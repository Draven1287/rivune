...
fn acceptance_overlapping_providers_restore_durable_checkpoint_without_replay() {
    acceptance_overlap_checkpoint_case(false, false);
}

#[cfg(unix)]
#[test]
fn acceptance_completed_sibling_survives_restart_and_running_sibling_cancellation() {
    acceptance_overlap_checkpoint_case(true, false);
}

#[cfg(unix)]
#[test]
fn acceptance_later_sibling_is_drained_after_first_completion_persistence_failure() {
    acceptance_overlap_checkpoint_case(true, true);
}

#[cfg(unix)]
fn acceptance_overlap_checkpoint_case(complete_first: bool, fail_first_completion_save: bool) {
...
if role == "decide":
    ...
    while not (os.path.exists(os.path.join(root, "release")) or (member == "member-1" and os.path.exists(os.path.join(root, "release-a")))):
...
"#.replace("__FIXTURE_ROOT__", &serde_json::to_string(&fixtures.to_string_lossy()).unwrap());
    ...
    if complete_first {
        if fail_first_completion_save {
            host.fail_checkpoint_saves("memberCompleted", PersistFault::AfterWrite, 2);
        }
        fs::write(fixtures.join("release-a"), b"release A only").unwrap();
        let durable_deadline = Instant::now() + Duration::from_secs(5);
        loop {
            let workspace = host.workspace.lock().unwrap();
            let public = public_snapshot(&workspace).unwrap();
            if public["runs"][0]["memberResults"].as_array().unwrap().len() == 1 {
                assert_eq!(
                    public["runs"][0]["memberResults"][0]["memberID"],
                    "member-1"
                );
                assert_eq!(
                    workspace.runs[0].status,
                    if fail_first_completion_save {
                        "failed"
                    } else {
                        "running"
            ...
            assert!(
                ...
                "A must be persisted and projected while B is latched"
            );
            thread::sleep(Duration::from_millis(10));
        }
        if fail_first_completion_save {
            fs::write(&release, b"release B").unwrap();
            assert_eq!(worker.join().unwrap().state, "uncertain");
            assert_eq!(
                calls(),
                3,
                "A persistence failure must not start integration"
            );
            {
                let workspace = host.workspace.lock().unwrap();
                assert_eq!(workspace.runs[0].status, "failed");
                ...
                for member in ["member-1", "member-2"] {
                    assert!(results.iter().any(|result| {
                        result["memberID"] == member
                            && result["text"].as_str().unwrap().contains(member)
                    }));
                }
            }
            drop(host);
            let reopened = HostState::open(path.clone()).unwrap();
            {
                let workspace = reopened.workspace.lock().unwrap();
                let restored =
                    TeamSession::restore(&workspace.constellation_sessions[0].checkpoint).unwrap();
                let unresolved = restored.unresolved_invocations();
                assert_eq!(unresolved.len(), 1);
                assert_eq!(unresolved[0].binding.role, TeamRole::Integrate);
                let results = public_snapshot(&workspace).unwrap()["runs"][0]["memberResults"]
                    .as_array()
                    .unwrap()
                    .clone();
                assert_eq!(results.len(), 2);
            ...
            assert_eq!(
                reopened.reconcile("acceptance-overlap-run").state,
                "rejected"
            );
            assert_eq!(
                calls(),
                3,
                "Recovery must not replay members or start synthesis"
            );
            drop(reopened);
            drop(release_guard);
            fs::remove_dir_all(path).unwrap();
            fs::remove_dir_all(captured).unwrap_or(());
    ...
    let recovered = HostState::open(captured.clone()).unwrap();
    {
        ...
        let unresolved = restored.unresolved_invocations();
        assert_eq!(unresolved.len(), if complete_first { 1 } else { 2 });
        ...
        let members = public["runs"][0]["memberResults"].as_array().unwrap();
        assert_eq!(members.len(), if complete_first { 1 } else { 0 });
        if complete_first {
            assert_eq!(members[0]["memberID"], "member-1");
            assert_eq!(
                members[0]["text"],
                "Independent overlap answer from member-1\n"
            );
        }
    }
    let bindings = {
    ...
    if complete_first {
        host.lifecycle
            ...
            .unwrap()
            .cancellations
            .get("acceptance-overlap-run")
            .expect("Original B is still active")
    ...
    assert_eq!(
        calls(),
        if complete_first { 3 } else { 4 },
        "Cancelled B must not integrate; normal execution integrates once"
    );
    {
        let workspace = host.workspace.lock().unwrap();
        assert_eq!(
        ...
        let public = public_snapshot(&workspace).unwrap();
        let results = public["runs"][0]["memberResults"].as_array().unwrap();
        assert_eq!(results.len(), if complete_first { 1 } else { 2 });
        let expected_members: &[&str] = if complete_first {
            &["member-1"]
        } else {
            &["member-1", "member-2"]
        };
