    ...
    <div className={`host-layout ${results.visible ? 'has-results' : ''}`}>
      ...
      <main id="host-chat" inert={results.visible&&narrowResults} aria-hidden={results.visible&&narrowResults?true:undefined} className={`host-chat ${context ? 'is-hidden' : ''} ${results.visible&&narrowResults ? 'results-replaced' : ''}`}>
        ...
        {conversation && <section className="host-composer" aria-label="Write a message">
          ...
          <div className="host-composer-actions"><button disabled={!!state.configurationConfirmations[conversation.id] || locked || !!state.pending || conversation.readOnly} onClick={() => void run(c => c.saveDraft(conversation.id))}>Save draft</button><button disabled={!!state.configurationConfirmations[conversation.id] || locked || !!state.pending || !!activeRuns.length || conversation.readOnly || !conversation.draft.trim()} onClick={() => void run(c => c.send(conversation.id))}>Send</button></div>
