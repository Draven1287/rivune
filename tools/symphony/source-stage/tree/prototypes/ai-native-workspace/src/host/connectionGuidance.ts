...
export function connectionGuidance(provider: CatalogProvider): string {
  ...
  if (provider.installation === 'missing') return 'Install the provider’s command-line app using its official instructions, then choose Find installed providers to look again. Installation and sign-in happen outside Rivune.';
  ...
  if (provider.responseTest === 'notTested') return 'No successful response is recorded. Close Settings and explicitly send a short message when you are ready. Sending starts a real provider request; this panel does not run a hidden test.';
