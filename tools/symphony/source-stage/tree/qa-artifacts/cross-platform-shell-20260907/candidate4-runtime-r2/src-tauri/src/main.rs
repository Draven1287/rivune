...
fn load_startup_profile(profile: PathBuf) -> Result<StartupWorkspace, rivune_desktop::host::ProfileInUse> {
...
fn classify_startup(result: Result<HostState, Box<dyn std::error::Error>>) -> Result<StartupWorkspace, rivune_desktop::host::ProfileInUse> {
    Ok(match result {
        ...
        Err(error) if error.is::<rivune_desktop::host::ProfileInUse>() => return Err(rivune_desktop::host::ProfileInUse),
...
fn generated_startup_context() -> tauri::Context<tauri::Wry> {tauri::generate_context!()}

fn defer_startup_windows(windows: &mut [tauri::utils::config::WindowConfig]) {
    for window in windows {window.create=false;}
}
fn admitted_startup_ui(
    startup: Result<StartupWorkspace,rivune_desktop::host::ProfileInUse>,
    initialize: impl FnOnce(StartupWorkspace)->Result<(),Box<dyn std::error::Error>>,
) -> Result<(),Box<dyn std::error::Error>> { initialize(startup?) }
fn startup_ui_constructors(
    window: impl FnOnce()->Result<(),Box<dyn std::error::Error>>,
    tray: impl FnOnce()->Result<(),Box<dyn std::error::Error>>,
) -> Result<(),Box<dyn std::error::Error>> {window()?;tray()}
// Tauri invokes setup on Ready. Never return a setup error into its panic path.
fn complete_startup_setup(
    result: Result<(),Box<dyn std::error::Error>>,
    admitted: &std::sync::atomic::AtomicBool,
    exit: impl FnOnce(i32),
) -> Result<(),Box<dyn std::error::Error>> {
    match result {
        Ok(())=>admitted.store(true,Ordering::SeqCst),
        Err(error)=>{eprintln!("Rivune startup stopped: {error}");exit(1);}
    }
    Ok(())
}
fn admitted_event(admitted: &std::sync::atomic::AtomicBool, event: RunEvent, handle: impl FnOnce(RunEvent)) {
    if admitted.load(Ordering::SeqCst) {handle(event);}
}
// Uses the built app's supported PathResolver before entering app.run/Ready.
fn preflight_startup(profile: Result<PathBuf,Box<dyn std::error::Error>>) -> Result<StartupWorkspace,std::process::ExitCode> {
    let profile=profile.map_err(|error|{eprintln!("Rivune startup stopped: {error}");std::process::ExitCode::FAILURE})?;
    load_startup_profile(profile).map_err(|_|std::process::ExitCode::SUCCESS)
}

fn startup_failure_exit(contended: bool) -> std::process::ExitCode {
    if contended {std::process::ExitCode::SUCCESS} else {std::process::ExitCode::FAILURE}
}
...
fn main() -> std::process::ExitCode {
    let mut context = generated_startup_context();
    // Defer automatic windows even when QA overrides replace window config.
    defer_startup_windows(&mut context.config_mut().app.windows);
    let startup_slot=Arc::new(Mutex::new(None::<StartupWorkspace>));
    let setup_slot=startup_slot.clone();
    let admitted=Arc::new(std::sync::atomic::AtomicBool::new(false));
    let setup_admitted=admitted.clone();
    let exit_status=Arc::new(std::sync::atomic::AtomicI32::new(0));
    let setup_exit=exit_status.clone();
    let app = tauri::Builder::default()
        .setup(move |app| {
            let initialized=(|| -> Result<(),Box<dyn std::error::Error>> {
            let startup=setup_slot.lock().map_err(|_|"Startup admission unavailable")?.take().ok_or("Startup was not admitted before Ready")?;
            ...
            complete_startup_setup(initialized,&setup_admitted,|code|{setup_exit.store(code,Ordering::SeqCst);app.handle().exit(code);})
        })
    ...
    let app = match app {
        Ok(app) => app,
        Err(error) => {
            // Builder failure precedes profile admission and Ready/setup.
            eprintln!("Rivune startup stopped: {error}");
            return startup_failure_exit(false);
        }
    };

    let profile=(|| -> Result<PathBuf,Box<dyn std::error::Error>> {
        Ok(resolved_profile_path(std::env::var_os("RIVUNE_ISOLATED_PROFILE_DIR"),app.path().app_data_dir()?.join("profile-v1"))?)
    })();
    let startup=match preflight_startup(profile){Ok(startup)=>startup,Err(code)=>return code};
    *startup_slot.lock().expect("startup slot before event loop")=Some(startup);
    app.run(move |app, event| admitted_event(&admitted,event,|event| match event {
        #[cfg(target_os = "macos")]
        RunEvent::Reopen { .. } => {
    ...
    if exit_status.load(Ordering::SeqCst)==0 {std::process::ExitCode::SUCCESS} else {std::process::ExitCode::FAILURE}
}

#[cfg(test)]
mod tests {
    #[test]
    fn startup_preflight_contention_never_reaches_ready_and_fatal_resolution_fails() {
        let (owner,path)=host("startup-preflight-contention");
        match preflight_startup(Ok(path.clone())) {Err(code)=>assert_eq!(code,std::process::ExitCode::SUCCESS),Ok(_)=>panic!("Contended preflight admitted")};
        match preflight_startup(Err("Invalid synthetic profile path".into())) {Err(code)=>assert_eq!(code,std::process::ExitCode::FAILURE),Ok(_)=>panic!("Invalid preflight admitted")};
        drop(owner);let startup=preflight_startup(Ok(path.clone())).unwrap();assert!(startup.host.is_some());drop(startup);std::fs::remove_dir_all(path).unwrap();
    }
    #[test]
    fn startup_ready_failure_returns_ok_requests_exit_and_skips_events_without_state() {
        let admitted=std::sync::atomic::AtomicBool::new(false);let exit=std::cell::Cell::new(None);
        // This is the exact setup completion boundary invoked by the Ready closure.
        assert!(complete_startup_setup(Err("Synthetic window/tray construction failure".into()),&admitted,|code|exit.set(Some(code))).is_ok());
        assert_eq!(exit.get(),Some(1));assert!(!admitted.load(Ordering::SeqCst));
        for event in [RunEvent::Ready,RunEvent::Exit] {admitted_event(&admitted,event,|_|panic!("accessed missing managed state"));}
        // No prevent_exit or normal request_quit callback can execute while unadmitted.
        let handled=std::cell::Cell::new(0);assert!(complete_startup_setup(Ok(()),&admitted,|_|panic!("Healthy startup requested exit")).is_ok());
        admitted_event(&admitted,RunEvent::Ready,|_|handled.set(handled.get()+1));assert_eq!(handled.get(),1);
    }

    #[test]
    fn startup_contention_blocks_both_constructor_seams_and_exit_policy() {
        let windows=std::cell::Cell::new(0);let trays=std::cell::Cell::new(0);
        let result=admitted_startup_ui(classify_startup(Err(Box::new(rivune_desktop::host::ProfileInUse))), |_|startup_ui_constructors(||{windows.set(windows.get()+1);Ok(())},||{trays.set(trays.get()+1);Ok(())}));
        assert!(result.is_err());assert_eq!((windows.get(),trays.get()),(0,0));
        assert_eq!(startup_failure_exit(true),std::process::ExitCode::SUCCESS);assert_eq!(startup_failure_exit(false),std::process::ExitCode::FAILURE);
    }
    #[test]
    fn startup_valid_and_recovery_admit_once_after_status_initialization() {
        let (h,path)=host("startup-admission-seams");drop(h);
        for startup in [load_startup_profile(path.clone()).unwrap(),classify_startup(Err(Box::new(std::io::Error::new(std::io::ErrorKind::PermissionDenied,"synthetic permission failure")))).unwrap()] {
            let recovery=startup.status.recovery_required;let initialized=std::cell::Cell::new(false);let windows=std::cell::Cell::new(0);let trays=std::cell::Cell::new(0);
            admitted_startup_ui(Ok(startup),|s|{assert_eq!(s.host.is_none(),recovery);initialized.set(true);startup_ui_constructors(||{assert!(initialized.get());windows.set(windows.get()+1);Ok(())},||{trays.set(trays.get()+1);Ok(())})}).unwrap();assert_eq!((windows.get(),trays.get()),(1,1));
        }
        std::fs::remove_dir_all(path).unwrap();
    }
    #[test]
    fn startup_defers_effective_windows_including_override() {
        let mut context=generated_startup_context();
        context.config_mut().app.windows.push(serde_json::from_value(serde_json::json!({"label":"synthetic-qa","title":"QA override","create":true})).unwrap());
        assert!(context.config().app.windows.iter().any(|w|w.create));defer_startup_windows(&mut context.config_mut().app.windows);assert!(context.config().app.windows.iter().all(|w|!w.create));
    }
    ...
    fn startup_corrupt_latest_profile_returns_no_host_and_preserves_every_file_byte() {
        ...
        let startup = load_startup_profile(path.clone()).unwrap();
