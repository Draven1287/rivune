...
export function BrowserWorkspace() {
  ...
  const previousNavigation = useRef({ narrow, navigation });
  const resultHeading = useRef<HTMLHeadingElement>(null);
  const resultRows = useRef(new Map<string, HTMLButtonElement>());
  const resultListPosition = useRef({ id: '', scrollTop: 0 });
  const resultFocus = useRef<'heading' | 'row' | null>(null);
  const [selectedResult, setSelectedResult] = useState<string | null>(null);
  const [copyStatus, setCopyStatus] = useState('');
  const [copyPending, setCopyPending] = useState(false);
  const copyGeneration = useRef(0);
  const copyOperation = useRef<symbol | null>(null);
  ...
  useLayoutEffect(() => { invalidateCopy(); }, [workspace.activeConversationId, result?.id, result?.content]);
  useLayoutEffect(() => {
    const query = window.matchMedia('(max-width: 620px)');
    const changed = () => { setNarrow(query.matches); if (!query.matches) setNavigation(false); };
    query.addEventListener('change', changed);
    return () => query.removeEventListener('change', changed);
  }, []);
  useLayoutEffect(() => {
    const previous = previousNavigation.current;
    if (narrow && navigation) newConversationButton.current?.focus();
    else if (previous.narrow && previous.navigation) {
      if (narrow) navigationToggle.current?.focus();
      else newConversationButton.current?.focus();
    } else if (narrow && !previous.narrow && sidebar.current?.contains(document.activeElement)) {
      navigationToggle.current?.focus();
    }
    previousNavigation.current = { narrow, navigation };
  }, [narrow, navigation]);
  useLayoutEffect(() => {
    if (!results.current?.open || !resultFocus.current) return;
    if (resultFocus.current === 'heading') {
      results.current.scrollTop = 0;
      resultHeading.current?.focus({ preventScroll: true });
    }
    else {
      const row = resultRows.current.get(resultListPosition.current.id);
      (row ?? resultHeading.current)?.focus({ preventScroll: true });
      results.current.scrollTop = resultListPosition.current.scrollTop;
    }
    resultFocus.current = null;
  }, [selectedResult]);
  function navigationKeys(event: KeyboardEvent<HTMLElement>) {
    if (!narrow || !navigation) return;
    if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); setNavigation(false); }
    if (event.key !== 'Tab') return;
    const controls = Array.from(sidebar.current?.querySelectorAll<HTMLElement>('a[href], button:not([disabled]), input:not([disabled])') ?? [])
  ...
  async function copyResult() {
    if (!result || !results.current?.open || copyOperation.current) return;
    const generation = copyGeneration.current;
    const operation = Symbol('sample-copy');
    copyOperation.current = operation;
    setCopyPending(true);
    setCopyStatus('Copying sample text…');
    let status: string;
    try {
      await navigator.clipboard.writeText(result.content);
      status = 'Copied sample text';
    } catch {
      status = 'Copy unavailable. Select the text to copy it.';
    }
    // An issued clipboard write cannot be cancelled. Only its feedback is scoped
    // to this result/open lifetime; the lock also prevents overlapping writes.
    if (copyOperation.current !== operation) return;
    copyOperation.current = null;
    if (!mounted.current) return;
    setCopyPending(false);
    if (copyGeneration.current === generation && results.current?.open) setCopyStatus(status);
  }
  function chooseResult(id: string | null) {
    invalidateCopy();
    if (id !== null) {
      resultListPosition.current = { id, scrollTop: results.current?.scrollTop ?? 0 };
      resultFocus.current = 'heading';
    } else resultFocus.current = 'row';
    setSelectedResult(id);
  }
  function closeResults() { invalidateCopy(); results.current?.close(); }
  function focusComposer() { requestAnimationFrame(() => composer.current?.focus()); }
  function selectConversation(id: string) { invalidateCopy(); workspace.selectConversation(id); setSelectedResult(null); setNavigation(false); focusComposer(); }
  function newConversation() { invalidateCopy(); workspace.newConversation(); setSelectedResult(null); setNavigation(false); focusComposer(); }
  function openResults(id: string | null = null) {
    resultListPosition.current = { id: '', scrollTop: 0 };
    chooseResult(id);
    results.current?.showModal();
    if (id === null) resultFocus.current = null;
    else requestAnimationFrame(() => { if (results.current?.open) resultHeading.current?.focus({ preventScroll: true }); });
  }
  return <div className={`browser-workspace ${navigation ? 'navigation-open' : ''}`}>
    <aside ref={sidebar} className="bw-sidebar" aria-label="Conversations" role={narrow && navigation ? 'dialog' : undefined} aria-modal={narrow && navigation ? true : undefined} onKeyDown={navigationKeys}>
      {narrow && navigation && <Button variant="ghost" size="icon" className="bw-navigation-close" aria-label="Close conversations" onClick={() => setNavigation(false)}><X size={18}/></Button>}
      <a className="bw-brand" href="#conversation"><img src={mark} alt=""/><span>Rivune</span></a>
      <Button variant="outline" className="bw-new" ref={newConversationButton} onClick={newConversation}><Plus size={17}/>New conversation</Button>
    ...
    <main id="conversation" className="bw-main" inert={narrow && navigation}>
      <header className="bw-header"><Button variant="ghost" size="icon" className="bw-menu" ref={navigationToggle} aria-label="Toggle conversations" aria-expanded={navigation} onClick={() => setNavigation(!navigation)}><PanelLeft size={18}/></Button><h1>{title}</h1><Button variant="ghost" className="bw-results-trigger" onClick={() => openResults()}><FileText size={16}/>Results<span>{artifacts.length}</span></Button></header>
    ...
    <dialog ref={results} onCancel={invalidateCopy} onClose={invalidateCopy} className="bw-dialog bw-results" aria-labelledby="bw-results-title"><header><div><span className="bw-kicker">SAMPLE WORKSPACE</span><h2 ref={resultHeading} tabIndex={-1} id="bw-results-title">{result ? result.title : 'Results'}</h2></div><Button variant="ghost" size="icon" aria-label="Close results" onClick={closeResults}><X size={19}/></Button></header>{result ? <><button className="bw-back" onClick={() => chooseResult(null)}>← All results</button><pre>{result.content}</pre><Button variant="outline" disabled={copyPending} title={copyPending ? 'A clipboard write is still pending' : undefined} onClick={copyResult}>Copy sample text</Button><p role="status">{copyStatus}</p></> : artifacts.length ? artifacts.map(artifact => <button className="bw-result-card" key={artifact.id} ref={element => { if (element) resultRows.current.set(artifact.id, element); else resultRows.current.delete(artifact.id); }} onClick={() => chooseResult(artifact.id)}><FileText size={20}/><span><strong>{artifact.title}</strong><small>Sample · {artifact.kind}</small></span><ArrowUpRight size={17}/></button>) : <p className="bw-dialog-intro">No results in this conversation yet.</p>}</dialog>
  </div>;
}
