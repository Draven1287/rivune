import { parseArtifactRequest, parseArtifactInspection, type HostArtifactRequest, type HostArtifactInspection } from './contracts.ts';
...
export interface HostBridgeAdapter {
  inspectArtifact?(request: HostArtifactRequest): Promise<HostArtifactInspection>;
...
export function createTauriWorkspaceAdapter(bridge: unknown, options: { reservedSubmission?: boolean } = {}): HostBridgeAdapter | null {
  ...
  try {
    ...
    inspect = Object.getOwnPropertyDescriptor(bridge, 'inspectArtifact')?.value;
  } catch { return null; }
  ...
  return Object.freeze({
    ...(typeof inspect === 'function' ? { async inspectArtifact(request: HostArtifactRequest) {
      const exact = parseArtifactRequest(request);
      // Keep validation identity private even if a custom bridge mutates its input.
      return parseArtifactInspection(await inspect.call(bridge, { ...exact }), exact);
    } } : {}),
