import type {HostArtifactSummary,HostArtifactRequest,HostTeamSelection} from '../src/host/contracts';
import { act, StrictMode } from 'react';
import {StartupBoundary,StartupLoader,StartupFallback} from '../src/StartupFallback';
...
async function mount(f: Fixture, preserveSession = false) {
  ...
  await act(async () => { root!.render(<HostWorkspace bridge={f.bridge} appearance={new URLSearchParams(location.search).get('scenario')==='quiet-workspace'?'quiet':'current'} />); });
...
teamFormTests.push(['connection-handoff',`Connection handoff: ${outcome}`,async f=>{
 ...
 await unmount();await mount(f);await type('Keep my first prompt 🪐');const input=messageInput();
...
async function handoffSettings(){if(innerWidth<=700)await click('Conversations');await click('Settings');}
async function handoffSave(){await click('Find installed providers');const select=container.querySelector<HTMLSelectElement>('#provider-route')!;await act(async()=>{select.value='codex:discovered:synthetic';select.dispatchEvent(new Event('change',{bubbles:true}));});await click('Save as workspace default');}
function handoffBridge(f:Fixture){Object.assign(f.bridge,{async discoverProviders(){return[{id:'codex:discovered:synthetic',kind:'codex',executablePath:'/synthetic/codex',installed:true}];},async configureSelectedProvider(provider:typeof f.snapshot.providers[number]){f.calls.push({name:'configure'});f.snapshot.providers.push(provider);f.snapshot.selectedProviderID=provider.id;return{schemaVersion:1,state:'durable',provider,selectedProviderID:provider.id};}});}
for(const outcome of ['applied','rejected','none'] as const)teamFormTests.push(['connection-handoff',`Connection recovery handoff: ${outcome}`,async f=>{
 await unmount();const provider={id:'codex:discovered:synthetic',kind:'codex',executablePath:'/synthetic/codex',model:null,timeoutMs:60000};const op={schemaVersion:1,operationID:'configuration:1',intent:{provider,select:true,guard:{schemaVersion:1,expectedProvider:null,expectedSelectedProviderID:null}},state:outcome==='applied'?'applied':'reserved',acknowledged:false};let reads=0,applies=0;
 if(outcome==='applied'){f.snapshot.providers.push(provider);f.snapshot.selectedProviderID=provider.id;}
 Object.assign(f.bridge,{async discoverProviders(){return[];},async reserveProviderConfiguration(){throw Error('No replay');},async applyProviderConfiguration(){applies++;throw Error('No replay');},async getProviderConfigurationOperation(){reads++;return outcome==='none'&&reads>1?null:clone(op);},async reconcileProviderConfiguration(){if(op.state==='reserved')op.state='rejected';return clone(op);},async acknowledgeProviderConfiguration(){op.acknowledged=true;return clone(op);}});
 await mount(f);await type('Recovery preserves draft');await handoffSettings();await waitFor(()=>!button('Check saved configuration').disabled,'Recovery not ready');await click('Check saved configuration');
 if(outcome==='applied'){await click('Continue to chat');await waitFor(()=>document.activeElement===messageInput(),'Recovery handoff lost focus');}else assert(!container.textContent?.includes('Continue to chat'),'Unapplied recovery authorized handoff');assert(messageInput().value==='Recovery preserves draft'&&applies===0,'Recovery replayed or lost draft');
}]);
for(const binding of ['pinned','team'] as const)teamFormTests.push(['connection-handoff',`Connection handoff preserves ${binding} and reopen`,async f=>{
 await unmount();const {members}=populateMockTeam(f);if(binding==='pinned')Object.assign(f.snapshot.conversations[0].richDraft,{selection:members[0],team:null});const before=clone(f.snapshot.conversations[0].richDraft);handoffBridge(f);await mount(f);await type('Bound draft');await handoffSettings();await handoffSave();await click('Continue to chat');await waitFor(()=>document.activeElement===messageInput(),'Handoff focus missing');assert(JSON.stringify(f.snapshot.conversations[0].richDraft)===JSON.stringify(before),'Saved binding changed');await handoffSettings();assert(!button('Continue to chat').disabled,'Same-session durable handoff lost on reopen');await click('Close settings');assert(messageInput().value==='Bound draft','Reopen lost draft');assert(f.calls.filter(c=>c.name==='configure').length===1,'Reopen configured again');
}]);
teamFormTests.push(['connection-handoff','Connection handoff rejects late old bridge save',async f=>{
 await unmount();handoffBridge(f);let finish!:(value:unknown)=>void;let requested:unknown;Object.assign(f.bridge,{async configureSelectedProvider(provider:unknown){requested=provider;return new Promise(resolve=>finish=resolve);}});await mount(f);await type('Old draft');await handoffSettings();await handoffSave();assert(requested,'Pending old save not reached');const fresh=createFixture();handoffBridge(fresh);await act(async()=>root!.render(<HostWorkspace bridge={fresh.bridge}/>));await settle();await act(async()=>finish({schemaVersion:1,state:'durable',provider:requested,selectedProviderID:'codex:discovered:synthetic'}));await settle();assert(!container.textContent?.includes('Continue to chat'),'Old save authorized new bridge');assert(!fresh.calls.some(c=>c.name==='configure'),'Old save configured new bridge');
}]);
for(const missing of ['routes','conversation'])teamFormTests.push(['connection-handoff',`Connection handoff missing ${missing}`,async f=>{
 await unmount();handoffBridge(f);if(missing==='routes')Object.assign(f.bridge,{async discoverProviders(){return[];}});else{f.snapshot.activeConversationID=null as unknown as string;f.snapshot.conversations=[];}
 if(missing==='conversation'){root=createRoot(container);await act(async()=>root!.render(<HostWorkspace bridge={f.bridge}/>));await settle();}else await mount(f);
 await handoffSettings();if(missing==='routes'){await click('Find installed providers');assert(!container.querySelector('#provider-route'),'Missing routes offered selection');}else await handoffSave();assert(![...container.querySelectorAll('button')].some(b=>b.textContent==='Continue to chat'),'Missing prerequisite offered handoff');
}]);

// Startup entry tests mount the production boundary/loader and App selector.
for (const mode of ['throwing-child','module-reject','module-pending','valid','demo','missing','null','primitive','accessor','partial','hydration-error','hydration-uncertain'] as const) {
 teamFormTests.push(['startup-fallback',`Startup fallback ${mode}`,async f=>{
  const keys=['__RIVUNE_DESKTOP_HOST__','__TAURI__','__TAURI_INTERNALS__','localStorage'];
  const before=new Map(keys.map(k=>[k,Object.getOwnPropertyDescriptor(window,k)]));
  let getters=0,writes=0,sessionWrites=0;
  const previousSet=memoryStorage.setItem,previousRemove=memoryStorage.removeItem,previousClear=memoryStorage.clear;
  const isolatedStorage:Storage={length:0,key:()=>null,getItem:()=>null,setItem:()=>{writes++;},removeItem:()=>{writes++;},clear:()=>{writes++;}};
  memoryStorage.setItem=()=>{sessionWrites++;};memoryStorage.removeItem=()=>{sessionWrites++;};memoryStorage.clear=()=>{sessionWrites++;};
  try {
   for(const key of keys)assert(Reflect.deleteProperty(window,key),`Cannot isolate ${key}`);
   Object.defineProperty(window,'localStorage',{configurable:true,value:isolatedStorage});
   if(mode!=='demo')Object.defineProperty(window,'__TAURI_INTERNALS__',{configurable:true,value:{}});
   if(mode==='accessor')Object.defineProperty(window,'__RIVUNE_DESKTOP_HOST__',{configurable:true,get(){getters++;throw Error('Private getter must never run');}});
   else if(mode!=='missing'&&mode!=='demo')Object.defineProperty(window,'__RIVUNE_DESKTOP_HOST__',{configurable:true,value:mode==='null'?null:mode==='primitive'?'invalid':mode==='partial'?{}:f.bridge});
   if(mode.startsWith('hydration-')){
    Object.assign(f.bridge,{getSnapshot:async()=>{f.calls.push({name:'snapshot'});throw Error('Private hydration failure');}});
    if(mode==='hydration-uncertain')Object.assign(f.bridge,{getSubmissionRecovery:async()=>({schemaVersion:1,requestID:'held',conversationID:'fixture-chat',state:'reserved'})});
   }
   function Broken():never {throw Error('Private child failure');}
   const load=mode==='module-reject'?()=>Promise.reject(Error('Private import failure')):mode==='module-pending'?()=>new Promise<{default:typeof App}>(()=>{}):async()=>({default:mode==='throwing-child'?Broken:App});
   root=createRoot(container);
   await act(async()=>root!.render(<StrictMode><StartupBoundary><StartupLoader load={load}/></StartupBoundary></StrictMode>));await settle();
   const text=container.textContent??'';
   assert(!text.includes('Private'),'Private exception rendered');assert(getters===0,'Accessor invoked');
   if(mode==='valid'){await waitFor(()=>!!container.querySelector('textarea[aria-label="Message"]'),'Valid desktop did not hydrate');assert(f.calls.some(c=>c.name==='snapshot'),'No host hydration');}
   else if(mode==='demo'){assert(text.includes('Local simulation')||text.includes('local demo')||text.includes('Demo workspace'),'Intentional demo absent');assert(container.querySelector('textarea'),'Demo composer absent');assert(f.calls.length===0,'Demo touched host');}
   else if(mode.startsWith('hydration-')){assert(!container.querySelector('.rivune-startup'),'Caught hydration error incorrectly mapped to boundary');assert(!text.includes('Rivune could not display'),'Hydration mapped to render exception');assert(container.querySelector('.host-workspace'),'Existing hydration surface absent');}
   else if(mode==='partial'){assert(text.includes('Desktop connection is incomplete'),'Partial bridge did not retain host validation');assert(f.calls.length===0,'Partial bridge touched host');}
   else {
    const expected=mode==='module-pending'?'Opening Rivune':mode==='module-reject'||mode==='throwing-child'?'Rivune could not display':'Desktop connection unavailable';
    assert(text.includes(expected),'Expected fallback missing');assert(container.querySelector(mode==='module-pending'?'[role=status]':'[role=alert]'),'Announcement absent');
    assert(!container.querySelector('textarea,button,input,a'),'Fallback offers interactive mutation/reload');assert(f.calls.length===0,'Fallback touched host');
    assert(document.documentElement.scrollWidth<=innerWidth,'Fallback horizontal overflow');
    const heading=container.querySelector('h1')!;assert(parseFloat(getComputedStyle(heading).fontSize)>=24,'Heading too small');
   }
   assert(!f.calls.some(c=>['save','submit','configure','create','open','recoveryReserve','recoveryClear','reconcile','cancel'].includes(c.name)),'Startup mutated host');
   if(mode==='demo'){
    const loaderWrites=sessionWrites;await unmount();sessionWrites=0;root=createRoot(container);await act(async()=>root!.render(<StrictMode><App/></StrictMode>));await settle();
    assert(writes===0&&sessionWrites===loaderWrites,'Loader changed intentional demo storage behavior');
   }else assert(writes===0&&sessionWrites===0,'Startup mutated storage');
  }finally{
   await unmount();memoryStorage.setItem=previousSet;memoryStorage.removeItem=previousRemove;memoryStorage.clear=previousClear;
   for(const key of keys){const d=before.get(key);if(d)Object.defineProperty(window,key,d);else Reflect.deleteProperty(window,key);}
  }
 }]);
}

const quietPrompt='Compare SQLite and JSON files for a local notes app. Recommend one and explain the trade-offs.';
const quietAnswer='Use SQLite for the working library.\n\nSQLite gives notes, tags, and search a consistent home. Transactions keep related changes together.\n\nUse JSON for export and interchange. It stays readable and portable, but requires more application code for safe concurrent updates.\n\n• Search and filtering as the library grows\n• Atomic updates across notes and tags\n• Portable JSON exports when needed';
async function quietFixture(f:Fixture){
 f.snapshot.runs=[];f.snapshot.artifacts=[];f.snapshot.conversations[0].title='Compare storage options';f.snapshot.conversations.push({...clone(f.snapshot.conversations[0]),id:'outline',title:'Project outline',draft:''});
 const {team,members}=populateMockTeam(f);team.members=members;team.leadIndex=2;Object.assign(f.snapshot.conversations[0].richDraft,{team,selection:members[2]});
 f.catalog.providers.forEach((p,i)=>p.label=['Research','Review','Lead'][i]);
 const id='quiet-sample',bytes=new TextEncoder().encode(quietAnswer),hash=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',bytes)),b=>b.toString(16).padStart(2,'0')).join('');
 const saved={id,conversationID:'fixture-chat',status:'completed',updatedAt:'2026-09-10T12:00:00Z',admitted:{requestID:id,conversationID:'fixture-chat',prompt:quietPrompt,mode:'constellation',provider:f.snapshot.providers[2],retryOf:null,team},answer:quietAnswer,error:null,memberResults:[],resolution:{schemaVersion:1,kind:'leadSynthesis',reviewed:false,reviewerMemberID:'member-3',providerID:f.snapshot.providers[2].id,summary:'Synthetic example: independent contributions combined by the lead.'}};
 f.snapshot.runs.push(saved);
 const record:HostArtifactSummary={schemaVersion:1,artifactID:'quiet-result',conversationID:'fixture-chat',requestID:id,origin:{kind:'finalAnswer',memberID:null,providerID:f.snapshot.providers[2].id},displayName:'Final answer',previewKind:'plainText',languageHint:null,byteLength:bytes.length,contentSHA256:hash,availability:'available',createdAt:'2026-09-10T12:00:00Z',supersedesArtifactID:null};f.snapshot.artifacts.push(record);
 Object.assign(f.bridge,{async inspectArtifact(q:HostArtifactRequest){f.calls.push({name:'inspectArtifact',value:clone(q)});if(q.artifactID!==record.artifactID||q.expectedSHA256!==hash)throw Error('Synthetic identity mismatch');return {schemaVersion:1,artifactID:record.artifactID,conversationID:record.conversationID,requestID:record.requestID,previewKind:record.previewKind,languageHint:null,byteLength:record.byteLength,contentSHA256:hash,availability:'available',text:quietAnswer};},async submitRun(q:{id:string;conversationID:string;prompt:string;mode:string}){f.calls.push({name:'submit',value:clone(q)});const c=f.snapshot.conversations.find(c=>c.id===q.conversationID)!;const savedTeam=q.mode==='constellation'?clone(c.richDraft.team) as HostTeamSelection:null;const admittedProvider=savedTeam?f.snapshot.providers.find(p=>p.id===savedTeam.members[savedTeam.leadIndex].providerID)!:f.snapshot.providers[0];c.draft='';c.richDraft.revision++;f.snapshot.runs.push({id:q.id,conversationID:q.conversationID,status:'completed',updatedAt:'2026-09-10T12:01:00Z',admitted:{requestID:q.id,conversationID:q.conversationID,prompt:q.prompt,mode:q.mode,provider:admittedProvider,retryOf:null,...(savedTeam?{team:savedTeam}:{})},answer:'Synthetic response: your local test request completed. No provider was called.',error:null});return{state:'accepted',requestID:q.id};}});
}
teamFormTests.push(['quiet-workspace','Quiet new/open/save/send safeguards',async f=>{
 await unmount();await quietFixture(f);await mount(f);if(innerWidth<=700)await click('Conversations');await click('New conversation');await type('Synthetic vertical slice');await click('Save draft');assert(f.calls.some(c=>c.name==='save'),'Save not reached');await click('Send');await waitFor(()=>container.textContent!.includes('Synthetic response:'),'No visible result');assert(f.calls.filter(c=>c.name==='submit').length===1,'Duplicate submit');assert(messageInput().value==='','Admitted unchanged draft not cleared');assert(button('Send').disabled,'Empty send enabled');
 if(innerWidth<=700)await click('Conversations');await click('Compare storage options');assert(container.textContent?.includes('Use SQLite'),'Saved conversation not reopened');
}]);
teamFormTests.push(['quiet-workspace','Quiet rejected save retains draft and prevents send',async f=>{
 await type('Keep this synthetic draft');f.setConflict(true);await click('Send');await settle();assert(messageInput().value==='Keep this synthetic draft','Draft lost');assert(!f.calls.some(c=>c.name==='submit'),'Failed save dispatched');assert(container.querySelector('[role=alert]'),'Honest error absent');
}]);
teamFormTests.push(['quiet-workspace','Quiet settings closure preserves draft and focus',async f=>{
 await type('Settings synthetic draft');if(innerWidth<=700)await click('Conversations');await click('Settings');assert(container.querySelector('dialog[open]'),'Settings absent');assert(container.textContent?.includes('Not tested'),'Readiness states missing');await click('Close settings');await settle();assert(!container.querySelector('dialog[open]'),'Settings stayed open');await waitFor(()=>!!document.activeElement?.textContent?.includes('Settings'),'Settings focus not restored');if(innerWidth<=700)await click('Chat');assert(messageInput().value==='Settings synthetic draft','Settings lost draft');assert(!f.calls.some(c=>['save','submit','configure'].includes(c.name)),'Settings mutated host');
}]);
teamFormTests.push(['quiet-workspace','Quiet result inspection preserves composer',async f=>{
 await unmount();await quietFixture(f);await mount(f);await type('Retain beside result');await click('Results (1)');await selectResult('quiet-result');await resultReady(quietAnswer);await click('Close results');assert(messageInput().value==='Retain beside result','Result navigation lost draft');assert(!f.calls.some(c=>['save','submit'].includes(c.name)),'Inspection mutated host');
}]);

teamFormTests.push(['quiet-workspace','Quiet connection facts stay separate and details preserve draft',async f=>{
 f.catalog.providers[0].authentication='authenticated';f.catalog.providers[0].responseTest='failed';await click('Refresh');await type('Keep this connection review draft');if(innerWidth<=700)await click('Conversations');await click('Settings');
 const card=container.querySelector<HTMLElement>('.quiet-connection-card')!;assert(card,'Connection card missing');const facts=[...card.querySelectorAll('dl>div')].map(row=>[row.querySelector('dt')?.textContent,row.querySelector('dd')?.textContent]);assert(JSON.stringify(facts)===JSON.stringify([['Installation','Installed'],['Sign-in','Host reports signed in'],['Response test','Failed']]),'Connection facts were combined or inferred');
 const detail=card.querySelector<HTMLDetailsElement>('details')!;assert(!detail.open,'Guidance unexpectedly expanded');await act(async()=>{detail.querySelector('summary')!.click();});assert(detail.open&&detail.textContent?.includes('host reports a failed response'),'Failed-response guidance missing');await click('Close settings');await waitFor(()=>!!document.activeElement?.textContent?.includes('Settings'),'Settings focus not restored');if(innerWidth<=700)await click('Chat');assert(messageInput().value==='Keep this connection review draft','Detail inspection lost draft');assert(!f.calls.some(c=>['save','submit','configure','discover'].includes(c.name)),'Detail inspection caused a host action');
}]);

const selection = rendererScenarioSelection(location.search);
teamFormTests.push(['quiet-workspace','Quiet visible configuration stays confirmed across changed snapshots',async f=>{
 for(const outcome of ['lost','malformed','refresh']){
  await unmount();f.snapshot.conversations[0].richDraft.team=null;f.snapshot.conversations[0].richDraft.selection=null;
  if(f.snapshot.providers.length===1)populateMockTeam(f);f.snapshot.conversations[0].richDraft.team=null;f.snapshot.conversations[0].richDraft.selection=null;
  const original=f.bridge.saveRichDraft,read=f.bridge.getSnapshot;let receipt:Awaited<ReturnType<typeof original>>|null=null,failRead=false;
  f.bridge.getSnapshot=async()=>{if(failRead){failRead=false;throw Error('Synthetic refresh failure after acknowledgement');}return read();};
  f.bridge.saveRichDraft=async request=>{if(receipt){f.calls.push({name:'save',value:clone(request)});return clone(receipt);}receipt=await original(request);if(outcome==='lost')throw Error('Applied reply lost');if(outcome==='malformed')return {...receipt,mutationID:'wrong'};failRead=true;return receipt;};
  await mount(f);await type('Applied but uncertain draft');const summary=()=>container.querySelector('.composer-execution summary')!.textContent;const before=summary();const assertConfirmed=()=>{const toolbar=container.querySelector<HTMLElement>('.quiet-execution-toolbar')!;assert(toolbar.textContent===before,'Visible toolbar promoted unconfirmed snapshot');assert(toolbar.getClientRects().length>0,'Confirmed presentation is hidden');assert(!toolbar.querySelector('button'),'Unconfirmed mode/provider controls still presented');};const countBefore=f.calls.filter(c=>c.name==='save').length;
  await click('Local test fixture');await chooseExecution('Execution mode','team');await act(async()=>{const inputs=container.querySelectorAll<HTMLInputElement>('.composer-execution input');inputs[0].click();inputs[1].click();});await chooseExecution('Constellation lead','second-provider');await click('Save configuration');assert(summary()===before,'Unconfirmed operation changed resting summary');assertConfirmed();assert(container.textContent?.includes('Configuration not confirmed'),'Missing uncertainty status');assert(button('Send').disabled,'Unconfirmed configuration left Send enabled');await act(async()=>{for(const modifier of ['metaKey','ctrlKey']){const event=new KeyboardEvent('keydown',{key:'Enter',[modifier]:true,bubbles:true,cancelable:true});messageInput().dispatchEvent(event);assert(event.defaultPrevented,'Uncertain keyboard shortcut retained default text behavior');}});await settle();assert(!f.calls.some(c=>c.name==='submit')&&messageInput().value==='Applied but uncertain draft','Keyboard Send bypassed uncertainty or changed draft');
  if(outcome!=='refresh'){await click('Refresh');assert(summary()===before,'Normal refresh promoted unconfirmed host configuration');assertConfirmed();assert(container.textContent?.includes('Configuration not confirmed'),'Refresh hid uncertainty');await click('Retry draft save');const saves=f.calls.filter(c=>c.name==='save').slice(countBefore);assert(saves.length===2&&JSON.stringify(saves[0].value)===JSON.stringify(saves[1].value),'Recovery replaced original mutation');}
  else {await click('Retry draft save');assert(f.calls.filter(c=>c.name==='save').length===countBefore+1,'Acknowledged refresh retry saved again');}
  assert(summary()==='Constellation · 2 members · Lead: Mock connection (second-provider)','Exact acknowledgement and refreshed state did not update summary');assert(!container.textContent?.includes('Showing the last confirmed configuration'),'Confirmation fence remained after recovery');assert(container.querySelector('.quiet-execution-toolbar')?.textContent?.includes('Constellation'),'Confirmed mode not restored');assert(container.querySelector('.quiet-execution-toolbar')?.textContent?.includes('Mock connection (second-provider)'),'Confirmed provider not restored');assert(!container.textContent?.includes('Configuration was not confirmed'),'Recovered editor retained stale uncertainty notice');assert(messageInput().value==='Applied but uncertain draft','Recovery lost draft');assert(!f.calls.some(c=>c.name==='submit'),'Recovery dispatched');
  f.bridge.saveRichDraft=original;f.bridge.getSnapshot=read;
 }
}]);

const selectionScope = selection.ids === null ? 'full suite' : selection.ids.includes('quiet-workspace') ? 'shadcn working direction · synthetic host only' : selection.ids.includes('startup-fallback') ? 'startup observability and containment only' : selection.ids.includes('sent-draft-mounted') ? 'selected sent-draft composer only' : selection.ids.some(id => id.startsWith('retry-')) ? 'selected invocation retries only' : 'selected team forms only';
...
for (const [name, run] of selectedTests) {
  ...
  try {
    if (!name.startsWith('Startup fallback ')) await mount(f); await run(f);
    if (selection.ids !== null) assert(!f.calls.some(c => [...((selection.ids?.includes('sent-draft-mounted')||selection.ids?.includes('quiet-workspace'))?[]:['submit']), 'beginShutdown', 'flushShutdownDraft', 'completeShutdown', 'abortShutdown'].includes(c.name)), 'Selected team form scenario invoked unrelated run or shutdown methods');
...
if (!selection.error && new URLSearchParams(location.search).get('preview') === '1') {
  ...
  if(new URLSearchParams(location.search).get('scenario')==='quiet-workspace'){await quietFixture(f);output.style.display='none';}
  await mount(f);
  if(new URLSearchParams(location.search).get('scenario')==='quiet-workspace'){const label=container.querySelector('.preview-label');if(label)label.textContent='Working design · synthetic host · no provider calls';}
  if(supersessionPreview){
...
if(new URLSearchParams(location.search).get('handoffKeyboard')==='1' && selection.ids?.includes('connection-handoff')){
 Object.defineProperty(window,'sessionStorage',{configurable:true,value:memoryStorage});const f=createFixture();handoffBridge(f);await mount(f);await type('Keyboard handoff draft');await handoffSettings();await handoffSave();output.textContent='Synthetic keyboard fixture: use Tab/Enter on Continue to chat. No provider requests.';output.style.pointerEvents='none';globalThis.IS_REACT_ACT_ENVIRONMENT=false;window.addEventListener('pagehide',restoreSessionStorage,{once:true});
}

if(new URLSearchParams(location.search).get('scenario')==='startup-fallback'&&new URLSearchParams(location.search).has('fallbackPreview')) {root=createRoot(container);const state=new URLSearchParams(location.search).get('fallbackPreview');await act(async()=>root!.render(<StartupFallback state={state==='loading'?'loading':state==='bridge-unavailable'?'bridge-unavailable':'failed'}/>));output.style.display='none';}
