...
test('compact composer confirmation survives applied lost or malformed reply and acknowledgement refresh failure',async t=>{
 const {composerConfigurationKey,composerSummary}=await import('../src/host/composerConfiguration.ts');const {configuredTeam}=await import('../src/host/teamConfiguration.ts');
 for(const outcome of ['lost','malformed','refresh']){
  const f=fixture();enableTeam(f);const c=f.controller();t.after(()=>c.dispose());await c.start();c.editDraft('chat','Retain uncertain text');
  const display=()=>c.getState().configurationConfirmations.chat?.confirmedSummary??composerSummary(c.getState().snapshot,c.getState().catalog,'chat');const before=display();const state=c.getState(),team=configuredTeam(state.snapshot,state.catalog,['codex','claude'],'claude');
  const save=f.bridge.saveRichDraft,read=f.bridge.getSnapshot;let receipt=null,failRead=false;
  f.bridge.getSnapshot=async()=>{if(failRead){failRead=false;throw Error('Synthetic refresh failure');}return read();};
  f.bridge.saveRichDraft=async request=>{if(receipt){f.calls.push(['save',clone(request)]);return clone(receipt);}receipt=await save(request);if(outcome==='lost')throw Error('Applied reply lost');if(outcome==='malformed')return {...receipt,mutationID:'wrong'};failRead=true;return receipt;};
  await assert.rejects(c.configureExecution('chat',{expectedKey:composerConfigurationKey(state.snapshot,state.catalog,'chat'),selection:team.members[1],team}));assert.equal(display(),before);assert.equal(c.getState().configurationConfirmations.chat.acknowledged,outcome==='refresh');
  if(outcome!=='refresh'){
   await c.refresh();assert.deepEqual(c.getState().snapshot.conversations[0].richDraft.team,team,'Global snapshot must remain truthful');assert.equal(display(),before);assert.equal(c.getState().drafts.chat,'Retain uncertain text');
   await assert.rejects(c.configureExecution('chat',{expectedKey:composerConfigurationKey(c.getState().snapshot,c.getState().catalog,'chat'),selection:null,team:null}));await assert.rejects(c.send('chat'));assert.equal(count(f,'save'),1);
   await c.retryDraftSave('chat');const saves=f.calls.filter(x=>x[0]==='save');assert.equal(saves.length,2);assert.deepEqual(saves[0][1],saves[1][1]);
  }else{await c.retryDraftSave('chat');assert.equal(count(f,'save'),1,'Acknowledged operation must only refresh, not create/replay a mutation');}
  assert.equal(c.getState().configurationConfirmations.chat,undefined);assert.match(display(),/^Constellation/);assert.equal(c.getState().drafts.chat,'Retain uncertain text');assert.equal(count(f,'submit'),0);
 }
});

test('compact composer explicitly accepts superseding acknowledged revision without replay or losing local text',async t=>{
 const {composerConfigurationKey}=await import('../src/host/composerConfiguration.ts');const {configuredTeam}=await import('../src/host/teamConfiguration.ts');
 for(const recovered of [false,true]){
  const f=fixture();enableTeam(f);const c=f.controller();t.after(()=>c.dispose());await c.start();c.editDraft('chat','Keep local text');const state=c.getState(),team=configuredTeam(state.snapshot,state.catalog,['codex','claude'],'claude');const save=f.bridge.saveRichDraft;let receipt;
  f.bridge.saveRichDraft=async request=>{if(receipt){f.calls.push(['save',clone(request)]);return clone(receipt);}receipt=await save(request);Object.assign(f.snapshot.conversations[0],{draft:'External message'});Object.assign(f.snapshot.conversations[0].richDraft,{revision:4,selection:null,team:null});if(recovered)throw Error('Lost applied reply');return receipt;};
  await assert.rejects(c.configureExecution('chat',{expectedKey:composerConfigurationKey(state.snapshot,state.catalog,'chat'),selection:team.members[1],team}));
  if(recovered){await c.refresh();await assert.rejects(c.acceptSupersedingConfiguration('chat',composerConfigurationKey(c.getState().snapshot,c.getState().catalog,'chat')));await assert.rejects(c.retryDraftSave('chat'));const saves=f.calls.filter(x=>x[0]==='save');assert.equal(saves.length,2);assert.deepEqual(saves[0][1],saves[1][1]);}
  assert(c.getState().configurationConfirmations.chat.acknowledged);const reviewed=composerConfigurationKey(c.getState().snapshot,c.getState().catalog,'chat');c.editDraft('chat','Newer local text');f.snapshot.conversations[0].richDraft.revision=5;f.snapshot.conversations[0].draft='Another writer';
  const saveCount=count(f,'save');await assert.rejects(c.acceptSupersedingConfiguration('chat',reviewed),/changed again/);assert(c.getState().configurationConfirmations.chat);assert.equal(c.getState().drafts.chat,'Newer local text');
  await c.acceptSupersedingConfiguration('chat',composerConfigurationKey(c.getState().snapshot,c.getState().catalog,'chat'));assert.equal(c.getState().configurationConfirmations.chat,undefined);assert.equal(c.getState().drafts.chat,'Newer local text');assert.equal(f.snapshot.conversations[0].draft,'Another writer');assert.equal(count(f,'save'),saveCount);assert.equal(count(f,'submit'),0);
  f.bridge.saveRichDraft=save;await c.saveDraft('chat');const last=f.calls.filter(x=>x[0]==='save').at(-1)[1];assert.equal(last.expectedRevision,5);assert.equal(last.draft,'Newer local text');assert.equal(last.team,null);assert.equal(last.selection,null);
 }
});

test('saved artifact controller is read only and rejects disposed late response',async t=>{
  const {createHash}=await import('node:crypto');const f=fixture();const text='saved 🪐';const contentSHA256=createHash('sha256').update(text).digest('hex');f.addRun({id:'r',conversationID:'chat',prompt:'p'},'completed').answer=text;
  const a={schemaVersion:1,artifactID:'artifact',conversationID:'chat',requestID:'r',origin:{kind:'finalAnswer',memberID:null,providerID:'codex'},displayName:'Final answer',previewKind:'plainText',languageHint:null,byteLength:Buffer.byteLength(text),contentSHA256,availability:'available',createdAt:'today',supersedesArtifactID:null};f.snapshot.artifacts=[a];
  const response={schemaVersion:1,artifactID:a.artifactID,conversationID:a.conversationID,requestID:a.requestID,contentSHA256,previewKind:'plainText',languageHint:null,byteLength:a.byteLength,availability:'available',text};let inspectCalls=0;f.bridge.inspectArtifact=async()=>{inspectCalls++;return response;};const c=f.controller();t.after(()=>c.dispose());await c.start();const q={artifactID:a.artifactID,conversationID:a.conversationID,requestID:a.requestID,expectedSHA256:contentSHA256};const before=clone(c.getState());const calls=clone(f.calls);
  assert.equal((await c.inspectArtifact(q)).text,text);assert.deepEqual(c.getState(),before);assert.deepEqual(f.calls,calls);await assert.rejects(()=>c.inspectArtifact({...q,conversationID:'wrong'}));assert.equal(inspectCalls,1);
  let resolve;f.bridge.inspectArtifact=()=>new Promise(r=>resolve=r);const pending=c.inspectArtifact(q);c.dispose();resolve(response);await assert.rejects(()=>pending);
});

test('saved artifact controller retains private expected identity with a custom adapter',async t=>{
 const {createHash}=await import('node:crypto');const f=fixture();const text='answer';const sha=createHash('sha256').update(text).digest('hex');f.addRun({id:'r',conversationID:'chat',prompt:'p'},'completed').answer=text;
 f.snapshot.artifacts=[{schemaVersion:1,artifactID:'a',conversationID:'chat',requestID:'r',origin:{kind:'finalAnswer',memberID:null,providerID:'codex'},displayName:'Final answer',previewKind:'plainText',languageHint:null,byteLength:Buffer.byteLength(text),contentSHA256:sha,availability:'available',createdAt:'today',supersedesArtifactID:null}];
 f.snapshot.artifacts.push({...f.snapshot.artifacts[0],artifactID:'b'});
 f.bridge.inspectArtifact=async q=>{q.artifactID='b';return {schemaVersion:1,artifactID:'b',conversationID:'chat',requestID:'r',contentSHA256:sha,previewKind:'plainText',languageHint:null,byteLength:Buffer.byteLength(text),availability:'available',text};};const c=f.controller();t.after(()=>c.dispose());await c.start();await assert.rejects(()=>c.inspectArtifact({artifactID:'a',conversationID:'chat',requestID:'r',expectedSHA256:sha}));
});

for(const timeline of ['accepted','newer','newer-distinct','uncertain','rejected','lost-reply','switch','restart'])test(`sent draft admission timeline: ${timeline}`,async t=>{
 const f=fixture();let request;let finish;const gate=new Promise(r=>finish=r);
 const admitted=q=>{const chat=f.snapshot.conversations.find(c=>c.id===q.conversationID);assert.equal(chat.richDraft.revision,q.richDraftRevision);assert.equal(chat.draft,q.prompt);f.addRun(q);chat.draft='';chat.richDraft.revision++;};
 f.bridge.submitRun=async q=>{request=q;await gate;if(timeline==='rejected')return{state:'rejected',requestID:q.id};if(timeline!=='uncertain')admitted(q);if(['lost-reply','restart'].includes(timeline))throw Error('lost reply');return{state:timeline==='uncertain'?'uncertain':'accepted',requestID:q.id};};
 const c=f.controller();t.after(()=>c.dispose());await c.start();c.editDraft('chat','Sent exactly');const sending=c.send('chat');while(!request)await flush();
 assert.equal(c.getState().drafts.chat,'Sent exactly');const savedRevision=request.richDraftRevision;
 if(timeline.startsWith('newer')){c.editDraft('chat','replacement');if(timeline==='newer')c.editDraft('chat','Sent exactly');}
 if(timeline==='switch'){f.snapshot.conversations.push({id:'other',title:'Other',readOnly:false,draft:'Other draft',richDraft:{schemaVersion:1,revision:0,attachmentIDs:[],selection:null,team:null}});f.snapshot.activeConversationID='other';const other=f.controller();await other.start();other.dispose();}
 finish();await sending;
 if(timeline==='uncertain'){assert.equal(c.getState().drafts.chat,'Sent exactly');await c.reconcile();assert.equal(c.getState().drafts.chat,'Sent exactly');admitted(request);await c.reconcile();}
 if(timeline==='lost-reply'){assert(c.getState().pending);await c.reconcile();await c.reconcile();}
 if(timeline==='restart'){c.dispose();const restarted=f.controller();t.after(()=>restarted.dispose());await restarted.start();assert.equal(restarted.getState().drafts.chat,'');assert.equal(f.snapshot.conversations[0].richDraft.revision,savedRevision+1);return;}
 if(timeline==='rejected'){assert.equal(f.snapshot.runs.length,0);assert.equal(c.getState().drafts.chat,'Sent exactly');assert.equal(f.snapshot.conversations[0].richDraft.revision,savedRevision);return;}
 assert.equal(f.snapshot.runs.length,1);assert.equal(f.snapshot.conversations[0].draft,'');assert.equal(f.snapshot.conversations[0].richDraft.revision,savedRevision+1);
 if(timeline.startsWith('newer')){assert.equal(c.getState().drafts.chat,timeline==='newer'?'Sent exactly':'replacement');await c.saveDraft('chat');const save=f.calls.filter(x=>x[0]==='save').at(-1)[1];assert.equal(save.expectedRevision,savedRevision+1);}
 else assert.equal(c.getState().drafts.chat,'');
 if(timeline==='switch')assert.equal(c.getState().drafts.other,'Other draft');
});
