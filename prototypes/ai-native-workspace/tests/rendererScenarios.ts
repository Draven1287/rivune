...
export function rendererScenarioSelection(search: string): { ids: readonly RendererScenario[] | null; error: string | null } {
  ...
  if (values.length === 1 && ['quiet-workspace','startup-fallback','connection-handoff','sent-draft-mounted','composer-supersession-preview','conversation-list','configuration-recovery','follow-default','conversation-binding','provider-refresh','provider-empty','provider-success','provider-setup','provider-stale','provider-missing','connection-guidance', ...resultsScenarioIDs, ...teamFormScenarioIDs, ...invocationRetryScenarioIDs].some(id => id === value)) return { ids: [value as RendererScenario], error: null };
