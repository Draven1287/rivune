...
export function ProviderSetup({bridge,disabled,onConfirmed,onContinue}:{bridge:unknown;disabled:boolean;onConfirmed:()=>Promise<void>;onContinue?:()=>void}){
 ...
 async function inspect(){
  ...
  setHandoffConfirmed(false);busy.current=true;setPhase('inspecting');setRoutes([]);setSelected('');setSnapshot(null);
 ...
 async function configure(){
  ...
  setHandoffConfirmed(false);busy.current=true;setPhase('saving');setNotice('Saving the selected default connection…');
  try{const result=await adapter.configure(route,snapshot,true);
   if(result.state==='durable'){setHandoffConfirmed(true);setRefreshContext('Default connection saved durably.');setDurabilityUnconfirmed(false);setPhase('idle');setRoutes([]);setSelected('');setNotice('Default connection saved. Sign-in and a successful response are still unverified. Continue to chat when you are ready; no message is sent automatically.');intended.current=null;try{await onConfirmed();setRefreshNeeded(false);}catch{setRefreshNeeded(true);setNotice('Default connection saved durably. The workspace display could not refresh; retry the display refresh without saving again.');}}
 ...
 return <section className="provider-setup" aria-labelledby="provider-setup-title"><h3 id="provider-setup-title">Add a connection</h3>
  ...
  {!available?<p>This host does not support guarded connection setup. Reopen with a compatible host; no fallback setup will run.</p>:<>
   ...
   {handoffConfirmed&&onContinue&&<button type="button" disabled={blocked||refreshNeeded||durabilityUnconfirmed} onClick={()=>{if(!busy.current&&!currentDisabled.current&&!refreshNeeded&&!durabilityUnconfirmed)onContinue();}}>Continue to chat</button>}
