...
root=Path.cwd();candidate=root/'tools/symphony/source-import-candidate';out=root/'qa-artifacts/startup-handoff-native-build-20260910';frontend=candidate/'prototypes/ai-native-workspace';native=candidate/'qa-artifacts/cross-platform-shell-20260907/candidate4-runtime-r2/src-tauri'
expected='2ea834a1e2385c34b8f516854b5317c6a08a0607';manifest=json.loads((root/'tools/symphony/source-stage/SOURCE_MANIFEST.json').read_text());sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
assert manifest['fileCount']==148 and manifest['treeSHA256']=='171ccf39b8cbee0189968505950f1085e12d5b261ecfeb61813e2e878d2c7f28'
...
def preserved():
 paths=[]
 # Hash prior QA app bundles and profile directories without following symlinks.
 for folder,dirs,files in os.walk(root/'qa-artifacts'):
  p=Path(folder)
  if p==out:dirs[:]=[];continue
  if p.name.endswith('.app') or 'profile' in p.name.lower():
   paths.extend(f for f in p.rglob('*') if f.is_file() and not f.is_symlink());dirs[:]=[]
 installed=Path('/Applications/Rivune.app')
 if installed.exists():paths.extend(f for f in installed.rglob('*') if f.is_file() and not f.is_symlink())
 paths.extend(root/f['path'] for f in manifest['files'] if (root/f['path']).is_file())
 return {str(p):sha(p) for p in sorted(set(paths))}
assert not (out/'build-receipt.json').exists();before=identity();save('source-before.json',before);save('source-manifest.json',manifest);(out/'processes-before.txt').write_text('No process inventory or process-control commands requested or issued.\n');old=preserved();save('preserved-before.json',old)
config={'productName':'Rivune Startup Handoff QA','identifier':'com.rivune.desktop.qa.startuphandoff20260910','build':{'frontendDist':str(out/'frontend-dist')},'app':{'windows':[{'label':'main','title':'Rivune Startup Handoff QA · build review only','width':1200,'height':900,'minWidth':760,'minHeight':560,'resizable':True}]}}
...
try:
 ...
 app=out/'Rivune Startup Handoff QA.app';binary=app/'Contents/MacOS/rivune';binary.parent.mkdir(parents=True);(app/'Contents/Resources').mkdir();shutil.copy2(root/'.toolchains/target-candidate4-r2/debug/rivune',binary);shutil.copy2(native/'icons/icon.icns',app/'Contents/Resources/icon.icns')
 info={'CFBundleDisplayName':'Rivune Startup Handoff QA','CFBundleName':'Rivune Startup Handoff QA','CFBundleExecutable':'rivune','CFBundleIdentifier':config['identifier'],'CFBundlePackageType':'APPL','CFBundleShortVersionString':'0.0.1','CFBundleVersion':'2026091006','CFBundleIconFile':'icon.icns','NSHighResolutionCapable':True,'LSMinimumSystemVersion':'11.0','LSEnvironment':{'RIVUNE_ISOLATED_PROFILE_DIR':str(out/'profile-reserved-not-launched'),'PATH':'/usr/bin:/bin'}}
 ...
 after=identity();save('source-after.json',after);receipt['sourceUnchanged']=before==after;now=preserved();save('preserved-after.json',now);receipt['priorBinariesProfilesUnchanged']=old==now;(out/'processes-after.txt').write_text('No process inventory or process-control commands requested or issued.\n');receipt['completedAtUTC']=datetime.datetime.now(datetime.timezone.utc).isoformat();save('build-receipt.json',receipt)
