...
final class CouncilSavedIntegrityTests: XCTestCase {
    ...
    func testCorruptNestedJournalRejectedBeforeAnyMutation() async throws {
        let team = TeamTestFactory.team()
        let participants = try team.participants(admittedBy: TeamTestFactory.evidence(team))
        for corruption in ["duplicate", "foreign", "conflict", "runID", "turnID", "prompt", "zero", "one", "runningOne", "runningRoute"] {
            let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
            defer { try? FileManager.default.removeItem(at: directory) }
            try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
            let file = directory.appendingPathComponent("runs.json")
            ...
            if ["zero", "one", "runningOne", "runningRoute"].contains(corruption) {
                ...
                if corruption == "runningRoute" {
                    savedIdentities.append(.init(id: AIExecutionRoute.claudeCodeCLI.transportID, providerID: "forged", adapterID: "forged", modelID: nil, displayName: "Claude"))
                }
