...
enum WorkspaceRunError: LocalizedError {
    case duplicateConflict, deletedRequest, busy, storageUnavailable, journalFull, modeUnavailable
...
final class RivuneRunCoordinator: ObservableObject {
    ...
    private let teamEvidence: () -> TeamAdmissionEvidence
        ...
        do {
            ...
            for run in runs {
                ...
                guard run.turn.mode == .council, council.id == run.id,
                      council.turnID == run.turn.id, council.prompt == run.turn.prompt else {
    ...
    func submit(
        ...
        guard turn.mode != .swarm, turn.mode != .council || (codexRoute == .codexCLI && claudeRoute == .claudeCodeCLI) else { throw WorkspaceRunError.modeUnavailable }
        ...
        if let run = try existing(id: id, requestKey: requestKey) {
            guard run.turn.councilRun?.teamConfiguration == teamConfiguration else { throw WorkspaceRunError.duplicateConflict }
            return run
        }
        ...
        if let teamConfiguration {
            // Persist frozen team before publishing running state or any call.
            var frozen = CouncilRunRecord(id: id, turnID: turn.id, prompt: turn.prompt,
                approvedContext: priorContext + "\n" + (RivuneStore.preparedAttachmentContext(turn.attachments) ?? ""),
                criteria: CouncilRunner.defaultCriteria, participants: configuredParticipants!.map(\.identity))
        ...
        let run = WorkspaceRun(id: id, conversationID: conversationID, requestKey: requestKey,
                               turn: admittedTurn, status: .running, stage: .asking, updatedAt: .now)
        runs.append(run)
        ...
        tasks[id] = Task { [weak self] in
            ...
            if turn.mode == .council {
                ...
                let request = CouncilRequest(runID: id, turnID: turn.id, prompt: turn.prompt, approvedContext: context, criteria: CouncilRunner.defaultCriteria, participants: participants, teamConfiguration: teamConfiguration)
    ...
    func retryCouncil(_ id: UUID) throws {
        ...
        guard runs[index].turn.mode == .council, previous.id == runs[index].id,
              previous.turnID == runs[index].turn.id, previous.prompt == runs[index].turn.prompt else {
            throw CouncilRecordValidationError.invalidSavedResults
        }
        ...
        let request = CouncilRequest(runID: id, turnID: previous.turnID, prompt: previous.prompt,
            approvedContext: previous.approvedContext, criteria: previous.criteria, participants: participants, previous: previous, teamConfiguration: previous.teamConfiguration)
        let gated = GatedCouncilTextRunner(base: textRunner, gate: councilGate)
        tasks[id] = Task { [weak self] in
            let record = await CouncilRunner(textRunner: gated).run(request) { [weak self] record in
