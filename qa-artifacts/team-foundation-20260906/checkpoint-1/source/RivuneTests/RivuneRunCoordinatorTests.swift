...
private enum TeamTestFactory {
    ...
    @MainActor static func submit(_ coordinator: RivuneRunCoordinator, team: TeamRunConfiguration, id: UUID = UUID(), conversation: UUID = UUID()) throws -> WorkspaceRun {
        try coordinator.submit(id: id, conversationID: conversation, requestKey: id.uuidString,
            turn: .init(prompt: "Frozen team task", mode: .council), priorContext: "Only original supplied context",
