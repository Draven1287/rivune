...
root=Path.cwd();candidate=root/'tools/symphony/source-import-candidate';out=root/'qa-artifacts/sent-draft-native-build-20260910';frontend=candidate/'prototypes/ai-native-workspace';native=candidate/'qa-artifacts/cross-platform-shell-20260907/candidate4-runtime-r2/src-tauri'
expected='c84cc2d07fffc1050c2be52ef31b62fbd3c1181a';manifest=json.loads((root/'tools/symphony/source-stage/SOURCE_MANIFEST.json').read_text());sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
assert manifest['fileCount']==148 and manifest['treeSHA256']=='ac15ded7187b8c85fbbfb600bfc196e4e55234f2d2c8132e41fbcb0584acacc8'
...
config={'productName':'Rivune Sent Draft QA','identifier':'com.rivune.desktop.qa.sentdraft20260910','build':{'frontendDist':str(out/'frontend-dist')},'app':{'windows':[{'label':'main','title':'Rivune Sent Draft QA · build review only','width':1200,'height':900,'minWidth':760,'minHeight':560,'resizable':True}]}}
...
try:
 ...
 app=out/'Rivune Sent Draft QA.app';binary=app/'Contents/MacOS/rivune';binary.parent.mkdir(parents=True);(app/'Contents/Resources').mkdir();shutil.copy2(root/'.toolchains/target-candidate4-r2/debug/rivune',binary);shutil.copy2(native/'icons/icon.icns',app/'Contents/Resources/icon.icns')
 info={'CFBundleDisplayName':'Rivune Sent Draft QA','CFBundleName':'Rivune Sent Draft QA','CFBundleExecutable':'rivune','CFBundleIdentifier':config['identifier'],'CFBundlePackageType':'APPL','CFBundleShortVersionString':'0.0.1','CFBundleVersion':'2026091005','CFBundleIconFile':'icon.icns','NSHighResolutionCapable':True,'LSMinimumSystemVersion':'11.0','LSEnvironment':{'RIVUNE_ISOLATED_PROFILE_DIR':str(out/'profile-reserved-not-launched'),'PATH':'/usr/bin:/bin'}}
