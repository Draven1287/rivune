...
class StaticOutputTests(unittest.TestCase):
    ...
    def test_current_page_has_no_dmg_or_fake_download(self):
        ...
        self.assertIn('No verified Tauri download exists for macOS, Windows, or Linux.', app)
    ...
    def test_installer_readiness_does_not_enable_pending_modes(self):
        for output in (self.output, self.fixture):
            with self.subTest(installer_state=output.name):
                ...
                self.assertIn('First Tauri milestone · not verified', text)
                self.assertIn('Legacy Mac development only', text)
                self.assertIn('Tauri port pending', text)
                self.assertIn('No live Tauri provider request has been verified yet.', text)
    ...
    def test_tauri_direction_is_explicit_without_transferring_legacy_readiness(self):
        pages = {route: (self.output / route / 'index.html').read_text()
                 for route in ('app', 'how-it-works', 'faq', 'download', 'privacy', 'council-vs-swarm')}
        home = (self.output / 'index.html').read_text()
        self.assertIn('Tauri desktop app in development for macOS, Windows, and Linux', home)
        self.assertIn('No live Tauri provider request has been verified yet.', pages['how-it-works'])
        self.assertIn('No accepted macOS, Windows, or Linux installer exists', pages['council-vs-swarm'])
