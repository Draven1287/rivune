...
Copy follows the current product direction: one Tauri desktop app is in development for macOS, Windows, and Linux. No built and running Tauri app, distributable installer, or live Tauri provider request has been verified yet. The legacy SwiftUI Mac source and installed legacy build do not establish Tauri readiness. Normal AI through the real host pipeline is the first milestone; Council and Swarm ports remain pending. The public source preview predates this architecture. No superiority, commercial pricing, or cloud availability claim is made.
