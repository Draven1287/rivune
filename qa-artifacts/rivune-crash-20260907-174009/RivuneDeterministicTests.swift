...
final class TranscriptScrollPolicyTests: XCTestCase {
    func testExistingTurnUpdatesFollowBottomWithoutRewindingToTurnStart() {
        XCTAssertEqual(
            TranscriptScrollPolicy.action(
                newTurnAdded: false,
                completedTogetherTurnID: nil,
                isNearBottom: true
            ),
            .transcriptBottom
        )
    ...
    func testNewTurnCompletionAndReaderPositionRemainDistinct() {
        let turnID = UUID()
        XCTAssertEqual(
            TranscriptScrollPolicy.action(newTurnAdded: true, completedTogetherTurnID: nil, isNearBottom: true),
            .latestTurnStart
        )
        XCTAssertEqual(
            TranscriptScrollPolicy.action(newTurnAdded: false, completedTogetherTurnID: turnID, isNearBottom: true),
            .finalAnswer(turnID)
        )
        XCTAssertEqual(
            TranscriptScrollPolicy.action(newTurnAdded: false, completedTogetherTurnID: turnID, isNearBottom: false),
            .showNewContent
        )
        XCTAssertEqual(
            TranscriptScrollPolicy.action(newTurnAdded: false, completedTogetherTurnID: nil, isNearBottom: false),
            .showNewContent
        )
    }
}
...
#if os(macOS)
@MainActor
private final class RivuneRecoverableTestWindow: NSWindow {
    override var canBecomeMain: Bool { true }
}

@MainActor
final class RivuneLifecycleRecorderTests: XCTestCase {
    func testSessionRecordsLaunchAndOrderlyTerminationWithoutPrivateContent() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let recorder = RivuneLifecycleRecorder(directory: root, now: { Date(timeIntervalSince1970: 1_700_000_000) }, processID: 42, build: "fixture-build", isProcessAlive: { _ in false })
        recorder.beginSession(); XCTAssertTrue(recorder.hasActiveMarker)
        recorder.record(.windowVisible); recorder.finishSession()
        XCTAssertFalse(recorder.hasActiveMarker)
        XCTAssertEqual(recorder.events().map(\.kind), [.launch, .windowVisible, .orderlyTermination])
        let text = try XCTUnwrap(String(data: JSONEncoder().encode(recorder.events()), encoding: .utf8))
        XCTAssertTrue(text.contains("fixture-build")); XCTAssertFalse(text.contains("prompt")); XCTAssertFalse(text.contains("attachment")); XCTAssertFalse(text.contains("token"))
    }

    func testDeadMarkerUsesPriorIdentityAndSessionsCannotDeleteEachOther() {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let first = RivuneLifecycleRecorder(directory: root, processID: 101, build: "old", isProcessAlive: { _ in false })
        first.beginSession(); let oldSession = first.sessionID
        let second = RivuneLifecycleRecorder(directory: root, processID: 202, build: "new", isProcessAlive: { _ in false })
        second.beginSession()
        let prior = second.events().dropLast().last
        XCTAssertEqual(prior?.kind, .previousSessionUnclosed); XCTAssertEqual(prior?.processID, 101); XCTAssertEqual(prior?.build, "old"); XCTAssertEqual(prior?.sessionID, oldSession)
        first.finishSession(); XCTAssertTrue(second.hasActiveMarker, "One session must never delete another session's marker")
        second.finishSession(); XCTAssertFalse(second.hasActiveMarker)
    }

    func testLogIsBoundedAndOnlyWorkspaceWindowIsRecoverable() {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let recorder = RivuneLifecycleRecorder(directory: root, processID: 12, build: "c", limit: 16, isProcessAlive: { _ in false })
        recorder.beginSession(); for _ in 0..<30 { recorder.record(.becameActive) }; XCTAssertEqual(recorder.events().count, 16)
        let workspace = RivuneRecoverableTestWindow(contentRect: .init(x: 0, y: 0, width: 300, height: 200), styleMask: [.titled, .closable], backing: .buffered, defer: false)
        workspace.title = RivuneBrand.displayName; workspace.orderOut(nil)
        let settings = NSWindow(contentRect: .init(x: 0, y: 0, width: 300, height: 200), styleMask: [.titled, .closable], backing: .buffered, defer: false)
        settings.title = "Rivune Settings"; settings.orderOut(nil)
        XCTAssertTrue(RivuneWindowRecovery.recoverableWorkspaceWindow(in: [settings, workspace]) === workspace)
        workspace.title = "Conversation content"
        XCTAssertNil(RivuneWindowRecovery.recoverableWorkspaceWindow(in: [settings, workspace]))
    }

    func testRealDelegateReopensHiddenWorkspaceWindowAndRecordsRequest() {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString, isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let recorder = RivuneLifecycleRecorder(directory: root, processID: 303, build: "delegate", isProcessAlive: { _ in false })
        let delegate = RivuneTerminationDelegate(lifecycle: recorder, allowsIsolatedCallbacks: true)
        recorder.beginSession()
        let window = RivuneRecoverableTestWindow(contentRect: .init(x: 0, y: 0, width: 300, height: 200), styleMask: [.titled, .closable], backing: .buffered, defer: false)
        window.title = RivuneBrand.displayName; window.orderFront(nil); window.orderOut(nil)
        XCTAssertTrue(NSApplication.shared.windows.contains(where: { $0 === window }))
        XCTAssertTrue(delegate.applicationShouldHandleReopen(.shared, hasVisibleWindows: false))
        XCTAssertTrue(window.isVisible)
        XCTAssertEqual(recorder.events().last?.kind, .reopenRequested)
        window.close(); recorder.finishSession()
    }
}
#endif
