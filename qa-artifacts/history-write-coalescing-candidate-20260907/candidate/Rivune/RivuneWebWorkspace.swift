...
extension IntelligenceMode {
    var workspaceID: String {
        switch self { case .chatGPT: "codex"; case .claude: "claude"; case .together: "rivune"; case .council: "council"; case .swarm: "swarm" }
    }
...
enum NativeSettingsDestination: String, Codable, CaseIterable, Sendable {
    case connections, models, privacy, devices, account, general, appearance, about

    static func destination(from url: URL) -> Self? {
        guard url.scheme?.lowercased() == "rivune", url.host?.lowercased() == "settings",
              url.user == nil, url.password == nil, url.port == nil, url.query == nil, url.fragment == nil else { return nil }
        let components = url.path.split(separator: "/")
        if components.isEmpty { return .general }
        guard components.count == 1 else { return nil }
        return Self(rawValue: String(components[0]))
    }
}
...
extension RivuneStore {
    ...
    func resolveWorkspaceModelSelections(_ selections: [String: BrowserModelSelection]?, for mode: IntelligenceMode) throws -> WorkspaceResolvedModelOptions {
        ...
        let relevantProviders: Set<String> = (mode == .together || mode == .council) ? ["codex", "claude"] : [mode.workspaceID]
    ...
    func submitWorkspaceRun(
        ...
        if let existing = try runCoordinator.existing(id: id, requestKey: requestKey) {
            guard existing.turn.selectedArtifact == selectedArtifact else { throw WorkspaceRunError.duplicateConflict }
            return existing
        }
        if let selectedArtifact {
            guard isWorkspaceDraftReadable else { throw ArtifactContinuationError.unreadableDraft }
            try validateArtifactSource(selectedArtifact)
        }
        guard !historyArtifactRecoveryRequired else { throw ArtifactContinuationError.unreadableHistory }
        ...
        let promptContext = ApprovedPromptContext(
            priorConversation: memoryEnabled
                ? Self.roleAwareConversationContext(from: existingTurns.filter { $0.id != selectedArtifact?.sourceTurnID }, mode: requestMode)
                : .disabled,
            approvedProjectInstructions: approvedProjectInstructions,
            userSelectedDocuments: attachments,
            selectedArtifactReference: selectedArtifact
        )
        let turn = ChatTurn(prompt: cleanPrompt, mode: requestMode, attachments: attachments,
                            togetherTrace: requestMode == .together ? TogetherTrace(phase: .planning) : nil, selectedArtifact: selectedArtifact)
        return try runCoordinator.submit(
            ...
            teamConfiguration: requestMode == .council ? savedTeam?.configuration : nil
        )
    }
    ...
    func mergeWorkspaceRun(_ run: WorkspaceRun, persist: Bool = true) {
        ...
        workspaceRevision += 1
        markWorkspaceHistoryProjectionDirty()
        if persist, run.status != .running { flushWorkspaceHistoryProjection() }
    }
    ...
    func handleWorkspaceRequest(_ request: LocalWorkspaceRequest) -> LocalWorkspaceResponse {
        do {
            ...
            if request.method == "POST", request.path == "/v1/runs" {
                ...
                if (mode == .together || mode == .council) && !input.shareWithTeam { throw WorkspaceInputError.sharingRequired }
...
struct WorkspaceRunDTO: Encodable {
    ...
    @MainActor init(run: WorkspaceRun) {
        ...
        case .together, .council, .swarm: fullResult = run.turn.combinedAnswer?.content; error = run.turn.combinedError
        }
