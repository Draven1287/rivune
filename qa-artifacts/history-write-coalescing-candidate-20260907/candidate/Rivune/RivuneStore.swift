...
final class RivuneStore: ObservableObject {
    ...
    private let conversationHistorySaver: ([Conversation], RivuneHistoryStorage.SaveMode) -> Bool
    private(set) var workspaceHistoryProjectionDirty = false
    ...
    func saveAdmissionForTermination() -> RivuneSaveAdmission {
        RivuneSaveAdmission.evaluate(active: hasActiveProviderRuns || loadingProjects,
            ...
            history: { self.flushWorkspaceHistoryProjection(force: true) },
            projects: {
    ...
    private struct CachedRemoteResult {
        let fingerprint: String
        let update: BridgePromptUpdate
    }
    private var remoteResultCache: [UUID: CachedRemoteResult] = [:]
    private var remoteInFlightFingerprints: [UUID: String] = [:]
    private var remoteInFlightUpdates: [UUID: BridgePromptUpdate] = [:]
    ...
    init(
        ...
        conversationHistorySaver: (([Conversation], RivuneHistoryStorage.SaveMode) -> Bool)? = nil
    ) {
        self.bridge = bridge
        self.remoteTextRunnerOverride = remoteTextRunner
        self.conversationHistorySaver = conversationHistorySaver ?? { conversations, mode in
            guard !RivuneLaunchContext.isIsolated else { return true }
            return RivuneHistoryStorage.save(conversations, mode: mode)
        }
        ...
        bridge.onConnectionChanged = { [weak self] connected in
            Task { @MainActor [weak self] in
                ...
                if connected {
                    ...
                    self.remoteInFlightFingerprints.removeAll()
                    self.remoteInFlightUpdates.removeAll()
                    #else
    ...
    private func handleBridgeEnvelope(_ envelope: BridgeEnvelope) {
        ...
        case .cancel:
            ...
            remoteInFlightFingerprints.removeValue(forKey: requestID)
            remoteInFlightUpdates.removeValue(forKey: requestID)
            #endif
        }
    }
        ...
        try? bridge.send(
            .readiness(
                BridgeReadiness(
                    ...
                    codexRoute: codexReadiness.isReady ? selectedCodexRoute : nil,
                    claudeRoute: claudeReadiness.isReady ? selectedClaudeRoute : nil
                )
            )
        )
        #endif
    ...
    func handleRemoteRequest(_ request: BridgePromptRequest) -> RemoteRequestDisposition {
        let fingerprint = Self.remoteRequestFingerprint(request)
        if let cached = remoteResultCache[request.id] {
            ...
            sendBridgeUpdate(cached.update)
            return .replayed
        }
        if let activeFingerprint = remoteInFlightFingerprints[request.id] {
            guard activeFingerprint == fingerprint else {
                sendBridgeUpdate(Self.remoteFailureUpdate(request, rejection: .requestIDConflict))
                return .rejected(.requestIDConflict)
            }
            if let update = remoteInFlightUpdates[request.id] {
        ...
        if let rejection = Self.remoteAdmissionRejection(for: request) {
            cacheAndSendRemoteUpdate(
                Self.remoteFailureUpdate(request, rejection: rejection),
        ...
        if request.mode == .council || request.mode == .swarm {
            ...
            turn.combinedError = "This mode is not yet available through iPhone pairing. Use Council on your Mac."
            cacheAndSendRemoteUpdate(
                .init(requestID: request.id, stage: .failed, turn: turn, isComplete: true),
                fingerprint: fingerprint
            )
        ...
        guard request.contextVersion == BridgePromptRequest.currentContextVersion,
            ...
            cacheAndSendRemoteUpdate(
                Self.remoteFailureUpdate(request, rejection: .legacyConversationFormat),
                fingerprint: fingerprint
            )
            return .rejected(.legacyConversationFormat)
        }
        let admittedCodexRoute = selectedCodexRoute
        let admittedClaudeRoute = selectedClaudeRoute
        let routeMatches = switch request.mode {
        case .chatGPT:
            codexReadiness.isReady
                && admittedCodexRoute != nil
                && request.codexRoute == admittedCodexRoute
        case .claude:
            claudeReadiness.isReady
                && admittedClaudeRoute != nil
                && request.claudeRoute == admittedClaudeRoute
        case .together:
            codexReadiness.isReady
                && claudeReadiness.isReady
                && admittedCodexRoute != nil
        ...
        guard routeMatches else {
            cacheAndSendRemoteUpdate(
                Self.remoteFailureUpdate(
                    request,
                    rejection: .incompatibleExecutionRoute
                ),
                fingerprint: fingerprint
            )
        ...
        guard remoteGenerationTasks.count < 2 else {
            ...
            case .together, .council, .swarm: busyTurn.combinedError = busyMessage
            }
            let update = BridgePromptUpdate(
                requestID: request.id,
                stage: .failed,
            ...
            cacheAndSendRemoteUpdate(update, fingerprint: fingerprint)
            return .rejected(.hostBusy)
        }

        remoteInFlightFingerprints[request.id] = fingerprint
        let executionCodexRoute = request.codexRoute ?? .codexCLI
        let executionClaudeRoute = request.claudeRoute ?? .claudeCodeCLI
        let task = Task { [weak self] in
            guard let self else { return }
            await executeRemoteRequest(
                request,
                codexRoute: executionCodexRoute,
                claudeRoute: executionClaudeRoute
            )
            remoteGenerationTasks.removeValue(forKey: request.id)
            remoteInFlightFingerprints.removeValue(forKey: request.id)
            remoteInFlightUpdates.removeValue(forKey: request.id)
        }
    ...
    private func executeRemoteRequest(
        ...
        codexRoute: AIExecutionRoute,
        claudeRoute: AIExecutionRoute
    ) async {
        guard let roleAwareContext = request.roleAwareContext else { return }
        var turn = ChatTurn(
            id: request.turnID,
            prompt: request.prompt,
            mode: request.mode,
        ...
        let codexProvenance = executionDescription(
            route: codexRoute,
            provider: .chatGPT,
            requestedModel: request.codexModel.title,
            requestedEffort: request.codexEffort.title
        )
        let claudeProvenance = executionDescription(
            route: claudeRoute,
            provider: .claude,
            requestedModel: request.claudeModel.title,
            requestedEffort: request.claudeEffort.title
        )
        ...
        case .chatGPT, .claude:
            ...
            let selectedRoute = isCodex ? codexRoute : claudeRoute
            let attempt = await Self.attempt(
                service: remoteTextRunnerOverride ?? terminalService,
                route: selectedRoute,
                prompt: answerPrompt,
                options: selectedRoute.transportKind == .api
                    ? .accountDefault
                    : (isCodex ? codexOptions : claudeOptions)
            )
        ...
        case .together, .council, .swarm:
            let runner = RivuneCollaborationRunner(
                textRunner: RouteMappedTextRunner(
                    base: remoteTextRunnerOverride ?? terminalService,
            ...
            let collaborationRequest = RivuneCollaborationRequest(
                ...
                prompt: request.prompt,
                priorContext: Self.encodedPromptContext(roleAwareContext),
                attachments: request.attachments,
    ...
    private func sendRemoteUpdate(
        ...
        remoteInFlightUpdates[requestID] = update
        sendBridgeUpdate(update)
    }

    private func cacheAndSendRemoteUpdate(
        _ update: BridgePromptUpdate,
        fingerprint: String? = nil
    ) {
        let fitted = Self.fittedBridgeUpdate(update)
        let resolvedFingerprint = fingerprint ?? remoteInFlightFingerprints[fitted.requestID]
        if let resolvedFingerprint {
            remoteResultCache[fitted.requestID] = CachedRemoteResult(
    ...
    func newChat() {
        flushWorkspaceHistoryProjection()
    ...
    func saveConversations(
        ...
        guard conversationHistorySaver(conversations, mode) else {
            showPrototypeNotice(
                "Rivune could not save this history change. Check available storage and try again."
            )
            return false
        }
        workspaceHistoryProjectionDirty = false
        return true
    }

    func markWorkspaceHistoryProjectionDirty() {
        workspaceHistoryProjectionDirty = true
    }

    @discardableResult
    func flushWorkspaceHistoryProjection(force: Bool = false) -> Bool {
        guard force || workspaceHistoryProjectionDirty else { return true }
