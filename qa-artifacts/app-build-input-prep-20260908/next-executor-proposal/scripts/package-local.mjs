...
import { collectAndValidateMacArtifact, deriveMacArtifactPolicy, preflightMacArtifactValidation } from "./mac-artifact-evidence.mjs";
...
const TAURI_CLI_SCHEMA = "rivune-workspace-tauri-cli-v1";
const TAURI_CLI_VERSION = "tauri-cli 2.11.4";
const MAC_FEATURES = ["custom-protocol"];
const ACCEPTED_BUILD_ENVIRONMENT = ["CARGO_HOME", "RUSTUP_HOME", "CARGO_TARGET_DIR", "SDKROOT", "PATH", "CARGO_NET_OFFLINE"];
const BUILD_OVERRIDE_KEYS = new Set([
  "CC", "CXX", "CFLAGS", "CXXFLAGS", "CPPFLAGS", "LDFLAGS", "AR",
  "MACOSX_DEPLOYMENT_TARGET", "DEVELOPER_DIR", "NODE_OPTIONS", "NODE_PATH"
]);

function isBuildOverride(key) {
  return key.startsWith("CARGO_") || key.startsWith("RUST") || key.startsWith("TAURI_") ||
    key.startsWith("APPLE_") || key.startsWith("DYLD_") || BUILD_OVERRIDE_KEYS.has(key);
}

export function sanitizeBuildEnvironment(environment, accepted) {
  const sanitized = {};
  for (const [key, value] of Object.entries(environment || {})) {
    if (!isBuildOverride(key)) sanitized[key] = value;
  }
  for (const key of ACCEPTED_BUILD_ENVIRONMENT) sanitized[key] = accepted[key];
  return sanitized;
}

function macTargetTriple(architecture) {
  if (architecture === "arm64") return "aarch64-apple-darwin";
  throw new Error(`Installed Tauri CLI receipt supports only the current arm64 Mac, not ${architecture}`);
}
...
async function assertFileHash(path, acceptedHash, label) {
  const actual = await sha256File(path).catch(() => null);
  if (!actual || actual !== requireHash(acceptedHash, label)) throw new Error(`${label} does not match`);
  return actual;
}

export async function validateMacToolchain(binding, env) {
  if (!binding || typeof binding !== "object") throw new Error("Mac build receipt must bind toolchain");
  const receiptPath = assertAbsolute(binding.tauriCliReceiptPath, "toolchain tauriCliReceiptPath");
  await assertFileHash(receiptPath, binding.tauriCliReceiptSHA256, "Tauri CLI receipt SHA-256");
  const receipt = JSON.parse(await readFile(receiptPath, "utf8"));
  if (receipt.schema !== TAURI_CLI_SCHEMA || receipt.cliVersion !== TAURI_CLI_VERSION) {
    throw new Error("Tauri CLI receipt schema/version is not pinned");
  }
  const receiptRoot = dirname(receiptPath);
  if (receipt.executable !== "node_modules/.bin/tauri" || receipt.executableSymlinkTarget !== "../@tauri-apps/cli/tauri.js") {
    throw new Error("Tauri CLI receipt does not bind the expected executable link");
  }
  const cliPath = resolve(receiptRoot, receipt.executable);
  const wrapperPath = resolve(receiptRoot, "node_modules/@tauri-apps/cli/tauri.js");
  const mainPath = resolve(receiptRoot, "node_modules/@tauri-apps/cli/main.js");
  const nativePath = resolve(receiptRoot, "node_modules/@tauri-apps/cli-darwin-arm64/cli.darwin-arm64.node");
  if (!inside(receiptRoot, cliPath) || !inside(receiptRoot, wrapperPath) || !inside(receiptRoot, mainPath) || !inside(receiptRoot, nativePath)) {
    throw new Error("Tauri CLI receipt paths must stay inside the toolchain root");
  }
  if (await readlink(cliPath).catch(() => null) !== receipt.executableSymlinkTarget ||
      await realpath(cliPath).catch(() => null) !== await realpath(wrapperPath).catch(() => null)) {
    throw new Error("Tauri CLI executable link does not match receipt");
  }
  await assertFileHash(cliPath, receipt.files?.["node_modules/@tauri-apps/cli/tauri.js"], "Tauri CLI executable SHA-256");
  await assertFileHash(wrapperPath, receipt.files?.["node_modules/@tauri-apps/cli/tauri.js"], "Tauri CLI wrapper SHA-256");
  await assertFileHash(mainPath, receipt.files?.["node_modules/@tauri-apps/cli/main.js"], "Tauri CLI main module SHA-256");
  await assertFileHash(nativePath, receipt.files?.["node_modules/@tauri-apps/cli-darwin-arm64/cli.darwin-arm64.node"], "Tauri CLI native binding SHA-256");
  const cargoPath = assertAbsolute(binding.cargoPath, "toolchain cargoPath");
  await assertFileHash(cargoPath, binding.cargoSHA256, "Cargo executable SHA-256");
  const targetRoot = assertAbsolute(binding.cargoTargetDir, "toolchain cargoTargetDir");
  if (resolve(env.CARGO_TARGET_DIR || "") !== targetRoot) throw new Error("CARGO_TARGET_DIR does not match accepted shared target");
  return { receiptPath, receipt, cliPath, mainPath, nativePath, cargoPath, targetRoot };
}
...
export function commandFor(options = {}) {
  ...
  if (platform === "macos") {
    if (options.phase === "build-reviewable-app") {
      const tauriCliPath = assertAbsolute(options.tauriCliPath, "Tauri CLI path");
      const cargoPath = assertAbsolute(options.cargoPath, "Cargo runner path");
      return { platform, phase: options.phase, cwd, plan, command: [tauriCliPath, "build", "--runner", cargoPath, "--features", ...MAC_FEATURES, "--bundles", "app", "--", "--locked", "--offline"] };
    }
...
export async function validateExecution(options, actualOS = process.platform) {
  ...
  if (platform === "macos" && options.phase === "bundle-accepted-app") {
    ...
    const output = assertAbsolute(options.out, "--out");
    if (await lstat(output).catch(() => null)) throw new Error("Mac DMG output must not already exist");
    ...
    const exact = commandFor({ ...options, stagingRoot, out: output });
  ...
  const infoPlistPath = join(cwd, "src-tauri", "Info.plist");
  const rustHostPath = join(cwd, "src-tauri", "src", "host.rs");
  const rustMainPath = join(cwd, "src-tauri", "src", "main.rs");
  const rustLibPath = join(cwd, "src-tauri", "src", "lib.rs");
  const constellationProjectionPath = join(cwd, "src-tauri", "src", "constellation_projection.rs");
  const constellationAcceptanceTestsPath = join(cwd, "src-tauri", "src", "constellation_acceptance_tests.rs");
  const teamStrategyPath = join(cwd, "src-tauri", "src", "team_strategy.rs");
  const r5ContractFixturePath = join(cwd, "src-tauri", "examples", "r5_contract_fixture.rs");
  const webCorePath = join(cwd, "web", "core.mjs");
  const webAppPath = join(cwd, "web", "app.mjs");
  const webDesktopHostPath = join(cwd, "web", "desktop-host.mjs");
  for (const required of [tauriConfigPath, cargoManifestPath, rustBuildScriptPath, infoPlistPath, rustHostPath, rustMainPath, rustLibPath, constellationProjectionPath, constellationAcceptanceTestsPath, teamStrategyPath, r5ContractFixturePath, webCorePath, webAppPath, webDesktopHostPath]) {
  ...
  let executionEnvironment = offlineEnv(suppliedEnvironment);
  if (platform === "macos" && options.phase === "build-reviewable-app") {
    const expectedEnvironment = receipt.buildEnvironment;
    if (!expectedEnvironment || typeof expectedEnvironment !== "object") throw new Error("Mac build receipt must bind buildEnvironment");
    for (const key of ACCEPTED_BUILD_ENVIRONMENT) {
      const supplied = key === "CARGO_NET_OFFLINE" ? "true" : suppliedEnvironment[key];
      if (supplied !== expectedEnvironment[key]) throw new Error(`Build environment ${key} does not match acceptance`);
    }
    executionEnvironment = sanitizeBuildEnvironment(suppliedEnvironment, expectedEnvironment);
  }
  const cargoConfigPaths = await discoverCargoConfigs(cwd, executionEnvironment);
  ...
  let macToolchain;
  if (platform === "macos" && options.phase === "build-reviewable-app") {
    macToolchain = await validateMacToolchain(receipt.toolchain, executionEnvironment);
    if (inside(cwd, macToolchain.targetRoot)) throw new Error("Accepted shared target must remain outside the source tree");
    tool = { invocationPath: macToolchain.cargoPath, resolvedPath: await realpath(macToolchain.cargoPath) };
    const sourceInputs = receipt.sourceInputs;
    if (!sourceInputs || typeof sourceInputs !== "object") throw new Error("Mac build receipt must bind sourceInputs");
    await Promise.all([
      assertFileHash(tauriConfigPath, sourceInputs.tauriConfigSHA256, "Tauri config SHA-256"),
      assertFileHash(cargoManifestPath, sourceInputs.cargoManifestSHA256, "Cargo manifest SHA-256"),
      assertFileHash(rustBuildScriptPath, sourceInputs.rustBuildScriptSHA256, "Rust build script SHA-256"),
      assertFileHash(cargoLockPath, sourceInputs.cargoLockSHA256, "Cargo.lock SHA-256"),
      assertFileHash(infoPlistPath, sourceInputs.infoPlistSHA256, "Info.plist SHA-256"),
      assertFileHash(rustHostPath, sourceInputs.rustHostSHA256, "Rust host.rs SHA-256"),
      assertFileHash(rustMainPath, sourceInputs.rustMainSHA256, "Rust main.rs SHA-256"),
      assertFileHash(rustLibPath, sourceInputs.rustLibSHA256, "Rust lib.rs SHA-256"),
      assertFileHash(constellationProjectionPath, sourceInputs.constellationProjectionSHA256, "Constellation projection SHA-256"),
      assertFileHash(constellationAcceptanceTestsPath, sourceInputs.constellationAcceptanceTestsSHA256, "Constellation acceptance tests SHA-256"),
      assertFileHash(teamStrategyPath, sourceInputs.teamStrategySHA256, "Team strategy SHA-256"),
      assertFileHash(r5ContractFixturePath, sourceInputs.r5ContractFixtureSHA256, "R5 contract fixture SHA-256"),
      assertFileHash(webCorePath, sourceInputs.webCoreSHA256, "Web core SHA-256"),
      assertFileHash(webAppPath, sourceInputs.webAppSHA256, "Web app SHA-256"),
      assertFileHash(webDesktopHostPath, sourceInputs.webDesktopHostSHA256, "Web desktop host SHA-256")
    ]);
    const filterPlatform = macTargetTriple(actualArchitecture);
    if (JSON.stringify(receipt.metadataFeatures) !== JSON.stringify(MAC_FEATURES) || receipt.metadataFilterPlatform !== filterPlatform) {
      throw new Error("Cargo metadata feature/platform receipt does not match the Mac build");
    }
  } else {
    tool = await findCommandEvidence("cargo", { platform, env: options.env ?? process.env });
  ...
  if (macToolchain) metadataCommand.push("--features", ...MAC_FEATURES, "--filter-platform", macTargetTriple(actualArchitecture));
  const metadataRunner = options.metadataRunner ?? runCapture;
  const metadata = await metadataRunner(metadataCommand, { cwd, env: executionEnvironment });
  const productIdentity = await validateProductIdentity(cwd, tauriConfig, cargoManifestText, metadata);
  ...
  const expectedOutput = macToolchain ? join(macToolchain.targetRoot, "release", "bundle", "macos", `${productIdentity.productName}.app`) : null;
  if (expectedOutput && await lstat(expectedOutput).catch(() => null)) throw new Error("Mac app output must be fresh");
  const exact = commandFor(macToolchain ? { ...options, tauriCliPath: macToolchain.cliPath, cargoPath: macToolchain.cargoPath } : options);
  return {
    platform, phase: exact.phase, cwd, receipt, tool, command: macToolchain ? exact.command : [tool.invocationPath, ...exact.command.slice(1)],
    inputs: { tauriConfigPath, cargoManifestPath, rustBuildScriptPath, cargoLockPath, infoPlistPath, rustHostPath, rustMainPath, rustLibPath, constellationProjectionPath, constellationAcceptanceTestsPath, teamStrategyPath, r5ContractFixturePath, webCorePath, webAppPath, webDesktopHostPath, cargoConfigPaths, frontendDist: rendererRoot, productIdentity, metadataCommand, externalLocalDependencies, macToolchain, expectedOutput },
    actual: { project, renderer, host, cargoConfig }, executionEnvironment
  };
}
...
export async function executePackaging(options, actualOS = process.platform, runner = run, artifactRunner) {
  const validated = await validateExecution(options, actualOS);
  let artifactPolicy = null;
  if (validated.platform === "macos" && validated.phase === "build-reviewable-app") {
    const tauriConfig = JSON.parse(await readFile(validated.inputs.tauriConfigPath, "utf8"));
    artifactPolicy = deriveMacArtifactPolicy(tauriConfig, validated.inputs.productIdentity, validated.receipt);
    await preflightMacArtifactValidation(artifactPolicy, validated.cwd);
  }
  await runner(validated.command, { cwd: validated.cwd, env: validated.executionEnvironment ?? offlineEnv(options.env ?? process.env) });
  const artifactQualification = artifactPolicy ? await collectAndValidateMacArtifact({
    appPath: validated.inputs.expectedOutput,
    sourceRoot: validated.cwd,
    policy: artifactPolicy,
    runner: artifactRunner,
    env: validated.executionEnvironment ?? offlineEnv(options.env ?? process.env)
  }) : null;
  return { ...validated, artifactQualification };
}
...
async function main() {
  ...
  if (!options.execute) {
    console.log(JSON.stringify({ mode: "plan-only", ...commandFor(options), gate: "--execute requires filesystem-bound v5 acceptance" }, null, 2));
    return;
  }
