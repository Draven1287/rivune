...
function ConnectedHostWorkspace({ bridge }: { bridge: unknown }) {
  ...
  const [narrowResults,setNarrowResults]=useState(()=>window.matchMedia('(max-width:1000px)').matches);
  useEffect(()=>{const media=window.matchMedia('(max-width:1000px)');const update=()=>setNarrowResults(media.matches);media.addEventListener('change',update);return()=>media.removeEventListener('change',update);},[]);
