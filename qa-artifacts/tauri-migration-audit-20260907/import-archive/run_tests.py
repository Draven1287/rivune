...
files = ["Cargo.toml", "src/lib.rs", "tests/transactions.rs"]
if (crate / "Cargo.lock").exists(): files.append("Cargo.lock")
inputs = [crate / name for name in files] + sorted((crate.parent / "fixtures").glob("*.json")) + [crate.parent / "import-preview" / name for name in ["Cargo.toml", "src/lib.rs"]]
def source_hashes():
...
receipt = {"scope":"Isolated archive filesystem tests with synthetic data and owned child test processes only; no app or provider calls", "results":results, "source_hashes":after, "inputs_unchanged_during_execution":unchanged}
