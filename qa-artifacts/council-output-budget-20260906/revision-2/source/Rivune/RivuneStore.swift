...
final class RivuneStore: ObservableObject {
    ...
    var councilIsAvailable: Bool {
        #if os(macOS)
        codexReadiness.isReady && claudeReadiness.isReady && currentCodexRoute == .codexCLI && currentClaudeRoute == .claudeCodeCLI
        #else
        false
        #endif
    }
    ...
    var connectionSummary: String {
        ...
        if mode == .council && !councilIsAvailable { return "Council currently needs both Codex CLI and Claude Code CLI connected on this Mac." }
        #if os(iOS)
        guard bridge.state.isConnected else { return bridge.state.label }
        switch mode {
        case .chatGPT:
    ...
    func retry(_ turn: ChatTurn) {
        ...
        if turn.mode == .council {
            ...
            #if os(macOS)
            guard councilIsAvailable else { showPrototypeNotice("Reconnect the original Council CLI participants before retrying."); return }
