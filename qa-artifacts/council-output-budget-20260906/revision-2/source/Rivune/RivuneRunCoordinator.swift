...
final class RivuneRunCoordinator: ObservableObject {
    ...
    func submit(
        ...
        tasks[id] = Task { [weak self] in
            ...
            if turn.mode == .council {
                ...
                let request = CouncilRequest(runID: id, turnID: turn.id, prompt: turn.prompt, approvedContext: context, criteria: CouncilRunner.defaultCriteria, participants: participants)
                let record = await CouncilRunner(textRunner: runner).run(request) { [weak self] record in
    ...
    func retryCouncil(_ id: UUID) throws {
        ...
        let participants = try previous.participants.map { identity -> CouncilParticipant in
            guard let route = [AIExecutionRoute.codexCLI, .claudeCodeCLI].first(where: {
            ...
            return CouncilParticipant(identity: identity, route: route,
                options: TerminalRunOptions(model: identity.modelID, effort: identity.requestedEffort))
        }
        ...
        let request = CouncilRequest(runID: id, turnID: previous.turnID, prompt: previous.prompt,
            approvedContext: previous.approvedContext, criteria: previous.criteria, participants: participants, previous: previous)
        tasks[id] = Task { [weak self, textRunner] in
            let record = await CouncilRunner(textRunner: textRunner).run(request) { [weak self] record in
