...
struct CouncilOutputReceipt: Codable, Hashable, Sendable {
    ...
    init(kind: String, output: CouncilParticipantResult, budget: CouncilOutputBudget?) {
        ...
        wordCount = count
        wordLimitPassed = budget.flatMap { budget in count.map { $0 <= budget.maximumWords && output.error == nil } }
...
struct CouncilRunner: Sendable {
    ...
    func run(_ request: CouncilRequest,
        ...
        if let previous = request.previous {
            ...
            if previous.outputReceipts?.contains(where: { $0.wordLimitPassed == false }) == true {
                return previous
            }
            record.results = previous.results.filter { $0.text != nil && $0.error == nil }
        ...
        await withTaskGroup(of: CouncilParticipantResult.self) { group in
            ...
            for await result in group {
                ...
                if !Task.isCancelled {
                    ...
                    record.events.append(.init(message: result.text == nil ? "An answer could not finish" : "Independent answer received", participantID: result.participant.id))
                    await onUpdate(record)
        ...
        let successful = record.results.filter { $0.text != nil && $0.error == nil }
        ...
        for participant in eligible {
            ...
            if let text = result.text, result.error == nil {
                if receipt.wordLimitPassed == false {
                    ...
                    guard repairReceipt.wordLimitPassed == true, let repairedText = repaired.text, repaired.error == nil else {
    ...
    static func synthesisPrompt(_ record: CouncilRunRecord) -> String {
        let drafts = record.results.filter { $0.text != nil && $0.error == nil }.sorted { $0.participant.id < $1.participant.id }
