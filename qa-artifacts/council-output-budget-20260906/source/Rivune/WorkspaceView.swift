        ...
        DisclosureGroup {
            VStack(alignment: .leading, spacing: 14) {
                ...
                ForEach(record.results, id: \.participant.id) { result in
                    DisclosureGroup(result.participant.displayName + (result.text == nil ? " · did not finish" : " · independent answer")) {
                        Text(result.text ?? result.error ?? "No answer").textSelection(.enabled)
                            .frame(maxWidth: .infinity, alignment: .leading)
                    }
                }
            ...
            Text("Council · \(record.phase.rawValue.capitalized) · \(record.results.filter { $0.text != nil }.count) independent answers")
                .font(.subheadline)
        }
        .padding(16)
