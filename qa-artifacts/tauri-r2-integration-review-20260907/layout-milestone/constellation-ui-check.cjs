...
(async()=>{
 ...
 await p.evaluate(()=>{const run=testHost.state.runs[0];run.memberResults=[{schemaVersion:1,memberID:'member-1',providerID:'fixture:lead',role:'independentAnswer',text:'<img src=x onerror="window.projectionInjected=true"> First independent answer.',truncated:false},{schemaVersion:1,memberID:'member-2',providerID:'fixture:second',role:'independentAnswer',text:'Second independent answer.',truncated:true}];run.resolution={schemaVersion:1,kind:'leadSynthesis',reviewed:true,reviewerMemberID:'member-1',providerID:'fixture:lead',summary:'The lead synthesized the independent answers.'};window.__polls.forEach(f=>f())});await p.waitForTimeout(100);
 check('synthetic agreed projection stays collapsed beside primary answer',await p.locator('#member-contributions').isVisible()&&await p.locator('#member-contributions').evaluate(e=>!e.open));
 await p.locator('#member-contributions summary').focus();await p.keyboard.press('Enter');
 check('member body is text-safe and truncation explicit',await p.locator('#contribution-cards img').count()===0&&(await p.locator('#contribution-cards').textContent()).includes('<img src=x')&&(await p.locator('#contribution-status').textContent()).includes('shortened'));
 check('projection inspection preserves final and draft',(await p.locator('#transcript').textContent()).includes(actual.completedRun.answer.trim())&&await p.locator('#prompt').inputValue()==='Keep my follow-up');
 await p.screenshot({path:path.join(root,'contribution-disclosure-synthetic.png')});
 await p.evaluate(()=>{testHost.state.runs[0].memberResults[0].role='decide';window.__polls.forEach(f=>f())});await p.waitForTimeout(100);
 check('invalid projection does not erase final answer',(await p.locator('#contribution-status').textContent()).includes('could not be read')&&(await p.locator('#transcript').textContent()).includes(actual.completedRun.answer.trim()));
 fs.writeFileSync(path.join(root,'CONSTELLATION_UI_CHECKS.json'),JSON.stringify({results,errors,scope:'exact normalized host public fixture with synthetic transport; no live provider'},null,2));console.log(JSON.stringify({checks:results.length,errors}));
