...
(async()=>{
 ...
 try {
 const catalog=JSON.parse(fs.readFileSync('/Users/Aaravshah/Documents/ChatGPT/App for me to integrate all my AI/qa-artifacts/tauri-r2-integration-review-20260907/r5-model-catalog/fixtures/catalog.json'));
 const {page:p}=await setup({catalog});
 await p.locator('#prompt').fill('Preserve my question');await p.waitForTimeout(240);
 await p.locator('#choose-model').click();await p.locator('#model-options button').first().waitFor();
 check('provider modal displays actual catalog',await p.locator('#model-dialog').isVisible());
 await p.locator('#model-options button').filter({hasText:'fixture-north'}).click();
 await p.locator('#model-options button').filter({hasText:'North Builder'}).click();
 await p.locator('#model-options button').filter({hasText:'High'}).click();
 await p.setViewportSize({width:760,height:560});await p.screenshot({path:path.join(root,'model-picker-760.png')});
 check('model dialog fits compact window',await p.locator('#model-dialog').evaluate(e=>{const r=e.getBoundingClientRect();return r.left>=0&&r.right<=innerWidth&&r.top>=0&&r.bottom<=innerHeight}));
 await p.locator('#apply-model').click();await p.locator('#model-dialog').waitFor({state:'hidden'});
 check('choice saves exact model and effort with text',await p.evaluate(()=>{const r=testHost.calls.filter(x=>x[0]==='rich-save').at(-1)[1];return r.draft==='Preserve my question'&&r.selection.modelID==='north-builder'&&r.selection.effortID==='high'&&r.team===null}));
 check('model save retains composer text',await p.locator('#prompt').inputValue()==='Preserve my question');
 await p.locator('#choose-model').click();await p.locator('#model-options button').filter({hasText:'Provider default'}).click();
 await p.evaluate(()=>testHost.uncertainSave=true);await p.locator('#apply-model').click();await p.waitForTimeout(100);
 check('uncertain model save blocks send and retains text',await p.locator('#submit').isDisabled()&&await p.locator('#prompt').inputValue()==='Preserve my question');
 await p.locator('#close-model').click();await p.evaluate(()=>testHost.uncertainSave=false);await p.locator('#retry-draft-save').click();await p.waitForTimeout(100);
 check('retry saves original pending model choice',await p.evaluate(()=>{const r=testHost.calls.filter(x=>x[0]==='rich-save').at(-1)[1];return r.selection.modelID===null&&r.selection.effortID===null}));
 check('no model selection dispatches AI',await p.evaluate(()=>!testHost.calls.some(x=>x[0]==='submit')));
 await p.evaluate(()=>{testHost.state.runs.push({id:'activity-run',conversationID:'c1',status:'completed',updatedAt:'now',admitted:{requestID:'activity-run',conversationID:'c1',prompt:'Fixture task',mode:'direct',provider:testHost.state.providers[0],approvedContext:{},retryOf:null},answer:'A real stored fixture result.',error:null,activity:{schemaVersion:1,baseSequence:4,entries:[{schemaVersion:1,eventID:'activity-run:4',requestID:'activity-run',conversationID:'c1',sequence:4,kind:'finalCompleted',phase:'final',state:'completed',memberID:null,providerID:null,role:null,summary:'Lead result saved',textDelta:null,error:null}]}});window.__polls.forEach(f=>f())});await p.waitForTimeout(100);
 check('real snapshot activity is collapsed and secondary',await p.locator('#run-activity').isVisible()&&await p.locator('#run-activity').evaluate(e=>!e.open)&&(await p.locator('#transcript').textContent()).includes('A real stored fixture result.'));
 await p.locator('#run-activity summary').click();check('activity uses exact host summary',(await p.locator('#activity-entries').textContent()).includes('Lead result saved'));
 await p.evaluate(()=>{testHost.state.runs.at(-1).activity.entries[0].sequence=7;window.__polls.forEach(f=>f())});await p.waitForTimeout(100);
 check('malformed activity keeps final answer and draft',await p.locator('#activity-error').textContent()!==''&&(await p.locator('#transcript').textContent()).includes('A real stored fixture result.')&&await p.locator('#prompt').inputValue()==='Preserve my question');
 fs.writeFileSync(path.join(root,'MODEL_UI_CHECKS.json'),JSON.stringify({results,errors,scope:'canonical renderer with synthetic catalog/host'},null,2));console.log(JSON.stringify({checks:results.length,errors}));
