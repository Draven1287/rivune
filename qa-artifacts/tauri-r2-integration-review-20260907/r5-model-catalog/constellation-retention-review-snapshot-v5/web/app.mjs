import { createHostAdapter, createSubmissionGate, newestRunForConversation, parseAcknowledgement, parseSnapshot, parseRichDraftReceipt, validateAttachmentMetadata, modelChoiceIdentity, parseModelCatalog, parseRunActivity, subscribeRunUpdates, parseModelSelection, parseTeamSelection, parseConstellationDetails } from "./core.mjs";
import { createTranscript } from "./transcript.mjs";
import { initializeArchiveInspector } from "./archive-inspector.mjs";

const pendingMemberRetries = new Map();
const pendingCancellations = new Set();
let teamDialogGeneration = 0;
let teamControls = null;
let teamEditorConversationID = null;
...
function render() {
  ...
  renderConstellationDetails(run);
  ...
  const isConstellation = run?.admitted?.mode === "constellation";
  if (run && ["completed","cancelled"].includes(run.status)) pendingCancellations.delete(run.id);
  if (run && pendingMemberRetries.has(run.id) && ((run.failedInvocationID !== pendingMemberRetries.get(run.id).invocationID || run.failedAttemptID !== pendingMemberRetries.get(run.id).failedAttemptID) || ["running","completed","cancelled"].includes(run.status))) pendingMemberRetries.delete(run.id);
  const retryMember = document.querySelector("#retry-member");
  retryMember.hidden = !isConstellation || run.status !== "failed" || !run.failedInvocationID || !run.failedAttemptID || !host?.retryConstellationInvocation;
  if (isConstellation && run.status === "failed" && (!run.failedInvocationID || !run.failedAttemptID)) runStatus.textContent = "Team recovery needs host confirmation. You can cancel this task.";
  retryMember.disabled = shutdownFrozen || pendingMemberRetries.has(run?.id) || pendingCancellations.has(run?.id);
  cancel.hidden = !host?.cancelRun || (!(["running","queued"].includes(run?.status)) && !(isConstellation && run.status === "failed"));
  cancel.disabled = pendingCancellations.has(run?.id) || shutdownFrozen;
  submit.hidden = ["running","queued"].includes(run?.status);
  const importedRun = run?.admitted?.provider?.kind === "imported" || run?.admitted?.mode?.startsWith("imported-read-only");
  retryRun.hidden = isConstellation || !historyIsValid || !run || selectionIsReadOnly || importedRun || !["failed", "cancelled"].includes(run.status) || typeof host?.retryRun !== "function";
  ...
  submit.disabled = (isConstellation && run.status === "failed" && (!run.failedInvocationID || !run.failedAttemptID)) || pendingMemberRetries.has(run?.id) || pendingCancellations.has(run?.id) || ["running","queued"].includes(run?.status) || Boolean(selectedConversationID && richState(selectedConversationID).team && snapshot?.runtimeCapabilities?.constellation !== "available") || dirtyDrafts.has(selectedConversationID) || draftWrites.has(selectedConversationID) || !host?.saveRichDraft || attachmentBusy || Boolean(richDrafts.get(selectedConversationID)?.pending) || richDrafts.get(selectedConversationID)?.conflictRevision != null || shutdownFrozen || !selectionIsValid || selectionIsReadOnly || !historyIsValid || !providerIsConfigured || submissionPreparing || retryPreparing || gate.pending || Boolean(gate.unresolvedRequestID) || Boolean(unresolvedRetry);
  ...
  const teamButton = document.querySelector("#choose-team");
  if (teamButton) teamButton.disabled = shutdownFrozen || !selectionIsValid || selectionIsReadOnly || !host?.getModelCatalog || teamSaving;
  if (teamControls) teamControls.update({catalog:modelCatalog,capability:snapshot?.runtimeCapabilities ?? {schemaVersion:1,constellation:"unavailable",minimumMembers:2,reasonCode:"ENGINE_NOT_CONNECTED"},currentTeam:richState(teamEditorConversationID).team,contextKey:teamEditorConversationID,disabled:teamSaving||shutdownFrozen||selectionIsReadOnly||selectedConversationID!==teamEditorConversationID});
  composerHint.textContent = selectionIsReadOnly ? "Preserved legacy conversation" : (selectedConversationID && richState(selectedConversationID).team) ? (snapshot?.runtimeCapabilities?.constellation === "available" ? "Constellation" : "Team draft · execution unavailable") : "Single AI conversation";
...
async function refresh(clearPinnedStatus = false, isActive = () => true) {
  if (!isActive() || shutdownFrozen || navigationPending) return;
  ...
  try {
    ...
    if (!isActive() || revision < appliedRefreshRevision || navigation !== navigationRevision) return;
...
cancel.addEventListener("click", async () => {
  ...
  if (!requestID || !host?.cancelRun || pendingCancellations.has(requestID) || shutdownFrozen) return;
  pendingCancellations.add(requestID);render();
  try {
    const acknowledgement = parseAcknowledgement(await host.cancelRun(requestID), requestID);
    if (acknowledgement.state === "rejected") pendingCancellations.delete(requestID);
    setStatus(acknowledgement.state === "accepted" ? "Stopping this task…" : acknowledgement.error ?? "Cancellation is not confirmed. Check the connection to refresh task status.", acknowledgement.state === "accepted" ? "ready" : "warning",true);
    await refresh();
  } catch { setStatus("Cancellation is not confirmed. Check the connection to refresh task status.","warning",true); }
  finally {render();}
});
function memberRetryPayload(run) {
  return Object.freeze({requestID:run.id,invocationID:run.failedInvocationID,failedAttemptID:run.failedAttemptID});
}
document.querySelector("#retry-member").addEventListener("click",async()=>{
  const run=newestRunForConversation(snapshot??{runs:[]},selectedConversationID);
  if(shutdownFrozen||!host?.retryConstellationInvocation||run?.admitted?.mode!=="constellation"||run.status!=="failed"||!run.failedInvocationID||!run.failedAttemptID||pendingMemberRetries.has(run.id)||pendingCancellations.has(run.id))return;
  const request=memberRetryPayload(run);pendingMemberRetries.set(run.id,request);render();
  try{
    const result=parseAcknowledgement(await host.retryConstellationInvocation(request),run.id);
    if(result.state==="rejected")pendingMemberRetries.delete(run.id);
    setStatus(result.state==="accepted"?"Retrying the failed member. Completed contributions are retained.":result.error??"Member retry is not confirmed. Check the connection to refresh task status.",result.state==="accepted"?"ready":"warning",true);
    await refresh();
  }catch{setStatus("Member retry is not confirmed. Check the connection to refresh task status.","warning",true);}
  finally{render();}
});
...
form.addEventListener("submit", async (event) => {
  ...
  if (submittedConversationID && richState(submittedConversationID).team && snapshot?.runtimeCapabilities?.constellation !== "available") return setStatus("Your team draft is saved, but Constellation execution is not available yet.", "warning", true);
  ...
  const currentRun = newestRunForConversation(snapshot??{runs:[]},submittedConversationID);
  if ((currentRun?.admitted?.mode === "constellation" && currentRun.status === "failed" && (!currentRun.failedInvocationID || !currentRun.failedAttemptID)) || ["running","queued"].includes(currentRun?.status) || pendingMemberRetries.has(currentRun?.id) || pendingCancellations.has(currentRun?.id)) return;
  ...
  try {
    ...
    recovery.mode = recovery.team ? "constellation" : "direct";
    persistUnresolved(recovery);
    const acknowledgement = await gate.submit({ conversationID: submittedConversationID, prompt: submittedDraft, id: recovery.id, richDraftRevision, mode: recovery.team ? "constellation" : "direct" });
    if (shutdownFrozen) return;
    if (acknowledgement.state === "accepted") {
      persistUnresolved(null);
...
// Only notification resources are detached; persistence and host shutdown retain their own lifecycle.
if (host?.onRunEvent) {
  let disposeUpdates = null;
  const connectUpdates = () => {
    disposeUpdates?.();
    disposeUpdates = subscribeRunUpdates(host, {
      refresh: isActive => refresh(false, isActive),
      onError: () => setStatus("Live task updates are unavailable. Rivune will continue checking saved task status.", "warning"),
    });
  };
  connectUpdates();
  globalThis.addEventListener?.("pagehide", () => { disposeUpdates?.(); disposeUpdates = null; });
  globalThis.addEventListener?.("pageshow", event => { if (event.persisted) connectUpdates(); });
}


document.querySelector("#choose-team").addEventListener("click", async () => {
  if(shutdownFrozen||teamSaving||!selectedConversationID||!host?.getModelCatalog)return;
  const id=selectedConversationID;const generation=++teamDialogGeneration;teamEditorConversationID=id;
  const dialog=document.querySelector("#team-dialog");const status=document.querySelector("#team-status");
  status.textContent="Loading your configured providers…";dialog.showModal();
  try{
    const {mountTeamControls}=await import("./team-controls.mjs");
    const catalog=parseModelCatalog(await host.getModelCatalog());
    if(generation!==teamDialogGeneration||!dialog.open||teamEditorConversationID!==id||selectedConversationID!==id||shutdownFrozen)return;
    modelCatalog=catalog;status.textContent="";
    teamControls=mountTeamControls({container:document.querySelector("#team-controls"),catalog,capability:snapshot?.runtimeCapabilities??{schemaVersion:1,constellation:"unavailable",minimumMembers:2,reasonCode:"ENGINE_NOT_CONNECTED"},currentTeam:richState(id).team,contextKey:id,disabled:false,onChange:async value=>{
      if(shutdownFrozen||teamSaving||selectedConversationID!==id||snapshot?.conversations.find(c=>c.id===id)?.readOnly)throw new Error("This conversation is not editable.");
      parseModelSelection(value.selection);parseTeamSelection(value.team);
      if(value.team&&modelChoiceIdentity({selection:value.selection})!==modelChoiceIdentity({selection:value.team.members[value.team.leadIndex]}))throw new Error("Select a lead from this team.");
      teamSaving=true;
      const rich=richState(id);rich.selection=structuredClone(value.selection);rich.team=structuredClone(value.team);render();
      try{await touchRichDraft(id);status.textContent=value.team?"Team draft saved. Execution depends on available engine support.":"Team draft cleared.";}
      finally{teamSaving=false;render();}
    }});
  }catch(error){if(generation!==teamDialogGeneration)return;status.textContent=`Team configuration unavailable: ${error.message}. Your draft is retained.`;}
});
document.querySelector("#close-team").addEventListener("click",()=>{if(!teamSaving)document.querySelector("#team-dialog").close();});
document.querySelector("#team-dialog").addEventListener("cancel",event=>{if(teamSaving)event.preventDefault();});
document.querySelector("#team-dialog").addEventListener("close",()=>{teamDialogGeneration++;teamControls?.destroy();teamControls=null;teamEditorConversationID=null;document.querySelector("#team-controls").replaceChildren();document.querySelector("#choose-team").focus();});


function renderConstellationDetails(run) {
  const panel=document.querySelector("#member-contributions");if(!panel)return;
  const cards=document.querySelector("#contribution-cards");const status=document.querySelector("#contribution-status");
  if(panel.dataset.runId!==run?.id){panel.open=false;cards.replaceChildren();delete panel.dataset.signature;panel.dataset.runId=run?.id??"";}
  panel.hidden=run?.memberResults==null&&run?.resolution==null;if(panel.hidden)return;
  try{
    const details=parseConstellationDetails({memberResults:run.memberResults??[],resolution:run.resolution??null});const signature=JSON.stringify(details);
    if(panel.dataset.signature===signature)return;
    const focused=panel.contains(document.activeElement);cards.replaceChildren();
    const addCard=(title,meta,body,truncated)=>{
      const card=document.createElement("article");card.className="contribution-card";
      const heading=document.createElement("h3");heading.textContent=title;card.append(heading);
      const context=document.createElement("p");context.className="contribution-meta";context.textContent=meta;card.append(context);
      const text=document.createElement("p");text.textContent=body;card.append(text);
      if(truncated){const note=document.createElement("p");note.className="contribution-meta";note.textContent=body?"This member answer is shortened for display.":"This member answer is omitted from this view because of the display limit.";card.append(note);}
      cards.append(card);
    };
    for(const item of details.memberResults)addCard("Independent answer",`Member ${item.memberID}${item.providerID?` · ${item.providerID}`:""}`,item.text,item.truncated);
    if(details.resolution)addCard("Lead synthesis",`Lead ${details.resolution.reviewerMemberID}${details.resolution.providerID?` · ${details.resolution.providerID}`:""}`,details.resolution.summary,false);
    status.textContent=details.memberResults.some(item=>item.truncated)?"Some member answers are shortened for display. The final answer remains in the conversation.":details.memberResults.length?"These are the independent answers recorded by this task.":"No member answers have been recorded yet.";
    panel.dataset.signature=signature;
    if(focused&&!panel.contains(document.activeElement))panel.querySelector("summary").focus();
  }catch{cards.replaceChildren();delete panel.dataset.signature;status.textContent="Member answers could not be read. The final answer and your draft are retained.";}
}
