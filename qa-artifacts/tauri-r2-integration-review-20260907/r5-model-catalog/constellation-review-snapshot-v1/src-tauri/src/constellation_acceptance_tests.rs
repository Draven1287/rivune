...
fn acceptance_council_restart_retry_preserves_provenance_and_failed_retry_is_atomic() {
    ...
    let (failed_id, failed_checkpoint, completed_before) = {
        ...
        (failed_id, persisted.checkpoint.clone(), completed)
    };
    drop(host);
    let host = HostState::open(path.clone()).unwrap();
    {
    ...
    let (admitted, cancellation, _) = host.admit_constellation_retry(&retry).unwrap();
    assert_eq!(serde_json::to_value(&admitted).unwrap(), original_admitted);
    assert_eq!(
        ...
        .remove(&retry.request_id);
    {
        ...
        assert_eq!(
            ...
            Some("Recovered combined answer\n")
        );
    ...
    let reopened = HostState::open(path.clone()).unwrap();
    assert_eq!(
        reopened.workspace.lock().unwrap().runs[0].answer.as_deref(),
        Some("Recovered combined answer\n")
    );
    drop(reopened);
    fs::remove_dir_all(path).unwrap();
