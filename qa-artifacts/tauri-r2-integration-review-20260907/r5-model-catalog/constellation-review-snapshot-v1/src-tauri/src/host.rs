...
impl HostState {
    ...
    fn save_constellation_checkpoint(
        ...
        match self.save(&candidate) {
            ...
            Err(error) if error.committed => {
                *workspace = candidate;
                Err(format!("{TERMINAL_PERSISTENCE_PREFIX} {}", error.message))
            }
    ...
    fn execute_constellation(
        ...
        loop {
            if cancellation.load(Ordering::SeqCst) {
                ...
                if let Ok(event) = self.save_constellation_checkpoint(
                    &admitted.request_id,
                    &session,
                    (
                        "cancelled",
                ...
                return Acknowledgement::accepted(&admitted.request_id);
            }
    ...
    fn cancel_persisted_constellation(&self, request_id: &str) -> Acknowledgement {
        ...
        if status != "failed"
            || !matches!(
                session.status(),
                TeamStatus::Failed | TeamStatus::Uncertain | TeamStatus::RecoveryRequired
            )
        {
            return Acknowledgement::rejected(
                request_id,
