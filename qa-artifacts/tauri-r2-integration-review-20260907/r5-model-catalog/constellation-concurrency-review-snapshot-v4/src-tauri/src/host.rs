            ...
            if ready.len() > 1 {
                ...
                let handles = prepared
                    ...
                    .map(|(invocation, provider, internal)| {
                        ...
                        thread::spawn(move || {
                            (invocation, provider, run_direct(&internal, cancellation))
                        })
                    })
                    .collect::<Vec<_>>();
                ...
                if outcomes
                    .iter()
                    .any(|(_, _, outcome)| matches!(outcome, Err(RunFailure::Cancelled)))
                {
                    let _ = session.cancel();
                    continue;
                }
                for (invocation, provider, outcome) in outcomes {
