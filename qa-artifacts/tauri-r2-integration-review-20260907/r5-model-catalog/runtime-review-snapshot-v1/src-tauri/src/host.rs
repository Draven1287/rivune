    ...
    if let Some(object) = value.as_object_mut() {
        ...
        object.insert(
            ...
            serde_json::to_value(RuntimeCapabilities {
                schema_version: 1,
                constellation: "unavailable".into(),
                minimum_members: 2,
                reason_code: Some("ENGINE_NOT_CONNECTED".into()),
            })
...
fn prepare_submission(
    ...
    if request.mode == "constellation" {
        return Err(Acknowledgement::rejected(
            &request.id,
            "ENGINE_NOT_CONNECTED: Constellation is not available in this source checkpoint.",
        ));
    }
    if request.mode != "direct" || conversation.rich_draft.team.is_some() {
        return Err(Acknowledgement::rejected(
            &request.id,
...
mod tests {
    ...
    #[test]
    fn stale_or_unverified_model_choices_and_unconnected_teams_fail_closed() {
        let mut workspace = WorkspaceSnapshot::default();
        ...
        .unwrap_err();
        assert_eq!(rejected.state, "rejected");
        assert!(rejected.error.unwrap().contains("ENGINE_NOT_CONNECTED"));
    }
