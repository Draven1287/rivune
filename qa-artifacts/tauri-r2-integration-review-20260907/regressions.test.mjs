...
const source=process.env.RIVUNE_RENDERER_SOURCE || fileURLToPath(new URL('./reference-web/',import.meta.url));
...
async function setup(options={}){
 ...
 globalThis.__RIVUNE_DESKTOP_HOST__={
  getSnapshot:async()=>{if(options.failSnapshot)throw Error("temporary connection fixture");return structuredClone(state)},
...
test('unsaved text survives a failed refresh and reconnection',async()=>{
 const options={failSaves:true,failSnapshot:false};
 try{
  const app=await setup(options);app.get('#prompt').value='Unsaved work must survive reconnect';app.get('#prompt').listeners.input();await app.flush();
  options.failSnapshot=true;await app.tick();
  options.failSnapshot=false;await app.tick();
  assert.equal(app.get('#prompt').value,'Unsaved work must survive reconnect');
 }finally{await cleanup()}
});
