...
function render() {
  ...
  submit.disabled = !selectionIsValid || !providerIsConfigured || submissionPreparing || gate.pending || Boolean(gate.unresolvedRequestID);
...
prompt.addEventListener("input", () => {
  ...
  if (conversationID && host?.saveDraft) {
    draftSaveTimer = setTimeout(() => host.saveDraft(conversationID, draft).catch(() => setStatus("We couldn’t save this draft. Your text is still here. Keep this window open and try again.", "warning", true)), 180);
  }
});
prompt.addEventListener("change", async () => {
  if (!selectedConversationID || !host?.saveDraft) return;
  try {
    await host.saveDraft(selectedConversationID, prompt.value);
  } catch {
    setStatus("We couldn’t save this draft. Your text is still here. Keep this window open and try again.", "warning", true);
  }
});
