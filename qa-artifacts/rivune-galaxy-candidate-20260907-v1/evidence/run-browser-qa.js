...
const base = 'http://127.0.0.1:4293/rivune/';
...
(async () => {
  ...
  try {
    ...
    assert(await page.locator('.demo-panel').evaluate(el => el === document.activeElement), 'demo panel did not receive sensible focus');
    ...
    report.demo = { exampleSelection: true, stageSelection: true, panelFocus: true, reset: true, disclosure: await page.locator('.demo-disclosure').textContent() };

    for (const route of ['', 'app/']) {
      await page.goto(base + route);
      assert(await page.locator('.workspace-preview').count() === 1, `${route || 'home'} DOM workspace preview missing`);
      assert(await page.locator('img[src*="native-workspace"],.product-image-zoom').count() === 0, `${route || 'home'} still references an app screenshot`);
      report.productPreview[route || 'home'] = { domBuilt: true, screenshotReferences: 0, disclosure: await page.locator('.preview-disclosure').textContent() };
    }
