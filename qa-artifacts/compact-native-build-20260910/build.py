...
root=Path.cwd();candidate=root/'tools/symphony/source-import-candidate';out=root/'qa-artifacts/compact-native-build-20260910';frontend=candidate/'prototypes/ai-native-workspace';native=candidate/'qa-artifacts/cross-platform-shell-20260907/candidate4-runtime-r2/src-tauri'
expected='3bc19ce0bd7e7a9a1f59dbf11c16a35f0d089f8d';manifest=json.loads((root/'tools/symphony/source-stage/SOURCE_MANIFEST.json').read_text());sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
config={'productName':'Rivune Compact QA','identifier':'com.rivune.desktop.qa.compact20260910','build':{'frontendDist':str(out/'frontend-dist')},'app':{'windows':[{'label':'main','title':'Rivune Compact QA · build review only','width':1200,'height':900,'minWidth':760,'minHeight':560,'resizable':True}]}}
...
try:
 ...
 app=out/'Rivune Compact QA.app';binary=app/'Contents/MacOS/rivune';binary.parent.mkdir(parents=True);(app/'Contents/Resources').mkdir();shutil.copy2(root/'.toolchains/target-candidate4-r2/debug/rivune',binary);shutil.copy2(native/'icons/icon.icns',app/'Contents/Resources/icon.icns')
 info={'CFBundleDisplayName':'Rivune Compact QA','CFBundleName':'Rivune Compact QA','CFBundleExecutable':'rivune','CFBundleIdentifier':config['identifier'],'CFBundlePackageType':'APPL','CFBundleShortVersionString':'0.0.1','CFBundleVersion':'2026091002','CFBundleIconFile':'icon.icns','NSHighResolutionCapable':True,'LSMinimumSystemVersion':'11.0','LSEnvironment':{'RIVUNE_ISOLATED_PROFILE_DIR':str(out/'profile-reserved-not-launched'),'PATH':'/usr/bin:/bin'}}
