...
export function ComposerExecutionControl({ snapshot, catalog, conversationID, disabled, onApply, onReviewConnections }: {
  ...
  useEffect(() => { if (!dirty && baseKey !== savedKey) loadSaved(); }, [savedKey, baseKey, dirty]);
  function close() { if (saving) return; loadSaved(); setOpen(false); summary.current?.focus(); }
  ...
  async function apply() {
    ...
    try { await onApply({expectedKey:baseKey, selection, team}); setDirty(false); setOpen(false); summary.current?.focus(); }
  ...
  return <details className="composer-execution" open={open} onKeyDown={event => { if (event.key === 'Escape' && open) { event.preventDefault(); event.stopPropagation(); close(); } }}>
    <summary ref={summary} title={composerSummary(snapshot, catalog, conversationID)} onClick={event => { event.preventDefault(); if (open) close(); else { loadSaved(); setOpen(true); } }}>{composerSummary(snapshot, catalog, conversationID)}</summary>
    <div className="composer-execution-editor" aria-label="Execution configuration">
      <p>Provider default · exact model not reported</p>
      <p>{dirty ? 'Not saved' : 'Saved configuration'} · Changes apply to future sends.</p>
