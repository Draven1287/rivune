...
test('direct send saves exact CAS/default selection and journals identity before one submission',async t=>{
  ...
  assert.equal(save.expectedRevision,2);assert.deepEqual(save.selection,{schemaVersion:1,providerID:'codex',modelID:null,effortID:null,catalogRevision:'revision'});assert.deepEqual(save.attachmentIDs,[]);assert.equal(save.team,null);
...
test('conversation binding uncertain save preserves draft and prevents a replacement mutation',async t=>{const f=fixture();let saves=0;f.bridge.saveRichDraft=async()=>{saves++;throw Error('lost reply')};const c=f.controller();t.after(()=>c.dispose());await c.start();c.editDraft('chat','Keep me');const choice={provider:clone(f.snapshot.providers[0]),revision:2,catalogRevision:'revision'};await assert.rejects(c.configureConversationProvider('chat',choice));await assert.rejects(c.configureConversationProvider('chat',choice));assert.equal(saves,1);assert.equal(c.getState().drafts.chat,'Keep me');assert.equal(count(f,'submit'),0);});
 ...
 test('reviewer binding preserves attachment IDs and blocks explicit models and teams',async t=>{
 for(const variant of ['attachments','model','team']){
 const f=fixture();const chat=f.snapshot.conversations[0];const sel={schemaVersion:1,providerID:'codex',modelID:null,effortID:null,catalogRevision:'revision'};
 if(variant==='attachments'){chat.richDraft.attachmentIDs=['retained-attachment'];f.snapshot.attachments=[{attachmentID:'retained-attachment',conversationID:'chat',displayName:'Synthetic',byteLength:1,sha256:'a'.repeat(64),sourceState:'valid'}];}
 if(variant==='model')chat.richDraft.selection={...sel,modelID:'explicit'};
 if(variant==='team'){const second={...f.snapshot.providers[0],id:'second'};f.snapshot.providers.push(second);chat.richDraft.team={schemaVersion:1,leadIndex:0,members:[sel,{...sel,providerID:'second'}]};chat.richDraft.selection=sel;}
 const c=f.controller();t.after(()=>c.dispose());await c.start();c.editDraft('chat','Retained local text');
 const choice={provider:clone(f.snapshot.providers[0]),revision:2,catalogRevision:'revision'};
 if(variant==='attachments'){await c.configureConversationProvider('chat',choice);assert.deepEqual(chat.richDraft.attachmentIDs,['retained-attachment']);assert.equal(count(f,'save'),1);}
 else{await assert.rejects(c.configureConversationProvider('chat',choice));assert.equal(count(f,'save'),0);}
 assert.equal(c.getState().drafts.chat,'Retained local text');assert.equal(f.snapshot.selectedProviderID,'codex');assert.equal(count(f,'submit'),0);
 }
 });
