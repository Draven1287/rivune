...
if (!selection.error && new URLSearchParams(location.search).get('preview') === '1') {
  ...
  await mount(f);
  if(['conversation-list','compact-composer'].includes(new URLSearchParams(location.search).get('scenario')??'')){
    const panel=document.createElement('details'),summary=document.createElement('summary');
    summary.textContent=`Synthetic checks: ${window.__HOST_RENDERER_RESULTS__.status.toUpperCase()} (${window.__HOST_RENDERER_RESULTS__.results.length})`;
    panel.append(summary,output);document.body.prepend(panel);
    ...
    const resize=new ResizeObserver(()=>{(container.firstElementChild as HTMLElement).style.height=`calc(100dvh - ${panel.getBoundingClientRect().height}px)`;});resize.observe(panel);
