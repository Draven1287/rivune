...
function ConnectedHostWorkspace({ bridge }: { bridge: unknown }) {
  ...
  async function run(action: (c: Controller) => Promise<unknown>, focus = false) {
    ...
    try { await action(controller.current); if (focus) { focusChatAfterOpen.current=true;setContext(false); } }
    ...
    <div className="host-layout">
      ...
      <main id="host-chat" className={`host-chat ${context ? 'is-hidden' : ''}`}>
        ...
        {conversation && <section className="host-composer" aria-label="Write a message">
          ...
          <label htmlFor="host-message">Message</label><textarea id="host-message" ref={message} aria-label="Message" value={conversation.draft} readOnly={conversation.readOnly || frozen || unavailable} onChange={event => { try { controller.current?.editDraft(conversation.id, event.target.value); } catch { setNotice('This draft exceeds the supported size. Your previous text is retained.'); } }} onKeyDown={event => { if (event.key === 'Enter' && (event.metaKey || event.ctrlKey) && !locked && !state.pending && !activeRuns.length && !state.configurationConfirmations[conversation.id]) { event.preventDefault(); void run(c => c.send(conversation.id)); } }}/>
