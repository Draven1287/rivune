...
export function createModelDiscoveryAdapter(bridge: unknown) {
  ...
  return {cancel, async discover(value:ModelDiscoveryRequest,signal?:AbortSignal):Promise<ModelDiscoveryReport>{
    ...
    const stop=(state:'cancelled'|'timeout',resolve:(value:ModelDiscoveryReport)=>void)=>{void cancel(bound.requestID).catch(()=>{});resolve(failure(bound,state,state==='cancelled'?'DISCOVERY_CANCELLED':'DISCOVERY_TIMEOUT'));};
    const interrupted=new Promise<ModelDiscoveryReport>(resolve=>{abort=()=>stop('cancelled',resolve);signal?.addEventListener('abort',abort,{once:true});timer=setTimeout(()=>stop('timeout',resolve),10000);});
    try {return await Promise.race([Promise.resolve().then(()=>signal?.aborted?failure(bound,'cancelled','DISCOVERY_CANCELLED'):discover.call(bridge,bound)).then(v=>parseModelDiscovery(v,bound)).catch(()=>failure(bound,'error','DISCOVERY_PROTOCOL_ERROR')),interrupted]);}
