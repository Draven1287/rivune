...
tests.push(['Constellation configuration saves lead and members; saved results remain separate from final answer', async f => {
  ...
  await act(async () => {
    container.querySelector<HTMLDetailsElement>('.host-team-settings')!.open = true;
    for (const input of container.querySelectorAll<HTMLInputElement>('.host-team-settings input[type="checkbox"]')) input.click();
  }); await settle();
  ...
  await click('Save team configuration');
  ...
  assert(container.textContent?.includes('Swarm is not available in this version'), 'Unsupported strategy limitation missing');
}]);
tests.push(['Missing Constellation capability leaves direct chat available and team configuration disabled', async f => {
  ...
  assert(container.querySelector<HTMLFieldSetElement>('.host-team-settings fieldset')?.disabled, 'Team configuration enabled without capability');
...
tests.push(['Team form synchronizes pristine changes and requires explicit conflict resolution for local choices', async f => {
  ...
  await act(async () => { container.querySelector<HTMLDetailsElement>('.host-team-settings')!.open = true; container.querySelector<HTMLInputElement>('.host-team-settings input')!.click(); }); await settle();
  ...
  await click('Refresh');
  assert(container.textContent?.includes('The saved team changed'), 'Local choices silently overwritten');
  assert(button('Save team configuration').disabled, 'Conflicting team could overwrite refreshed settings');
  await click('Load saved team');
  assert(select().value === 'third-provider', 'Explicit reload failed to load saved lead');
  const checks = [...container.querySelectorAll<HTMLInputElement>('.host-team-settings input')].map(input => input.checked);
  ...
  assert(button('Save team configuration').disabled, 'Removed provider left Save enabled');
}]);
...
const teamChecks = () => [...container.querySelectorAll<HTMLInputElement>('.host-team-settings input')].map(input => input.checked);
async function editThirdParticipant() {
  await act(async () => { container.querySelector<HTMLDetailsElement>('.host-team-settings')!.open = true; container.querySelectorAll<HTMLInputElement>('.host-team-settings input')[2].click(); }); await settle();
}
...
teamFormTests.push(['team-pristine', 'Team form: pristine refresh loads the same conversation saved team', async f => {
  ...
  assert(!container.textContent?.includes('The saved team changed'), 'Pristine form incorrectly requires conflict resolution');
...
teamFormTests.push(['team-dirty', 'Team form: dirty refresh retains choices and blocks overwrite', async f => {
  ...
  externalTeam(f, members); await click('Refresh');
  assert(container.textContent?.includes('The saved team changed'), 'Dirty refresh hid the conflict');
  assert(teamLead() === 'second-provider' && teamChecks().every(Boolean), 'Dirty local choices were replaced');
  assert(button('Save team configuration').disabled, 'Dirty conflict permits overwrite');
  assert(!f.calls.some(c => c.name === 'save'), 'Dirty refresh wrote state');
  await click('Load saved team');
  assert(teamLead() === 'third-provider' && JSON.stringify(teamChecks()) === '[false,true,true]', 'Load saved team did not restore remote participant IDs and lead');
  assert(!container.textContent?.includes('The saved team changed'), 'Explicit saved-team load left the conflict unresolved');
  assert(!f.calls.some(c => c.name === 'save'), 'Loading the saved team wrote state');
}]);
teamFormTests.push(['team-keep', 'Team form: Keep my team choices waits for explicit save using refreshed revision', async f => {
  const { members } = populateMockTeam(f); await click('Refresh'); await editThirdParticipant();
  externalTeam(f, members); await click('Refresh'); await click('Keep my team choices');
  assert(teamLead() === 'second-provider' && teamChecks().every(Boolean), 'Keep choices discarded edits');
  assert(!button('Save team configuration').disabled && !f.calls.some(c => c.name === 'save'), 'Keep choices saved without explicit action');
  await click('Save team configuration');
...
teamFormTests.push(['team-save-ack', 'Team form: durable save acknowledgement makes later refresh pristine', async f => {
  ...
  assert(!container.textContent?.includes('The saved team changed'), 'Own save acknowledgement appeared as conflict');
  externalTeam(f, members); await click('Refresh');
  ...
  assert(!container.textContent?.includes('The saved team changed'), 'Acknowledged form incorrectly conflicts');
}]);
teamFormTests.push(['team-save-failed', 'Team form: rejected save keeps local choices and original saved team', async f => {
  ...
  await editThirdParticipant(); f.setConflict(true); await click('Save team configuration');
  ...
  externalTeam(f, members); await click('Refresh');
  assert(container.textContent?.includes('The saved team changed') && button('Save team configuration').disabled, 'External team change after failed save did not require explicit resolution');
...
teamFormTests.push(['conversation-binding','Explicit conversation pin preserves draft and workspace default',async f=>{
 ...
 const panel=container.querySelector('#conversation-provider')!.closest('details')!;await act(async()=>{panel.open=true;});
 const picker=container.querySelector<HTMLSelectElement>('#conversation-provider')!;await act(async()=>{picker.value=p.id;picker.dispatchEvent(new Event('change',{bubbles:true}));});await settle();await click('Save conversation connection');
 assert(container.textContent?.includes('Pinned to Second connection'),'Pinned identity not visible');assert(f.snapshot.selectedProviderID==='fixture-cli','Workspace default silently changed');assert(messageInput().value==='Keep binding draft','Binding changed draft');assert(!f.calls.some(c=>c.name==='submit'),'Binding submitted prompt');
}]);
...
teamFormTests.push(['conversation-list','Failed draft save blocks filtered navigation and preserves mobile pane',async f=>{
 await unmount();f.snapshot.conversations.push({...clone(f.snapshot.conversations[0]),id:'other',title:'Other saved chat'});f.setConflict(true);await mount(f);await type('Keep failed-navigation draft');if(window.innerWidth<=700)await click('Conversations');await click('Other saved chat');assert(f.snapshot.activeConversationID==='fixture-chat','Failed save changed selection');assert(messageInput().value==='Keep failed-navigation draft','Failed save lost draft');assert(!f.calls.some(c=>c.name==='open'),'Failed save reached open');if(window.innerWidth<=700)assert(container.querySelector('.host-sidebar.is-visible'),'Failed save switched away from Conversations');
}]);
const selection = rendererScenarioSelection(location.search);
const selectionScope = selection.ids === null ? 'full suite' : selection.ids.some(id => id.startsWith('retry-')) ? 'selected invocation retries only' : 'selected team forms only';
...
for (const [name, run] of selectedTests) {
  ...
  try {
    ...
    if (selection.ids !== null) assert(!f.calls.some(c => ['submit', 'beginShutdown', 'flushShutdownDraft', 'completeShutdown', 'abortShutdown'].includes(c.name)), 'Selected team form scenario invoked unrelated run or shutdown methods');
