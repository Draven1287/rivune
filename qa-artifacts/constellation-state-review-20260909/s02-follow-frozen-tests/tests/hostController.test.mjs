...
test('reviewer inherited send reaches legacy provider model override',async t=>{const f=fixture();f.snapshot.providers[0].model='unverified-legacy-model';const c=f.controller();t.after(()=>c.dispose());await c.start();await c.send('chat');assert.equal(f.snapshot.conversations[0].richDraft.selection,null);assert.equal(f.snapshot.runs[0].admitted.provider.model,'unverified-legacy-model');assert.equal(count(f,'submit'),1);});
