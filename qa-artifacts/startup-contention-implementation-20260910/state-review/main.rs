...
fn main() -> std::process::ExitCode {
    ...
    let contention=Arc::new(std::sync::atomic::AtomicBool::new(false));
    let setup_contention=contention.clone();
    let app = tauri::Builder::default()
        .setup(move |app| {
            ...
            if startup.is_err() {setup_contention.store(true,Ordering::SeqCst);}
            admitted_startup_ui(startup, |startup| {
    ...
    let app = match app {
        ...
        Err(error) => {
            ...
            return startup_failure_exit(contention.load(Ordering::SeqCst));
        }
    };
