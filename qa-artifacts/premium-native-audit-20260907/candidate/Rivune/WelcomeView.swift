...
struct WelcomeView: View {
                ...
                VStack(spacing: 0) {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        if !bridge.state.isConnected {
                            Button("Connect this iPhone to your Mac") { store.presentSettings(.devices) }
                                .font(.system(size: 12))
                                .foregroundStyle(RivunePalette.secondaryText)
                                .buttonStyle(.plain)
                                .padding(.top, 18)
    ...
    private func modeButton(_ mode: IntelligenceMode) -> some View {
        ...
        let isAvailable = mode != .swarm
        let ready = isReady(mode)
        let title = switch mode {
        case .swarm: "Swarm · unavailable"
        case .together, .council: mode.displayName
        ...
        .accessibilityValue(isAvailable ? (ready ? "Ready" : "Connection required") : "Not available in this build")
        .accessibilityHint(isAvailable ? "Select this assistant" : "Swarm cannot be selected in this build")
        .accessibilityAddTraits(selected ? .isSelected : [])
        .help(isAvailable ? mode.shortDescription : "Swarm is not available in this build")
    }
    ...
    private func isReady(_ mode: IntelligenceMode) -> Bool {
        ...
        case .council: store.councilIsAvailable
        case .swarm: false
