...
final class CouncilSavedIntegrityTests: XCTestCase {
    ...
    func testCorruptNestedJournalRejectedBeforeAnyMutation() async throws {
        let team = TeamTestFactory.team()
        let participants = try team.participants(admittedBy: TeamTestFactory.evidence(team))
        for corruption in ["duplicate", "foreign", "conflict", "appointment", "runID", "turnID", "prompt", "zero", "one", "runningOne", "runningRoute", "runningIdentity"] {
            let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
            defer { try? FileManager.default.removeItem(at: directory) }
            try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
            let file = directory.appendingPathComponent("runs.json")
            ...
            if corruption == "appointment" {
                nested.appointments = [.init(
                    participant: .init(id: "outside", providerID: "forged", adapterID: "forged", modelID: "forged", displayName: "Forged lead"),
                    policyVersion: "selected-orchestrator-v1", reason: "Forged", evidenceBasis: "Forged", replacesParticipantID: nil
                )]
            }
    ...
    func testSavedLeadAppointmentsRequireExactSuccessfulPolicyChain() throws {
        let team = TeamTestFactory.team(fallback: .orderedMemberIDs(["a"]))
        let participants = try team.participants(admittedBy: TeamTestFactory.evidence(team))
        let successful = participants.map { CouncilParticipantResult(participant: $0.identity, text: "Exact draft \($0.identity.id)") }
        let selected = CouncilLeadAppointment(participant: participants[1].identity,
            policyVersion: "selected-orchestrator-v1", reason: "Selected orchestrator.", evidenceBasis: "Frozen team.", replacesParticipantID: nil)
        let fallback = CouncilLeadAppointment(participant: participants[0].identity,
            policyVersion: "selected-orchestrator-v1", reason: "Saved fallback.", evidenceBasis: "Frozen team.", replacesParticipantID: participants[1].identity.id)
        func record(_ appointments: [CouncilLeadAppointment], results: [CouncilParticipantResult],
                    team: TeamRunConfiguration) -> CouncilRunRecord {
            var value = CouncilRunRecord(id: UUID(), turnID: UUID(), prompt: "Frozen", approvedContext: "", criteria: "Criteria", participants: participants.map(\.identity))
            value.phase = .partial; value.results = results; value.teamConfiguration = team; value.appointments = appointments
            return value
        }
        XCTAssertNoThrow(try record([selected, fallback], results: successful, team: team).validateSavedResults())

        let foreign = CouncilLeadAppointment(participant: .init(id: "outside", providerID: "forged", adapterID: "forged", modelID: "forged", displayName: "Forged"),
            policyVersion: selected.policyVersion, reason: selected.reason, evidenceBasis: selected.evidenceBasis, replacesParticipantID: nil)
        let wrongPolicy = CouncilLeadAppointment(participant: selected.participant,
            policyVersion: "forged-policy", reason: selected.reason, evidenceBasis: selected.evidenceBasis, replacesParticipantID: nil)
        let brokenChain = CouncilLeadAppointment(participant: fallback.participant,
            policyVersion: fallback.policyVersion, reason: fallback.reason, evidenceBasis: fallback.evidenceBasis, replacesParticipantID: "outside")
        let failedSelected = CouncilParticipantResult(participant: participants[1].identity, error: "Failed")
        let stopTeam = TeamTestFactory.team(fallback: .stop)
        for invalid in [
            record([foreign], results: successful, team: team),
            record([wrongPolicy], results: successful, team: team),
            record([selected, brokenChain], results: successful, team: team),
            record([selected], results: [successful[0], failedSelected], team: team),
            record([fallback], results: successful, team: stopTeam)
        ] {
            XCTAssertThrowsError(try invalid.validateSavedResults())
        }
    }
