...
export function commandFor(options = {}) {
  ...
  if (platform === "macos") {
    if (options.phase === "build-reviewable-app") {
      return { platform, phase: options.phase, cwd, plan, command: ["cargo", "tauri", "build", "--bundles", "app"] };
    }
...
export async function loadAcceptance(path) {
  ...
  if (receipt.schema !== "rivune-tauri-installer-input-v4") throw new Error("Unsupported acceptance receipt schema");
...
export async function validateExecution(options, actualOS = process.platform) {
  ...
  await assertNoExternalCargoPathDependencies(cwd, project.records);
  ...
  return {
    ...
    inputs: { tauriConfigPath, cargoManifestPath, rustBuildScriptPath, cargoLockPath, cargoConfigPaths, frontendDist: rendererRoot, productIdentity },
...
export async function assertNoExternalCargoPathDependencies(cwd, treeRecords) {
  const physicalRoot = await realpath(cwd);
  const manifests = treeRecords.filter(record => record.type === "file" && (record.path === "Cargo.toml" || record.path.endsWith("/Cargo.toml")));
  for (const manifest of manifests) {
    const manifestPath = join(cwd, manifest.path);
    const source = (await readFile(manifestPath, "utf8")).replace(/#.*$/gm, "");
    for (const match of source.matchAll(/\bpath\s*=\s*["']([^"']+)["']/g)) {
      const dependency = resolve(dirname(manifestPath), match[1]);
      const physicalDependency = await realpath(dependency).catch(() => null);
      if (!physicalDependency) throw new Error(`Cargo path dependency does not exist: ${match[1]}`);
      if (!inside(physicalRoot, physicalDependency)) {
        throw new Error(`External Cargo path dependency is unsupported by this acceptance gate: ${match[1]}`);
      }
    }
  }
}
...
export async function executePackaging(options, actualOS = process.platform, runner = run) {
  ...
  await runner(validated.command, { cwd: validated.cwd, env: options.env ?? process.env });
  return validated;
}
