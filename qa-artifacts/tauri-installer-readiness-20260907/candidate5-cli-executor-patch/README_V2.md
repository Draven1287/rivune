# Candidate5 macOS executor patch v2

This supersedes the preserved `v1/` overlay. V2 adds binding for the executed CLI `main.js` module and uses one sanitized environment for metadata and the eventual build runner.
...
patch -d /tmp/candidate5-next -p1 < candidate5-cli-executor-patch/CANDIDATE5_EXECUTOR_V2.patch
...
- verifies the npm CLI symlink, JavaScript wrapper, executed `main.js` module, arm64 native binding, workspace Cargo executable, and their receipt-bound hashes;
- binds `CARGO_HOME`, `RUSTUP_HOME`, `CARGO_TARGET_DIR`, `SDKROOT`, `PATH`, and forced `CARGO_NET_OFFLINE=true` to the accepted environment;
- strips inherited `CARGO_*`, `RUST*`, compiler/linker, Node preload/path, Tauri/Apple signing, dynamic-loader, deployment-target and developer-directory overrides, then passes that same sanitized environment to metadata and the build runner;
...
Syntax checks passed. All 21 tests passed, including rejection of changed `tauri.js` and `main.js` payloads, wrong CLI receipt hash, wrong target, wrong source receipt, existing app output, dirty DMG staging, and wrong accepted app hash. A focused test proves build-affecting inherited overrides are absent from both metadata and build-runner environments. The emitted app-only command and metadata flags are asserted exactly. Patch application to a fresh candidate5 copy was also verified.
