...
class StaticOutputTests(unittest.TestCase):
    ...
    def test_static_package_has_no_dynamic_workspace_or_fixture(self):
        ...
        self.assertFalse(any('workspace' in p or 'fixture' in p or p.endswith('.py') for p in paths))
    ...
    def test_installer_readiness_does_not_enable_pending_modes(self):
        for output in (self.output, self.fixture):
            with self.subTest(installer_state=output.name):
                ...
                self.assertIn('First development stage', text)
                self.assertIn('Future milestone', text)
                self.assertIn('public source preview 4 does not include this configurable team experience', text)
                self.assertIn('automatic workflow selection remain future milestones', text)
    ...
    def test_guide_is_linked_and_has_descriptive_unique_metadata(self):
        ...
        self.assertIn('Multi-Model AI Workspace for Mac', home.meta['og:title'])
        self.assertIn('in development', home.meta['description'])
        self.assertIn('planned AI collaboration modes', guide.meta['og:title'])
        self.assertIn('first development stage', guide.meta['og:description'])
    ...
    def test_preview_artifact_is_explicit_and_noncommercial(self):
        ...
        for page in self.output.rglob('*.html'):
            ...
            for forbidden in ('<form', '<script', 'data-installer', 'checkout', '$10', 'stripe.com'):
                self.assertNotIn(forbidden, text)
        guide = (self.output / 'council-vs-swarm/index.html').read_text()
        self.assertIn('Rivune should propose an eligible team and manager', guide)
        self.assertIn('Multiple members may use the same provider tool', guide)
        self.assertIn('temporary stage, not the final product scope', guide)
