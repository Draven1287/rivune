...
const source = fs.readFileSync(path.join(__dirname, 'BrowserWorkspace.tsx'), 'utf8');
...
function fixture() {
  let slots = [], index = 0, effects = [], dirty = false, tree, unmounted = false, writesAfterUnmount = 0;
  const requests = [];
...
(async () => {
  ...
  for (const transition of ['selection', 'back', 'close-reopen', 'escape-reopen', 'conversation', 'content', 'unmount']) {
    for (const outcome of ['resolve', 'reject']) {
      ...
      if (transition === 'content') { f.current.artifacts[0].content='CHANGED TEXT'; f.render(); }
  ...
  const sync=fixture();sync.open();const callback=sync.copyButton().props.onClick;const first=callback();const second=callback();assert.equal(sync.requests.length,1,'same-tick duplicate write');sync.requests[0].resolve();await Promise.all([first,second]);checks++;
