...
export function BrowserWorkspace() {
  ...
  function chooseResult(id: string | null) { invalidateCopy(); setSelectedResult(id); }
  ...
  function openResults(id: string | null = null) { chooseResult(id); results.current?.showModal(); }
  return <div className={`browser-workspace ${navigation ? 'navigation-open' : ''}`}>
    ...
    <dialog ref={results} onCancel={invalidateCopy} onClose={invalidateCopy} className="bw-dialog bw-results" aria-labelledby="bw-results-title"><header><div><span className="bw-kicker">SAMPLE WORKSPACE</span><h2 id="bw-results-title">{result ? result.title : 'Results'}</h2></div><Button variant="ghost" size="icon" aria-label="Close results" onClick={closeResults}><X size={19}/></Button></header>{result ? <><button className="bw-back" onClick={() => chooseResult(null)}>← All results</button><pre>{result.content}</pre><Button variant="outline" disabled={copyPending} title={copyPending ? 'A clipboard write is still pending' : undefined} onClick={copyResult}>Copy sample text</Button><p role="status">{copyStatus}</p></> : artifacts.length ? artifacts.map(artifact => <button className="bw-result-card" key={artifact.id} onClick={() => chooseResult(artifact.id)}><FileText size={20}/><span><strong>{artifact.title}</strong><small>Sample · {artifact.kind}</small></span><ArrowUpRight size={17}/></button>) : <p className="bw-dialog-intro">No results in this conversation yet.</p>}</dialog>
  </div>;
}
