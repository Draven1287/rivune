...
struct WorkspaceView: View {
    ...
    private var headerActions: some View {
        RivuneActionMenu(title: "Workspace actions", actions: [
            .init(title: "API conversations", systemImage: "globe") { store.showUniversalAPI = true },
            .init(title: "Project", systemImage: "folder") { store.showProjectWorkspace = true },
            .init(title: isWorkPanelPresented ? "Hide work inspector" : "Show work inspector", systemImage: "sidebar.right") { isWorkPanelPresented.toggle() },
            .init(title: "Include previous messages", systemImage: "brain.head.profile", isSelected: store.memoryEnabled, action: toggleConversationContext),
            .init(title: "Connections", systemImage: "network") { store.showConnections = true },
            .init(title: "Settings", systemImage: "gearshape") { store.presentSettings() }
        ]) {
    ...
    private var mobileOptionsMenu: some View {
        RivuneActionMenu(title: "Chat options", actions: [
            .init(title: "Include previous messages", isSelected: store.memoryEnabled, action: toggleConversationContext),
            .init(title: "Settings", systemImage: "gearshape") { store.presentSettings() }
        ]) {
        ...
        ScrollViewReader { proxy in
                    ...
                    LazyVStack(spacing: 36) {
                        ForEach(store.turns) { turn in
                            TurnView(
                                ...
                                onContinueArtifact: store.canContinueGeneratedFiles ? { store.selectArtifactForContinuation(answerID: $0.id) } : nil
                            )
                            .id(turn.id)
                        }
            ...
            .onChange(of: store.turns) { oldTurns, newTurns in
                let action = TranscriptScrollPolicy.action(
                    newTurnAdded: newTurns.count > oldTurns.count,
                    completedTogetherTurnID: newlyCompletedTogetherTurnID(from: oldTurns, to: newTurns),
                    isNearBottom: isNearTranscriptBottom
                )
                switch action {
                ...
                case .transcriptBottom:
                    scrollToBottom(proxy)
                case .finalAnswer(let turnID):
                    scrollToFinalAnswer(proxy, turnID: turnID)
                case .showNewContent:
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                HStack(spacing: 10) {
                    ...
                    RivuneSelect(title: "Show conversations", selection: $store.sidebarDestination, options: SidebarDestination.allCases.map { ($0.rawValue, $0) })
                    .fixedSize()
                }
                .font(.system(size: 12))
                .padding(13)
...
private struct TurnView: View {
    ...
    let onRetry: () -> Void
    let onContinueArtifact: ((AIAnswer) -> Void)?
    ...
    var body: some View {
        VStack(spacing: 14) {
            ...
            case .council:
                togetherFinalAnswer
                if let record = turn.councilRun { CouncilRunDisclosure(record: record) }
            case .swarm:
                Text("Swarm is not available yet. Real worker execution is being integrated.")
            case .together:
    ...
    private var retryAction: (() -> Void)? {
        guard allowsRetry else { return nil }
        if turn.mode == .council { return turn.councilRun?.retryEligibility.allowsRetry == true ? onRetry : nil }
        return onRetry
    }
    ...
    private var togetherFinalAnswer: some View {
        ResponseCard(
            ...
            onRetry: retryAction,
            retryLabel: turn.mode == .council ? "Retry Council" : "Run collaboration again",
            eyebrow: "Final answer",
            emptyStateMessage: finalAnswerEmptyStateMessage,
            isPrimary: true,
            onContinueArtifact: turn.mode == .council ? onContinueArtifact : nil
        )
...
struct ReturnedCodeSnippet: Identifiable, Equatable {
    ...
    static func extract(from turn: ChatTurn?) -> [ReturnedCodeSnippet] {
        ...
        case .together, .council, .swarm: answer = turn.combinedAnswer
        }
...
private struct DeveloperWorkPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            RivuneSelect(title: "Inspect", selection: $section, options: [("Activity", "Activity"), ("Code", "Code"), ("Context", "Context")])
            .padding(.horizontal, 16)
            .padding(.bottom, 16)

            if !store.turns.isEmpty {
                RivuneSelect(title: "Request", selection: $selectedTurnID, options: [("Latest request", nil)] + store.turns.dropLast().map { (String($0.prompt.prefix(70)), Optional($0.id)) })
                .font(.caption)
                .padding(.horizontal, 18)
                .padding(.bottom, 14)
            }
    ...
    @ViewBuilder private var contextContent: some View {
        if selectedTurn != nil {
            RivuneSelect(title: "Context source", selection: $inspectDraft, options: [("This request", false), ("Next message", true)])
        }
...
private struct CouncilRunDisclosure: View {
    let record: CouncilRunRecord
    var body: some View {
        DisclosureGroup {
            VStack(alignment: .leading, spacing: 14) {
                if let team = record.teamConfiguration {
                    Text("Frozen team for this request").font(.headline)
                    ForEach(team.members, id: \.memberID) { member in
                        Text("\(member.displayName) · \(member.routeRef.registeredRoute == .codexCLI ? "Codex CLI" : "Claude Code CLI") · \(member.requestedModelID ?? "Account default") · \(member.requestedEffort ?? "Automatic")\(member.memberID == team.orchestratorMemberID ? " · Orchestrator" : "")")
                            .font(.caption)
                    }
                }
                if let appointment = record.appointments.last {
                    Text("Lead: \(appointment.participant.displayName)").font(.headline)
                    Text(appointment.reason).font(.caption)
                    Text(appointment.evidenceBasis).font(.caption).foregroundStyle(.secondary)
                    Text("Requested model: \(appointment.participant.modelID ?? "Account default") · Resolved model: unknown").font(.caption)
                }
                ForEach(record.results, id: \.participant.id) { result in
                    DisclosureGroup(result.disclosureLabel) {
                        VStack(alignment: .leading, spacing: 8) {
                            if let error = result.error { Text(error).font(.caption) }
                            Text(result.text ?? "No output returned").textSelection(.enabled)
                        }.frame(maxWidth: .infinity, alignment: .leading)
                    }
                }
                if let attempts = record.retainedFailedDraftAttempts, !attempts.isEmpty {
                    DisclosureGroup("Earlier failed draft attempts") {
                        ForEach(Array(attempts.enumerated()), id: \.offset) { _, result in
                            VStack(alignment: .leading, spacing: 8) {
                                Text(result.disclosureLabel).font(.caption)
                                if let error = result.error { Text(error).font(.caption) }
                                Text(result.text ?? "No output returned").textSelection(.enabled)
                            }.frame(maxWidth: .infinity, alignment: .leading)
                        }
                ...
                if (record.phase == .partial || record.phase == .failed), let explanation = record.retryEligibility.explanation {
                    Text(explanation).font(.caption)
                }
                if let assessment = record.outputBudgetAssessment {
                    Text(assessment.budget?.description ?? "Word limit: \(assessment.status)")
                        .font(.caption).foregroundStyle(.secondary)
                }
                if let receipts = record.outputReceipts {
                    ForEach(Array(receipts.enumerated()), id: \.offset) { _, receipt in
                        DisclosureGroup(receipt.kind.capitalized + " · " + (receipt.wordLimitPassed.map { $0 ? "word count passed" : "word count failed" } ?? "word limit unchecked")) {
                            VStack(alignment: .leading, spacing: 8) {
                                if let count = receipt.wordCount { Text("\(count) words across the entire output").font(.caption) }
                                Text(receipt.output.isSuccessful ? "Output accepted by transport limits" : "Output invalid or provider failed; cannot be a final answer").font(.caption)
                                Text(receipt.semanticGrounding).font(.caption).foregroundStyle(.secondary)
                                if let error = receipt.output.error { Text(error).font(.caption) }
                                Text(receipt.output.text ?? "No output returned").textSelection(.enabled)
                            }.frame(maxWidth: .infinity, alignment: .leading)
                        }
                    }
                }
                DisclosureGroup("Activity") {
                    ForEach(record.events) { event in
                        Text(event.message).font(.caption).frame(maxWidth: .infinity, alignment: .leading)
                    }
                }
            }.padding(.vertical, 12)
        } label: {
            Text("Council · \(record.phase.rawValue.capitalized) · \(record.successfulAnswerCount) independent answer\(record.successfulAnswerCount == 1 ? "" : "s")")
                .font(.subheadline)
        }
        .padding(16)
