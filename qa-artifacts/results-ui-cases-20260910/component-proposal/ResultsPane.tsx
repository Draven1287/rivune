...
import type { HostArtifactSummary } from '../../../prototypes/ai-native-workspace/src/host/contracts';
import { createSavedResultCopy } from '../../../prototypes/ai-native-workspace/src/host/savedResultCopy';
...
export function ResultsPane({records,state,runLabels,onSelect,onRetry,onBack,onClose,notice}:ResultsPaneProps){
 ...
 return <aside className="results-proposal" aria-labelledby={title} onKeyDown={e=>{
  if(e.key==='Escape'&&!e.defaultPrevented){e.preventDefault();e.stopPropagation();onClose();}
 }}>
  <header className="results-proposal-header">
...
function ExactText({text}:{text:string}){
 ...
 useEffect(()=>{const current=session.current;current.open(text);return()=>current.close();},[text]);
 ...
 return <div className="results-proposal-text"><textarea ref={field} aria-label="Saved result text" value={text} readOnly spellCheck={false} wrap="off"/>
  <div className="results-proposal-actions"><button type="button" disabled={copying} onClick={()=>void copy()}>{copying?'Copying…':'Copy saved text'}</button><button type="button" onClick={()=>{field.current?.focus();field.current?.select();}}>Select all text</button></div><p role="status">{status}</p>
 </div>;
}
