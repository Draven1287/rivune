...
public enum EnvironmentPolicy {
    ...
    public static func sanitized(_ source: [String: String], platform: HostPlatform = .macOS) -> [String: String] {
        source.reduce(into: [:]) { result, entry in
            ...
            if platform == .windowsNative {
                canonicalKey = allowed.first { $0.caseInsensitiveCompare(entry.key) == .orderedSame }
            } else {
                canonicalKey = allowed.contains(entry.key) ? entry.key : nil
            }
            guard let canonicalKey, !entry.value.contains("\0"), !entry.value.contains("\n") else { return }
