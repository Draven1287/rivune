...
if case .missing = ExecutableDiscovery.resolve(.init(executable: "codex", host: win, environment: ["PATH": "C:\\Trusted", "Path": "C:\\Other"]), fileSystem: FS(paths: ["C:\\Trusted\\codex.EXE", "C:\\Other\\codex.EXE"])) { check(true, "ambiguous Windows PATH fails closed") } else { check(false, "ambiguous Windows PATH fails closed") }
...
let explicitScript = "C:\\Fixture\\codex.cmd"
if case .unsupported(let message) = ExecutableDiscovery.resolve(.init(executable: "codex", host: win, environment: [:], explicitPath: explicitScript), fileSystem: FS(paths: [explicitScript])) { check(message.contains("direct .exe"), "explicit Windows script fail closed") } else { check(false, "explicit Windows script fail closed") }
let directScriptRunner = Launcher()
do {
    _ = try ProviderRunner(launcher: directScriptRunner).run(.init(host: win, executablePath: "C:\\Fixture\\codex.CMD", arguments: [], standardInput: nil, workingDirectory: nil, environment: [:]), support: .init(status: .proposedAdapter, explanation: "fixture"))
    check(false, "direct Windows script runner block")
} catch { check(directScriptRunner.requests.isEmpty, "direct Windows script runner block") }
let nativeWindowsRunner = Launcher()
_ = try ProviderRunner(launcher: nativeWindowsRunner).run(.init(host: win, executablePath: "C:\\Program Files\\Codex\\codex.exe", arguments: ["--version"], standardInput: nil, workingDirectory: nil, environment: ["Path": "C:\\Windows"]), support: .init(status: .proposedAdapter, explanation: "fixture"))
check(nativeWindowsRunner.requests.count == 1, "native Windows executable allowed")
