...
final class CrossPlatformProviderCLITests: XCTestCase {
    ...
    func testAmbiguousWindowsPathKeysFailClosed() {
        let outcome = ExecutableDiscovery.resolve(
            .init(executable: "codex", host: x64Windows, environment: ["PATH": "C:\\Trusted", "Path": "C:\\Other"]),
            fileSystem: MemoryFileSystem(executablePaths: ["C:\\Trusted\\codex.EXE", "C:\\Other\\codex.EXE"])
        )
        guard case .missing = outcome else { return XCTFail("Expected ambiguous PATH to fail closed") }
    }
    ...
    func testExplicitWindowsScriptWrapperFailsClosed() {
        let script = "C:\\Fixture\\codex.cmd"
        let outcome = ExecutableDiscovery.resolve(
            .init(executable: "codex", host: x64Windows, environment: [:], explicitPath: script),
            fileSystem: MemoryFileSystem(executablePaths: [script])
        )
        guard case .unsupported(let message) = outcome else { return XCTFail("Expected unsupported explicit wrapper") }
        XCTAssertTrue(message.contains("direct .exe"))
    }

    func testRunnerRejectsWindowsScriptEvenWhenDiscoveryIsBypassed() {
        let launcher = RecordingLauncher()
        let request = CommandRequest(host: x64Windows, executablePath: "C:\\Fixture\\codex.CMD", arguments: [], standardInput: nil, workingDirectory: nil, environment: [:])
        XCTAssertThrowsError(try ProviderRunner(launcher: launcher).run(request, support: .init(status: .proposedAdapter, explanation: "fixture")))
        XCTAssertTrue(launcher.requests.isEmpty)
    }

    func testRunnerAllowsNativeWindowsExecutable() throws {
        let launcher = RecordingLauncher()
        let request = CommandRequest(host: x64Windows, executablePath: "C:\\Program Files\\Codex\\codex.exe", arguments: ["--version"], standardInput: nil, workingDirectory: nil, environment: ["Path": "C:\\Windows"])
        _ = try ProviderRunner(launcher: launcher).run(request, support: .init(status: .proposedAdapter, explanation: "fixture"))
        XCTAssertEqual(launcher.requests.single?.executablePath, request.executablePath)
    }
