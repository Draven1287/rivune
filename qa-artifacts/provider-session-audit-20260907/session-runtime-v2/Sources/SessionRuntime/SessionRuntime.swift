...
public struct ApprovedTurnInput: Codable, Equatable, Sendable {
    ...
    func validate() throws {
        guard !message.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty, message.utf8.count <= 16_384,
              documents.count <= 6, documents.reduce(0, { $0 + $1.bytes.count }) + (projectInstructions?.utf8.count ?? 0) <= 20_000,
...
public actor ConversationSessionRuntime {
    ...
    public func receive(_ e: SessionEnvelope) throws -> EventEffect {
        ...
        case .acknowledged(let session, let turn):
            guard session == e.sessionID, turn == e.turnID,
                  ...
                  !next.bindings.contains(where: { $0.id != old.bindingID && $0.key.route == old.key.route && $0.providerSessionID == session }),
    ...
    private static func validate(_ s: Snapshot) throws {
        ...
        var providerSessions: Set<String> = []
        for b in s.bindings {
            if let id = b.providerSessionID {
                guard validID(id), providerSessions.insert(b.key.route.rawValue + ":" + id).inserted else { throw SessionError.corruptStore }
            }
        }
        for r in s.records {
