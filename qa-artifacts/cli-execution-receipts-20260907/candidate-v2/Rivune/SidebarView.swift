...
struct SidebarView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            brandHeader
                .padding(.bottom, RivuneInterface.sidebarSectionGap)
        ...
        Button(action: newThread) {
            HStack(spacing: 9) {
                ...
                Text("New conversation")
                    .font(RivuneTypography.sidebarLabel.weight(.medium))
                Spacer(minLength: 0)
                #if os(macOS)
                Text("⌘ N")
                    .font(.system(size: 10))
    ...
    private var visibleProjects: [RivuneProject] {
        Array(store.projects.filter { !$0.isArchived && (store.searchText.isEmpty || $0.name.localizedCaseInsensitiveContains(store.searchText)) }.prefix(4))
    }
            ...
            ForEach(visibleProjects) { project in
                HStack {
                    Button { store.selectedProjectID = project.id; section = .projects; openDetail() } label: {
                        Label(project.name, systemImage: "folder").font(RivuneTypography.sidebarLabel).lineLimit(1).frame(maxWidth: .infinity, alignment: .leading).padding(8)
                    }.buttonStyle(.plain)
    ...
    private func navigationRow(_ title: String, symbol: String, destination: WorkspaceSection, action: @escaping () -> Void) -> some View {
        ...
        return Button {
            ...
            HStack(spacing: 10) {
                ...
                Text(title).font(RivuneTypography.sidebarLabel.weight(selected ? .medium : .regular))
                Spacer(minLength: 0)
            }
            .foregroundStyle(selected ? RivunePalette.primaryText : RivunePalette.secondaryText)
            .padding(.horizontal, 11)
            .frame(height: RivuneInterface.sidebarRowHeight)
    ...
    private var searchRow: some View {
        HStack(spacing: 9) {
            ...
            TextField("Search conversations", text: $store.searchText)
                ...
                .font(RivuneTypography.sidebarLabel)
    ...
    private var workspaceHeading: some View {
        HStack(spacing: 7) {
            Text("RECENT")
                .font(RivuneTypography.sectionLabel)
                .tracking(0.8)
                .foregroundStyle(RivunePalette.tertiaryText)
                .lineLimit(1)
            Spacer(minLength: 2)
    ...
    private var workspaceActions: [RivuneMenuAction] {
        ...
        #endif
        actions.append(RivuneMenuAction(title: "Settings", id: "settings", systemImage: "gearshape", section: "Workspace") { store.presentSettings() })
        return actions
    }
        ...
        return HStack(spacing: 0) {
            Button {
                ...
                HStack(spacing: 9) {
                    ...
                    Text(conversation.title)
                        .font(RivuneTypography.sidebarLabel.weight(selected ? .medium : .regular))
                        .lineLimit(1)
                ...
                .padding(.leading, 10)
                .frame(maxWidth: .infinity, minHeight: RivuneInterface.sidebarRowHeight, alignment: .leading)
            ...
            RivuneActionMenu(title: "Actions for \(conversation.title)", actions: conversationActions(conversation)) {
                Image(systemName: "ellipsis")
                    .font(.system(size: 10))
                    .foregroundStyle(RivunePalette.tertiaryText)
                    .frame(width: 25, height: RivuneInterface.sidebarRowHeight)
            }
            .fixedSize()
    ...
    // Reading the verified account projection is inert; provider CLI readiness is unrelated.
    ...
    private var accountFooterActions: [RivuneMenuAction] {
        [
            RivuneMenuAction(title: "Rivune Account", id: "rivune-account", systemImage: "person.crop.circle") {
                store.presentSettings(.account)
            },
            RivuneMenuAction(title: "Settings", id: "settings", systemImage: "gearshape") {
    ...
    private var footerControls: some View {
        VStack(spacing: 8) {
            ...
            HStack(spacing: 6) {
                ...
                #endif
                RivuneActionMenu(title: "Rivune Account and Settings, \(accountFooterName), \(accountFooterStatus)", actions: accountFooterActions) {
                    HStack(spacing: 10) {
                        ZStack {
                            Circle().fill(RivunePalette.surfaceRaised)
                            if let identity = verifiedAccountIdentity {
                        ...
                        VStack(alignment: .leading, spacing: 3) {
                            Text(accountFooterName)
                                .font(.system(size: 12, weight: .medium))
                                .foregroundStyle(RivunePalette.primaryText)
                                .lineLimit(1)
                                .truncationMode(.middle)
                            Text(accountFooterStatus)
                                .font(.system(size: 10))
                                .foregroundStyle(RivunePalette.secondaryText)
                                .lineLimit(1)
                        }
                    ...
                    .background(RivunePalette.surface.opacity(0.82), in: RoundedRectangle(cornerRadius: 10))
                    .overlay(RoundedRectangle(cornerRadius: 10).stroke(RivunePalette.hairline, lineWidth: 0.6))
                    .contentShape(RoundedRectangle(cornerRadius: 10))
                }
                .help("Open Rivune Account or Settings")
            }
        }
        .padding(.top, 8)
    }
