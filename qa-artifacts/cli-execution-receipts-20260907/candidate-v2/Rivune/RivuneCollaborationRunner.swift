...
struct RivuneCollaborationRunner: Sendable {
    ...
    func run(
        ...
        turn.togetherTrace?.proposedPlan = proposedPlanResult.text
        ...
        turn.togetherTrace?.reviewedPlan = challengedPlan.result?.text
        ...
        guard !codexContributionPrompt.isEmpty, !claudeContributionPrompt.isEmpty else {
            return await failed(turn, phase: .contributing, message: "The complete shared contract exceeds the request size limit. Nothing was truncated; shorten the plan and retry.", onUpdate: onUpdate)
        }
        ...
        turn.togetherTrace?.chatGPTRawContribution = codexContribution.result?.text
        turn.togetherTrace?.claudeRawContribution = claudeContribution.result?.text
        ...
        guard RivuneStore.contributionRespectsOwnership(codexText, plan: sharedPlan, role: "Codex"),
              RivuneStore.contributionRespectsOwnership(claudeText, plan: sharedPlan, role: "Claude") else {
            return await failed(turn, phase: .contributing,
                message: "A contribution did not match its assigned files or handoff contract. The complete drafts are preserved; integration was stopped.", onUpdate: onUpdate)
        }
        ...
        guard !reviewPrompts.chatGPT.isEmpty, !reviewPrompts.claude.isEmpty else {
            return await failed(turn, phase: .reviewing, message: "The complete artifacts exceed the review size limit. Drafts are preserved without truncation; reduce their size and retry.", onUpdate: onUpdate)
        }
        ...
        turn.togetherTrace?.chatGPTRawReview = codexReviewResult.result?.text
        turn.togetherTrace?.claudeRawReview = claudeReviewResult.result?.text
        ...
        guard !synthesisPrompt.isEmpty else {
            return await failed(turn, phase: .integrating, message: "The complete artifacts and reviews exceed the integration size limit. They are preserved without truncation; reduce their size and retry.", onUpdate: onUpdate)
        }
        ...
        if let text = integrated, !RivuneStore.integratedFilesRespectPlan(text, plan: sharedPlan) { integrated = nil }
        ...
        turn.togetherTrace?.integrationProvider = provenance
    ...
    private func attempt(
        ...
        guard !prompt.isEmpty else { return RunnerAttempt(result: nil, error: .promptTooLong) }
        do {
