...
struct RootView: View {
    ...
    var body: some View {
        ...
        #if os(macOS)
        .sheet(isPresented: Binding(get: { store.showUniversalAPI || store.showSettingsAPI }, set: { value in
            if !value { store.showUniversalAPI = false; store.showSettingsAPI = false }
        })) {
    ...
    private var appContent: some View {
        ZStack {
            workspace
                #if os(macOS)
                .background(WorkspaceSettingsFocusRestoration(store: store))
                #endif
                .opacity(isSettingsPresented ? 0 : 1)
                .disabled(isSettingsPresented)
                .allowsHitTesting(!isSettingsPresented)
                .accessibilityElement(children: isSettingsPresented ? .ignore : .contain)
                .accessibilityHidden(isSettingsPresented)

            if isSettingsPresented {
                SettingsView(store: store) { store.closeWorkspaceSettings() }
                    .accessibilityIdentifier("rivune.workspace.settings")
            }
        }
    }

    private var isSettingsPresented: Bool { store.showSettings }
...
#if os(macOS)
/// Keeps the workspace responder while the same mounted workspace is covered by Settings.
private struct WorkspaceSettingsFocusRestoration: NSViewRepresentable {
    @ObservedObject var store: RivuneStore
    ...
    func makeNSView(context: Context) -> NSView {
        ...
        context.coordinator.observation = store.$showSettings.removeDuplicates().sink { [weak coordinator = context.coordinator] presented in
            coordinator?.settingsChanged(presented)
        }
        return view
    }
    ...
    @MainActor final class Coordinator {
        ...
        weak var previousWindow: NSWindow?
        var observation: AnyCancellable?
        var wasPresented = false

        func settingsChanged(_ presented: Bool) {
            if presented, !wasPresented, let window = view?.window {
                previousWindow = window
                previousResponder = window.firstResponder
            } else if !presented, wasPresented {
                let responder = previousResponder
                let window = previousWindow
                // Wait for SwiftUI to re-enable the retained workspace. Never activate another window or app.
                DispatchQueue.main.async { [weak self, weak responder, weak window] in
                    guard let self, !self.wasPresented, let window, window.isKeyWindow,
                          window.attachedSheet == nil, let responder else { return }
                    if let responderView = responder as? NSView, responderView.window !== window { return }
