...
struct UniversalAPIWorkspaceView: View {
    ...
    private var connectionBar: some View {
        HStack(spacing: 12) {
            RivuneSelect(title: "Provider", selection: $api.selectedID,
                options: api.snapshot.connections.map { ($0.name, Optional($0.id)) })
                .frame(maxWidth: 230)
            ...
            Spacer(minLength: 5)
            RivuneActionMenu(title: "API connection settings", actions: connectionActions) {
    ...
    private var connectionActions: [RivuneMenuAction] {
        var actions: [RivuneMenuAction] = []
        if api.selected?.usesKey == true {
            actions.append(.init(title: "Replace API key", id: "replace-key", systemImage: "key") {
                replacementKey = ""; replacingKey = true
            })
        }
        actions.append(.init(title: "Check available models", id: "check-models", systemImage: "arrow.clockwise") {
            Task { await api.checkModels() }
        })
        actions.append(.init(title: "New conversation", id: "new-conversation", systemImage: "square.and.pencil") { confirmClear = true })
        actions.append(.init(title: "Remove connection", id: "remove-connection", systemImage: "trash",
            section: "Connection", destructive: true) { confirmRemoval = true })
        return actions
    }
        ...
        ZStack {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    RivuneSelect(title: "Provider", selection: $presetID,
                        options: UniversalAPIPreset.all.map { ($0.name, $0.id) })
                    ...
                    RivuneSelect(title: "API format", selection: $style,
                        options: UniversalAPIStyle.allCases.map { ($0.title, $0) })
