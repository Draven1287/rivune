...
final class RivuneAccount: ObservableObject {
    enum SignOutRecovery: String, Equatable, Sendable {
        case none
        case localSessionRemovalPending
    }
    ...
    @Published private(set) var signOutRecovery: SignOutRecovery = .none
    ...
    private var acceptsRefresh = false
    ...
    var signOutPending: Bool { signOutRecovery != .none }
    ...
    init(configuration: RivuneAccountConfiguration? = .bundled, defaults: UserDefaults = .standard,
        ...
        if defaults.bool(forKey: "rivune.account.explicitlySignedOut.\(configuration?.url.host ?? "unconfigured").pending") {
            self.signOutRecovery = .localSessionRemovalPending
        }
    }
    ...
    // Synchronous event admission lets the observer acknowledge refreshes even
    // while an operation awaits the backend. Repeated events coalesce.
    func receive(_ event: RivuneAccountEvent) {
        switch event {
        case .signedOut:
            invalidateIdentity()
            challengeEmail = nil
        case .refreshed:
            guard acceptsRefresh, !signOutPending else { return }
            refreshPending = true
            scheduleRefresh()
        }
    }
    private func scheduleRefresh() {
        guard refreshPending, !busy, acceptsRefresh else { return }
        refreshPending = false
        let ticket = revision
        Task { [weak self] in
            guard let self, self.revision == ticket, self.acceptsRefresh else { return }
            // A user action may have begun before this task was scheduled.
            if self.busy { self.refreshPending = true; return }
            await self.restore()
        }
    }
    private func finishOperation(_ ticket: UInt64) {
        guard revision == ticket else { return }
        busy = false
        scheduleRefresh()
    }
    private func invalidateIdentity() {
        refreshPending = false; acceptsRefresh = false
    ...
    func restore() async {
        guard sessionAccessRequested, acceptsRefresh, let client, !busy, !defaults.bool(forKey: signedOutKey) else { return }
        revision &+= 1; let ticket = revision
        busy = true; identity = nil; verifiedUserID = nil
        defer { finishOperation(ticket) }
        do {
    ...
    func signOut() async {
        ...
        busy = true; signOutRecovery = .localSessionRemovalPending; challengeEmail = nil; verificationFailed = false
        ...
        do {
            ...
            signOutRecovery = .none; defaults.set(false, forKey: pendingKey); busy = false
            message = "Signed out on this device. Your Rivune account, AI connections, and local conversations are unchanged."
        } catch {
            busy = false
            message = "Local sign-out is incomplete. Your account is hidden, but Rivune could not confirm removal of the saved session on this device. No remote account or AI-provider access was revoked."
        }
    }
    var canCancel: Bool { busy && !signOutPending && !signOutInFlight }
    func cancelCode() {
        guard !signOutPending, !signOutInFlight else { return }
        invalidateIdentity()
        challengeEmail = nil; message = nil; verificationFailed = false
    }
}
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if account.signOutPending {
                Text("Local sign-out needs another attempt").font(.headline)
                Text("Rivune could not confirm removal of the saved session on this device. Your account stays hidden and your local workspace remains available. This does not revoke your Rivune account or any AI-provider account.")
                Button("Retry local sign-out") { Task { await account.signOut() } }
            ...
            if account.busy {
                HStack {
                    ...
                    if account.canCancel {
                        Button("Cancel sign-in") { account.cancelCode(); code = "" }
                    }
                }
            }
