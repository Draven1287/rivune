...
// BEGIN Rivune appearance preference policy (pure values; no UserDefaults writes).
enum RivuneBackgroundPreset: String, CaseIterable, Identifiable {
    case graphite, orbit, cosmos
    var id: String { rawValue }
    var title: String {
        switch self { case .graphite: "Graphite"; case .orbit: "Orbit"; case .cosmos: "Cosmos" }
    }
    var detail: String {
        switch self {
        case .graphite: "Quiet, flat darkness"
        case .orbit: "Subtle paths of light"
        case .cosmos: "Depth and starlight"
        }
    }
}

struct RivuneAppearanceConfiguration: Equatable {
    let preset: RivuneBackgroundPreset?
    let galaxy: Bool
    let stars: Bool
    let dim: Double
}

enum RivuneAppearancePolicy {
    static let presetKey = "rivune.appearance.backgroundPreset"
    static func preset(from rawValue: String?) -> RivuneBackgroundPreset? {
        rawValue.flatMap(RivuneBackgroundPreset.init(rawValue:))
    }
    static func configuration(rawPreset: String?, galaxy: Bool, stars: Bool, dim: Double, presetStars: Bool? = nil, presetDim: Double? = nil) -> RivuneAppearanceConfiguration {
        guard let preset = preset(from: rawPreset) else {
            // Missing/unknown presets are old preferences, not an implicit new default.
            return .init(preset: nil, galaxy: galaxy, stars: stars, dim: dim)
        }
        let chosenDim = presetDim ?? dim
        let restrainedDim = chosenDim.isFinite ? min(max(chosenDim, 0.30), 0.85) : 0.35
        return .init(preset: preset, galaxy: preset == .cosmos,
                     stars: preset == .cosmos && (presetStars ?? stars), dim: restrainedDim)
    }
}
// END Rivune appearance preference policy.
...
struct RivuneBackground: View {
    ...
    @AppStorage(RivuneAppearancePolicy.presetKey) private var rawPreset = ""
    @AppStorage("rivune.appearance.presetDim") private var presetDim: Double?
    @AppStorage("rivune.appearance.presetStars") private var presetStars: Bool?
    var body: some View {
        RivuneBackgroundSurface(configuration: RivuneAppearancePolicy.configuration(
            rawPreset: rawPreset, galaxy: galaxy, stars: stars, dim: dim, presetStars: presetStars, presetDim: presetDim
        ))
        .ignoresSafeArea()
    }
}

/// Shared by the actual workspace and thumbnails. Entirely static, including Orbit.
struct RivuneBackgroundSurface: View {
    let configuration: RivuneAppearanceConfiguration
    var body: some View {
        ZStack {
            ...
            GeometryReader { geometry in
                Image("RivuneWorkspaceCosmos")
                    ...
                    .clipped()
                    .opacity(configuration.galaxy ? 0.92 : 0)
            }
            .accessibilityHidden(true)
            if configuration.preset == .orbit { RivuneOrbitBackground() }
            Color.black.opacity(configuration.dim)
            RivuneStarfield(density: 210, opacity: configuration.stars ? 0.62 : 0)
        }
...
private struct RivuneOrbitBackground: View {
    var body: some View {
        Canvas { context, size in
            let scale = max(size.width, size.height)
            for index in 0..<4 {
                let offset = CGFloat(index) * 0.13
                let center = CGPoint(x: size.width * (0.16 + offset), y: size.height * (0.26 + offset * 0.6))
                var arc = Path()
                arc.addArc(center: center, radius: scale * (0.48 + offset),
                           startAngle: .degrees(-76), endAngle: .degrees(228), clockwise: false)
                let ink = Color(red: 0.48, green: 0.65, blue: 0.79)
                context.stroke(arc, with: .color(ink.opacity(0.06)), lineWidth: 9)
                context.stroke(arc, with: .color(ink.opacity(index == 1 ? 0.28 : 0.16)), lineWidth: 1)
            }
        }
        .clipped()
        .allowsHitTesting(false)
...
struct RivuneBackgroundPresetCard: View {
    let preset: RivuneBackgroundPreset
    let isSelected: Bool
    let stars: Bool
    let dim: Double
    ...
    var body: some View {
        Button(action: action) {
            VStack(alignment: .leading, spacing: 9) {
                RivuneBackgroundSurface(configuration: RivuneAppearancePolicy.configuration(
                    rawPreset: preset.rawValue, galaxy: true, stars: stars, dim: dim
                ))
                .frame(height: 82)
                ...
                HStack(spacing: 6) {
                    ...
                    Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
                        .foregroundStyle(isSelected ? RivunePalette.rivune : RivunePalette.tertiaryText)
                        .accessibilityHidden(true)
                }
                Text(preset.detail).font(.caption).foregroundStyle(RivunePalette.secondaryText)
                    .fixedSize(horizontal: false, vertical: true)
            }
            .padding(10).frame(maxWidth: .infinity, alignment: .leading)
            .foregroundStyle(RivunePalette.primaryText)
            .background(RivunePalette.composer, in: RoundedRectangle(cornerRadius: 12))
            .overlay(RoundedRectangle(cornerRadius: 12).stroke(
                focused || isSelected ? RivunePalette.rivune.opacity(0.85) : RivunePalette.hairline,
                lineWidth: focused || isSelected ? 1.5 : 1
            ))
...
/// Rivune-drawn brightness control with pointer, keyboard and adjustable accessibility input.
struct RivuneBackgroundBrightnessControl: View {
    @Binding var value: Double
    let range: ClosedRange<Double>
    @FocusState private var focused: Bool
    @Environment(\.isEnabled) private var enabled
    private var safeValue: Double { min(max(value.isFinite ? value : 0.65, range.lowerBound), range.upperBound) }
    private var percentage: Int { Int((safeValue * 100).rounded()) }
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Text("Background brightness").font(.subheadline)
                Spacer(minLength: 8)
                Text("\(percentage)%").font(.caption.monospacedDigit()).foregroundStyle(RivunePalette.secondaryText)
            }
            HStack(spacing: 10) {
                Button { adjust(-0.05) } label: { Image(systemName: "minus").frame(width: 28, height: 28) }
                    .buttonStyle(RivuneButtonStyle()).disabled(!enabled || safeValue <= range.lowerBound)
                    .accessibilityLabel("Decrease background brightness")
                GeometryReader { geometry in
                    let travel = max(geometry.size.width - 14, 1)
                    let fraction = (safeValue - range.lowerBound) / (range.upperBound - range.lowerBound)
                    ZStack(alignment: .leading) {
                        Capsule().fill(RivunePalette.surfaceRaised).frame(height: 5)
                        Capsule().fill(RivunePalette.rivune.opacity(0.6)).frame(width: max(7, travel * fraction + 7), height: 5)
                        Circle().fill(RivunePalette.rivune).frame(width: 14, height: 14).offset(x: travel * fraction)
                    }
                    .frame(height: 28)
                    .contentShape(Rectangle())
                    .gesture(DragGesture(minimumDistance: 0).onChanged { drag in
                        guard enabled else { return }
                        let fraction = min(max((drag.location.x - 7) / travel, 0), 1)
                        value = range.lowerBound + fraction * (range.upperBound - range.lowerBound)
                    })
                }.frame(height: 28)
                Button { adjust(0.05) } label: { Image(systemName: "plus").frame(width: 28, height: 28) }
                    .buttonStyle(RivuneButtonStyle()).disabled(!enabled || safeValue >= range.upperBound)
                    .accessibilityLabel("Increase background brightness")
            }
        }
        .padding(10)
        .background(RivunePalette.control, in: RoundedRectangle(cornerRadius: 10))
        .overlay(RoundedRectangle(cornerRadius: 10).stroke(focused ? RivunePalette.rivune : .clear, lineWidth: 1))
        .focusable().focused($focused).focusEffectDisabled()
        .onKeyPress(.leftArrow) { adjust(-0.05); return .handled }
        .onKeyPress(.rightArrow) { adjust(0.05); return .handled }
        .accessibilityElement(children: .ignore)
        .accessibilityLabel("Background brightness")
        .accessibilityValue("\(percentage) percent")
        .accessibilityAdjustableAction { direction in
            switch direction { case .increment: adjust(0.05); case .decrement: adjust(-0.05); @unknown default: break }
        }
    }
    private func adjust(_ delta: Double) {
        guard enabled else { return }
        value = min(max(safeValue + delta, range.lowerBound), range.upperBound)
    }
}
...
enum RivuneInterface {
    ...
    static let composerControlGap: CGFloat = 8
    static let composerContentInset: CGFloat = 16
    static let contextInset: CGFloat = 10
    static let contextLineGap: CGFloat = 4
    static let contextRemoveTarget: CGFloat = 32
    static let sidebarRowHeight: CGFloat = 36
    static let sidebarSectionGap: CGFloat = 16
