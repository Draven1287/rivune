...
final class ArtifactContinuationTests: XCTestCase {
    ...
    func testMixedMalformedTurnContainersCannotHideInvalidNewMetadata() throws {
        let (conversation, _) = try fixture()
        let containers: [Any] = [
            [["selectedArtifact": false], NSNull()],
            [["selectedArtifact": false], 7],
            [NSNull(), ["selectedArtifact": false]],
            [[ ["selectedArtifact": false] ]],
            ["selectedArtifact": false],
            [["councilRun": [["selectedArtifact": false]]]]
        ]
        for container in containers {
            ...
            let backup = current.deletingLastPathComponent().appendingPathComponent("conversations.backup.json")
            let good = try JSONEncoder().encode([conversation]); try good.write(to: backup)
            XCTAssertTrue(RivuneHistoryStorage.hasInvalidArtifactSelections(bytes))
            XCTAssertTrue(RivuneHistoryStorage.load(from: root).isEmpty)
            XCTAssertFalse(RivuneHistoryStorage.save([conversation], in: root))
