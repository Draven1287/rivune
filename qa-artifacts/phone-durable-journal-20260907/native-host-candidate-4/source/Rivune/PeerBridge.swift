...
struct BridgePeerContext: Equatable, Sendable {
...
final class PeerBridge: ObservableObject, @unchecked Sendable {
    ...
    /// Host-derived identity for the mutually authenticated paired peer.
    @Published private(set) var authenticatedPrincipal: String?
    @Published private(set) var connectionGeneration: UUID?

    var onEnvelope: ((BridgeEnvelope, BridgePeerContext?) -> Void)?
    ...
    // Queue-owned authorization state. Every application envelope captures this
    // exact peer and connection generation before crossing to the main actor.
    private var activePrincipal: String?
    ...
    @MainActor
    func currentPeerContext() -> BridgePeerContext? {
        guard let authenticatedPrincipal, let connectionGeneration else { return nil }
        return .init(principal: authenticatedPrincipal, generation: connectionGeneration)
    }

    /// Selects and authorizes the transport in one queue transaction so a
    /// re-pair between a UI check and send cannot redirect an application frame.
        ...
        candidate.stateUpdateHandler = { [weak self, weak candidate] newState in
            ...
            self.queue.async { [self] in
                ...
                case .ready:
                    ...
                    if let authenticatedCredential = self.credential {
                        let principal = SHA256.hash(data: Data((authenticatedCredential.serviceID + ":" + authenticatedCredential.identity.base64EncodedString()).utf8))
                            .map { String(format: "%02x", $0) }.joined()
                        let generation = UUID()
                        self.activePrincipal = principal
                        self.activeConnectionGeneration = generation
                        DispatchQueue.main.async { [weak self] in
                            self?.authenticatedPrincipal = principal
    ...
    private func parseFrames(on candidate: NWConnection) {
        while receiveBuffer.count >= MemoryLayout<UInt32>.size {
            ...
            default:
                #if os(macOS)
                // Initial-pairing transport is not yet a durable authenticated principal.
                guard pendingPairingExpiresAt == nil, pendingReconnectCredential == nil else { continue }
                #endif
                break
            }
            let peerContext = activePrincipal.flatMap { principal in
                activeConnectionGeneration.map { BridgePeerContext(principal: principal, generation: $0) }
            }
            DispatchQueue.main.async { [weak self] in
                self?.onEnvelope?(envelope, peerContext)
            }
        }
    }
    ...
    private func onConnectionChangedOnMain(_ connected: Bool) {
        DispatchQueue.main.async { [weak self] in
            if !connected {
                self?.authenticatedPrincipal = nil
    ...
    #if DEBUG
    @MainActor
    func setTestAuthenticatedPeer(_ context: BridgePeerContext?) {
        authenticatedPrincipal = context?.principal
        connectionGeneration = context?.generation
        queue.sync {
            activePrincipal = context?.principal
            activeConnectionGeneration = context?.generation
        }
    }
    #endif
