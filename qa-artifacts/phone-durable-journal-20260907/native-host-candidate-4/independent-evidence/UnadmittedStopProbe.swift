...
precondition(!update.accepted && update.terminalState==nil && host.runCoordinator.cancelCount==0)
precondition(journal.record(requestID:id)==nil)
var pendingPhoneStopRequested=true
var isGenerating=true
// Exact first condition of the iOS terminal-settlement gate. Other request/conversation matches are valid in this fixture.
...
precondition(host.notices.allSatisfy {!$0.accepted && $0.terminalState==nil})
print("REPRODUCED: Stop before Mac admission is rejected without terminal status; phone remains pending and generating across repeated reconnect Stop attempts")
print("No journal tombstone is recorded for the unadmitted request")
