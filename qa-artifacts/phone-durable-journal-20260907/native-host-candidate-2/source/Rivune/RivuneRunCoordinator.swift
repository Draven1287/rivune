...
final class RivuneRunCoordinator: ObservableObject {
    ...
    /// Phone Stop remains a request until the owned provider task exits. This
    /// keeps the durable bridge from claiming termination before it is observed.
    func requestCancel(_ id: UUID) {
        guard runs.contains(where: { $0.id == id && $0.status == .running }) else { return }
        tasks[id]?.cancel()
    }
