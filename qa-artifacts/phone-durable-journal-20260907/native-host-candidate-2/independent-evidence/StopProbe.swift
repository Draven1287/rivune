...
struct BridgeStopUpdate { let requestID: UUID; let accepted: Bool; let message: String }
struct Run { let status: String; let answer: String }
extension String { static var running: String { "running" } }
final class Bridge { func isCurrent(_ peer: BridgePeerContext) -> Bool { true } }
final class Coordinator { var cancelCount=0; var snapshot: Run?; func requestCancel(_ id: UUID) { cancelCount += 1 }; func remoteSnapshot(id: UUID) -> Run? { snapshot } }
...
final class Host {
 let remotePrincipalOverride: String? = nil
 let bridge = Bridge()
 let remoteJournal: RemoteRequestJournal?
 struct Observation { let fingerprint: String; let peer: BridgePeerContext }
 var remoteObservations: [UUID:Observation] = [:]
 let runCoordinator = Coordinator()
 var sentAnswers: [String] = []
 ...
 static func bridgeUpdate(run:Run) -> String {run.answer}
 func sendBridgeUpdate(_ answer:String,peerContext:BridgePeerContext) {sentAnswers.append(answer)}
 func sendStopUpdate(_ update:BridgeStopUpdate,peerContext:BridgePeerContext) {}
...
let journal=try RemoteRequestJournal(storage:Memory()),id=UUID(),fp=SHA256Digest.hash(string:"original request")
let identity=RemoteAcceptedIdentity(requestID:id,turnID:UUID(),mode:.chatGPT,routes:[Host.remoteDeviceIdentity(principal:"phone-A")],requestedModelIDs:[],requestedEfforts:[],contextVersion:2,attachmentSetDigest:.hash(string:"none"))
_ = try journal.admit(identity:identity,requestFingerprint:fp)
let host=Host(journal),stop=BridgeStopRequest(requestID:id,requestFingerprint:fp.hex)
host.handleRemoteStop(stop,peerContext:.init(principal:"phone-B",generation:UUID()))
precondition(host.runCoordinator.cancelCount==0)
precondition(journal.record(requestID:id)?.state == .running)
print("PASS: exact candidate2 Stop method rejects foreign owner")
let verified=try RemoteResultReference(workspaceRunID:id,revision:1,resultDigest:.hash(string:"verified original payload"))
try journal.complete(requestID:id,requestFingerprint:fp,result:verified)
host.runCoordinator.snapshot=Run(status:"complete",answer:"CORRUPTED payload not matching journal reference")
host.handleRemoteStop(stop,peerContext:.init(principal:"phone-A",generation:UUID()))
precondition(host.sentAnswers==["CORRUPTED payload not matching journal reference"])
print("CONFIRMED: completed Stop path sends raw coordinator snapshot without validating stored result reference")
