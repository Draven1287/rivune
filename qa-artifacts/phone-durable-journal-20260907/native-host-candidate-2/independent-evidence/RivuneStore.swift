...
final class RivuneStore: ObservableObject {
    ...
    private func handleBridgeEnvelope(_ envelope: BridgeEnvelope, peerContext: BridgePeerContext?) {
        ...
        case .stopUpdate:
            #if os(iOS)
            guard let update = envelope.stopUpdate, update.requestID == activeBridgeRequest?.id else { return }
            showPrototypeNotice(update.message)
            if update.accepted, let request = activeBridgeRequest, let conversationID = request.conversationID,
               let conversationIndex = conversations.firstIndex(where: { $0.id == conversationID }),
               let turnIndex = conversations[conversationIndex].turns.firstIndex(where: { $0.id == request.turnID }) {
                ...
                switch turn.mode {
                case .chatGPT: if turn.chatGPTAnswer == nil { turn.chatGPTError = "The local Rivune task stopped before it finished." }
                case .claude: if turn.claudeAnswer == nil { turn.claudeError = "The local Rivune task stopped before it finished." }
                case .together, .council, .swarm: if turn.combinedAnswer == nil { turn.combinedError = "The local Rivune task stopped before it finished." }
                }
                ...
                _ = clearPendingPhoneRequest()
                activeRequestID = nil; isGenerating = false; councilStage = .cancelled
            }
            #endif
        }
    }
    ...
    private func handleDurableRemoteRunUpdate(_ run: WorkspaceRun) {
        ...
        do {
            if case .stopRequested = journal.record(requestID: run.id)?.state, run.status == .cancelled {
                ...
                if let observation = remoteObservations[run.id] {
                    sendStopUpdate(.init(requestID: run.id, accepted: true, message: "The local Rivune task stopped"), peerContext: observation.peer)
                }
    ...
    func handleRemoteStop(_ stop: BridgeStopRequest, peerContext: BridgePeerContext) {
        ...
        do {
            ...
            case .alreadyTerminal:
                if let run = runCoordinator.remoteSnapshot(id: stop.requestID), run.status != .running {
                    sendBridgeUpdate(Self.bridgeUpdate(run: run), peerContext: peerContext)
                } else {
                    sendStopUpdate(.init(requestID: stop.requestID, accepted: false, message: "This request already finished, but its saved result is unavailable"), peerContext: peerContext)
                }
            case .conflict:
    ...
    private func sendStopUpdate(_ update: BridgeStopUpdate, peerContext: BridgePeerContext) {
        if remotePrincipalOverride == nil,
           !bridge.isCurrent(peerContext) { return }
        do {
            try bridge.send(.stopUpdate(update))
        } catch {
            bridge.disconnect()
        }
    }
    ...
    @discardableResult
    private func persistPendingPhoneRequest(_ request: BridgePromptRequest, stopRequested: Bool = false) -> Bool {
        if RivuneLaunchContext.isIsolated { return true }
        guard let url = pendingPhoneRequestURL,
              let data = try? JSONEncoder.sorted.encode(PendingPhoneOperation(request: request, stopRequested: stopRequested)),
    ...
    private func restorePendingPhoneRequest() {
        guard !RivuneLaunchContext.isIsolated, let url = pendingPhoneRequestURL,
              ...
              let operation = (try? JSONDecoder().decode(PendingPhoneOperation.self, from: data))
                ?? (try? JSONDecoder().decode(BridgePromptRequest.self, from: data)).map({ PendingPhoneOperation(request: $0, stopRequested: false) }),
    ...
    func send() {
        ...
        guard persistPendingPhoneRequest(bridgeRequest) else {
    ...
    private func cancelGeneration() {
        ...
        bridgeRequestWatchdog?.cancel()
        bridgeRequestWatchdog = nil
        guard persistPendingPhoneRequest(request, stopRequested: true) else {
            showPrototypeNotice("Rivune could not save the Stop request. It was not sent.")
            return
        }
        pendingPhoneStopRequested = true
        do {
            try bridge.send(.stop(.init(requestID: requestID, requestFingerprint: Self.bridgeRequestFingerprint(request))))
