...
@main struct Review {
 static func main() throws {
  ...
  let key = "root-review:" + UUID().uuidString
  var lease: JournalStateLease? = registry.claim(key)!
  let stale = lease!.token;lease=nil
  var successor:JournalStateLease? = registry.claim(key)!
  registry.release(key,ownedBy:stale)
  withExtendedLifetime(successor) {
   if registry.claim(key) != nil {exit(8)}
   print("staleReleaseCannotRemoveSuccessor")
  }
  successor=nil
  let lastLease=registry.claim(key)!
  withExtendedLifetime(lastLease) {print("leaseReacquiredAfterValidOwnerRelease")}
  ...
  do {_=try RemoteRequestJournal(storage:s);exit(9)}catch RemoteJournalError.journalAlreadyOwned {print("postFailureSuccessorRemainsExclusive")}
