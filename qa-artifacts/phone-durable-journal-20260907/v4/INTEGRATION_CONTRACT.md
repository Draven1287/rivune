# Durable phone request journal v4 — integration contract
...
2. Freeze `RemoteAcceptedIdentity` from the admitted request and exact advertised routes. The candidate persists typed mode/version fields and canonical SHA-256 digests for route/model/effort/attachment metadata. It never persists those raw strings. Do not include prompt/history/document contents, credentials, outputs, or diagnostics.
...
- `RemoteRequestJournal` independently claims `storage.journalLeaseKey` through a token-owned RAII lease. A second journal over the same storage object fails with `journalAlreadyOwned`, so two cached snapshots cannot both authorize dispatch. Initialization failure and normal deinitialization release only the lease carrying the matching ownership token; a stale release cannot remove a successor's lease.
