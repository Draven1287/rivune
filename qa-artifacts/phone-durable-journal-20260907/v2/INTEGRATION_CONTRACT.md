# Durable phone request journal v2 — integration contract
...
- Construct exactly one `FileRemoteJournalStorage` for the path. It holds both an in-process ownership lease and an OS advisory writer lock for its lifetime; a second writer fails before journal construction.
