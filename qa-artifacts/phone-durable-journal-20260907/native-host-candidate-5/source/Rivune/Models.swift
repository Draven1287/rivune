...
enum IntelligenceMode: String, CaseIterable, Identifiable, Codable, Sendable {
    ...
    var symbol: String {
        ...
        case .together, .council, .swarm: "arrow.triangle.merge"
    ...
    var accent: Color {
        ...
        case .together, .council, .swarm: RivunePalette.rivune
    ...
    var shortDescription: String {
        ...
        case .together: "Legacy coordinated workflow"
        case .council: "Independent answers, reviewed by a lead"
        case .swarm: "Agents build different parts of a project · unavailable"
...
struct IntelligenceModeSelectionOption: Identifiable, Hashable, Sendable {
    ...
    static func available(
        ...
        var options = [Self(
            mode: .council,
            title: "Council",
            detail: IntelligenceMode.council.shortDescription,
            symbol: IntelligenceMode.council.symbol
        )]
        options.append(Self(mode: .swarm, title: "Swarm · unavailable", detail: IntelligenceMode.swarm.shortDescription, symbol: IntelligenceMode.swarm.symbol))
        var includedModes: Set<IntelligenceMode> = [.council, .swarm]
...
enum PromptHistoryRole: String, Codable, Hashable, Sendable {
    case user
    case assistant
}

struct PromptHistoryMessage: Codable, Hashable, Sendable {
    let turnID: UUID
    let role: PromptHistoryRole
    let content: String
}

struct PromptConversationHistory: Codable, Hashable, Sendable {
    static let currentVersion = 1
    let version: Int
    let messages: [PromptHistoryMessage]

    init(version: Int = Self.currentVersion, messages: [PromptHistoryMessage]) {
        self.version = version
        self.messages = messages
    }
}

struct PromptHistoryField: Codable, Hashable, Sendable {
    ...
    let state: State
    let history: PromptConversationHistory?
    let legacyReference: String?

    static let disabled = Self(state: .disabled, history: nil, legacyReference: nil)
    static func roleAware(_ history: PromptConversationHistory) -> Self {
        Self(state: .roleAware, history: history, legacyReference: nil)
    }
    static func legacyUntrusted(_ reference: String) -> Self {
        Self(state: .legacyUntrusted, history: nil, legacyReference: reference)
    }
}

/// Every authority boundary supplied to a provider. Only the current request,
/// explicit user-role history, and separately approved project instructions
/// may carry instructions. Documents and artifacts remain quoted reference.
struct ApprovedPromptContext: Codable, Hashable, Sendable {
    static let currentVersion = 1
    let version: Int
    let priorConversation: PromptHistoryField
    let approvedProjectInstructions: String?
    let userSelectedDocuments: [PromptAttachment]
    let selectedArtifactReference: ArtifactContinuation?

    init(
        version: Int = Self.currentVersion,
        priorConversation: PromptHistoryField,
        approvedProjectInstructions: String? = nil,
        userSelectedDocuments: [PromptAttachment] = [],
        selectedArtifactReference: ArtifactContinuation? = nil
    ) {
        self.version = version
        self.priorConversation = priorConversation
        self.approvedProjectInstructions = approvedProjectInstructions
        self.userSelectedDocuments = userSelectedDocuments
        self.selectedArtifactReference = selectedArtifactReference
    }

    var isStructurallyValid: Bool {
        guard version == Self.currentVersion,
              (approvedProjectInstructions?.utf8.count ?? 0) <= 8_000,
              userSelectedDocuments.count <= 6,
              userSelectedDocuments.reduce(0, { $0 + $1.byteCount }) + (approvedProjectInstructions?.utf8.count ?? 0) <= 20_000 else { return false }
        switch priorConversation.state {
        case .disabled:
            return priorConversation.history == nil && priorConversation.legacyReference == nil
        case .legacyUntrusted:
            return priorConversation.history == nil && priorConversation.legacyReference != nil
        case .roleAware:
            guard let history = priorConversation.history,
                  priorConversation.legacyReference == nil,
                  history.version == PromptConversationHistory.currentVersion,
                  history.messages.count <= 16,
                  let encoded = try? JSONEncoder().encode(history), encoded.count <= 12_000 else { return false }
            return true
        }
    }
}
...
struct TogetherTrace: Codable, Hashable, Sendable {
    ...
    var reviewedPlan: String?
    var chatGPTRawContribution: String?
    var claudeRawContribution: String?
    var chatGPTRawReview: String?
    var claudeRawReview: String?
...
struct ChatTurn: Identifiable, Codable, Hashable, Sendable {
    ...
    var councilRun: CouncilRunRecord?
    ...
    init(
        ...
        selectedArtifact: ArtifactContinuation? = nil
    ) {
        self.id = id
        self.prompt = prompt
        self.mode = mode
...
enum CouncilStage: Equatable, Codable, Sendable {
    ...
    var title: String {
        switch self {
        case .idle: "Ready"
        case .asking: "Writing independent answers"
        case .comparing: "Reviewing the answers"
        case .synthesizing: "Combining the result"
...
struct BridgeReadiness: Codable, Sendable {
    ...
    /// Exact, Mac-admitted routes. A phone must echo these values with a
    /// request; it may not choose a different or paid fallback transport.
    ...
    init(
        ...
        codexRoute: AIExecutionRoute? = nil,
        claudeRoute: AIExecutionRoute? = nil
    ) {
        self.codex = codex
        self.claude = claude
...
struct BridgePromptRequest: Codable, Sendable {
    ...
    let id: UUID
    let turnID: UUID
    /// Mac workspace destination selected by the phone; the authenticated pairing owns the mapping.
    let conversationID: UUID?
    let prompt: String
    let mode: IntelligenceMode
    let attachments: [PromptAttachment]
    let priorContext: String
    /// Version 2 carries explicit roles and authority channels. `priorContext`
    /// remains decode-only compatibility data and is never promoted to roles.
    let contextVersion: Int?
    let roleAwareContext: ApprovedPromptContext?
    ...
    let togetherWorkflowVersion: Int?
    /// Routes advertised by the connected Mac and selected when the user sent
    /// this request. The Mac validates them again before any provider call.
    let codexRoute: AIExecutionRoute?
    let claudeRoute: AIExecutionRoute?
    ...
    init(
        ...
        attachments: [PromptAttachment],
        priorContext: String,
        contextVersion: Int? = Self.currentContextVersion,
        roleAwareContext: ApprovedPromptContext? = nil,
        ...
        self.roleAwareContext = roleAwareContext
...
struct BridgeStopRequest: Codable, Sendable {
    let requestID: UUID
    let requestFingerprint: String
}

struct BridgeStopUpdate: Codable, Sendable {
    let requestID: UUID
    let accepted: Bool
    let message: String
    let terminalState: TurnExecutionState?

    init(requestID: UUID, accepted: Bool, message: String, terminalState: TurnExecutionState? = nil) {
...
struct BridgeEnvelope: Codable, Sendable {
    ...
    let stopRequest: BridgeStopRequest?
    let stopUpdate: BridgeStopUpdate?
    ...
    private init(
        ...
        stopRequest: BridgeStopRequest? = nil,
        stopUpdate: BridgeStopUpdate? = nil,
    ...
    static func stop(_ value: BridgeStopRequest) -> Self {
        Self(kind: .stopRequest, stopRequest: value)
    }

    static func stopUpdate(_ value: BridgeStopUpdate) -> Self {
        Self(kind: .stopUpdate, stopUpdate: value)
    }
