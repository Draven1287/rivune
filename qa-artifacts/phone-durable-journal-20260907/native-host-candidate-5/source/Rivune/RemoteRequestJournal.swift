...
public enum RemoteAdmissionDisposition: Equatable, Sendable {
    ...
    case conflict
    /// A matching owner durably stopped this request before first admission.
...
public final class RemoteRequestJournal: @unchecked Sendable {
    ...
    private struct PreAdmissionStop: Codable, Equatable {
        let requestID: UUID
        let requestFingerprint: SHA256Digest
        let owner: RemoteRouteIdentity
        let stoppedAt: Date
    }
    ...
    private struct Snapshot: Codable, Equatable {
        ...
        var preAdmissionStops: [PreAdmissionStop]

        private enum CodingKeys: String, CodingKey {
            case schemaVersion, records, tombstones, preAdmissionStops
        }

        init(
            schemaVersion: Int = 2,
            records: [RemoteJournalRecord],
            tombstones: [Tombstone],
            preAdmissionStops: [PreAdmissionStop] = []
        ) {
            self.schemaVersion = schemaVersion
            self.records = records
            self.tombstones = tombstones
            self.preAdmissionStops = preAdmissionStops
        }

        init(from decoder: Decoder) throws {
            let values = try decoder.container(keyedBy: CodingKeys.self)
            schemaVersion = try values.decodeIfPresent(Int.self, forKey: .schemaVersion) ?? 1
            records = try values.decode([RemoteJournalRecord].self, forKey: .records)
            tombstones = try values.decode([Tombstone].self, forKey: .tombstones)
            preAdmissionStops = try values.decodeIfPresent([PreAdmissionStop].self, forKey: .preAdmissionStops) ?? []
        }
    }
    ...
    public init(
        ...
        var requiresSchemaUpgrade = false
        do {
            if let data = try storage.read() {
                ...
                guard decoded.schemaVersion == 1 || decoded.schemaVersion == 2,
                      ...
                      Set(decoded.preAdmissionStops.map(\.requestID)).count == decoded.preAdmissionStops.count else {
                    throw RemoteJournalError.corruptOrUnreadable
                }
                requiresSchemaUpgrade = decoded.schemaVersion == 1
                decoded.schemaVersion = 2
                snapshot = decoded
            } else {
                snapshot = Snapshot(records: [], tombstones: [], preAdmissionStops: [])
            }
        ...
        do {
            ...
            if changed || requiresSchemaUpgrade { try persistCurrentSnapshot() }
        } catch let error as RemoteJournalError {
            throw error
        } catch {
            throw RemoteJournalError.corruptOrUnreadable
    ...
    public func admit(
        ...
        try withLock {
            ...
            if let stopped = snapshot.preAdmissionStops.first(where: { $0.requestID == identity.requestID }) {
                guard stopped.requestFingerprint == requestFingerprint,
                      identity.routes.first == stopped.owner else { return .conflict }
    ...
    public func requestStop(
        requestID: UUID,
        requestFingerprint: SHA256Digest,
        owner: RemoteRouteIdentity
    ) throws -> RemoteStopDisposition {
        try withLock {
            if let index = snapshot.records.firstIndex(where: { $0.identity.requestID == requestID }) {
                guard snapshot.records[index].requestFingerprint == requestFingerprint,
                      snapshot.records[index].identity.routes.first == owner else { return .conflict }
                switch snapshot.records[index].state {
                case .running:
                    try commit {
                        $0.records[index].state = .stopRequested
            ...
            if let stopped = snapshot.preAdmissionStops.first(where: { $0.requestID == requestID }) {
                guard stopped.requestFingerprint == requestFingerprint, stopped.owner == owner else { return .conflict }
                return .settledBeforeAdmission
            }
            if snapshot.tombstones.contains(where: { $0.requestID == requestID }) { return .conflict }
            guard snapshot.tombstones.count + snapshot.preAdmissionStops.count < maxTombstones else {
                throw RemoteJournalError.journalFull
            }
            let stopped = PreAdmissionStop(
                requestID: requestID,
                requestFingerprint: requestFingerprint,
                owner: owner,
                stoppedAt: now()
            )
            try commit { $0.preAdmissionStops.append(stopped) }
            return .settledBeforeAdmission
        }
    }
    ...
    public func hasConsumedRequestID(_ requestID: UUID) -> Bool {
        withLockNoThrow {
            ...
            snapshot.preAdmissionStops.contains { $0.requestID == requestID }
        }
    }
    ...
    private func terminalIndexToPruneIfNeeded() throws -> Int? {
        ...
        }), snapshot.tombstones.count + snapshot.preAdmissionStops.count < maxTombstones else {
            throw RemoteJournalError.journalFull
        }
        return oldestIndex
    }
    ...
    private func validateAll(_ candidate: Snapshot) throws {
        guard candidate.records.count <= maxRecords,
              candidate.tombstones.count + candidate.preAdmissionStops.count <= maxTombstones else {
            throw RemoteJournalError.journalFull
        }
        let recordIDs = Set(candidate.records.map(\.identity.requestID))
        let tombstoneIDs = Set(candidate.tombstones.map(\.requestID))
        let stoppedIDs = Set(candidate.preAdmissionStops.map(\.requestID))
        guard recordIDs.count == candidate.records.count,
              tombstoneIDs.count == candidate.tombstones.count,
              stoppedIDs.count == candidate.preAdmissionStops.count,
              recordIDs.isDisjoint(with: tombstoneIDs),
              recordIDs.isDisjoint(with: stoppedIDs),
              tombstoneIDs.isDisjoint(with: stoppedIDs) else {
            throw RemoteJournalError.corruptOrUnreadable
        }
