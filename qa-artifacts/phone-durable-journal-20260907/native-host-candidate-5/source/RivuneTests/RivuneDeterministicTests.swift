...
private final class RemoteEnvelopeCapture {
    var values: [BridgeEnvelope] = []
}
...
final class DurableRemoteHostEdgeTests: XCTestCase {
    ...
    func testPendingPhoneOperationResumesOnlyOnOriginalHost() {
        let original = BridgePeerContext(principal: "mac-a", generation: UUID())
        let rotatedConnection = BridgePeerContext(principal: "mac-a", generation: UUID())
        let replacementMac = BridgePeerContext(principal: "mac-b", generation: UUID())
        XCTAssertTrue(RivuneStore.shouldResumePendingPhoneOperation(originalHost: "mac-a", currentPeer: original))
        XCTAssertTrue(RivuneStore.shouldResumePendingPhoneOperation(originalHost: "mac-a", currentPeer: rotatedConnection))
        XCTAssertFalse(RivuneStore.shouldResumePendingPhoneOperation(originalHost: "mac-a", currentPeer: replacementMac))
        XCTAssertFalse(RivuneStore.shouldResumePendingPhoneOperation(originalHost: nil, currentPeer: original), "Legacy unbound pending records must fail closed")
        XCTAssertFalse(RivuneStore.shouldResumePendingPhoneOperation(originalHost: "mac-a", currentPeer: nil), "Revoked pairing must not auto-replay")
    }

    func testOfflinePhoneStopQueuesForOriginalHostWithoutRequestFallback() {
        XCTAssertEqual(
            RivuneStore.pendingPhoneStopTransport(originalHost: "mac-a", currentPeer: nil),
            .queuedForOriginalHost("mac-a")
        )
        XCTAssertEqual(
            RivuneStore.pendingPhoneStopTransport(
                originalHost: "mac-a",
                currentPeer: .init(principal: "mac-b", generation: UUID())
            ),
            .queuedForOriginalHost("mac-a")
        )
        XCTAssertEqual(
            RivuneStore.pendingPhoneStopTransport(originalHost: nil, currentPeer: nil),
            .legacyUnbound
        )
    }

    func testPhoneStopSendsOnlyToOriginalHostAcrossNewConnectionGeneration() {
        let current = BridgePeerContext(principal: "mac-a", generation: UUID())
        XCTAssertEqual(
            RivuneStore.pendingPhoneStopTransport(originalHost: "mac-a", currentPeer: current),
            .send(current)
        )
    }

    func testStopBeforeAdmissionSettlesAndLateRequestNeverDispatchesAcrossRestart() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        let runURL = root.appendingPathComponent("runs.json")
        let remoteURL = root.appendingPathComponent("remote.json")
        let value = request()
        let fingerprint = RivuneStore.remoteRequestFingerprint(value)
        let initialCapture = RemoteEnvelopeCapture()
        var initialStore: RivuneStore? = RivuneStore(
            runCoordinator: RivuneRunCoordinator(textRunner: DurableRemoteRunner(), journalURL: runURL),
            remoteTextRunner: DurableRemoteRunner(),
            remoteJournalStorage: try FileRemoteJournalStorage(url: remoteURL),
            remotePrincipal: "phone",
            remoteEnvelopeSink: { initialCapture.values.append($0) }
        )
        initialStore?.selectedCodexRoute = .codexCLI
        initialStore?.codexReadiness = .ready
        initialStore?.handleRemoteStop(.init(requestID: value.id, requestFingerprint: fingerprint))
        XCTAssertEqual(initialCapture.values.compactMap(\.stopUpdate).last?.accepted, true)
        XCTAssertEqual(initialCapture.values.compactMap(\.stopUpdate).last?.terminalState, .cancelled)
        initialStore = nil

        let lateRunner = DurableRemoteRunner()
        let lateCapture = RemoteEnvelopeCapture()
        let restarted = RivuneStore(
            runCoordinator: RivuneRunCoordinator(textRunner: lateRunner, journalURL: runURL),
            remoteTextRunner: lateRunner,
            remoteJournalStorage: try FileRemoteJournalStorage(url: remoteURL),
            remotePrincipal: "phone",
            remoteEnvelopeSink: { lateCapture.values.append($0) }
        )
        restarted.selectedCodexRoute = .codexCLI
        restarted.codexReadiness = .ready
        XCTAssertEqual(restarted.handleRemoteRequest(value), .replayed)
        let lateCalls = await lateRunner.calls
        XCTAssertEqual(lateCalls, 0)
        XCTAssertEqual(lateCapture.values.compactMap(\.update).last?.turn.executionState, .cancelled)
        XCTAssertEqual(lateCapture.values.compactMap(\.update).last?.isComplete, true)
    }

    func testPreAdmissionStopIsBoundToOwnerAndFingerprint() throws {
        ...
        let id = UUID()
        let fingerprint = SHA256Digest.hash(string: "request")
        let owner = RivuneStore.remoteDeviceIdentity(principal: "phone-one")
        XCTAssertEqual(
            try journal.requestStop(requestID: id, requestFingerprint: fingerprint, owner: owner),
            .settledBeforeAdmission
        )
        XCTAssertEqual(
            try journal.requestStop(requestID: id, requestFingerprint: fingerprint, owner: owner),
            .settledBeforeAdmission
        ...
        XCTAssertEqual(
            try journal.requestStop(
                requestID: id,
                requestFingerprint: fingerprint,
                owner: RivuneStore.remoteDeviceIdentity(principal: "phone-two")
            ),
            .conflict
        )
        XCTAssertEqual(
            try journal.requestStop(requestID: id, requestFingerprint: .hash(string: "changed"), owner: owner),
            .conflict
        )
        func identity(owner: RemoteRouteIdentity) -> RemoteAcceptedIdentity {
            .init(
                requestID: id,
                turnID: UUID(),
                mode: .chatGPT,
                routes: [owner, .init(providerID: "openai", transportID: "cli", adapterID: "codex")],
                requestedModelIDs: [],
                requestedEfforts: [],
                contextVersion: 2,
        ...
        XCTAssertEqual(
            try journal.admit(identity: identity(owner: owner), requestFingerprint: fingerprint),
            .cancelledBeforeDispatch
        )
        XCTAssertEqual(
            try journal.admit(
                identity: identity(owner: RivuneStore.remoteDeviceIdentity(principal: "phone-two")),
                requestFingerprint: fingerprint
            ),
            .conflict
        )
        XCTAssertEqual(
            try journal.admit(identity: identity(owner: owner), requestFingerprint: .hash(string: "changed")),
            .conflict
        )
    }
    ...
    func testCompletedStopUsesVerifiedReferenceAndSettlesUnavailableReplay() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        let runURL = root.appendingPathComponent("runs.json"), remoteURL = root.appendingPathComponent("remote.json")
        let value = request(), key = RivuneStore.remoteRequestFingerprint(value)
        var originalStore: RivuneStore? = RivuneStore(
            runCoordinator: RivuneRunCoordinator(textRunner: DurableRemoteRunner(), journalURL: runURL),
            remoteTextRunner: DurableRemoteRunner(),
            remoteJournalStorage: try FileRemoteJournalStorage(url: remoteURL), remotePrincipal: "phone")
        originalStore?.selectedCodexRoute = .codexCLI; originalStore?.codexReadiness = .ready
        XCTAssertEqual(originalStore?.handleRemoteRequest(value), .accepted)
        try await Task.sleep(for: .milliseconds(180))
        originalStore = nil

        var saved = try String(contentsOf: runURL, encoding: .utf8)
        XCTAssertTrue(saved.contains("Durable remote result"))
        saved = saved.replacingOccurrences(of: "Durable remote result", with: "Unverified changed result")
        try Data(saved.utf8).write(to: runURL, options: .atomic)
        let capture = RemoteEnvelopeCapture()
        let restarted = RivuneStore(
            runCoordinator: RivuneRunCoordinator(textRunner: DurableRemoteRunner(), journalURL: runURL),
            remoteTextRunner: DurableRemoteRunner(),
            remoteJournalStorage: try FileRemoteJournalStorage(url: remoteURL), remotePrincipal: "phone",
            remoteEnvelopeSink: { capture.values.append($0) })
        restarted.handleRemoteStop(.init(requestID: value.id, requestFingerprint: key))
        restarted.handleRemoteStop(.init(requestID: value.id, requestFingerprint: key))
        let stopUpdates = capture.values.compactMap(\.stopUpdate)
        XCTAssertEqual(stopUpdates.count, 2)
        XCTAssertTrue(stopUpdates.allSatisfy { $0.accepted && $0.terminalState == .interrupted })
        XCTAssertTrue(capture.values.compactMap(\.update).isEmpty, "Unverified answer must never bypass result resolution")
    }
