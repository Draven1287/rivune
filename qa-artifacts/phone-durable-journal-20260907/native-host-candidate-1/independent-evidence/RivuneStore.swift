...
final class RivuneStore: ObservableObject {
    ...
    private var remoteObservedFingerprints: [UUID: String] = [:]
    ...
    private func handleBridgeEnvelope(_ envelope: BridgeEnvelope) {
        ...
        case .stopRequest:
            #if os(macOS)
            guard let stop = envelope.stopRequest else { return }
            handleRemoteStop(stop)
            #endif
        case .stopUpdate:
            #if os(iOS)
            guard let update = envelope.stopUpdate, update.requestID == activeRequestID else { return }
    ...
    func handleRemoteRequest(_ request: BridgePromptRequest) -> RemoteRequestDisposition {
        ...
        guard let fingerprint = try? SHA256Digest(hex: fingerprintHex),
            ...
            sendBridgeUpdate(Self.remoteFailureUpdate(request, rejection: .legacyConversationFormat))
        ...
        if let rejection = Self.remoteAdmissionRejection(for: request) {
            sendBridgeUpdate(Self.remoteFailureUpdate(request, rejection: rejection))
            return .rejected(rejection)
        }
        if request.mode == .council || request.mode == .swarm {
            sendBridgeUpdate(Self.remoteFailureUpdate(request, rejection: .unsupportedMode))
        ...
        if request.mode == .together,
           request.togetherWorkflowVersion != BridgePromptRequest.currentTogetherWorkflowVersion {
            sendBridgeUpdate(Self.remoteFailureUpdate(request, rejection: .incompatibleWorkflow))
        ...
        // Bind the request to the mutually authenticated peer and its allowed Mac conversation.
        identities.insert(RemoteRouteIdentity(providerID: "rivune-paired-device", transportID: principal, adapterID: conversationID.uuidString), at: 0)
        ...
        if !isKnown {
            ...
            guard routeMatches else {
                sendBridgeUpdate(Self.remoteFailureUpdate(request, rejection: .incompatibleExecutionRoute))
                return .rejected(.incompatibleExecutionRoute)
            }
            guard runCoordinator.canAcceptRemote(conversationID: conversationID) else {
                sendBridgeUpdate(Self.remoteFailureUpdate(request, rejection: .hostBusy))
        ...
        do {
            ...
            case .conflict:
                sendBridgeUpdate(Self.remoteFailureUpdate(request, rejection: .requestIDConflict))
                return .rejected(.requestIDConflict)
            case .expiredResult:
                sendBridgeUpdate(Self.remoteInterruptedUpdate(request, message: "This completed result is no longer retained on your Mac. Rivune did not run it again."))
                return .replayed
            case .replay(let reference):
                let run = try runCoordinator.resolve(reference: reference, requestKey: fingerprintHex)
                sendBridgeUpdate(Self.bridgeUpdate(run: run))
                return .replayed
            case .reattach(let record):
                remoteObservedFingerprints[request.id] = fingerprintHex
                if let run = runCoordinator.remoteSnapshot(id: request.id) {
                    ...
                    sendBridgeUpdate(Self.remoteInterruptedUpdate(request, message: message))
                }
            ...
            remoteJournalError = "Phone execution needs local journal recovery."
            sendBridgeUpdate(Self.remoteInterruptedUpdate(request, message: remoteJournalError!))
            return .rejected(.hostBusy)
        }
        ...
        do {
            ...
            sendBridgeUpdate(Self.remoteInterruptedUpdate(request, message: error.localizedDescription))
            return .rejected(.hostBusy)
        }
    }

    func detachRemoteObservations() {
        for (requestID, fingerprint) in remoteObservedFingerprints {
            if let digest = try? SHA256Digest(hex: fingerprint) {
                _ = try? remoteJournal?.detachObservation(requestID: requestID, requestFingerprint: digest)
            }
        }
        remoteObservedFingerprints.removeAll()
    }
    ...
    private func handleDurableRemoteRunUpdate(_ run: WorkspaceRun) {
        ...
        if run.status == .running {
            if remoteObservedFingerprints[run.id] != nil { sendBridgeUpdate(Self.bridgeUpdate(run: run)) }
            return
        }
        do {
            if case .stopRequested = journal.record(requestID: run.id)?.state, run.status == .cancelled {
                ...
                try? bridge.send(.stopUpdate(.init(requestID: run.id, accepted: true, message: "Request stopped")))
            ...
            if remoteObservedFingerprints[run.id] != nil { sendBridgeUpdate(Self.bridgeUpdate(run: run)) }
            remoteObservedFingerprints.removeValue(forKey: run.id)
    ...
    private func handleForgottenRemoteRun(_ run: WorkspaceRun) {
        guard let fingerprintHex = remoteExecutionFingerprints[run.id] ?? remoteObservedFingerprints[run.id],
              let fingerprint = try? SHA256Digest(hex: fingerprintHex), let journal = remoteJournal else { return }
        try? journal.fail(requestID: run.id, requestFingerprint: fingerprint)
        remoteExecutionFingerprints.removeValue(forKey: run.id)
        if remoteObservedFingerprints.removeValue(forKey: run.id) != nil {
            var turn = run.turn
            ...
            sendBridgeUpdate(.init(requestID: run.id, stage: .failed, turn: turn, isComplete: true))
        }
    }

    func handleRemoteStop(_ stop: BridgeStopRequest) {
        guard (remotePrincipalOverride ?? bridge.authenticatedPrincipal) != nil,
              let fingerprint = try? SHA256Digest(hex: stop.requestFingerprint),
              let journal = remoteJournal else { return }
        do {
            switch try journal.requestStop(requestID: stop.requestID, requestFingerprint: fingerprint) {
            case .cancelProvider:
                remoteObservedFingerprints[stop.requestID] = stop.requestFingerprint
                runCoordinator.cancel(stop.requestID)
            case .alreadyTerminal:
                try? bridge.send(.stopUpdate(.init(requestID: stop.requestID, accepted: false, message: "This request already finished")))
            case .conflict:
                try? bridge.send(.stopUpdate(.init(requestID: stop.requestID, accepted: false, message: "Stop request did not match the active request")))
            }
        } catch {
            try? bridge.send(.stopUpdate(.init(requestID: stop.requestID, accepted: false, message: "Rivune could not verify this stop request")))
        }
    }
    ...
    private func persistPendingPhoneRequest(_ request: BridgePromptRequest) {
        guard !RivuneLaunchContext.isIsolated, let url = pendingPhoneRequestURL,
              let data = try? JSONEncoder.sorted.encode(request) else { return }
        try? FileManager.default.createDirectory(at: url.deletingLastPathComponent(), withIntermediateDirectories: true)
        try? data.write(to: url, options: .atomic)
        try? FileManager.default.setAttributes([.posixPermissions: 0o600], ofItemAtPath: url.path)
    }

    private func clearPendingPhoneRequest() {
        guard let url = pendingPhoneRequestURL else { return }
    ...
    private func restorePendingPhoneRequest() {
        guard !RivuneLaunchContext.isIsolated, let url = pendingPhoneRequestURL,
              ...
              let request = try? JSONDecoder().decode(BridgePromptRequest.self, from: data),
              let conversationID = request.conversationID,
              let conversation = conversations.first(where: { $0.id == conversationID }),
              conversation.turns.contains(where: { $0.id == request.turnID }) else { return }
    ...
    func send() {
        ...
        persistPendingPhoneRequest(bridgeRequest)
        do {
    ...
    private func cancelGeneration() {
        ...
        showPrototypeNotice("Stopping on your Mac…")
        #else
