...
struct StopUpdate { let requestID: UUID; let accepted: Bool; let message: String }
enum Envelope { case stopUpdate(StopUpdate) }
final class Bridge { let authenticatedPrincipal: String? = "phone-B"; func send(_ value: Envelope) throws {} }
final class Coordinator { var cancelCount = 0; func cancel(_ id: UUID) { cancelCount += 1 } }
final class Memory: RemoteJournalStorage, @unchecked Sendable { let journalLeaseKey = "stop-probe"; var data: Data?; func read() throws -> Data? { data }; func writeAtomically(_ data: Data) throws { self.data = data } }
final class Host {
let remotePrincipalOverride: String? = nil
let bridge = Bridge()
let remoteJournal: RemoteRequestJournal?
var remoteObservedFingerprints: [UUID:String] = [:]
...
let requestID = UUID(), fingerprint = SHA256Digest.hash(string:"same known original request")
let identity = RemoteAcceptedIdentity(requestID:requestID, turnID:UUID(), mode:.chatGPT, routes:[RemoteRouteIdentity(providerID:"rivune-paired-device",transportID:"phone-A",adapterID:"mapped-phone-A-conversation")], requestedModelIDs:[],requestedEfforts:[],contextVersion:2,attachmentSetDigest:.hash(string:"empty"))
_ = try journal.admit(identity:identity, requestFingerprint:fingerprint)
let host = Host(journal)
host.handleRemoteStop(BridgeStopRequest(requestID:requestID, requestFingerprint:fingerprint.hex))
precondition(host.runCoordinator.cancelCount == 1)
precondition(journal.record(requestID:requestID)?.state == .stopRequested)
print("CONFIRMED: exact production handleRemoteStop cancels phone-A record while authenticated as phone-B, with matching original request fingerprint")
