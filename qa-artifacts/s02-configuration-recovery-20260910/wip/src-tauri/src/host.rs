...
fn validate_recovery_binding(workspace: &WorkspaceSnapshot) -> Result<(), String> {
    if let Some(op)=&workspace.configuration_operation { validate_configuration_operation(op)?; if op.operation_id != format!("configuration:{}",workspace.configuration_sequence) { return Err("CONFIGURATION_INVALID_RECORD".into()); } }
...
fn validate_configuration_operation(op: &ConfigurationOperation) -> Result<(),String> {
    ...
    validate_provider(&op.intent.provider)
}
...
fn apply_configuration_in(host:&HostState,intent:ConfigurationIntent,operation_id:&str,route_current:bool) -> Result<ConfigurationOperation,String> {
    let _mutation=host.lock_mutation()?;let mut w=host.workspace.lock().map_err(|_|"Workspace unavailable")?;
    let mut op=w.configuration_operation.clone().filter(|o|o.operation_id==operation_id).ok_or("CONFIGURATION_ID_MISMATCH")?;
...
pub fn apply_provider_configuration(intent:ConfigurationIntent,operation_id:String,state:State<'_,Arc<HostState>>) -> Result<ConfigurationOperation,String> {
    let p=&intent.provider;
    let current=discover_provider_executables(&provider_search_directories()).into_iter().any(|(kind,path)|kind==p.kind && path.to_string_lossy()==p.executable_path && discovered_route_id(&kind,&path).as_deref()==Some(p.id.as_str()));
    apply_configuration_in(state.inner(),intent,&operation_id,current)
}
...
mod provider_configuration_tests {
    ...
    fn configuration_persist_failure_has_no_receipt_and_success_preserves_draft_on_reopen() {
        ...
        let host=HostState::open(path.clone()).unwrap();
        let mut current=host.workspace.lock().unwrap().clone();
        current.conversations[0].draft="Preserve synthetic draft".into();host.save(&current).unwrap();
