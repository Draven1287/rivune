import { composerConfigurationKey, composerSummary, type ComposerConfiguration } from './composerConfiguration.ts';
import { parseModelCatalog, type ModelCatalog } from '../hooks/workspaceAdapter.ts';
import { parseArtifactRequest, parseArtifactInspection, type HostArtifactRequest, parseHostSnapshot, parseHostAcknowledgement, parseHostRunEvent, type HostSnapshot, type HostProviderConfig } from './contracts.ts';
...
export interface HostControllerState {
  configurationConfirmations: Readonly<Record<string, { confirmedSummary: string; acknowledged: boolean }>>;
...
export function createHostWorkspaceController(adapter: HostBridgeAdapter | null, journal: RecoveryJournal, options: { id?: () => string; pollMs?: number } = {}) {
  ...
  let state: HostControllerState = { phase: 'disconnected', snapshot: null, catalog: null, pending: null, retry: null, retryNotice: null, error: null, drafts: {}, configurationConfirmations: {} };
  ...
  const configurationConfirmations = new Map<string, { request: SaveRichDraftRequest; confirmedSummary: string; acknowledged: boolean }>();
  const confirmationState = () => Object.fromEntries([...configurationConfirmations].map(([key, value]) => [key, {confirmedSummary:value.confirmedSummary, acknowledged:value.acknowledged}]));
  function confirmsConfiguration(snapshot: HostSnapshot, request: SaveRichDraftRequest) {
    const saved=snapshot.conversations.find(c=>c.id===request.conversationID);
    return saved && saved.richDraft.revision===request.expectedRevision+1 && saved.draft===request.draft && equal(saved.richDraft.selection,request.selection) && equal(saved.richDraft.team,request.team) && equal(saved.richDraft.attachmentIDs,request.attachmentIDs);
  }
  const dirty = new Set<string>();
  const draftBases = new Map<string, number>();
  const sentDrafts = new Map<string,{requestID:string;text:string;generation:number;revision:number}>();
  ...
  async function refreshOnce() {
    ...
    for (const c of snapshot.conversations) {
      const sent=sentDrafts.get(c.id);
      if(sent){
        const admitted=snapshot.runs.some(r=>r.id===sent.requestID&&r.conversationID===c.id&&r.admitted.prompt===sent.text);
        if(admitted&&c.draft===''&&c.richDraft.revision>=sent.revision+1){
          if((editVersions.get(c.id)??0)===sent.generation&&drafts[c.id]===sent.text){drafts[c.id]='';dirty.delete(c.id);}else dirty.add(c.id);
          draftBases.set(c.id,c.richDraft.revision);sentDrafts.delete(c.id);
        }
        continue;
      }
    ...
    for (const [conversationID, confirmation] of configurationConfirmations) {
      if (confirmation.acknowledged && confirmsConfiguration(snapshot, confirmation.request)) configurationConfirmations.delete(conversationID);
    }
    publish({ snapshot, catalog, drafts, configurationConfirmations:confirmationState() });
  ...
  function selectionForSave(conversationID: string) {
    const c=currentConversation(conversationID);
    const admitted=sendSelection(conversationID); // Validate even when inheritance stays null.
    return !c.richDraft.team && c.richDraft.selection===null ? null : admitted;
  }
  async function save(conversationID: string, draft: string, forSend = false, execution?: Pick<SaveRichDraftRequest, 'selection' | 'team'>): Promise<number> {
    const saveGeneration=editVersions.get(conversationID)??0;
    if (!adapter) throw new HostActionError('Desktop host unavailable.');
    const c = currentConversation(conversationID);
    const old = pendingSaves.get(conversationID);
    if (configurationConfirmations.has(conversationID) && (execution || forSend || !old)) throw new HostActionError('Confirm the earlier configuration save before changing or sending this draft.');
    if (old && execution) throw new HostActionError('Resolve the earlier draft save before changing the team.');
    if (old && forSend && (!equal(old.selection, selectionForSave(conversationID)) || !equal(old.team, c.richDraft.team))) throw new HostActionError('Resolve the earlier draft save before selecting a provider and sending.');
    ...
    const request = old ?? { conversationID, mutationID: id(), expectedRevision: c.richDraft.revision, draft, attachmentIDs: [...c.richDraft.attachmentIDs], selection: forSend ? selectionForSave(conversationID) : c.richDraft.selection, team: c.richDraft.team, ...execution };
    ...
    if (execution && !configurationConfirmations.has(conversationID)) configurationConfirmations.set(conversationID,{request,confirmedSummary:composerSummary(state.snapshot!,state.catalog,conversationID),acknowledged:false});
    publish({ phase: 'saving', configurationConfirmations:confirmationState() });
    let receipt: unknown;
    ...
    if (r.mutationID === request.mutationID && r.conversationID === conversationID && r.state === 'rejected') { pendingSaves.delete(conversationID); configurationConfirmations.delete(conversationID); publish({configurationConfirmations:confirmationState()}); }
    ...
    const confirmation=configurationConfirmations.get(conversationID);
    if(confirmation){confirmation.acknowledged=true;publish({configurationConfirmations:confirmationState()});}
    ...
    if (state.drafts[conversationID] === request.draft && (editVersions.get(conversationID)??0)===saveGeneration) dirty.delete(conversationID);
  ...
  async function reconcilePending() {
    ...
    if (journal.authoritative && await journal.read() === null) {
      await refresh();
      sentDrafts.delete(token.conversationID);
    ...
    if(ack.state==='rejected')sentDrafts.delete(token.conversationID);
  ...
  return {
    async inspectArtifact(request: HostArtifactRequest) {
      const exact = parseArtifactRequest(request);
      const selected = () => state.snapshot?.artifacts.find(a => a.artifactID === exact.artifactID && a.conversationID === exact.conversationID && a.requestID === exact.requestID && a.contentSHA256 === exact.expectedSHA256);
      if (!started || disposed || !adapter?.inspectArtifact || !selected()) throw new HostActionError('Saved result inspection is unavailable. Refresh and choose the result again.');
      const result = await parseArtifactInspection(await adapter.inspectArtifact({ ...exact }), exact);
      if (disposed || !selected()) throw new HostActionError('Saved result selection is no longer available.');
      return result;
    },
    ...
    retryDraftSave: (conversationID: string) => action(async () => {
      if(configurationConfirmations.get(conversationID)?.acknowledged){await refresh();if(configurationConfirmations.has(conversationID))throw new HostActionError('Acknowledged configuration does not match the saved workspace. Review the saved state.');return;}
    ...
    acceptSupersedingConfiguration: (conversationID: string, reviewedKey: string) => action(async () => {
      const confirmation=configurationConfirmations.get(conversationID);
      if(!confirmation?.acknowledged || pendingSaves.has(conversationID)) throw new HostActionError('Acknowledge the original configuration save before reviewing its replacement.');
      await refresh();
      const current=currentConversation(conversationID);
      if(!state.snapshot || composerConfigurationKey(state.snapshot,state.catalog,conversationID)!==reviewedKey) throw new HostActionError('Saved configuration changed again. Review the latest configuration before accepting it.');
      if(current.richDraft.revision<=confirmation.request.expectedRevision+1) throw new HostActionError('Review a newer saved configuration before resolving the acknowledged change.');
      // Explicitly accept this reviewed host revision without writing or replacing local text.
      draftBases.set(conversationID,current.richDraft.revision);
      dirty.add(conversationID);
      configurationConfirmations.delete(conversationID);
      publish({configurationConfirmations:confirmationState(),phase:'ready',error:null});
    }),
    ...
    followWorkspaceDefault: (conversationID: string, expectedRevision: number, expectedDefaultID: string | null) => action(async () => {
      await refresh();
      if(state.pending || state.snapshot?.runs.some(run=>active(run.status))) throw new HostActionError('Resolve the pending run before changing the connection.');
      const c=currentConversation(conversationID);
      if(c.richDraft.team || c.richDraft.selection?.modelID || c.richDraft.selection?.effortID) throw new HostActionError('Review the saved team or explicit model choices separately.');
      if(c.richDraft.revision!==expectedRevision || state.snapshot?.selectedProviderID!==expectedDefaultID) throw new HostActionError('Saved settings or workspace default changed. Review before following it.');
      await save(conversationID,state.drafts[conversationID]??c.draft,false,{team:null,selection:null});
    }),
    ...
    configureExecution: (conversationID: string, configuration: ComposerConfiguration) => {
      const intended = structuredClone(configuration);
      return action(async () => {
        await refresh();
        if (!state.snapshot || !state.catalog) throw new HostActionError('Review connections before saving.');
        if (state.snapshot.runs.some(run => active(run.status))) throw new HostActionError('Wait for the active run before changing configuration.');
        if (composerConfigurationKey(state.snapshot, state.catalog, conversationID) !== intended.expectedKey) throw new HostActionError('Saved configuration or connections changed. Review the latest settings before saving.');
        const c = currentConversation(conversationID);
        if (intended.team) {
          try { validateSavedTeam(state.snapshot, state.catalog, intended.team); const rebuilt=configuredTeam(state.snapshot,state.catalog,intended.team.members.map(m=>m.providerID),intended.team.members[intended.team.leadIndex].providerID); if(!equal(rebuilt,intended.team)) throw new Error('Review the current provider-default team.'); }
          catch (error) { throw new HostActionError(error instanceof Error ? error.message : 'Review the team.'); }
          if (!equal(intended.selection, intended.team.members[intended.team.leadIndex])) throw new HostActionError('Selection must match the chosen team lead.');
        } else {
          const providerID = intended.selection?.providerID ?? state.snapshot.selectedProviderID;
          if (!teamRoutes(state.snapshot, state.catalog).some(p => p.id === providerID)) throw new HostActionError('Choose an available Single AI connection.');
          if (intended.selection && !equal(intended.selection, {schemaVersion:1, providerID, modelID:null, effortID:null, catalogRevision:state.catalog.revision})) throw new HostActionError('Review the current provider-default route.');
        }
        await save(conversationID, state.drafts[conversationID] ?? c.draft, false, {selection:intended.selection, team:intended.team});
      });
    },
    ...
    send: (conversationID: string) => action(async () => {
      ...
      const generation=editVersions.get(conversationID)??0;
      ...
      sentDrafts.set(conversationID,{requestID:pending.requestID,text:draft,generation,revision});
      ...
      if(ack.state==='rejected')sentDrafts.delete(conversationID);
    }),
