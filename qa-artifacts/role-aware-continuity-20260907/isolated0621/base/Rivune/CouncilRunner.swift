...
struct CouncilRunner: Sendable {
    ...
    private static func frozenTask(_ record: CouncilRunRecord) -> String {
        ...
        struct TaskPayload: Encodable { let request: String; let approvedContext: String; let criteria: String; let team: TeamRunConfiguration?; let selectedArtifact: ArtifactContinuation? }
        return String(decoding: try! JSONEncoder().encode(TaskPayload(request: record.prompt, approvedContext: record.approvedContext, criteria: record.criteria, team: record.teamConfiguration, selectedArtifact: record.selectedArtifact)), as: UTF8.self)
    }
