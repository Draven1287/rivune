...
final class RivuneStore: ObservableObject {
    ...
    private func handleRemoteRequest(_ request: BridgePromptRequest) {
        if request.mode == .council || request.mode == .swarm {
            ...
            cacheAndSendRemoteUpdate(.init(requestID: request.id, stage: .failed, turn: turn, isComplete: true))
            return
        }
        ...
        guard request.contextVersion == BridgePromptRequest.currentContextVersion,
            ...
            var incompatibleTurn = ChatTurn(id: request.turnID, prompt: request.prompt, mode: request.mode, attachments: request.attachments)
            incompatibleTurn.executionState = .failed
            let message = "This iPhone request uses an older conversation format. Update Rivune on both devices and send it again. Legacy text was not interpreted as user instructions."
            switch request.mode {
            case .chatGPT: incompatibleTurn.chatGPTError = message
            case .claude: incompatibleTurn.claudeError = message
            case .together, .council, .swarm: incompatibleTurn.combinedError = message
            }
            cacheAndSendRemoteUpdate(.init(requestID: request.id, stage: .failed, turn: incompatibleTurn, isComplete: true))
            return
        }
