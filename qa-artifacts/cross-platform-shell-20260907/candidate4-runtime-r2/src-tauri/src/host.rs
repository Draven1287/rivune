...
pub enum SelectionCapabilityReason {
    AdapterDiscoveryUnavailable,
}
...
pub struct SelectionCapabilities {
    pub schema_version: u32,
    pub model_override: OverrideCapability,
    pub effort_override: OverrideCapability,
    pub reason_code: SelectionCapabilityReason,
}

impl SelectionCapabilities {
    fn provider_defaults_only() -> Self {
        Self {
            schema_version: 1,
            model_override: OverrideCapability::Unsupported,
            effort_override: OverrideCapability::Unsupported,
            reason_code: SelectionCapabilityReason::AdapterDiscoveryUnavailable,
        }
    }

    fn validate(&self, selection: &ModelSelection) -> Result<(), String> {
        match (&self.model_override, &self.effort_override) {
            (OverrideCapability::Unsupported, OverrideCapability::Unsupported)
                if selection.model_id.is_some() || selection.effort_id.is_some() =>
                    Err("MODEL_CAPABILITIES_UNKNOWN".into()),
            _ => Ok(()),
        }
    }
}
...
pub struct CatalogProvider {
    ...
    pub selection_capabilities: SelectionCapabilities,
...
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct SubmissionRecoveryIdentity {
    #[serde(rename = "requestID")]
    pub request_id: String,
    #[serde(rename = "conversationID")]
    pub conversation_id: String,
...
fn validate_submission_recovery(value: &SubmissionRecovery) -> Result<(), String> {
    if value.schema_version != 1 || value.request_id.trim().is_empty()
        || value.request_id.len() > 128 || value.conversation_id.trim().is_empty()
        || value.conversation_id.len() > 128
        || !matches!(value.state.as_str(), "reserved" | "rejected") {
        return Err("Invalid saved submission recovery record.".into());
    }
    Ok(())
}

fn validate_recovery_binding(workspace: &WorkspaceSnapshot) -> Result<(), String> {
    if let Some(op)=&workspace.configuration_operation { validate_configuration_operation(op)?; if workspace.configuration_sequence==0 || op.operation_id != format!("configuration:{}",workspace.configuration_sequence) { return Err("CONFIGURATION_INVALID_RECORD".into()); } }
    if let Some(record) = &workspace.submission_recovery {
        validate_submission_recovery(record)?;
        if !workspace.conversations.iter().any(|c| c.id == record.conversation_id)
            || workspace.runs.iter().any(|run| run.id == record.request_id
                && (run.conversation_id != record.conversation_id || record.state == "rejected")) {
            return Err("Saved submission recovery identity is inconsistent.".into());
        }
    }
    Ok(())
}
...
pub struct WorkspaceSnapshot {
    ...
    #[serde(default)]
    pub artifacts: Vec<saved_artifacts::PersistedArtifact>,
    pub schema_version: u32,
    ...
    #[serde(default)]
    pub submission_recovery: Option<SubmissionRecovery>,
    #[serde(default)]
    pub configuration_operation: Option<ConfigurationOperation>,
    #[serde(default)]
    pub configuration_sequence: u64,
}
...
// Internal only: retries reuse admitted inputs and never carry a composer-clear guard.
#[derive(Clone, Debug)]
struct DraftAdmissionGuard { conversation_id: String, prompt: String, revision: Option<u64> }
impl DraftAdmissionGuard {
    fn from_request(request: &SubmitRunRequest) -> Self { Self { conversation_id:request.conversation_id.clone(), prompt:request.prompt.clone(), revision:request.rich_draft_revision } }
}
fn admission_candidate(workspace: &WorkspaceSnapshot, admitted: &AdmittedRequest, guard: Option<&DraftAdmissionGuard>) -> Result<WorkspaceSnapshot,String> {
    if let Some(existing)=workspace.runs.iter().find(|r|r.id==admitted.request_id) {
        return if existing.admitted==*admitted {Ok(workspace.clone())} else {Err("Request ID is already bound to different content.".into())};
    }
    let mut candidate=workspace.clone();
    if let Some(guard)=guard {
        if admitted.retry_of.is_some() || guard.conversation_id!=admitted.conversation_id || guard.prompt!=admitted.prompt {return Err("The submitted draft identity changed.".into());}
        let conversation=candidate.conversations.iter_mut().find(|c|c.id==guard.conversation_id).ok_or("The conversation no longer exists.")?;
        if conversation.read_only || guard.revision!=Some(conversation.rich_draft.revision) || guard.prompt!=conversation.draft {return Err("The saved execution draft changed. Save it durably before sending.".into());}
        let revision=conversation.rich_draft.revision.checked_add(1).ok_or("Draft revision limit reached.")?;
        conversation.draft.clear();conversation.rich_draft.revision=revision;
    }
    candidate.runs.push(RunRecord {id:admitted.request_id.clone(),conversation_id:admitted.conversation_id.clone(),status:"running".into(),updated_at:now_marker(),admitted:admitted.clone(),answer:None,error:None});
    Ok(candidate)
}
...
mod contention_classification_tests {
    use super::*;
    #[test]
    fn startup_contention_two_open_and_reopen_preserve_snapshot() {
        let path=std::env::temp_dir().join(format!("rivune-startup-contention-{}-{}",std::process::id(),std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
        let first=HostState::open(path.clone()).unwrap();
        let before=serde_json::to_vec(&*first.workspace.lock().unwrap()).unwrap();
        let snapshots=|path:&Path|{let mut files=fs::read_dir(path.join("snapshots")).unwrap().map(|e|{let p=e.unwrap().path();(p.file_name().unwrap().to_owned(),fs::read(p).unwrap())}).collect::<Vec<_>>();files.sort();files};
        let bytes=snapshots(&path);
        let error=match HostState::open(path.clone()){Ok(_)=>panic!("second open acquired lock"),Err(e)=>e};assert!(error.is::<ProfileInUse>());
        assert_eq!(serde_json::to_vec(&*first.workspace.lock().unwrap()).unwrap(),before);assert_eq!(snapshots(&path),bytes);drop(first);
        let reopened=HostState::open(path.clone()).unwrap();assert_eq!(snapshots(&path),bytes);drop(reopened);fs::remove_dir_all(path).unwrap();
    }
    #[test]
...
impl HostState {
    ...
    pub fn open(profile: PathBuf) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let mut workspace = if let Some(path) = latest_path {
            ...
            validate_recovery_binding(&decoded)?;
            decoded
        } else if has_pending {
            return Err(
        ...
        repaired |= saved_artifacts::materialize(&mut workspace)?;
        if repaired {
    ...
    fn save(&self, workspace: &mut WorkspaceSnapshot) -> Result<(), PersistError> {
        ...
        *generation = generation.saturating_add(1);
        let mut candidate = workspace.clone();
        saved_artifacts::materialize(&mut candidate).map_err(PersistError::before)?;
        persist_with_fault(&self.profile, &candidate, allocated, fault)?;
        *workspace = candidate;
        Ok(())
    }
        ...
        loop {
            ...
            if ready.len() > 1 {
                ...
                for invocation in &ready {
                    ...
                    prepared.push((invocation.clone(), provider, internal));
                }
                let bindings = ready
                    .iter()
                    .map(|invocation| invocation.binding.clone())
                    .collect::<Vec<_>>();
                if let Err(error) = session.mark_batch_dispatched(&bindings) {
                    return Acknowledgement::rejected(&admitted.request_id, error);
                }
                for (invocation, provider, _) in &prepared {
                ...
                let handles = prepared
                    .into_iter()
                    .map(|(invocation, provider, internal)| {
                        let cancellation = cancellation.clone();
                        let sender = sender.clone();
                        thread::spawn(move || {
                            let outcome = run_direct(&internal, cancellation);
                            let _ = sender.send((invocation, provider, outcome));
                        })
                    })
                    .collect::<Vec<_>>();
                drop(sender);
                ...
                for _ in 0..handles.len() {
                    let (invocation, provider, outcome) = match receiver.recv() {
                        Ok(outcome) => outcome,
                        Err(_) => {
                            pending_acknowledgement =
                                Some(Acknowledgement::uncertain(&admitted.request_id));
                            break;
                        }
                    };
                    ...
                    {
                        pending_acknowledgement.get_or_insert_with(|| {
                            Acknowledgement::rejected(&admitted.request_id, error)
                        });
                        continue;
                    }
                    let failed = error_text.is_some();
                    match self.save_constellation_checkpoint(
                        &admitted.request_id,
                        &session,
                        (
                ...
                if let Some(acknowledgement) = pending_acknowledgement {
                    return acknowledgement;
                }
                continue;
            }
            let Some(invocation) = ready.into_iter().next() else {
                let failure = session
                    .failure()
    ...
    fn execute_with_app(
        ...
        self.execute_with_recovery_guard(admitted, app, false)
    }

    fn execute_with_recovery_guard(
        &self,
        admitted: AdmittedRequest,
        app: Option<AppHandle>,
        require_recovery: bool,
    ) -> Acknowledgement {
        self.execute_with_draft_guard(admitted, app, require_recovery, None)
    }
    fn execute_with_draft_guard(&self, admitted: AdmittedRequest, app: Option<AppHandle>, require_recovery: bool, draft_guard: Option<DraftAdmissionGuard>) -> Acknowledgement {
        ...
        let cancellation = Arc::new(AtomicBool::new(false));
        {
            ...
            let mut workspace = self.workspace.lock().expect("workspace lock poisoned");
            // Same barrier as reconcile/clear: a delayed reserved dispatch cannot
            // start after a no-run reconciliation rejected or cleared its intent.
            if let Err(error) = recovery_admission_guard(&workspace, &admitted.request_id,
                &admitted.conversation_id, require_recovery) {
                return Acknowledgement::rejected(&admitted.request_id, error);
            }
            ...
            let mut candidate = match admission_candidate(&workspace,&admitted,draft_guard.as_ref()) {
                Ok(candidate)=>candidate,Err(error)=>return Acknowledgement::rejected(&admitted.request_id,error),
            };
            if let Some(constellation) = constellation {
                candidate.constellation_sessions.push(constellation);
            }
            if let Err(error) = self.save(&mut candidate) {
                if error.committed {
                    if let Some(run) = candidate
                        .runs
                        .iter_mut()
    ...
    pub fn prepare_shutdown(&self, timeout: Duration) -> Result<(), String> {
        ...
        self.save(&mut workspace).map_err(|error| {
            format!(
        ...
        if let Err(error) = self.save(&mut candidate) {
            {
                if let Some(run) = candidate.runs.iter_mut().find(|run| run.id == request_id) {
                    ...
                    if !error.committed { run.status = "failed".into(); }
                    run.error = Some(format!(
    ...
    fn recovery_record(&self) -> Result<Option<SubmissionRecovery>, String> {
        let workspace = self.workspace.lock().map_err(|_| "Workspace is unavailable.")?;
        validate_recovery_binding(&workspace)?;
        // Finish any previous post-rename durability uncertainty before reporting
        // either a reservation or its absence. No provider or replay side effect.
        sync_directory(&self.profile.join("snapshots")).map_err(|_| "Recovery durability could not be verified.")?;
        Ok(workspace.submission_recovery.clone())
    }

    fn reserve_recovery(&self, identity: SubmissionRecoveryIdentity) -> Result<SubmissionRecovery, String> {
        let record = SubmissionRecovery { schema_version: 1, request_id: identity.request_id,
            conversation_id: identity.conversation_id, state: "reserved".into() };
        validate_submission_recovery(&record)?;
        let _mutation = self.lock_mutation()?;
        let mut workspace = self.workspace.lock().map_err(|_| "Workspace is unavailable.")?;
        validate_recovery_binding(&workspace)?;
        if !workspace.conversations.iter().any(|c| c.id == record.conversation_id && !c.read_only) {
            return Err("Choose an editable conversation before reserving a request.".into());
        }
        let reserved = match &workspace.submission_recovery {
            Some(existing) if existing.request_id == record.request_id && existing.conversation_id == record.conversation_id => existing.clone(),
            Some(_) => return Err("Reconcile the existing submission recovery record first.".into()),
            None => record,
        };
        let mut candidate = workspace.clone();
        candidate.submission_recovery = Some(reserved.clone());
        validate_recovery_binding(&candidate)?;
        match self.save(&mut candidate) {
            Ok(()) => { *workspace = candidate; Ok(reserved) }
            Err(error) => {
                if error.committed { *workspace = candidate; }
                Err("Submission reservation durability is unconfirmed. Read recovery before sending.".into())
            }
        }
    }

    fn reject_reserved_locked(&self, workspace: &mut WorkspaceSnapshot, request_id: &str) -> Acknowledgement {
        if workspace.runs.iter().any(|run| run.id == request_id) {
            return Acknowledgement::uncertain(request_id);
        }
        let Some(record) = workspace.submission_recovery.as_ref().filter(|r| r.request_id == request_id) else {
            return Acknowledgement::uncertain(request_id);
        };
        let mut rejected = record.clone(); rejected.state = "rejected".into();
        let mut candidate = workspace.clone(); candidate.submission_recovery = Some(rejected);
        match self.save(&mut candidate) {
            Ok(()) => { *workspace = candidate; Acknowledgement::rejected(request_id, "This reserved request was not dispatched.") }
            Err(error) => {
                if error.committed { *workspace = candidate; }
                Acknowledgement::uncertain(request_id)
            }
    ...
    fn clear_recovery(&self, identity: SubmissionRecoveryIdentity) -> Result<SubmissionRecovery, String> {
        let cleared = SubmissionRecovery { schema_version: 1, request_id: identity.request_id,
            conversation_id: identity.conversation_id, state: "cleared".into() };
        validate_submission_recovery(&SubmissionRecovery { state: "reserved".into(), ..cleared.clone() })?;
        // Reject the wrong identity before reconciliation can mutate its outcome.
        {
            let workspace = self.workspace.lock().map_err(|_| "Workspace is unavailable.")?;
            validate_recovery_binding(&workspace)?;
            if !workspace.submission_recovery.as_ref().is_some_and(|record|
                record.request_id == cleared.request_id && record.conversation_id == cleared.conversation_id) {
                return Err("No matching submission recovery record exists.".into());
            }
        }
        // Reconciliation establishes durable acceptance/rejection, including
        // pre-dispatch reservations. Unknown IDs are never treated as rejected.
        let acknowledged = self.reconcile(&cleared.request_id);
        if !matches!(acknowledged.state.as_str(), "accepted" | "rejected") {
            return Err("The request outcome is unresolved; recovery was retained.".into());
        }
        let _mutation = self.lock_mutation()?;
        let mut workspace = self.workspace.lock().map_err(|_| "Workspace is unavailable.")?;
        validate_recovery_binding(&workspace)?;
        let Some(record) = &workspace.submission_recovery else {
            return Err("No matching submission recovery record exists.".into());
        };
        if record.request_id != cleared.request_id || record.conversation_id != cleared.conversation_id {
            return Err("Submission recovery identity does not match.".into());
        }
        if record.state != "rejected" && !workspace.runs.iter().any(|run|
            run.id == cleared.request_id && run.conversation_id == cleared.conversation_id) {
            return Err("Submission outcome has no authoritative record.".into());
        }
        let mut candidate = workspace.clone(); candidate.submission_recovery = None;
        match self.save(&mut candidate) {
            Ok(()) => { *workspace = candidate; Ok(cleared) }
            Err(error) => {
                if error.committed { *workspace = candidate; }
                Err("Recovery clear durability is unconfirmed. Read recovery before sending.".into())
            }
        }
    }

    fn prepare_reserved_submission(&self, request: SubmitRunRequest, require_recovery: bool) -> Result<AdmittedRequest, Acknowledgement> {
        let request_id = request.id.clone();
        let _mutation = self.lock_mutation().map_err(|error| Acknowledgement::rejected(&request_id, error))?;
        let mut workspace = self.workspace.lock().map_err(|_| Acknowledgement::uncertain(&request_id))?;
        recovery_admission_guard(&workspace, &request_id, &request.conversation_id, require_recovery)
            .map_err(|error| Acknowledgement::rejected(&request_id, error))?;
        match prepare_submission(&workspace, request) {
            Ok(admitted) => Ok(admitted),
            Err(ack) => {
                if workspace.submission_recovery.as_ref().is_some_and(|r| r.request_id == request_id)
                    && !workspace.runs.iter().any(|run| run.id == request_id) {
                    return Err(self.reject_reserved_locked(&mut workspace, &request_id));
                }
                Err(ack)
            }
        }
    ...
    fn reconcile(&self, request_id: &str) -> Acknowledgement {
        ...
        let mut workspace = self.workspace.lock().expect("workspace lock poisoned");
        if validate_recovery_binding(&workspace).is_err() {
            return Acknowledgement::uncertain(request_id);
        }
        let Some(run) = workspace.runs.iter().find(|run| run.id == request_id) else {
            // No execution can cross this lifecycle/workspace barrier after the
            // reserved record is marked rejected. Never dispatch during recovery.
            if workspace.submission_recovery.as_ref().is_some_and(|r| r.request_id == request_id) {
                return self.reject_reserved_locked(&mut workspace, request_id);
            }
            return Acknowledgement::uncertain(request_id);
        };
...
fn model_catalog(workspace: &WorkspaceSnapshot) -> ModelCatalog {
    ...
    let catalog_providers = providers
        ...
        .map(|provider| CatalogProvider {
            ...
            selection_capabilities: SelectionCapabilities::provider_defaults_only(),
            error_code: None,
        })
        .collect::<Vec<_>>();
    let revision = format!(
...
fn resolve_model_selection(
    ...
    entry.selection_capabilities.validate(selection)?;
...
fn prepare_constellation_state(
    ...
    let session = TeamSession::new_with_concurrency(
        TeamFrozenRun {
...
fn commit_candidate(
    ...
    mut candidate: WorkspaceSnapshot,
) -> Result<(), String> {
    match host.save(&mut candidate) {
        Ok(()) => {
            *current = candidate;
            Ok(())
        }
...
fn public_snapshot(workspace: &WorkspaceSnapshot) -> Result<serde_json::Value, String> {
    ...
    if let Some(object) = value.as_object_mut() {
        object.remove("artifactSchemaVersion");
        object.insert("artifacts".into(), serde_json::to_value(saved_artifacts::summaries(workspace)?).map_err(|_| "Saved result metadata is unavailable.")?);
        object.remove("submissionRecovery");
        object.remove("configurationOperation");
        object.remove("configurationSequence");
        ...
        for run in runs {
            ...
            if let Some(session) = request_id.as_deref().and_then(|id| {
                ...
                if let Some(object) = run.as_object_mut() {
                    ...
                    let admitted_team = request_id.as_deref().and_then(|id| {
                        workspace
                            .runs
                            .iter()
                            .find(|record| record.id == id)
                            .and_then(|record| record.admitted.team.as_ref())
                    });
                    let projection = restored.as_ref().zip(admitted_team).map(|(state, team)| {
                        let providers = team
                            .members
                            .iter()
                            .enumerate()
                            .map(|(index, member)| {
                                (format!("member-{}", index + 1), member.provider_id.clone())
                            })
                    ...
                    object.insert(
                        ...
                        serde_json::to_value(
                            projection
                                .as_ref()
                                .map(|value| value.member_results.as_slice())
                                .unwrap_or_default(),
                        )
                        .map_err(|error| error.to_string())?,
                    );
                    object.insert(
                        "resolution".into(),
                        serde_json::to_value(projection.and_then(|value| value.resolution))
                            .map_err(|error| error.to_string())?,
                    );
...
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq)]
#[serde(rename_all="camelCase", deny_unknown_fields)]
pub struct ProviderConfigurationGuard {
    pub schema_version: u32,
    pub expected_provider: Option<ProviderConfig>,
    #[serde(rename="expectedSelectedProviderID")]
    pub expected_selected_provider_id: Option<String>,
}
#[derive(Serialize)]
#[serde(rename_all="camelCase")]
pub struct ProviderConfigurationReceipt {
    schema_version: u32, state: &'static str, provider: ProviderConfig,
    #[serde(rename="selectedProviderID")]
    selected_provider_id: Option<String>,
}
fn validate_configuration_guard(workspace: &WorkspaceSnapshot, provider: &ProviderConfig, guard: &ProviderConfigurationGuard, route_current: bool) -> Result<(), String> {
    if guard.schema_version != 1 || !matches!(provider.kind,ProviderKind::Codex|ProviderKind::Claude) || provider.model.is_some() {
        return Err("CONFIGURATION_UNSUPPORTED".into());
    }
    if workspace.providers.iter().find(|p| p.id == provider.id) != guard.expected_provider.as_ref()
        || workspace.selected_provider_id != guard.expected_selected_provider_id {
        return Err("CONFIGURATION_STALE".into());
    }
    if !route_current { return Err("CONFIGURATION_ROUTE_STALE".into()); }
    Ok(())
}
fn discovered_route_id(kind: &ProviderKind, path: &Path) -> Option<String> {
    let prefix=match kind {ProviderKind::Codex=>"codex",ProviderKind::Claude=>"claude",_=>return None};
    let identity=fs::canonicalize(path).ok()?;
    let hash=format!("{:x}",Sha256::digest(identity.to_string_lossy().as_bytes()));
    Some(format!("{prefix}:discovered:{}",&hash[..16]))
}

#[derive(Clone, Debug, Deserialize, Serialize, PartialEq)]
#[serde(rename_all="camelCase", deny_unknown_fields)]
pub struct ConfigurationIntent { provider: ProviderConfig, select: bool, guard: ProviderConfigurationGuard }
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq)]
#[serde(rename_all="camelCase", deny_unknown_fields)]
...
fn validate_configuration_operation(op: &ConfigurationOperation) -> Result<(),String> {
    if op.schema_version!=1 || !op.operation_id.starts_with("configuration:") || op.operation_id.len()>64 || !matches!(op.state.as_str(),"reserved"|"applied"|"rejected") || (op.acknowledged && op.state=="reserved") || op.intent.guard.schema_version!=1 { return Err("CONFIGURATION_INVALID_RECORD".into()); }
    let p=&op.intent.provider;
    if !matches!(p.kind,ProviderKind::Codex|ProviderKind::Claude) || p.model.is_some() || p.id.is_empty() || p.id.len()>128 || !Path::new(&p.executable_path).is_absolute() || p.executable_path.len()>4096 || p.timeout_ms<1000 || p.timeout_ms>MAX_TIMEOUT_MS {return Err("CONFIGURATION_INVALID_RECORD".into());}
    Ok(())
}
fn reserve_configuration_in(host:&HostState, intent:ConfigurationIntent) -> Result<ConfigurationOperation,String> {
    validate_provider(&intent.provider)?;
    let _mutation=host.lock_mutation()?;let mut w=host.workspace.lock().map_err(|_|"Workspace unavailable")?;
    if let Some(op)=&w.configuration_operation { if !op.acknowledged { return Err("CONFIGURATION_RECOVERY_REQUIRED".into()); } }
    if w.submission_recovery.is_some() || w.runs.iter().any(|r|matches!(r.status.as_str(),"queued"|"running")) { return Err("CONFIGURATION_RUN_PENDING".into()); }
    validate_configuration_guard(&w,&intent.provider,&intent.guard,true)?;
    let mut c=w.clone();c.configuration_sequence=c.configuration_sequence.checked_add(1).ok_or("CONFIGURATION_ID_EXHAUSTED")?;
    let op=ConfigurationOperation{schema_version:1,operation_id:format!("configuration:{}",c.configuration_sequence),intent,state:"reserved".into(),acknowledged:false};
    c.configuration_operation=Some(op.clone());commit_candidate(host,&mut w,c)?;Ok(op)
}
#[tauri::command]
pub fn reserve_provider_configuration(intent:ConfigurationIntent,state:State<'_,Arc<HostState>>) -> Result<ConfigurationOperation,String> { reserve_configuration_in(state.inner(),intent) }
#[tauri::command]
pub fn get_provider_configuration_operation(state:State<'_,Arc<HostState>>) -> Result<Option<ConfigurationOperation>,String> {
    let w=state.workspace.lock().map_err(|_|"Workspace unavailable")?;validate_recovery_binding(&w)?;Ok(w.configuration_operation.clone().filter(|o|!o.acknowledged))
}
fn reconcile_configuration_in(host:&HostState,operation_id:&str,acknowledge:bool) -> Result<ConfigurationOperation,String> {
    let _mutation=host.lock_mutation()?;let mut w=host.workspace.lock().map_err(|_|"Workspace unavailable")?;
    let mut op=w.configuration_operation.clone().filter(|o|o.operation_id==operation_id).ok_or("CONFIGURATION_ID_MISMATCH")?;
    if acknowledge && op.state=="reserved" {return Err("CONFIGURATION_RECOVERY_REQUIRED".into());}
    if !acknowledge && op.state=="reserved" {op.state="rejected".into();}
    if acknowledge {op.acknowledged=true;}
    let mut c=w.clone();c.configuration_operation=Some(op.clone());commit_candidate(host,&mut w,c)?;Ok(op)
}
#[tauri::command]
pub fn reconcile_provider_configuration(operation_id:String,state:State<'_,Arc<HostState>>) -> Result<ConfigurationOperation,String> {reconcile_configuration_in(state.inner(),&operation_id,false)}
#[tauri::command]
pub fn acknowledge_provider_configuration(operation_id:String,state:State<'_,Arc<HostState>>) -> Result<ConfigurationOperation,String> {reconcile_configuration_in(state.inner(),&operation_id,true)}
fn apply_configuration_in(host:&HostState,intent:ConfigurationIntent,operation_id:&str,route_current:impl FnOnce()->bool) -> Result<ConfigurationOperation,String> {
    let _mutation=host.lock_mutation()?;let mut w=host.workspace.lock().map_err(|_|"Workspace unavailable")?;
    let mut op=w.configuration_operation.clone().filter(|o|o.operation_id==operation_id).ok_or("CONFIGURATION_ID_MISMATCH")?;
    if op.intent!=intent {return Err("CONFIGURATION_PAYLOAD_MISMATCH".into());}
    if op.acknowledged || op.state=="rejected" {return Err("CONFIGURATION_OPERATION_CLOSED".into());}
    if op.state=="applied" {let c=w.clone();commit_candidate(host,&mut w,c)?;return Ok(op);}
    if w.submission_recovery.is_some() || w.runs.iter().any(|r|matches!(r.status.as_str(),"queued"|"running")) {return Err("CONFIGURATION_RUN_PENDING".into());}
    validate_configuration_guard(&w,&intent.provider,&intent.guard,route_current())?;
    let mut c=w.clone();set_provider_configuration(&mut c,intent.provider,intent.select);op.state="applied".into();c.configuration_operation=Some(op.clone());commit_candidate(host,&mut w,c)?;Ok(op)
}
#[tauri::command]
pub fn apply_provider_configuration(intent:ConfigurationIntent,operation_id:String,state:State<'_,Arc<HostState>>) -> Result<ConfigurationOperation,String> {
    let p=intent.provider.clone();
    apply_configuration_in(state.inner(),intent,&operation_id,||discover_provider_executables(&provider_search_directories()).into_iter().any(|(kind,path)|kind==p.kind && path.to_string_lossy()==p.executable_path && discovered_route_id(&kind,&path).as_deref()==Some(p.id.as_str())))
}
...
pub fn configure_provider(
    provider: ProviderConfig,
    select: bool,
    guard: Option<ProviderConfigurationGuard>,
    state: State<'_, Arc<HostState>>,
) -> Result<Option<ProviderConfigurationReceipt>, String> {
    validate_provider(&provider)?;
    ...
    let mut workspace = state
        ...
        .lock()
        .map_err(|_| "Workspace is unavailable.")?;
    if workspace.configuration_operation.as_ref().is_some_and(|o|!o.acknowledged) {return Err("CONFIGURATION_RECOVERY_REQUIRED".into());}
    if let Some(guard) = &guard {
        let current = discover_provider_executables(&provider_search_directories()).into_iter().any(|(kind,path)| {
            kind == provider.kind && path.to_string_lossy() == provider.executable_path
                && discovered_route_id(&kind,&path).as_deref() == Some(provider.id.as_str())
        });
        validate_configuration_guard(&workspace,&provider,guard,current)?;
    }
    commit_provider_configuration(state.inner().as_ref(), &mut workspace, provider, select, guard.is_some())
}
fn commit_provider_configuration(host: &HostState, workspace: &mut WorkspaceSnapshot, provider: ProviderConfig, select: bool, guarded: bool) -> Result<Option<ProviderConfigurationReceipt>,String> {
    let receipt_provider=provider.clone();
    let mut candidate = workspace.clone();
    set_provider_configuration(&mut candidate,provider,select);
    commit_candidate(host, workspace, candidate)?;
    Ok(guarded.then(|| ProviderConfigurationReceipt { schema_version:1,state:"durable",provider:receipt_provider,selected_provider_id:workspace.selected_provider_id.clone() }))
}

fn set_provider_configuration(candidate:&mut WorkspaceSnapshot,provider:ProviderConfig,select:bool) {
...
pub async fn discover_provider_models(
    request: crate::codex_model_discovery::Request,
    state: State<'_, Arc<HostState>>,
) -> Result<crate::codex_model_discovery::Report, String> {
    use crate::codex_model_discovery as discovery;
    if !discovery::valid_id(&request.request_id) || !discovery::valid_id(&request.provider_id) {
        return Ok(discovery::Report::failure(&request, "DISCOVERY_INVALID_REQUEST"));
    }
    let host = state.inner().clone();
    let provider = host.workspace.lock().ok().and_then(|workspace| workspace.providers.iter()
        .find(|p| p.id == request.provider_id && p.kind == ProviderKind::Codex).cloned());
    let Some(provider) = provider else { return Ok(discovery::Report::failure(&request, "DISCOVERY_UNSUPPORTED")); };
    let operation = match host.begin_operation() { Ok(lease) => lease, Err(_) => return Ok(discovery::Report::failure(&request, "DISCOVERY_UNAVAILABLE")) };
    let lease = match discovery::begin(&request) { Ok(lease) => lease, Err(code) => return Ok(discovery::Report::failure(&request, code)) };
    let copy = request.clone(); let executable = provider.executable_path.clone();
    let report = tauri::async_runtime::spawn_blocking(move || {
        let _operation = operation;
        let lease = lease;
        discovery::discover(Path::new(&executable), &copy, &lease.cancelled)
    }).await.unwrap_or_else(|_| discovery::Report::failure(&request, "DISCOVERY_TRANSPORT_ERROR"));
    let unchanged = host.workspace.lock().ok().is_some_and(|w| w.providers.iter().any(|p| p == &provider));
    Ok(finalize_model_discovery(&request, report, unchanged))
}

fn finalize_model_discovery(
    request: &crate::codex_model_discovery::Request,
    report: crate::codex_model_discovery::Report,
    provider_unchanged: bool,
) -> crate::codex_model_discovery::Report {
    use crate::codex_model_discovery::Report;
    match report.error_code {
        Some(code @ ("DISCOVERY_CLEANUP_PENDING" | "DISCOVERY_CLEANUP_FAILED")) => Report::failure(request, code),
        _ if provider_unchanged => report,
        _ => Report::failure(request, "DISCOVERY_PROVIDER_CHANGED"),
    }
}

#[tauri::command]
pub fn cancel_model_discovery(request_id: String) -> bool {
    crate::codex_model_discovery::cancel(&request_id)
}
...
pub fn save_rich_draft_in(
        ...
        .find(|item| item.mutation_id == request.mutation_id)
    {
        ...
        return match host.save(&mut workspace) {
            Ok(()) => {
    ...
    // A changed configuration must be enforceable now. Unchanged legacy/stale
    // selections may still accompany draft preservation; admission validates
    // them again and never silently falls back to a different model or lead.
    if request.selection != conversation.rich_draft.selection || request.team != conversation.rich_draft.team {
        if workspace.submission_recovery.is_some() || workspace.runs.iter().any(|run| matches!(run.status.as_str(), "queued" | "running")) {
            return Ok(mutation_receipt("rejected", &request, Some(conversation.rich_draft.revision), Some("CONFIGURATION_RUN_PENDING".into())));
        }
        let selections = request.team.as_ref().map(|team| team.members.as_slice())
            .unwrap_or_else(|| request.selection.as_slice());
        for selection in selections {
            if let Err(reason) = resolve_model_selection(&workspace, selection) {
                return Ok(mutation_receipt("rejected", &request,
                    Some(conversation.rich_draft.revision), Some(reason)));
            }
        }
    }
    let approved = host
...
fn recovery_admission_guard(workspace: &WorkspaceSnapshot, request_id: &str, conversation_id: &str, required: bool) -> Result<(), String> {
    validate_recovery_binding(workspace)?;
    match &workspace.submission_recovery {
        Some(record) if record.request_id == request_id && record.conversation_id == conversation_id && record.state == "reserved" => Ok(()),
        Some(_) => Err("This request does not match the active submission reservation.".into()),
        None if required => Err("The submission reservation is absent or already resolved. Nothing was dispatched.".into()),
        None => Ok(()),
    }
}
...
fn prepare_submission(
    ...
    if let Some(existing)=workspace.runs.iter().find(|r|r.id==request.id) {
        return if existing.conversation_id==request.conversation_id && existing.admitted.prompt==request.prompt && existing.admitted.mode==request.mode && existing.admitted.retry_of.is_none() {Ok(existing.admitted.clone())} else {Err(Acknowledgement::rejected(&request.id,"Request ID is already bound to different content."))};
    }
    let Some(conversation) = workspace
        .conversations
        .iter()
    ...
    let (provider, model_selection) = if let Some(selection) = &conversation.rich_draft.selection {
        ...
        let inherited = ModelSelection {
            schema_version: 1, provider_id: selected.clone(), model_id: None, effort_id: None,
            catalog_revision: model_catalog(workspace).revision,
        };
        match resolve_model_selection(workspace, &inherited) {
            Ok((provider, resolution)) => (provider, Some(resolution)),
            Err(error) => return Err(Acknowledgement::rejected(&request.id, error)),
        }
    };
    ...
    if request.rich_draft_revision != Some(conversation.rich_draft.revision)
...
#[tauri::command]
pub fn get_submission_recovery(state: State<'_, Arc<HostState>>) -> Result<Option<SubmissionRecovery>, String> {
    state.recovery_record()
}

#[tauri::command]
pub fn reserve_submission_recovery(request: SubmissionRecoveryIdentity, state: State<'_, Arc<HostState>>) -> Result<SubmissionRecovery, String> {
    state.reserve_recovery(request)
}

#[tauri::command]
pub fn clear_submission_recovery(request: SubmissionRecoveryIdentity, state: State<'_, Arc<HostState>>) -> Result<SubmissionRecovery, String> {
    state.clear_recovery(request)
}

async fn dispatch_submission(request: SubmitRunRequest, app: AppHandle, host: Arc<HostState>, require_recovery: bool) -> Result<Acknowledgement, String> {
    let draft_guard=DraftAdmissionGuard::from_request(&request);
    let admitted = match host.prepare_reserved_submission(request, require_recovery) {
        Ok(admitted) => admitted,
        Err(acknowledgement) => return Ok(acknowledgement),
    };
    tauri::async_runtime::spawn_blocking(move || {
        let request_id = admitted.request_id.clone();
        let ack = host.execute_with_draft_guard(admitted, Some(app), require_recovery, Some(draft_guard));
        if ack.state == "rejected" {
            // Validation or pre-rename persistence may reject before a run exists.
            // Reconcile fences the journal and durably records that no dispatch won.
            let has_reservation = host.workspace.lock().ok().is_some_and(|workspace|
                workspace.submission_recovery.as_ref().is_some_and(|r| r.request_id == request_id));
            if has_reservation { return host.reconcile(&request_id); }
        }
        ack
...
#[tauri::command]
pub async fn submit_run(request: SubmitRunRequest, app: AppHandle, state: State<'_, Arc<HostState>>) -> Result<Acknowledgement, String> {
    dispatch_submission(request, app, state.inner().clone(), false).await
}

#[tauri::command]
pub async fn submit_reserved_run(request: SubmitRunRequest, app: AppHandle, state: State<'_, Arc<HostState>>) -> Result<Acknowledgement, String> {
    dispatch_submission(request, app, state.inner().clone(), true).await
}
...
mod tests {
    ...
    fn recovery_identity(request_id: &str) -> SubmissionRecoveryIdentity {
        SubmissionRecoveryIdentity { request_id: request_id.into(), conversation_id: "welcome".into() }
    }

    #[test]
    fn submission_recovery_reservation_survives_restart_without_dispatch() {
        let path = profile("recovery-reserve");
        let host = HostState::open(path.clone()).unwrap();
        assert!(host.recovery_record().unwrap().is_none());
        let reserved = host.reserve_recovery(recovery_identity("intent-1")).unwrap();
        assert_eq!(reserved.state, "reserved");
        assert_eq!(host.reserve_recovery(recovery_identity("intent-1")).unwrap(), reserved);
        assert!(host.reserve_recovery(recovery_identity("other")).is_err());
        assert!(host.workspace.lock().unwrap().runs.is_empty());
        drop(host);
        let reopened = HostState::open(path.clone()).unwrap();
        assert_eq!(reopened.recovery_record().unwrap(), Some(reserved));
        assert!(reopened.workspace.lock().unwrap().runs.is_empty());
        drop(reopened); fs::remove_dir_all(path).unwrap();
    }

    #[test]
    fn submission_recovery_no_run_reconcile_fences_delayed_dispatch_and_clear() {
        let path = profile("recovery-fence"); let host = HostState::open(path.clone()).unwrap();
        host.reserve_recovery(recovery_identity("delayed")).unwrap();
        assert!(recovery_admission_guard(&host.workspace.lock().unwrap(), "delayed", "welcome", true).is_ok());
        assert_eq!(host.reconcile("delayed").state, "rejected");
        assert!(recovery_admission_guard(&host.workspace.lock().unwrap(), "delayed", "welcome", true).is_err());
        assert_eq!(host.clear_recovery(recovery_identity("delayed")).unwrap().state, "cleared");
        assert!(host.recovery_record().unwrap().is_none());
        assert!(recovery_admission_guard(&host.workspace.lock().unwrap(), "delayed", "welcome", true).is_err());
        // Exercise the actual admission entry point after clear; the required
        // fence returns before a run record or provider process can be created.
        let delayed = AdmittedRequest {
            request_id: "delayed".into(), conversation_id: "welcome".into(), prompt: "Never dispatch".into(), mode: "direct".into(),
            provider: ProviderConfig { id:"fixture:recovery-fence".into(), kind:ProviderKind::Fixture, executable_path:"/bin/cat".into(), model:None, timeout_ms:2000 },
            approved_context: ApprovedContext::default(), attachments:vec![], model_selection:None, team:None, retry_of:None,
        };
        let rejected = host.execute_with_recovery_guard(delayed, None, true);
        assert_eq!(rejected.state, "rejected");
        assert!(rejected.error.as_deref().unwrap_or_default().contains("reservation"));
        // Legacy renderer remains usable without a reservation; reserved renderer does not fall back.
        assert!(recovery_admission_guard(&host.workspace.lock().unwrap(), "legacy", "welcome", false).is_ok());
        assert!(host.workspace.lock().unwrap().runs.is_empty());
        drop(host); fs::remove_dir_all(path).unwrap();
    }

    #[test]
    fn submission_recovery_prepare_rejection_is_durable_and_exactly_clearable() {
        let path = profile("recovery-rejected"); let host = HostState::open(path.clone()).unwrap();
        host.reserve_recovery(recovery_identity("no-provider")).unwrap();
        let request = SubmitRunRequest { id: "no-provider".into(), conversation_id: "welcome".into(), prompt: "Never dispatch".into(), mode: "direct".into(), rich_draft_revision: Some(0) };
        assert_eq!(host.prepare_reserved_submission(request, true).unwrap_err().state, "rejected");
        assert_eq!(host.recovery_record().unwrap().unwrap().state, "rejected");
        drop(host);
        let host = HostState::open(path.clone()).unwrap();
        assert_eq!(host.reconcile("no-provider").state, "rejected");
        assert!(host.clear_recovery(SubmissionRecoveryIdentity { request_id:"no-provider".into(), conversation_id:"wrong".into() }).is_err());
        assert!(host.recovery_record().unwrap().is_some());
        assert_eq!(host.clear_recovery(recovery_identity("no-provider")).unwrap().state, "cleared");
        assert!(host.workspace.lock().unwrap().runs.is_empty());
        drop(host); fs::remove_dir_all(path).unwrap();
    }

    #[test]
    fn submission_recovery_unknown_clear_and_mismatched_request_never_discard_intent() {
        let path = profile("recovery-mismatch"); let host = HostState::open(path.clone()).unwrap();
        assert_eq!(host.reconcile("unknown").state, "uncertain");
        assert!(host.clear_recovery(recovery_identity("unknown")).is_err());
        host.reserve_recovery(recovery_identity("kept")).unwrap();
        assert!(host.clear_recovery(SubmissionRecoveryIdentity { request_id:"kept".into(), conversation_id:"wrong".into() }).is_err());
        assert_eq!(host.recovery_record().unwrap().unwrap().state, "reserved");
        assert!(recovery_admission_guard(&host.workspace.lock().unwrap(), "other", "welcome", false).is_err());
        assert!(host.clear_recovery(recovery_identity("other")).is_err());
        assert_eq!(host.recovery_record().unwrap().unwrap().request_id, "kept");
        drop(host); fs::remove_dir_all(path).unwrap();
    }

    #[test]
    fn submission_recovery_reserve_faults_keep_post_rename_candidate_and_restart_identity() {
        for fault in [PersistFault::AfterWrite, PersistFault::AfterSync, PersistFault::AfterRename] {
            let path = profile("recovery-reserve-fault"); let host = HostState::open(path.clone()).unwrap();
            host.fail_next_save(fault);
            assert!(host.reserve_recovery(recovery_identity("faulted")).is_err());
            assert_eq!(host.recovery_record().unwrap().is_some(), fault == PersistFault::AfterRename);
            assert!(host.workspace.lock().unwrap().runs.is_empty());
            drop(host);
            let host = HostState::open(path.clone()).unwrap();
            assert_eq!(host.recovery_record().unwrap().is_some(), fault == PersistFault::AfterRename);
    ...
    #[test]
    fn submission_recovery_rejection_fault_is_uncertain_and_never_allows_unfenced_dispatch() {
        for fault in [PersistFault::AfterWrite, PersistFault::AfterRename] {
            let path = profile("recovery-reject-fault"); let host = HostState::open(path.clone()).unwrap();
            host.reserve_recovery(recovery_identity("faulted")).unwrap(); host.fail_next_save(fault);
            assert_eq!(host.reconcile("faulted").state, "uncertain");
            let expected = if fault == PersistFault::AfterRename { "rejected" } else { "reserved" };
            assert_eq!(host.recovery_record().unwrap().unwrap().state, expected);
            assert_eq!(host.reconcile("faulted").state, "rejected");
            assert!(recovery_admission_guard(&host.workspace.lock().unwrap(), "faulted", "welcome", true).is_err());
            drop(host); fs::remove_dir_all(path).unwrap();
        }
    }

    #[test]
    fn submission_recovery_clear_fault_retains_or_commits_exact_atomic_outcome() {
        for fault in [PersistFault::AfterWrite, PersistFault::AfterRename] {
            let path = profile("recovery-clear-fault"); let host = HostState::open(path.clone()).unwrap();
            host.reserve_recovery(recovery_identity("clear-me")).unwrap();
            // Install a persisted synthetic terminal record so reconcile needs no write.
            {
                let mut workspace = host.workspace.lock().unwrap();
                configured_fixture(&mut workspace, "fixture:test-route");
                let provider = workspace.providers[0].clone();
                workspace.runs.push(RunRecord { id:"clear-me".into(),conversation_id:"welcome".into(),status:"completed".into(),updated_at:now_marker(),
                    admitted:AdmittedRequest {request_id:"clear-me".into(),conversation_id:"welcome".into(),prompt:"Synthetic persisted test only".into(),mode:"direct".into(),provider,approved_context:ApprovedContext::default(),attachments:vec![],model_selection:None,team:None,retry_of:None},answer:Some("Synthetic test result".into()),error:None });
                host.save(&mut workspace).unwrap();
            }
            host.fail_next_save(fault);
            assert!(host.clear_recovery(recovery_identity("clear-me")).is_err());
            assert_eq!(host.recovery_record().unwrap().is_some(), fault != PersistFault::AfterRename);
            drop(host);
            let host = HostState::open(path.clone()).unwrap();
            assert_eq!(host.recovery_record().unwrap().is_some(), fault != PersistFault::AfterRename);
    ...
    #[test]
    fn submission_recovery_corrupt_saved_state_fails_closed_on_load_and_read() {
        let path = profile("recovery-corrupt"); let host = HostState::open(path.clone()).unwrap();
        {
            let mut workspace = host.workspace.lock().unwrap();
            workspace.submission_recovery = Some(SubmissionRecovery {schema_version:1,request_id:"bad".into(),conversation_id:"welcome".into(),state:"unknown".into()});
            host.save(&mut workspace).unwrap();
        }
        assert!(host.recovery_record().is_err()); drop(host);
        assert!(HostState::open(path.clone()).is_err()); fs::remove_dir_all(path).unwrap();
    }

    /// The parent owns this newly-created directory, including cleanup on assertion failure.
    struct RecoverySubprocessRoot(PathBuf);

    impl Drop for RecoverySubprocessRoot {
        fn drop(&mut self) {
            let _ = fs::remove_dir_all(&self.0);
        }
    }

    fn recovery_subprocess_case(operation: &str, point: &str) {
        let root = profile("recovery-subprocess");
        fs::create_dir(&root).expect("fresh subprocess root must not already exist");
        let owned = RecoverySubprocessRoot(root);
        fs::write(owned.0.join("case.txt"), format!("{operation}:{point}")).unwrap();
        let output = std::process::Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact", "host::tests::submission_recovery_subprocess_child",
                "--ignored", "--nocapture", "--test-threads=1",
            ])
            .env_clear()
            .env("TMPDIR", std::env::temp_dir())
            .env("TEMP", std::env::temp_dir())
            .env("RIVUNE_RECOVERY_SUBPROCESS_ROOT", &owned.0)
            .output()
            .expect("spawn only the current native test executable");
        assert_eq!(output.status.code(), Some(73),
            "child must exit at the requested persistence boundary: {operation}:{point}\nstdout={}\nstderr={}",
            String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr));

        // No host destructor or subsequent directory-sync read ran in the child.
        // This proves process-exit recovery, not power-loss durability guarantees.
        let reopened = HostState::open(owned.0.join("profile")).unwrap();
        let record = reopened.recovery_record().unwrap();
        let committed = matches!(point, "after-rename" | "after-directory-sync");
        let should_have_record = if operation == "reserve" { committed } else { !committed };
        assert_eq!(record.is_some(), should_have_record, "{operation}:{point}");
        if let Some(record) = record {
            assert_eq!(record.schema_version, 1);
            assert_eq!(record.request_id, "subprocess-intent");
            assert_eq!(record.conversation_id, "welcome");
            assert_eq!(record.state, "reserved");
        }
        {
            let workspace = reopened.workspace.lock().unwrap();
            assert_eq!(workspace.runs.len(), if operation == "reserve" { 0 } else { 1 },
                "reopen must not dispatch or duplicate a request");
            if operation == "clear" {
                assert_eq!(workspace.runs[0].id, "subprocess-intent");
                assert_eq!(workspace.runs[0].status, "completed");
                assert_eq!(workspace.runs[0].answer.as_deref(), Some("Synthetic subprocess fixture"));
            }
            if !should_have_record {
                assert!(recovery_admission_guard(&workspace, "subprocess-intent", "welcome", true).is_err(),
                    "a delayed reserved command must remain fenced after clear or absent reservation");
            }
        }
        drop(reopened);
        // owned removes only this test's unique root after all handles are closed.
    }

    #[test]
    fn submission_recovery_reserve_subprocess_faults_reopen() {
        for point in ["after-write", "after-sync", "after-rename", "after-directory-sync"] {
            recovery_subprocess_case("reserve", point);
        }
    }

    #[test]
    fn submission_recovery_clear_subprocess_faults_reopen() {
        for point in ["after-write", "after-sync", "after-rename", "after-directory-sync"] {
            recovery_subprocess_case("clear", point);
        }
    }

    #[test]
    #[ignore = "Exact subprocess helper; parent supplies a fresh owned profile root"]
    fn submission_recovery_subprocess_child() {
        let root = PathBuf::from(std::env::var_os("RIVUNE_RECOVERY_SUBPROCESS_ROOT")
            .expect("subprocess helper requires an explicit owned root"));
        let canonical = fs::canonicalize(&root).unwrap();
        let temp = fs::canonicalize(std::env::temp_dir()).unwrap();
        assert!(canonical.starts_with(&temp));
        assert!(root.file_name().unwrap().to_string_lossy().starts_with("rivune-tauri-recovery-subprocess-"));
        let case = fs::read_to_string(root.join("case.txt")).unwrap();
        let (operation, point) = case.split_once(':').expect("explicit subprocess case");
        assert!(matches!(operation, "reserve" | "clear"));
        let fault = match point {
            "after-write" => PersistFault::AfterWrite,
            "after-sync" => PersistFault::AfterSync,
            "after-rename" => PersistFault::AfterRename,
            "after-directory-sync" => PersistFault::None,
            _ => panic!("unsupported subprocess fault point"),
        };
        let isolated_profile = root.join("profile");
        assert!(!isolated_profile.exists(), "helper only initializes a fresh owned profile");
        let host = HostState::open(isolated_profile).unwrap();
        if operation == "clear" {
            host.reserve_recovery(recovery_identity("subprocess-intent")).unwrap();
            // Seed one durable terminal record directly; no executable or provider is invoked.
            let mut workspace = host.workspace.lock().unwrap();
            configured_fixture(&mut workspace, "fixture:subprocess-clear");
            let provider = workspace.providers[0].clone();
            workspace.runs.push(RunRecord {
                id: "subprocess-intent".into(), conversation_id: "welcome".into(),
                status: "completed".into(), updated_at: now_marker(),
                admitted: AdmittedRequest {
                    request_id: "subprocess-intent".into(), conversation_id: "welcome".into(),
                    prompt: "Synthetic subprocess fixture".into(), mode: "direct".into(), provider,
                    approved_context: ApprovedContext::default(), attachments: vec![],
                    model_selection: None, team: None, retry_of: None,
                },
                answer: Some("Synthetic subprocess fixture".into()), error: None,
            });
            host.save(&mut workspace).unwrap();
        }
        host.fail_next_save(fault);
        let outcome = if operation == "reserve" {
            host.reserve_recovery(recovery_identity("subprocess-intent"))
        } else {
            host.clear_recovery(recovery_identity("subprocess-intent"))
        };
        assert_eq!(outcome.is_ok(), fault == PersistFault::None);
        // Deliberately bypass HostState/File destructors. In particular do not call
        // recovery_record(), which would add a directory durability barrier.
        std::process::exit(73);
    }
...
"#,
    ...
    fn actual_constellation_fixture_persists_each_phase_and_delivers_one_answer() {
        ...
        assert_eq!(
            public["runs"][0]["memberResults"].as_array().unwrap().len(),
            2
        );
        assert_eq!(
            public["runs"][0]["memberResults"][0]["role"],
            "independentAnswer"
        );
        assert_eq!(public["runs"][0]["resolution"]["kind"], "leadSynthesis");
        assert_eq!(public["runs"][0]["resolution"]["reviewed"], true);
    ...
    #[test]
    fn inherited_admission_resolves_current_default_without_legacy_override() {
        let mut workspace=WorkspaceSnapshot::default();
        configured_fixture(&mut workspace,"fixture:earlier");
        workspace.selected_provider_id=Some("fixture:earlier".into());
        let earlier_catalog=model_catalog(&workspace).revision;
        configured_fixture(&mut workspace,"fixture:later");
        workspace.providers.iter_mut().find(|p|p.id=="fixture:later").unwrap().model=Some("legacy-override".into());
        workspace.selected_provider_id=Some("fixture:later".into());
        let request=SubmitRunRequest{id:"inheritance-regression".into(),conversation_id:"welcome".into(),prompt:"Synthetic only".into(),mode:"direct".into(),rich_draft_revision:Some(0)};
        let admitted=prepare_submission(&workspace,request.clone()).unwrap();
        assert_eq!(admitted.provider.id,"fixture:later");assert!(admitted.provider.model.is_none());
        let resolution=admitted.model_selection.unwrap();assert_eq!(resolution.resolution,"providerManagedDefault");
        assert_eq!(resolution.requested.provider_id,"fixture:later");assert!(resolution.requested.model_id.is_none());
        assert_eq!(resolution.adapter_revision,model_catalog(&workspace).revision);assert_ne!(resolution.adapter_revision,earlier_catalog);
        assert!(workspace.conversations[0].rich_draft.selection.is_none());
        assert_eq!(workspace.providers.iter().find(|p|p.id=="fixture:later").unwrap().model.as_deref(),Some("legacy-override"));
        workspace.providers.iter_mut().find(|p|p.id=="fixture:later").unwrap().executable_path="/synthetic/missing-route".into();
        assert!(prepare_submission(&workspace,request).is_err());
    }

    #[test]
    fn model_capabilities_report_defaults_only_and_reject_overrides() {
        let mut workspace = WorkspaceSnapshot::default();
        let selection = configured_fixture(&mut workspace, "fixture:capability");
        let catalog = model_catalog(&workspace);
        let wire = serde_json::to_value(&catalog.providers[0].selection_capabilities).unwrap();
        assert_eq!(wire, serde_json::json!({"schemaVersion":1,"modelOverride":"unsupported",
            "effortOverride":"unsupported","reasonCode":"ADAPTER_DISCOVERY_UNAVAILABLE"}));
        assert!(catalog.providers[0].models.is_empty());
        assert_eq!(model_catalog(&workspace).revision, catalog.revision);
        let (provider, admitted) = resolve_model_selection(&workspace, &selection).unwrap();
        assert!(provider.model.is_none());
        assert!(admitted.effective_model_id.is_none());
        assert_eq!(admitted.resolution, "providerManagedDefault");
        for effort in [None, Some("unverified-effort".to_owned())] {
            let mut explicit = selection.clone(); explicit.model_id = Some("unverified-model".into()); explicit.effort_id = effort;
            assert_eq!(resolve_model_selection(&workspace, &explicit).unwrap_err(), "MODEL_CAPABILITIES_UNKNOWN");
        }
        let mut effort_only = selection; effort_only.effort_id = Some("unverified-effort".into());
        assert_eq!(resolve_model_selection(&workspace, &effort_only).unwrap_err(), "INVALID_SELECTION");
    }

    #[test]
    fn model_capabilities_legacy_team_preserves_draft_and_lead_until_explicit_review() {
        let path = profile("capability-migration");
        let host = HostState::open(path.clone()).unwrap();
        let legacy = {
            let mut w = host.workspace.lock().unwrap();
            let first = configured_fixture(&mut w, "fixture:first");
            let second = configured_fixture(&mut w, "fixture:second");
            let mut team = TeamSelection { schema_version: 1, lead_index: 1, members: vec![first, second] };
            for member in &mut team.members { member.catalog_revision = "legacy-catalog".into(); }
            w.conversations[0].rich_draft.selection = Some(team.members[1].clone());
            w.conversations[0].rich_draft.team = Some(team.clone());
            host.save(&mut w).unwrap(); team
        };
        drop(host);
        let host = HostState::open(path.clone()).unwrap();
        assert_eq!(host.workspace.lock().unwrap().conversations[0].rich_draft.team, Some(legacy.clone()));
        let request = SaveRichDraftRequest { conversation_id: "welcome".into(), mutation_id: "preserve-legacy".into(), expected_revision: 0,
            draft: "Preserve my draft".into(), attachment_ids: vec![], selection: Some(legacy.members[1].clone()), team: Some(legacy.clone()) };
        assert_eq!(save_rich_draft_in(&host, request.clone(), false).unwrap().state, "durable");
        assert_eq!(resolve_model_selection(&host.workspace.lock().unwrap(), &legacy.members[1]).unwrap_err(), "CATALOG_STALE");
        let mut reviewed = legacy;
        let revision = model_catalog(&host.workspace.lock().unwrap()).revision;
        for member in &mut reviewed.members { member.catalog_revision = revision.clone(); }
        let mut rejected = request.clone(); rejected.expected_revision = 1; rejected.mutation_id = "reject-override".into();
        rejected.team = Some(reviewed.clone()); rejected.team.as_mut().unwrap().members[0].model_id = Some("unsupported-model".into());
        rejected.selection = Some(reviewed.members[1].clone());
        let receipt = save_rich_draft_in(&host, rejected, false).unwrap();
        assert_eq!(receipt.state, "rejected"); assert_eq!(receipt.error.as_deref(), Some("MODEL_CAPABILITIES_UNKNOWN"));
        let mut applied = request; applied.expected_revision = 1; applied.mutation_id = "review-defaults".into();
        applied.selection = Some(reviewed.members[1].clone()); applied.team = Some(reviewed.clone());
        assert_eq!(save_rich_draft_in(&host, applied, false).unwrap().state, "durable");
        drop(host);
        let reopened = HostState::open(path.clone()).unwrap();
        assert_eq!(reopened.workspace.lock().unwrap().conversations[0].rich_draft.team, Some(reviewed));
        drop(reopened); fs::remove_dir_all(path).unwrap();
    }
    ...
    fn active_conversation_persists_and_missing_ids_fall_back_safely() {
        ...
        );
        {
            ...
            reopened.save(&mut workspace).unwrap();
        }
        drop(reopened);
    ...
    fn reopening_skips_a_pending_generation_before_the_next_save() {
        ...
        reopened.save(&mut accepted).unwrap();
    ...
    fn snapshot_retention_is_bounded() {
        ...
        for index in 0..(SNAPSHOT_RETENTION + 9) {
            ...
            host.save(&mut candidate).unwrap();
...
mod discovery_boundary_tests {
    use super::finalize_model_discovery;
    use crate::codex_model_discovery::{Request, Report, AdvertisedModel};
    #[test]
    fn discovery_cleanup_errors_survive_provider_change_and_unavailable_revalidation() {
        let request=Request { request_id:"synthetic-boundary".into(), provider_id:"synthetic-provider".into() };
        // false is the production path for changed/removed provider or failed lock.
        for unchanged in [false,true] {
            for code in ["DISCOVERY_CLEANUP_PENDING","DISCOVERY_CLEANUP_FAILED"] {
                let mut report=Report::failure(&request,code);
                report.models.push(AdvertisedModel { id:"must-discard".into(),model:"must-discard".into(),label:"Synthetic".into(),reasoning_efforts:vec!["default".into()],default_reasoning_effort:"default".into(),is_default:true });
                let final_report=finalize_model_discovery(&request,report,unchanged);
                assert_eq!(final_report.error_code,Some(code));assert_eq!(final_report.state,"error");
                assert!(final_report.models.is_empty());assert!(!final_report.selection_enabled);
                assert_eq!(final_report.request_id,request.request_id);assert_eq!(final_report.provider_id,request.provider_id);
            }
        }
        let report=Report::failure(&request,"DISCOVERY_CANCELLED");
        assert_eq!(finalize_model_discovery(&request,report,false).error_code,Some("DISCOVERY_PROVIDER_CHANGED"));
    }
}
...
mod provider_configuration_tests {
    use super::*;
    #[test]
    fn configuration_guard_rejects_stale_route_and_selection_without_mutating_workspace() {
        let mut w=WorkspaceSnapshot::default();
        let p=ProviderConfig {id:"codex:discovered:synthetic".into(),kind:ProviderKind::Codex,executable_path:"/synthetic/codex".into(),model:None,timeout_ms:60000};
        let g=ProviderConfigurationGuard {schema_version:1,expected_provider:None,expected_selected_provider_id:w.selected_provider_id.clone()};
        let before=serde_json::to_vec(&w).unwrap();
        assert!(validate_configuration_guard(&w,&p,&g,true).is_ok());
        assert_eq!(validate_configuration_guard(&w,&p,&g,false).unwrap_err(),"CONFIGURATION_ROUTE_STALE");
        assert_eq!(serde_json::to_vec(&w).unwrap(),before);
        w.selected_provider_id=Some("changed".into());assert_eq!(validate_configuration_guard(&w,&p,&g,true).unwrap_err(),"CONFIGURATION_STALE");
        w.selected_provider_id=g.expected_selected_provider_id.clone();w.providers.push(p.clone());assert_eq!(validate_configuration_guard(&w,&p,&g,true).unwrap_err(),"CONFIGURATION_STALE");
    }
    #[test]
    fn configuration_persist_failure_has_no_receipt_and_success_preserves_draft_on_reopen() {
        let path=std::env::temp_dir().join(format!("rivune-config-{}-{}",std::process::id(),std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
        let host=HostState::open(path.clone()).unwrap();
        let mut current=host.workspace.lock().unwrap().clone();
        current.conversations[0].draft="Preserve synthetic draft".into();host.save(&mut current).unwrap();
        let before=serde_json::to_vec(&current.conversations).unwrap();
        let executable=path.join("codex");fs::write(&executable,"synthetic file; never executed").unwrap();
        let p=ProviderConfig {id:"codex:discovered:synthetic".into(),kind:ProviderKind::Codex,executable_path:executable.to_string_lossy().into_owned(),model:None,timeout_ms:60000};
        host.fail_next_save(PersistFault::AfterSync);
        assert!(commit_provider_configuration(&host,&mut current,p.clone(),true,true).is_err());
        assert!(!current.providers.iter().any(|v|v.id==p.id));assert_eq!(serde_json::to_vec(&current.conversations).unwrap(),before);
        let receipt=commit_provider_configuration(&host,&mut current,p.clone(),true,true).unwrap().unwrap();
        assert_eq!(receipt.state,"durable");assert_eq!(receipt.provider,p);assert_eq!(receipt.selected_provider_id,Some(p.id.clone()));
        assert_eq!(serde_json::to_vec(&current.conversations).unwrap(),before);
        drop(host);let reopened=HostState::open(path.clone()).unwrap();let restored=reopened.workspace.lock().unwrap();
        assert!(restored.providers.contains(&p));assert_eq!(serde_json::to_vec(&restored.conversations).unwrap(),before);
        drop(restored);drop(reopened);fs::remove_dir_all(path).unwrap();
    }

    #[test]
    fn conversation_binding_pending_reservation_rejects_change_but_preserves_draft_save() {
        let path=std::env::temp_dir().join(format!("rivune-binding-{}",std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
        let host=HostState::open(path.clone()).unwrap();
        {let mut w=host.workspace.lock().unwrap();w.submission_recovery=Some(SubmissionRecovery {schema_version:1,request_id:"pending".into(),conversation_id:"welcome".into(),state:"reserved".into()});}
        let mut request=SaveRichDraftRequest {conversation_id:"welcome".into(),mutation_id:"binding-test".into(),expected_revision:0,draft:"Keep draft".into(),attachment_ids:vec![],selection:Some(ModelSelection {schema_version:1,provider_id:"synthetic".into(),model_id:None,effort_id:None,catalog_revision:"synthetic".into()}),team:None};
        let receipt=save_rich_draft_in(&host,request.clone(),false).unwrap();assert_eq!(receipt.state,"rejected");assert!(host.workspace.lock().unwrap().conversations[0].rich_draft.selection.is_none());
        request.selection=None;let saved=save_rich_draft_in(&host,request,false).unwrap();assert_eq!(saved.state,"durable");assert_eq!(host.workspace.lock().unwrap().conversations[0].draft,"Keep draft");
        drop(host);fs::remove_dir_all(path).unwrap();
    }

    fn recovery_fixture() -> (PathBuf,HostState,ConfigurationIntent) {
        static NEXT: std::sync::atomic::AtomicU64=std::sync::atomic::AtomicU64::new(0);
        let path=std::env::temp_dir().join(format!("rivune-config-recovery-{}-{}-{}",std::process::id(),NEXT.fetch_add(1,std::sync::atomic::Ordering::SeqCst),std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
        let host=HostState::open(path.clone()).unwrap();let executable=path.join("codex");fs::write(&executable,"synthetic; never executed").unwrap();
        let intent=ConfigurationIntent{provider:ProviderConfig{id:"codex:discovered:synthetic".into(),kind:ProviderKind::Codex,executable_path:executable.to_string_lossy().into_owned(),model:None,timeout_ms:60000},select:true,guard:ProviderConfigurationGuard{schema_version:1,expected_provider:None,expected_selected_provider_id:None}};
        (path,host,intent)
    }
    #[test]
    fn configuration_recovery_lost_ack_restart_same_id_and_payload_fence() {
        let (path,host,intent)=recovery_fixture();let op=reserve_configuration_in(&host,intent.clone()).unwrap();
        let mut different=intent.clone();different.select=false;
        assert_eq!(apply_configuration_in(&host,different,&op.operation_id,||true).unwrap_err(),"CONFIGURATION_PAYLOAD_MISMATCH");
        let applied=apply_configuration_in(&host,intent.clone(),&op.operation_id,||true).unwrap();assert_eq!(applied.state,"applied");
        drop(host);let host=HostState::open(path.clone()).unwrap();
        assert_eq!(host.workspace.lock().unwrap().configuration_operation.as_ref().unwrap().operation_id,op.operation_id);
        assert_eq!(apply_configuration_in(&host,intent.clone(),&op.operation_id,||panic!("Applied operation must not apply again")).unwrap().state,"applied");
        assert!(reserve_configuration_in(&host,intent.clone()).is_err());
        assert_eq!(reconcile_configuration_in(&host,&op.operation_id,false).unwrap().state,"applied");
        assert!(reconcile_configuration_in(&host,&op.operation_id,true).unwrap().acknowledged);
        assert!(apply_configuration_in(&host,intent,&op.operation_id,||true).is_err());
        drop(host);fs::remove_dir_all(path).unwrap();
    }
    #[test]
    fn configuration_recovery_reserved_restart_reconcile_fences_delayed_apply() {
        let (path,host,intent)=recovery_fixture();let op=reserve_configuration_in(&host,intent.clone()).unwrap();drop(host);
        let host=HostState::open(path.clone()).unwrap();assert_eq!(reconcile_configuration_in(&host,&op.operation_id,false).unwrap().state,"rejected");
        assert!(apply_configuration_in(&host,intent.clone(),&op.operation_id,||true).is_err());assert!(host.workspace.lock().unwrap().providers.is_empty());
        reconcile_configuration_in(&host,&op.operation_id,true).unwrap();let next=reserve_configuration_in(&host,intent.clone()).unwrap();assert_ne!(next.operation_id,op.operation_id);
        assert!(apply_configuration_in(&host,intent,&op.operation_id,||true).is_err());drop(host);fs::remove_dir_all(path).unwrap();
    }
    #[test]
    fn configuration_recovery_persist_faults_never_promote_visible_state() {
        for fault in [PersistFault::AfterSync,PersistFault::AfterRename] {
            let (path,host,intent)=recovery_fixture();let op=reserve_configuration_in(&host,intent.clone()).unwrap();host.fail_next_save(fault);
            assert!(apply_configuration_in(&host,intent,&op.operation_id,||true).is_err());drop(host);
            let host=HostState::open(path.clone()).unwrap();let state=host.workspace.lock().unwrap().configuration_operation.as_ref().unwrap().state.clone();
            assert_eq!(state,if fault==PersistFault::AfterRename{"applied"}else{"reserved"});
            host.fail_next_save(PersistFault::AfterRename);assert!(reconcile_configuration_in(&host,&op.operation_id,false).is_err());
            let receipt=reconcile_configuration_in(&host,&op.operation_id,false).unwrap();assert_eq!(receipt.state,if fault==PersistFault::AfterRename{"applied"}else{"rejected"});
            drop(host);fs::remove_dir_all(path).unwrap();
        }
    }
    #[test]
    fn configuration_recovery_reservation_fault_and_removed_executable_reopen() {
        let (path,host,intent)=recovery_fixture();host.fail_next_save(PersistFault::AfterRename);assert!(reserve_configuration_in(&host,intent).is_err());drop(host);
        fs::remove_file(path.join("codex")).unwrap();let host=HostState::open(path.clone()).unwrap();let op=host.workspace.lock().unwrap().configuration_operation.clone().unwrap();
        assert_eq!(reconcile_configuration_in(&host,&op.operation_id,false).unwrap().state,"rejected");
        let mut bad=host.workspace.lock().unwrap().clone();bad.configuration_sequence+=1;assert!(validate_recovery_binding(&bad).is_err());
        drop(host);fs::remove_dir_all(path).unwrap();
    }

    #[test]
    fn configuration_recovery_terminal_ack_fault_retry_and_reopen() {
        for terminal in ["applied", "rejected"] {
            for fault in [PersistFault::AfterSync, PersistFault::AfterRename] {
                for reopen_before_retry in [false, true] {
                    let (path, host, intent) = recovery_fixture();
                    let op = reserve_configuration_in(&host, intent.clone()).unwrap();
                    assert_eq!(reconcile_configuration_in(&host, &op.operation_id, true).unwrap_err(), "CONFIGURATION_RECOVERY_REQUIRED");
                    if terminal == "applied" {
                        apply_configuration_in(&host, intent.clone(), &op.operation_id, || true).unwrap();
                    } else {
                        reconcile_configuration_in(&host, &op.operation_id, false).unwrap();
                    }
                    let before = {
                        let w = host.workspace.lock().unwrap();
                        serde_json::to_vec(&(w.providers.clone(), w.selected_provider_id.clone(), w.conversations.clone())).unwrap()
                    };
                    host.fail_next_save(fault);
                    assert!(reconcile_configuration_in(&host, &op.operation_id, true).is_err(), "Failed acknowledgement must not return a receipt");
                    let host = if reopen_before_retry { drop(host); HostState::open(path.clone()).unwrap() } else { host };
                    let visible = host.workspace.lock().unwrap().configuration_operation.clone().unwrap();
                    assert_eq!(visible.operation_id, op.operation_id);
                    assert_eq!(visible.state, terminal);
                    assert_eq!(visible.acknowledged, fault == PersistFault::AfterRename);
                    if !visible.acknowledged {
                        assert_eq!(reserve_configuration_in(&host, intent.clone()).unwrap_err(), "CONFIGURATION_RECOVERY_REQUIRED");
                    }
                    // A pre-rename applied marker can be re-confirmed, but never applied twice.
                    // All rejected or acknowledged states must reject delayed application.
                    let delayed = apply_configuration_in(&host, intent.clone(), &op.operation_id, || panic!("Terminal operation must never apply again"));
                    if terminal == "applied" && !visible.acknowledged { assert_eq!(delayed.unwrap().state, "applied"); }
                    else { assert_eq!(delayed.unwrap_err(), "CONFIGURATION_OPERATION_CLOSED"); }
                    let receipt = reconcile_configuration_in(&host, &op.operation_id, true).unwrap();
                    assert!(receipt.acknowledged); assert_eq!(receipt.state, terminal);
                    drop(host);
                    let host = HostState::open(path.clone()).unwrap();
                    {
                        let w = host.workspace.lock().unwrap();
                        assert!(w.configuration_operation.as_ref().unwrap().acknowledged);
                        assert_eq!(serde_json::to_vec(&(w.providers.clone(), w.selected_provider_id.clone(), w.conversations.clone())).unwrap(), before);
                    }
                    let mut next_intent = intent.clone();
                    if terminal == "applied" {
                        next_intent.guard.expected_provider = Some(intent.provider.clone());
                        next_intent.guard.expected_selected_provider_id = Some(intent.provider.id.clone());
                    }
                    let next = reserve_configuration_in(&host, next_intent).unwrap();
                    assert_ne!(next.operation_id, op.operation_id);
                    assert_eq!(apply_configuration_in(&host, intent, &op.operation_id, || panic!("Replaced identity must not apply")).unwrap_err(), "CONFIGURATION_ID_MISMATCH");
                    drop(host); fs::remove_dir_all(path).unwrap();
                }
            }
        }
...
#[tauri::command]
pub fn inspect_artifact(request: saved_artifacts::InspectRequest, state: State<'_, Arc<HostState>>) -> Result<saved_artifacts::Inspection, String> {
    let workspace = state.workspace.lock().map_err(|_| "Workspace is unavailable.")?;
    saved_artifacts::inspect(&workspace, &request)
}

#[cfg(test)]
#[path = "saved_artifact_tests.rs"]
mod saved_artifact_tests;

#[cfg(test)]
#[path = "sent_draft_tests.rs"]
mod sent_draft_tests;
