...
export function parseSnapshot(value) {
  ...
  if (value.runtimeCapabilities != null) parseRuntimeCapabilities(value.runtimeCapabilities);
  ...
  for (const run of value.runs) {
    if (run.failedInvocationID != null) modelString(run.failedInvocationID);
    if (run.failedAttemptID != null) modelString(run.failedAttemptID);
...
export function createHostAdapter(host) {
  ...
  return Object.freeze({
    ...
    ...Object.fromEntries(["retryConstellationInvocation", "getModelCatalog", "refreshModelCatalog", "selectTextAttachments", "approveTextAttachment", "inspectTextAttachment", "validateDraftAttachments", "saveRichDraft"].filter(name => typeof host[name] === "function").map(name => [name, (...args) => host[name](...args)])),
    ...
    ...(typeof host.onRunEvent === "function" ? { onRunEvent: handler => host.onRunEvent(handler) } : {}),
...
export function parseRunActivity(value, run) {
  modelRecord(value, ["schemaVersion", "baseSequence", "entries"]);
  if (value.schemaVersion !== 1 || !Number.isSafeInteger(value.baseSequence) || value.baseSequence < 1 || !Array.isArray(value.entries) || value.entries.length > 256) throw new Error("Activity needs to be refreshed.");
  let bytes = 0;
  for (let index = 0; index < value.entries.length; index++) {
    const event = value.entries[index];
    modelRecord(event, ["schemaVersion", "eventID", "requestID", "conversationID", "sequence", "kind", "phase", "state", "memberID", "providerID", "role", "summary", "textDelta", "error"]);
    if (event.schemaVersion !== 1 || event.requestID !== run.id || event.conversationID !== run.conversationID || event.sequence !== value.baseSequence + index || event.eventID !== `${run.id}:${event.sequence}`) throw new Error("Activity sequence needs to be refreshed.");
    modelEnum(event.kind, ["admitted", "providerStarted", "answerDelta", "memberStarted", "memberCompleted", "leadReviewStarted", "finalCompleted", "failed", "cancelled", "recoveryRequired"]);
    modelEnum(event.phase, ["admission", "direct", "decide", "contribute", "integrate", "review", "final", "recovery"]);
    modelEnum(event.state, ["queued", "running", "completed", "failed", "cancelled", "uncertain"]);
    for (const key of ["memberID", "providerID", "role"]) if (event[key] !== null) modelString(event[key]);
    modelString(event.summary, 512);
    if (event.error !== null) modelString(event.error, 1024);
    if (event.textDelta !== null) { modelString(event.textDelta, 16384); bytes += new TextEncoder().encode(event.textDelta).length; }
    if (bytes > 262144) throw new Error("Activity exceeds its display limit.");
  }
  return value;
}

// Own the asynchronous subscription and coalescing timer as one disposable resource.
export function subscribeRunUpdates(host, { refresh, onError = () => {}, schedule = setTimeout, cancel = clearTimeout }) {
  let disposed = false;
  let timer = null;
  let unsubscribe = null;
  const seen = new Set();
  const detach = fn => { try { Promise.resolve(fn()).catch(() => {}); } catch { /* teardown is best effort */ } };
  Promise.resolve().then(() => {
    if (disposed) return null;
    return host.onRunEvent(event => {
      if (disposed || !event || event.schemaVersion !== 1 || typeof event.eventID !== "string" || event.eventID.length > 256 || seen.has(event.eventID)) return;
      seen.add(event.eventID);
      if (seen.size > 256) seen.delete(seen.values().next().value);
      if (timer !== null) return;
      timer = schedule(() => {
        timer = null;
        if (!disposed) Promise.resolve().then(() => { if (!disposed) return refresh(() => !disposed); }).catch(error => { if (!disposed) onError(error); });
      }, 40);
    });
  }).then(fn => {
    if (typeof fn !== "function") return;
    if (disposed) detach(fn);
    else unsubscribe = fn;
  ...
  return () => {
    ...
    seen.clear();
    if (unsubscribe) { detach(unsubscribe); unsubscribe = null; }
  };
}

export function parseRuntimeCapabilities(value) {
  modelRecord(value, ["schemaVersion", "constellation", "minimumMembers", "reasonCode"]);
  if (value.schemaVersion !== 1 || value.minimumMembers !== 2) throw new Error("Unreadable runtime capabilities.");
  modelEnum(value.constellation, ["available", "unavailable"]);
  if (value.reasonCode !== null) modelString(value.reasonCode);
  return value;
}

export function parseConstellationDetails(value) {
  modelRecord(value,["memberResults","resolution"]);
  if(!Array.isArray(value.memberResults)||value.memberResults.length>6)throw new Error("Unreadable member answers.");
  const identities=new Set();let bytes=0;
  for(const item of value.memberResults){
    modelRecord(item,["schemaVersion","memberID","providerID","role","text","truncated"]);
    if(item.schemaVersion!==1||item.role!=="independentAnswer"||typeof item.truncated!=="boolean")throw new Error("Unreadable member answer.");
    modelString(item.memberID);if(item.providerID!==null)modelString(item.providerID);
    if(identities.has(item.memberID))throw new Error("Duplicate member answer.");identities.add(item.memberID);
    if(typeof item.text!=="string")throw new Error("Unreadable member answer text.");
    const size=new TextEncoder().encode(item.text).length;bytes+=size;
    if(size>24576||bytes>98304)throw new Error("Member answer exceeds its display limit.");
  }
  if(value.resolution!==null){
    const r=value.resolution;modelRecord(r,["schemaVersion","kind","reviewed","reviewerMemberID","providerID","summary"]);
    if(r.schemaVersion!==1||r.kind!=="leadSynthesis"||typeof r.reviewed!=="boolean")throw new Error("Unreadable lead synthesis.");
    modelString(r.reviewerMemberID);if(r.providerID!==null)modelString(r.providerID);modelString(r.summary,512);
  }
  return value;
}
