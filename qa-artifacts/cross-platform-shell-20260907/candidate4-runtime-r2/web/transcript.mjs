...
const STATES = new Set(['queued','running','completed','failed','cancelled','preserved']);
...
export function createTranscript({container,scroller=container,copyText,openExternal,onStatus=()=>{}}) {
  ...
  // A completion from a removed conversation must not announce into the new one.
  const visible=entry=>nodes.get(entry.node.dataset.runId)===entry && entry.node.parentNode===container;
  ...
  function create(row){
    ...
    const copy=el('button','Copy response');copy.type='button';copy.className='secondary';copy.hidden=true;
    footer.append(document.createTextNode(' '),copy);
    assistant.append(header,response,error,footer);node.append(user,assistant);
    const entry={node,prompt,provider,state,response,error,footer,retry,copy,copyPending:false,answer:null,status:null,signature:null};
    copy.addEventListener('click',async()=>{
      if(typeof copyText!=='function'||entry.copyPending||!entry.answer?.trim()||!visible(entry))return;
      const answer=entry.answer,partial=['queued','running','failed','cancelled'].includes(entry.status);
      entry.copyPending=true;copy.textContent='Copying…';copy.setAttribute('aria-disabled','true');
      try {await copyText(answer);if(visible(entry))notify(partial?'Partial response copied.':'Response copied.');}
      catch {if(visible(entry))notify('Could not copy response. Select the response text and copy manually.');}
      finally {entry.copyPending=false;copy.textContent='Copy response';copy.removeAttribute('aria-disabled');}
    });
    return entry;
  }
  function update(entry,row){
    ...
    const labels={queued:'Queued',running:row.answer?'Responding · partial answer':'Working…',completed:row.answer?.trim()?'Complete':'Finished without an answer',failed:row.answer?'Failed · partial answer':'Failed',cancelled:row.answer?'Stopped · partial answer':'Stopped',preserved:'Preserved legacy state · inert'};
    ...
    if(entry.answer!==row.answer){
      if(!row.answer) entry.response.replaceChildren();
        ...
        const fresh=renderMarkdown(row.answer,{document,copyText,openExternal,report:message=>{if(visible(entry))notify(message)}});
    ...
    entry.copy.hidden=typeof copyText!=='function'||!row.answer?.trim();
