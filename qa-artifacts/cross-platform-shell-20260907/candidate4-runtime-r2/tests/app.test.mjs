...
test("renderer gates stale state and preserves newer or rejected drafts", async () => {
  ...
  globalThis.__RIVUNE_DESKTOP_HOST__ = {
    getSnapshot: async () => snapshotMode === "valid"
      ? { schemaVersion: 1, selectedProviderID: "codex:fixture", providers: [{ id: "codex:fixture", kind: "codex", executablePath: "/fixture/codex", model: null, timeoutMs: 2000 }], conversations: [{ id: "c1", title: "Real conversation" }, { id: "c2", title: "Other" }], runs: [] }
      : snapshotMode === "changed"
        ? { schemaVersion: 1, selectedProviderID: "codex:fixture", providers: [{ id: "codex:fixture", kind: "codex", executablePath: "/fixture/codex", model: null, timeoutMs: 2000 }], conversations: [{ id: "c2", title: "Other" }], runs: [] }
        : { schemaVersion: 2 },
    openConversation: async () => {},
    saveRichDraft: async r => ({state:"durable",mutationID:r.mutationID,conversationID:r.conversationID,revision:r.expectedRevision+1,attachmentIDs:r.attachmentIDs,selection:r.selection,team:r.team}),
  ...
  assert.match(get("#request-diagnostic-id").textContent, new RegExp(uncertainID));
  ...
  snapshotMode = "changed";
  await get("#retry").listeners.click();
  await get("#prompt").listeners.change();
  await settle();
  assert.equal(get("#submit").disabled, false, "current conversation becomes sendable after its rich draft is durable");
