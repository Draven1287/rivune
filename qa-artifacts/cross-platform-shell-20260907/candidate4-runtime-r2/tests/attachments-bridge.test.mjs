...
test('constellation retry forwards only actual host invocation identity',async()=>{
 const calls=[];globalThis.__TAURI__={core:{invoke:async(command,args)=>calls.push({command,args})}};
 await import(`../web/desktop-host.mjs?constellation=${Date.now()}`);
 await globalThis.__RIVUNE_DESKTOP_HOST__.retryConstellationInvocation({requestID:'constellation-retry-run',invocationID:'opaque-host-invocation-id',failedAttemptID:'opaque-host-attempt-id'});
 assert.deepEqual(calls,[{command:'retry_constellation_invocation',args:{request:{requestID:'constellation-retry-run',invocationID:'opaque-host-invocation-id',failedAttemptID:'opaque-host-attempt-id'}}}]);
 delete globalThis.__TAURI__;delete globalThis.__RIVUNE_DESKTOP_HOST__;
});
