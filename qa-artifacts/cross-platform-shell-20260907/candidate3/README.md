...
Opening `web/index.html` shows the truthful disconnected state. A real desktop host must inject `globalThis.__RIVUNE_DESKTOP_HOST__` before `app.mjs` loads. Submission remains disabled unless the current snapshot validates and the selected conversation still exists. This spike intentionally does not implement a fallback store, mock task, clipboard/screen access, network endpoint, native command, or provider dispatch.
...
globalThis.__RIVUNE_DESKTOP_HOST__ = {
  ...
  reconcileRun: async (requestID) => acknowledgement
};
```

The acknowledgement contract is exact and request-bound:

```js
{ state: "accepted", requestID }
{ state: "rejected", requestID, error?: "human-readable reason" }
{ state: "uncertain", requestID }
```

Missing, malformed, or mismatched acknowledgements normalize to `uncertain`. Accepted clears the composer only when its conversation, draft revision, and exact draft text are unchanged. That same submitted context is retained through uncertainty: a later confirmed-accepted reconciliation clears only the still-identical original draft, while edits or conversation changes remain untouched. Rejected and uncertain outcomes preserve the draft. A reconciliation transport failure also preserves the draft and original request ID, exposes a retryable status, and keeps the reconciliation action available. An uncertain request blocks a new submission and may call only `reconcileRun(requestID)`; it must never call `submitRun` again.

The host—not the renderer—must enforce authentication, capability readiness, durable deduplication, filesystem permission, execution location, and provider invocation.
