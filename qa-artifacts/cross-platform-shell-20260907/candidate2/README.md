...
Missing, malformed, or mismatched acknowledgements normalize to `uncertain`. Accepted clears the composer only when its conversation, draft revision, and exact draft text are unchanged. Rejected and uncertain outcomes preserve the draft. An uncertain request retains its original ID, blocks a new submission, and may call only `reconcileRun(requestID)`; it must never call `submitRun` again.
