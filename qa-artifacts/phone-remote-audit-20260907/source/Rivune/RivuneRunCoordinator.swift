...
final class RivuneRunCoordinator: ObservableObject {
    ...
    func submit(
        ...
        let approvedContext = priorContext + "\n" + (RivuneStore.preparedAttachmentContext(turn.attachments) ?? "")
        let directPrompt = try (turn.mode == .council ? nil : turn.selectedArtifact)?.independentPrompt(userPrompt: turn.prompt, history: priorContext,
            documents: RivuneStore.preparedAttachmentContext(turn.attachments) ?? "[]")
        ...
        tasks[id] = Task { [weak self] in
            ...
            if turn.mode == .council {
                let request = CouncilRequest(runID: id, turnID: turn.id, prompt: turn.prompt,
                    approvedContext: approvedContext, criteria: CouncilRunner.defaultCriteria, participants: participants,
                ...
                do {
                    let response = try await runner.run(
                        ...
                        prompt: directPrompt ?? RivuneStore.independentPrompt(userPrompt: turn.prompt, priorContext: priorContext, attachments: turn.attachments),
