# Rivune menu-bar quick access and explicit-text Review — candidate 2

Status: isolated source candidate; shared app source untouched; not independently accepted, installed, or rendered.

Candidate 2 preserves candidate 1 and closes the central review's truthful idle-state finding: zero active tasks now says `Idle` / `No tasks running` with a neutral icon. The real `engineFooterSummary` remains visible underneath, so connection readiness is neither duplicated nor invented. A successful Review transfer also clears the panel's local input/error before dismissal to prevent accidental reuse if the menu reopens.
...
Materialized copy: `/private/tmp/rivune-menubar-c2.vpMGRd`

Focused command:

```sh
xcodebuild -project Rivune.xcodeproj -scheme 'Rivune Mac' -configuration Debug -destination 'platform=macOS' -derivedDataPath /private/tmp/RivuneMenuBarC2Derived -clonedSourcePackagesDirPath /private/tmp/rivune-0621-tests/SourcePackages -disableAutomaticPackageResolution CODE_SIGNING_ALLOWED=NO test -only-testing:RivuneTests/RivuneMenuBarCommandRouterTests -only-testing:RivuneTests/RivuneMenuBarTaskSnapshotTests -only-testing:RivuneTests/RivuneReviewDraftPolicyTests
```
