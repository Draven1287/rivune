...
import { createReviewQuickDraftController } from "./review-quick-draft.mjs";
...
function renderReviewState(state) {
  ...
  reviewText.disabled = busy || state.kind === "blocked";
...
async function ensureReviewController(expectedScopeID = null) {
  ...
  if (!host?.getRecoveryScopeID || !host?.saveRichDraft) throw new Error("This Rivune host cannot save Review text yet.");
  reviewControllerPromise ??= (async () => {
    ...
    reviewController = createReviewQuickDraftController({
      ...
      applyDurable: async (request, receipt) => {
