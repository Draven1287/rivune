import { createHostAdapter, createSubmissionGate, newestRunForConversation, parseAcknowledgement, parseSnapshot, parseRichDraftReceipt, validateAttachmentMetadata } from "./core.mjs";
import { createTranscript } from "./transcript.mjs";
import { initializeArchiveInspector } from "./archive-inspector.mjs";
import { applyDurableIfCurrent, createReviewQuickDraftController } from "./review-quick-draft.mjs";
...
const retryReview = document.querySelector("#retry-review");
...
let reviewControllerPromise = null;
...
function richState(conversationID) {
  if (!richDrafts.has(conversationID)) {
    ...
    richDrafts.set(conversationID, { revision: saved?.revision ?? 0, attachmentIDs: [...(saved?.attachmentIDs ?? [])], pending: null, conflictRevision: null });
  }
  return richDrafts.get(conversationID);
}

function renderReviewState(state) {
  const busy = ["settling", "saving"].includes(state.kind);
  const uncertain = state.kind === "uncertain";
  reviewDialog.dataset.pending = String(busy || uncertain);
  reviewText.disabled = busy || ["blocked", "blockedRecovery"].includes(state.kind);
  reviewText.readOnly = uncertain || ["durable", "rejected"].includes(state.kind);
  addReview.disabled = state.kind !== "editing" || !reviewText.value.trim();
  retryReview.hidden = !uncertain;
  retryReview.disabled = shutdownFrozen;
  reviewStatus.textContent = state.message ?? (busy ? "Saving to this conversation…" : uncertain ? "Save outcome is uncertain." : "");
  if (["editing", "uncertain", "rejected"].includes(state.kind) && reviewText.value !== state.explicitText) reviewText.value = state.explicitText;
  if (state.kind === "durable") {
    if (reviewDialog.open) reviewDialog.close();
    setStatus("Review text was added to the draft. Nothing was sent to an AI.", "ready", true);
    prompt.focus();
  }
}

async function ensureReviewController(expectedScopeID = null) {
  if (reviewController) return reviewController;
  if (!host?.getRecoveryScopeID || !host?.getWorkspaceIncarnationID || !host?.saveRichDraft) throw new Error("This host cannot bind Review recovery to a workspace incarnation yet.");
  reviewControllerPromise ??= (async () => {
    const recoveryScopeID = await host.getRecoveryScopeID();
    const workspaceIncarnationID = await host.getWorkspaceIncarnationID();
    if (expectedScopeID && recoveryScopeID !== expectedScopeID) throw new Error("Review belongs to a different local workspace.");
    reviewController = createReviewQuickDraftController({
      storage: recoveryStorage,
      recoveryScopeID,
      workspaceIncarnationID,
      settleCurrentDraft: async () => {
        await saveCurrentDraft();
        const write = selectedConversationID ? draftWrites.get(selectedConversationID) : null;
        if (write) await write;
      },
      getCurrentTarget: async () => {
        const conversation = snapshot?.conversations.find(item => item.id === selectedConversationID);
        if (!conversation) return null;
        const rich = richState(conversation.id);
        return {
          recoveryScopeID,
          workspaceIncarnationID,
          conversationID: conversation.id,
          localDraftVersion: localDraftVersions.get(conversation.id) ?? 0,
          revision: rich.revision,
          draft: selectedConversationID === conversation.id ? prompt.value : localDrafts.get(conversation.id) ?? conversation.draft,
          attachmentIDs: [...rich.attachmentIDs],
          readOnly: conversation.readOnly,
          imported: conversation.readOnly,
          pending: rich.pending,
          conflictRevision: rich.conflictRevision,
        };
      },
      saveRichDraft: request => host.saveRichDraft(request),
      parseReceipt: parseRichDraftReceipt,
      applyDurable: async (request, receipt, guard) => {
        return applyDurableIfCurrent({
          guard,
          readCurrent: () => {
            const rich = richState(request.conversationID);
            const conversation = snapshot?.conversations.find(item => item.id === request.conversationID);
            return conversation ? {
              conversationID: request.conversationID,
              localDraftVersion: localDraftVersions.get(request.conversationID) ?? 0,
              richRevision: rich.revision,
              draft: localDrafts.get(request.conversationID) ?? conversation.draft,
              attachmentIDs: [...rich.attachmentIDs],
              pendingMutationID: rich.pending?.mutationID ?? null,
            } : null;
          },
          applyCurrent: async () => {
            const rich = richState(request.conversationID);
            ...
            rich.pending = null;
            rich.conflictRevision = null;
            rich.lastSaved = { draft: request.draft, attachmentIDs: [...request.attachmentIDs] };
            localDrafts.set(request.conversationID, request.draft);
            dirtyDrafts.delete(request.conversationID);
            const conversation = snapshot?.conversations.find(item => item.id === request.conversationID);
            if (conversation) {
              conversation.draft = request.draft;
              conversation.richDraft = { schemaVersion: 1, revision: receipt.revision, attachmentIDs: [...request.attachmentIDs] };
            }
            if (selectedConversationID === request.conversationID) prompt.value = request.draft;
            render();
          },
          refreshAuthoritative: () => refresh(),
        });
      },
      isFrozen: () => shutdownFrozen || shutdownPreparing || Boolean(hostShutdownToken),
      onChange: renderReviewState,
    });
    return reviewController;
  })().catch(error => { reviewControllerPromise = null; throw error; });
  return reviewControllerPromise;
}

host?.onOpenReview?.(async event => {
  try {
    const controller = await ensureReviewController(event?.recoveryScopeID);
    const state = controller.open(event);
    if (state.kind === "blocked") return setStatus(state.message, "warning", true);
    if (!reviewDialog.open) reviewDialog.showModal();
    renderReviewState(state);
    if (state.kind === "editing") reviewText.focus();
  } catch (error) {
    setStatus(`Review is unavailable: ${error.message ?? String(error)}`, "warning", true);
  }
});
...
async function writeRich(conversationID, draft, attachmentIDs, token, clientRevision) {
  const rich = richState(conversationID);
  if (token && rich.shutdownSaved?.token === token && rich.shutdownSaved.draft === draft && JSON.stringify(rich.shutdownSaved.attachmentIDs) === JSON.stringify(attachmentIDs)) return rich.revision;
  ...
  async function resolvePending() {
    ...
    rich.lastSaved = { draft: rich.pending.draft, attachmentIDs: [...rich.pending.attachmentIDs] };
    rich.pending = null;
  }
  const reconciledPending = Boolean(rich.pending);
  if (rich.pending) await resolvePending();
  if ((!token || reconciledPending) && rich.lastSaved?.draft === draft && JSON.stringify(rich.lastSaved.attachmentIDs) === JSON.stringify(attachmentIDs)) {
    if (token) rich.shutdownSaved = { token, draft, attachmentIDs: [...attachmentIDs] };
    return rich.revision;
  }
  rich.pending = Object.freeze({ conversationID, mutationID: crypto.randomUUID(), expectedRevision: rich.revision, draft, attachmentIDs: Object.freeze([...attachmentIDs]) });
...
function saveDraft(conversationID, draft, revision = localDraftVersions.get(conversationID) ?? 0, token = null) {
  ...
  const next = previous.catch(() => {}).then(() => writeRich(conversationID, draft, attachmentIDs, token, revision)).then(result => {
    const dirty = dirtyDrafts.get(conversationID);
    if (dirty?.draft === draft && dirty?.revision === revision && JSON.stringify(richState(conversationID).attachmentIDs) === JSON.stringify(attachmentIDs)) dirtyDrafts.delete(conversationID);
    return result;
  });
...
host?.onShutdownRequested?.(async token => {
  ...
  try {
    ...
    if (!pendingDrafts.length) {
      ...
      const receipt = await host.flushShutdownDraft({ token, conversationID: null, draft: "", clientRevision: 0, expectedRichRevision: null, mutationID, attachmentIDs: [] });
      if (receipt?.state !== "durable" || receipt.token !== token || receipt.mutationID !== mutationID || receipt.conversationID !== null || receipt.richDraftRevision !== null || receipt.clientRevision !== 0 || JSON.stringify(receipt.attachmentIDs) !== "[]") throw new Error("Empty shutdown flush was not durably acknowledged.");
    }
...
async function clearSubmittedDraftIfUnchanged(submission) {
  if (!submission || !Number.isSafeInteger(submission.richDraftRevision) || !Array.isArray(submission.attachmentIDs)) return false;
  ...
  const matches = () => {
    ...
    return versionMatches && text === submission.draft && rich.revision === submission.richDraftRevision && !rich.pending && rich.conflictRevision === null && JSON.stringify(rich.attachmentIDs) === JSON.stringify(submission.attachmentIDs);
  };
  if (!matches()) return false;
  const previous = draftWrites.get(id) ?? Promise.resolve();
  const next = previous.catch(() => {}).then(async () => {
    if (shutdownFrozen || !matches()) return false;
    const request = Object.freeze({ conversationID: id, mutationID: crypto.randomUUID(), expectedRevision: submission.richDraftRevision, draft: "", attachmentIDs: Object.freeze([]) });
    let receipt;
    ...
    rich.revision = receipt.revision;
    rich.lastSaved = { draft: "", attachmentIDs: [] };
...
function render() {
  ...
  const providerIsConfigured = Boolean(snapshot?.selectedProviderID);
...
retryRun.addEventListener("click", async () => {
  ...
  try {
    ...
    if (acknowledgement.state === "accepted") {
      ...
      setStatus(`Retry ${requestID} has an uncertain outcome. Reconcile it before any new request.`, "warning", true);
    }
  } catch (error) {
    setStatus(`${error.message ?? String(error)} Retry ${requestID} was not silently resubmitted; reconcile it.`, "warning", true);
  } finally {
...
refresh();
ensureReviewController().then(controller => {
  const recovered = controller.restore();
  if (["uncertain", "blockedRecovery"].includes(recovered.kind)) {
    if (!reviewDialog.open) reviewDialog.showModal();
    renderReviewState(recovered);
    retryReview.focus();
  }
}).catch(() => { /* Review remains unavailable without affecting ordinary workspace startup. */ });
...
reviewText.addEventListener("input", () => {
  ...
  renderReviewState(reviewController.setText(reviewText.value));
});
addReview.addEventListener("click", async () => {
  if (!reviewController) return;
  renderReviewState(await reviewController.add());
});
retryReview.addEventListener("click", async () => {
  if (!reviewController) return;
  renderReviewState(await reviewController.retry());
});
document.querySelector("#close-review").addEventListener("click", () => {
  reviewController?.close();
  reviewDialog.close();
});
reviewDialog.addEventListener("cancel", () => { reviewController?.close(); });
