...
impl HostState {
    pub fn recovery_scope_id(&self) -> String {
        self.recovery_scope_id.clone()
    }
    ...
    pub fn open(profile: PathBuf) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let recovery_scope_id = load_or_create_recovery_scope_id(&profile)?;
        Ok(Self {
            profile,
            recovery_scope_id,
...
fn load_or_create_recovery_scope_id(profile: &Path) -> Result<String, Box<dyn std::error::Error>> {
    let path = profile.join("recovery-scope-v1");
    if path.exists() {
        let value = fs::read_to_string(&path)?;
        let value = value.trim();
        if value.len() != 44
            || !value.starts_with("profile-")
            || Uuid::parse_str(&value[8..]).is_err()
        {
            return Err("invalid Rivune recovery scope identifier".into());
        }
        return Ok(value.to_owned());
    }
    let value = format!("profile-{}", Uuid::new_v4());
...
#[tauri::command]
pub fn get_recovery_scope_id(state: State<'_, Arc<HostState>>) -> String {
    state.recovery_scope_id()
}
...
pub fn save_rich_draft_in(
        ...
        .find(|item| item.mutation_id == request.mutation_id)
    {
        ...
        return match host.save(&workspace) {
            ...
            Err(error) if error.committed => Ok(mutation_receipt(
                "uncertain",
                &request,
                Some(revision),
                Some(error.message),
...
pub fn save_draft(
    ...
    if !conversation.rich_draft.attachment_ids.is_empty() {
        return Err("This draft contains attachments and must be saved through the revision-checked rich draft bridge.".into());
    }
...
fn prepare_submission(
    ...
    if !conversation.rich_draft.attachment_ids.is_empty() {
            ...
            || request.prompt != conversation.draft
        {
            return Err(Acknowledgement::rejected(
                &request.id,
                "The saved attachment draft changed. Save it durably before sending.",
            ));
        }
...
mod tests {
    ...
    fn profile_and_snapshot_permissions_are_private() {
        ...
        assert_eq!(
            fs::metadata(path.join("recovery-scope-v1"))
                .unwrap()
                .permissions()
                .mode()
                & 0o777,
    ...
    #[test]
    fn recovery_scope_is_stable_across_restart_and_isolated_by_profile() {
        let first_path = profile("recovery-scope-first");
        let second_path = profile("recovery-scope-second");
        let first = HostState::open(first_path.clone()).unwrap();
        let first_scope = first.recovery_scope_id();
        drop(first);
        let reopened = HostState::open(first_path.clone()).unwrap();
        assert_eq!(reopened.recovery_scope_id(), first_scope);
        let second = HostState::open(second_path.clone()).unwrap();
        assert_ne!(second.recovery_scope_id(), first_scope);
        drop(reopened);
        drop(second);
