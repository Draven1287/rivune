...
const base = 'http://127.0.0.1:4301/rivune/';
...
(async () => {
  ...
  try {
    ...
    assert(howText.includes('Single-AI chat is being built first; team workflows will follow.'), 'plain development sequence');
    report.modes = { labels, keyboard: true, plainStatusCopy: true };
    ...
    fs.writeFileSync(path.join(__dirname, 'browser-qa.log'), 'PASS 9 routes at 1280/768/390/320; contact, keyboard modes, platform states, plain development copy, overflow and browser errors\n');
  } finally {
    await browser.close();
  }
