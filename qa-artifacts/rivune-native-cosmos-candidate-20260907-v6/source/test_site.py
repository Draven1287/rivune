...
class StaticOutputTests(unittest.TestCase):
    ...
    def test_current_page_has_no_dmg_or_fake_download(self):
        ...
        self.assertIn('macOS download not yet available', download.split('<script>', 1)[0])
        ...
        self.assertIn('Downloads are not yet available for any platform.', app)
    ...
    def test_installer_readiness_does_not_enable_pending_modes(self):
        for output in (self.output, self.fixture):
            with self.subTest(installer_state=output.name):
                ...
                self.assertIn('Being built first', text)
                self.assertIn('Team workflow · in development', text)
                self.assertIn('Team workflow · planned', text)
                self.assertIn('Single-AI chat is being built first; team workflows will follow.', text)
    ...
    def test_sitemap_lists_only_intended_public_canonical_pages(self):
        ...
        expected = {
            ...
            'https://draven1287.github.io/rivune/contact/',
    ...
    def test_routes_have_unique_metadata_and_consistent_active_navigation(self):
        routes = ('', 'app/', 'how-it-works/', 'faq/', 'about/', 'contact/', 'download/', 'privacy/', 'council-vs-swarm/')
        ...
        for route in routes:
            ...
            self.assertEqual(text.count('aria-current="page"'), 1 if route == '' else (3 if route in ('privacy/', 'contact/') else 2))
        ...
        self.assertIn('/rivune/contact/', Document((self.output / 'about/index.html').read_text()).urls)
    ...
    def test_product_preview_is_dom_built_and_accessible(self):
        app = (self.output / 'app/index.html').read_text()
        home = (self.output / 'index.html').read_text()
        self.assertIn('cosmos-demo', home)
        self.assertNotIn('workspace-preview', home)
        self.assertIn('workspace-preview', app)
        self.assertIn('Illustrative interface preview — prewritten; no AI request is sent.', app)
    ...
    def test_demo_and_platform_choices_are_honest_and_interactive(self):
        home = (self.output / 'index.html').read_text()
        download = (self.output / 'download/index.html').read_text()
        self.assertIn('Illustrative example. No AI request is sent.', home)
        for value in ('Prewritten Council example', 'data-demo-stage="perspectives"', 'data-demo-stage="review"', 'data-demo-stage="final"'):
            self.assertIn(value, home)
        self.assertNotIn('data-demo-example=', home)
        self.assertNotIn('data-demo-reset', home)
        for value in ('data-platform="mac"', 'data-platform="windows"', 'data-platform="linux"', 'Windows download not yet available', 'Linux download not yet available'):
            self.assertIn(value, download)
        self.assertIn('Built in Denver, Colorado.', home)
        self.assertIn('/rivune/contact/', home)

    def test_contact_offers_manual_gmail_and_email_app_paths(self):
        contact = (self.output / 'contact/index.html').read_text()
        self.assertIn('rivune.crave757@slmails.com', contact)
        self.assertIn('data-copy-email="rivune.crave757@slmails.com"', contact)
        self.assertIn('data-copy-status aria-live="polite"', contact)
        self.assertIn('https://mail.google.com/mail/?view=cm&amp;fs=1&amp;to=rivune.crave757%40slmails.com', contact)
        self.assertIn('mailto:rivune.crave757@slmails.com', contact)
        self.assertIn('Copy failed. Select the email address above and copy it manually.', contact)

    def test_public_copy_is_simple_while_developer_disclosures_preserve_architecture_truth(self):
        pages = {route: (self.output / route / 'index.html').read_text()
                 for route in ('app', 'how-it-works', 'faq', 'download', 'privacy', 'council-vs-swarm')}
        home = (self.output / 'index.html').read_text()
        self.assertIn('open-source desktop app in development for macOS, Windows, and Linux', home)
        self.assertIn('Single-AI chat is being built first; team workflows will follow.', pages['how-it-works'])
        self.assertIn('Downloads are not yet available.', pages['council-vs-swarm'])
        self.assertIn('legacy SwiftUI Mac code', pages['download'])
        self.assertIn('forthcoming Tauri app', pages['download'])
        self.assertIn('Tauri storage and routing must be verified before release', pages['privacy'])
        self.assertIn('Storage and routing for the new desktop app are still being developed.', pages['council-vs-swarm'])
        readme = (ROOT / 'README.md').read_text()
        self.assertIn('one Tauri desktop app is in development for macOS, Windows, and Linux', readme)
        self.assertIn('legacy SwiftUI Mac source', readme)
        public_product_copy = home + pages['app'] + pages['how-it-works'] + pages['faq'] + pages['council-vs-swarm']
        for internal in ('real provider host pipeline', 'ports remain pending', 'Tauri runtime',
                         'No accepted macOS', 'currently installed Mac app'):
            self.assertNotIn(internal, public_product_copy)
        combined = home + ''.join(pages.values())
        for stale in ('Development preview · Native Mac app', 'native Mac workspace',
                      'current native preview is for Macs', 'Windows version is planned',
                      'Council is the first development stage'):
