...
const base = 'http://127.0.0.1:4296/rivune/';
...
(async () => {
  ...
  try {
    ...
    await page.setViewportSize({ width: 1280, height: 1000 });
    await page.goto(base);
    assert(await page.locator('[data-demo-stage]').count() === 3, 'demo should expose exactly three stage controls');
    assert(await page.locator('[data-demo-example],[data-demo-reset]').count() === 0, 'removed demo controls remain rendered');
    await page.locator('[data-demo-stage="review"]').focus();
    await page.keyboard.press('Enter');
    assert((await page.locator('[data-demo-label]').textContent()).trim() === 'Review', 'review stage did not render');
    assert(await page.locator('[data-demo-stage="review"]').getAttribute('aria-pressed') === 'true', 'review stage was not selected');
    assert(await page.locator('[data-demo-stage="review"]').evaluate(el => el === document.activeElement), 'demo control did not retain focus');
    await page.locator('[data-demo-stage="final"]').click();
    assert((await page.locator('[data-demo-label]').textContent()).trim() === 'Final draft', 'final stage did not render');
    report.demo = { threeStageControls: true, controlRetainsFocus: true, finalStage: true, disclosure: await page.locator('.cosmos-demo-foot p').textContent() };
    ...
    report.productPreview.home = { centeredHero: true, singleInteractiveDemo: true, workspacePreviewCount: 0, screenshotReferences: 0, backgroundImage };
