...
@Test func independentReceiptColonCollision() async throws {
    let base = makePlan()
    let special = SwarmWorkerIdentity(memberID: workerA.memberID, providerID: "openai:codex", adapterID: "cli", requestedModelID: "fixture-a", requestedEffort: "high")
    let plan = SwarmExecutionPlan(runID: base.runID, leadMemberID: base.leadMemberID, workers: [special, workerB], tasks: base.tasks, checks: base.checks)
    ...
    let tampered = try JSONDecoder().decode(SwarmIntegrationReceipt.self, from: JSONSerialization.data(withJSONObject: object))
    #expect(tampered.workers != receipt.workers)
    #expect(tampered.receiptSHA256 == receipt.receiptSHA256)
    #expect(tampered.verifyIntegrity()) // BUG: changed route still passes unchanged digest
    print("INDEPENDENT colon_collision: altered provider/adapter accepted by verifyIntegrity")
}

@Test func independentReceiptNilDefaultCollision() async throws {
    ...
    let tampered = try JSONDecoder().decode(SwarmIntegrationReceipt.self, from: JSONSerialization.data(withJSONObject: object))
    #expect(tampered.workers != receipt.workers)
    #expect(tampered.verifyIntegrity()) // BUG: nil and literal string collapse
    print("INDEPENDENT nil_default_collision: altered effort accepted by verifyIntegrity")
}

@Test func independentTargetAncestorConflict() async throws {
    let verifier = RecordingVerifier()
    let session = try SwarmExecutionSession(plan: makePlan(), worker: RecordingWorker(), verifier: verifier)
    let receipt = try await session.run(approvedContext: "", targetSnapshot: ["STYLES": Data("an existing file blocks styles/site.css".utf8)])
    #expect(receipt.state == .complete) // BUG: file ancestor makes proposed layout impossible
    #expect(await verifier.calls == 1)
    print("INDEPENDENT target_ancestor: complete with existing STYLES file blocking styles/site.css")
}

@Test func independentParentTaskCancellationDuringVerifier() async throws {
    let verifier = RecordingVerifier(delay: .seconds(5))
    let session = try SwarmExecutionSession(plan: makePlan(), worker: RecordingWorker(), verifier: verifier)
    let run = Task { try await session.run(approvedContext: "", targetSnapshot: [:]) }
    for _ in 0..<200 {
        if await verifier.calls == 1 { break }
    ...
    #expect(await verifier.cancellations == 0) // BUG: cancellation handler no longer covers verifier
    await session.cancel() // clean up promptly, do not leave verifier running
    let receipt = try await run.value
    #expect(receipt.state == .cancelled)
    #expect(await verifier.cancellations == 1)
    print("INDEPENDENT parent_cancellation: verifier survived run.cancel; explicit session.cancel stopped it")
}
