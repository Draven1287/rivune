...
@Test func independentPackageDescendantIsOverwrittenWithoutBase() async throws {
    ...
    await #expect(throws: SwarmFilesystemError.baseConflict) { try await session.stage() }
    #expect(try fixture.read("Existing.app/Contents/valuable.txt") == "existing-user-data")
    print("FIXED package: nil-base request rejected, existing file preserved")
}
...
@Test func independentRollbackOverwritesLaterUserEdit() async throws {
    ...
    await #expect(throws: SwarmFilesystemError.recoveryRequired) { try await session.apply() }
    #expect(try fixture.read("a.txt") == "new-user-edit")
    #expect(await session.currentReceipt()?.journalState == .recoveryRequired)
    print("FIXED rollback: new-user-edit preserved, recoveryRequired")
}
...
@Test func independentRollbackDeletesReplacementDirectory() async throws {
    ...
    _ = try await session.stage()
    await #expect(throws: SwarmFilesystemError.recoveryRequired) { try await session.apply() }
    #expect(try fixture.read("a.txt/keep.txt") == "user-content")
    #expect(await session.currentReceipt()?.journalState == .recoveryRequired)
    print("FIXED rollback: preserved replacement directory and keep.txt")
}


private struct SwapProjectRootForLink: SwarmFilesystemFaultInjecting {
    let project: URL
    let originalMoved: URL
    let external: URL
    func check(_ step: SwarmFilesystemApplyStep) throws {
        if step == .afterFile(index: 0, path: "a.txt") {
            try FileManager.default.moveItem(at: project, to: originalMoved)
            try FileManager.default.createSymbolicLink(at: project, withDestinationURL: external)
            throw InjectedFailure()
        }
    }
}
@Test func independentRollbackFollowsReplacedProjectRoot() async throws {
    let fixture = try Fixture(); defer { fixture.cleanup() }
    try fixture.write("a.txt", "original")
    ...
    try Data("proposal".utf8).write(to: external.appendingPathComponent("a.txt"))
    let moved = fixture.root.appendingPathComponent("moved-project")
    let session = try SwarmFilesystemApplySession(runID: UUID(), projectRoot: fixture.project, stagingParent: fixture.staging,
        files: [.init(path: "a.txt", expectedBaseSHA256: SwarmFilesystemApplySession.sha256(Data("original".utf8)), proposedBytes: Data("proposal".utf8))],
        faultInjector: SwapProjectRootForLink(project: fixture.project, originalMoved: moved, external: external))
    _ = try await session.stage()
    await #expect(throws: SwarmFilesystemError.applyFailed) { try await session.apply() }
    #expect(String(decoding: try Data(contentsOf: external.appendingPathComponent("a.txt")), as: UTF8.self) == "original")
    #expect(String(decoding: try Data(contentsOf: moved.appendingPathComponent("a.txt")), as: UTF8.self) == "proposal")
    #expect(await session.currentReceipt()?.journalState == .rolledBack)
    print("REPRO root replacement: rollback followed root symlink, overwrote external file and left actual proposal applied")
}
