...
private struct ReplaceProjectRootWithSymlink: SwarmFilesystemFaultInjecting {
    let project: URL
    let movedProject: URL
    let external: URL

    func check(_ step: SwarmFilesystemApplyStep) throws {
        if step == .afterFile(index: 0, path: "a.txt") {
            try FileManager.default.moveItem(at: project, to: movedProject)
...
@Test func rollbackRejectsReplacedProjectRootWithoutFollowingSymlink() async throws {
    let fixture = try Fixture(); defer { fixture.cleanup() }
    try fixture.write("a.txt", "original")
    let movedProject = fixture.root.appendingPathComponent("moved-project")
    ...
    let session = try SwarmFilesystemApplySession(
        ...
        files: [.init(path: "a.txt", expectedBaseSHA256: SwarmFilesystemApplySession.sha256(Data("original".utf8)),
                      proposedBytes: Data("proposal".utf8))],
        faultInjector: ReplaceProjectRootWithSymlink(project: fixture.project, movedProject: movedProject,
                                                     external: external))
    let staged = try await session.stage()
    await #expect(throws: SwarmFilesystemError.recoveryRequired) { try await session.apply() }
    #expect(String(decoding: try Data(contentsOf: external.appendingPathComponent("a.txt")), as: UTF8.self) == "proposal")
    #expect(String(decoding: try Data(contentsOf: movedProject.appendingPathComponent("a.txt")), as: UTF8.self) == "proposal")
    let receipt = await session.currentReceipt()
    #expect(receipt?.journalState == .recoveryRequired)
    #expect(receipt?.recoveryConflicts.first?.path == ".")
    #expect(receipt?.recoveryConflicts.first?.observedKind == "unsafe-root")
    #expect(FileManager.default.fileExists(atPath: staged.stagingDirectory + "/backups/a.txt"))
}
