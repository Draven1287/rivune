...
public actor SwarmFilesystemApplySession {
    ...
    private let projectBoundary: [FilesystemBoundaryComponent]
    ...
    public init(
        ...
        let canonicalProject = try Self.canonicalURL(standardizedProject)
        let canonicalStaging = try Self.canonicalURL(standardizedStaging)
        self.projectRoot = canonicalProject
        self.stagingParent = canonicalStaging
        self.projectBoundary = try Self.captureBoundary(for: canonicalProject)
        self.files = files
    ...
    private func validateProjectBoundary() throws {
        for expected in projectBoundary {
            let attributes = try fileManager.attributesOfItem(atPath: expected.path)
            guard attributes[.type] as? FileAttributeType == .typeDirectory,
                  Self.uint64Attribute(attributes[.systemNumber]) == expected.device,
                  Self.uint64Attribute(attributes[.systemFileNumber]) == expected.inode else {
                throw SwarmFilesystemError.unsafeRoot
            }
        }
        guard try Self.canonicalURL(projectRoot).path == projectRoot.path else {
            throw SwarmFilesystemError.unsafeRoot
        }
    }
    ...
    private func rollback(applied: [String], createdDirectories: [String]) -> [SwarmFilesystemRecoveryConflict] {
        ...
        do {
            ...
            return [.init(path: ".", reason: "admitted project root or ancestor identity changed",
                          observedKind: "unsafe-root", observedSHA256: nil)]
        }
    ...
    private static func captureBoundary(for projectRoot: URL) throws -> [FilesystemBoundaryComponent] {
        var urls: [URL] = []
        var cursor = projectRoot
        while true {
            urls.append(cursor)
        ...
        return try urls.reversed().map { url in
            let attributes = try FileManager.default.attributesOfItem(atPath: url.path)
            guard attributes[.type] as? FileAttributeType == .typeDirectory,
                  let device = uint64Attribute(attributes[.systemNumber]),
                  let inode = uint64Attribute(attributes[.systemFileNumber]) else {
                throw SwarmFilesystemError.unsafeRoot
            }
            return .init(path: url.path, device: device, inode: inode)
        }
    }
...
private struct FilesystemBoundaryComponent: Sendable {
    let path: String
    let device: UInt64
    let inode: UInt64
}
