...
public struct SwarmIntegrationReceipt: Codable, Hashable, Sendable {
    ...
    public func verifyIntegrity() -> Bool {
        ...
        return receiptSHA256 == canonicalReceiptDigest(
...
public actor SwarmExecutionSession {
    ...
    public func retryFailed(taskID: UUID, repairBrief: String) async throws -> SwarmIntegrationReceipt {
        ...
        return await withTaskCancellationHandler {
            await schedule(repair: [taskID: repairBrief])
        } onCancel: {
            Task { await self.cancel() }
        }
    }
    ...
    private func integrateIfPossible() async -> SwarmIntegrationReceipt {
        ...
        for task in plan.tasks {
            ...
            for file in output.files {
                ...
                let key = SwarmExecutionPlan.ownershipKey(file.path)
                let impossiblePrefix = targetByOwnershipKey.keys.contains { targetKey in
                    targetKey != key && (targetKey.hasPrefix(key + "/") || key.hasPrefix(targetKey + "/"))
                }
                let actual = targetByOwnershipKey[key].map(sha256)
                guard !impossiblePrefix, actual == expected, staged[file.path] == nil else {
    ...
    private func makeReceipt(state: SwarmRunState, integrated: [SwarmFileManifest],
        ...
        let receiptDigest = canonicalReceiptDigest(runID: plan.runID, planSHA256: planDigest,
            lead: plan.leadMemberID, workers: plan.workers, tasks: taskReceipts,
            integrated: integrated, checks: checks, state: state)
        return .init(schemaVersion: 2, runID: plan.runID, planSHA256: planDigest,
...
private struct ReceiptDigestPayload: Encodable {
    let schemaVersion: Int
    let runID: UUID
    let planSHA256: String
    let leadMemberID: String
    let workers: [SwarmWorkerIdentity]
    let tasks: [SwarmTaskReceipt]
    let integratedFiles: [SwarmFileManifest]
    let checks: [SwarmCheckReceipt]
    let state: SwarmRunState
}

private func canonicalReceiptDigest(runID: UUID, planSHA256: String, lead: String,
    ...
    let payload = ReceiptDigestPayload(
        ...
        leadMemberID: lead,
        workers: workers.sorted { $0.memberID < $1.memberID },
        tasks: tasks.sorted { $0.taskID.uuidString < $1.taskID.uuidString }.map { task in
            .init(taskID: task.taskID, parentLeaderID: task.parentLeaderID,
                  workerMemberID: task.workerMemberID, attempts: task.attempts.map { attempt in
                    .init(attemptID: attempt.attemptID, attempt: attempt.attempt, state: attempt.state,
                          outputManifest: attempt.outputManifest.sorted { $0.path < $1.path }, failure: attempt.failure)
                  }, state: task.state)
        },
        integratedFiles: integrated.sorted { $0.path < $1.path },
        checks: checks.sorted { $0.checkID < $1.checkID },
        state: state
    )
    return encodedSHA256(payload)
}
