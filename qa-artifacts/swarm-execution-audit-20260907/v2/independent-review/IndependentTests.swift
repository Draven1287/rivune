...
@Test func independentReceiptColonCollision() async throws {
    ...
    #expect(tampered.workers != receipt.workers)
    #expect(tampered.receiptSHA256 == receipt.receiptSHA256)
    #expect(!tampered.verifyIntegrity()) // Fixed: altered route fails unchanged digest
    print("INDEPENDENT colon_collision: altered provider/adapter rejected by verifyIntegrity")
}
...
@Test func independentReceiptNilDefaultCollision() async throws {
    ...
    let tampered = try JSONDecoder().decode(SwarmIntegrationReceipt.self, from: JSONSerialization.data(withJSONObject: object))
    #expect(tampered.workers != receipt.workers)
    #expect(!tampered.verifyIntegrity()) // Fixed: nil and literal default remain distinct
    print("INDEPENDENT nil_default_collision: altered effort rejected by verifyIntegrity")
}
...
@Test func independentTargetAncestorConflict() async throws {
    ...
    #expect(receipt.state == .conflict) // Fixed: existing ancestor blocks proposal
    #expect(await verifier.calls == 0)
    print("INDEPENDENT target_ancestor: conflict before verifier with existing STYLES file blocking styles/site.css")
}
...
@Test func independentParentTaskCancellationDuringVerifier() async throws {
    ...
    #expect(await verifier.cancellations == 1) // Fixed: parent cancellation covers verifier
    await session.cancel() // clean up promptly, do not leave verifier running
    let receipt = try await run.value
    #expect(receipt.state == .cancelled)
    #expect(await verifier.cancellations == 1)
    print("INDEPENDENT parent_cancellation: run.cancel propagated to verifier")
}
