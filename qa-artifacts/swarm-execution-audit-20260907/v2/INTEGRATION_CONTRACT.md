# Narrow native integration contract — candidate v2
...
- Before any eventual save, require receipt schema v2, `receipt.state == .complete`, `receipt.verifyIntegrity() == true`, the exact plan hash, and a second atomic comparison of every target base hash. A changed, missing, case/Unicode-equivalent, ancestor, or descendant target is a conflict, not a last-writer-wins merge.
