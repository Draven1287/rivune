...
- The durable-shaped receipt stores hashes and counts rather than approved context, proposed bytes, or check logs. Its digest binds the plan hash, lead, full worker identities, attempts, file manifests, checks, and terminal state; `verifyIntegrity()` detects alteration.
