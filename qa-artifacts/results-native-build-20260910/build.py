...
root=Path.cwd();candidate=root/'tools/symphony/source-import-candidate';out=root/'qa-artifacts/results-native-build-20260910';frontend=candidate/'prototypes/ai-native-workspace';native=candidate/'qa-artifacts/cross-platform-shell-20260907/candidate4-runtime-r2/src-tauri'
expected='5fa49168a9284efeec713b7222e0e5c74be16c8a';manifest=json.loads((root/'tools/symphony/source-stage/SOURCE_MANIFEST.json').read_text());sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
assert not (out/'build-receipt.json').exists();before=identity();save('source-before.json',before);save('source-manifest.json',manifest);(out/'processes-before.txt').write_text('Process listing unavailable in sandbox; no process-control commands issued.\n');old=preserved();save('preserved-before.json',old)
config={'productName':'Rivune Results QA','identifier':'com.rivune.desktop.qa.results20260910','build':{'frontendDist':str(out/'frontend-dist')},'app':{'windows':[{'label':'main','title':'Rivune Results QA · build review only','width':1200,'height':900,'minWidth':760,'minHeight':560,'resizable':True}]}}
...
try:
 ...
 app=out/'Rivune Results QA.app';binary=app/'Contents/MacOS/rivune';binary.parent.mkdir(parents=True);(app/'Contents/Resources').mkdir();shutil.copy2(root/'.toolchains/target-candidate4-r2/debug/rivune',binary);shutil.copy2(native/'icons/icon.icns',app/'Contents/Resources/icon.icns')
 info={'CFBundleDisplayName':'Rivune Results QA','CFBundleName':'Rivune Results QA','CFBundleExecutable':'rivune','CFBundleIdentifier':config['identifier'],'CFBundlePackageType':'APPL','CFBundleShortVersionString':'0.0.1','CFBundleVersion':'2026091004','CFBundleIconFile':'icon.icns','NSHighResolutionCapable':True,'LSMinimumSystemVersion':'11.0','LSEnvironment':{'RIVUNE_ISOLATED_PROFILE_DIR':str(out/'profile-reserved-not-launched'),'PATH':'/usr/bin:/bin'}}
 ...
 after=identity();save('source-after.json',after);receipt['sourceUnchanged']=before==after;now=preserved();save('preserved-after.json',now);receipt['priorBinariesProfilesUnchanged']=old==now;(out/'processes-after.txt').write_text('Process listing unavailable in sandbox; no process-control commands issued.\n');receipt['completedAtUTC']=datetime.datetime.now(datetime.timezone.utc).isoformat();save('build-receipt.json',receipt)
