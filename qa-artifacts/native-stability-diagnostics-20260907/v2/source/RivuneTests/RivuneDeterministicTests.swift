...
final class RivuneLifecycleRecorderTests: XCTestCase {
    ...
    func testOrdinaryTestHostDoesNotEnableInstalledLifecyclePolicy() {
        let delegate = RivuneTerminationDelegate()
    ...
    func testRealDelegateReopensHiddenWorkspaceWindowAndRecordsRequest() {
        ...
        var recoveryCalls = 0
        let delegate = RivuneTerminationDelegate(lifecycle: recorder, allowsIsolatedCallbacks: true) { _ in
            recoveryCalls += 1
            return true
        }
        ...
        XCTAssertEqual(recorder.events().last?.detail, "restore")
