...
#if os(macOS)
struct RivuneLifecycleEvent: Codable, Equatable {
    enum Kind: String, Codable {
        case launch, previousSessionUnclosed, becameActive, resignedActive
        case windowVisible, windowHidden, windowClosed, reopenRequested
        case terminationRequested, terminationCancelled, terminationApproved, orderlyTermination
    }
    let time: Date
    let build: String
    let processID: Int32
...
private struct RivuneLifecycleMarker: Codable {
    let build: String
    let processID: Int32
    let sessionID: UUID
}

enum RivuneWindowRecovery {
    static func recoverableWorkspaceWindow(in windows: [NSWindow], displayName: String = RivuneBrand.displayName) -> NSWindow? {
        windows.first { !$0.isVisible && $0.canBecomeMain && $0.level == .normal && $0.title == displayName }
    }
}

/// Keeps a bounded, local, content-free record. A dead session marker means
/// AppKit's orderly-termination callback was not reached; it is not proof of a
/// crash because force quit, power loss, and test interruption look the same.
@MainActor
final class RivuneLifecycleRecorder {
    static let shared = RivuneLifecycleRecorder()
    ...
    init(directory: URL? = nil, now: @escaping () -> Date = Date.init,
         ...
         limit: Int = 160, isProcessAlive: @escaping (Int32) -> Bool = { NSRunningApplication(processIdentifier: $0) != nil }) {
        ...
        logURL = root.appendingPathComponent("lifecycle-v1.json")
        self.now = now; self.processID = processID; self.build = build
        self.limit = max(16, limit); self.isProcessAlive = isProcessAlive
    }

    private var markerURL: URL { root.appendingPathComponent("active-session-v1-\(processID).json") }

    func beginSession() {
        let manager = FileManager.default
        try? manager.createDirectory(at: root, withIntermediateDirectories: true)
        let names = (try? manager.contentsOfDirectory(atPath: root.path)) ?? []
        for name in names where name.hasPrefix("active-session-v1-") && name.hasSuffix(".json") {
            let url = root.appendingPathComponent(name)
            guard let data = try? Data(contentsOf: url),
                  let marker = try? JSONDecoder().decode(RivuneLifecycleMarker.self, from: data) else { continue }
            if marker.processID == processID || !isProcessAlive(marker.processID) {
                append(.previousSessionUnclosed, detail: nil, build: marker.build,
                       processID: marker.processID, sessionID: marker.sessionID)
                try? manager.removeItem(at: url)
            }
        }
        sessionID = UUID()
        let marker = RivuneLifecycleMarker(build: build, processID: processID, sessionID: sessionID)
        if let data = try? JSONEncoder().encode(marker) { try? data.write(to: markerURL, options: .atomic) }
        append(.launch, detail: nil)
    }

    func record(_ kind: RivuneLifecycleEvent.Kind, detail: String? = nil) { append(kind, detail: detail) }
    func finishSession() { append(.orderlyTermination, detail: nil); try? FileManager.default.removeItem(at: markerURL) }
    func events() -> [RivuneLifecycleEvent] {
        guard let data = try? Data(contentsOf: logURL),
              let values = try? JSONDecoder().decode([RivuneLifecycleEvent].self, from: data) else { return [] }
        return values
    }
    var hasActiveMarker: Bool { FileManager.default.fileExists(atPath: markerURL.path) }

    private func append(_ kind: RivuneLifecycleEvent.Kind, detail: String?, build: String? = nil,
                        processID: Int32? = nil, sessionID: UUID? = nil) {
        var values = events()
        values.append(.init(time: now(), build: build ?? self.build, processID: processID ?? self.processID,
                            sessionID: sessionID ?? self.sessionID, kind: kind, detail: detail))
        if values.count > limit { values.removeFirst(values.count - limit) }
        if let data = try? JSONEncoder().encode(values) { try? data.write(to: logURL, options: .atomic) }
    }
}
#endif
...
final class RivuneTerminationDelegate: NSObject, NSApplicationDelegate {
    ...
    private let lifecycle: RivuneLifecycleRecorder
    private let allowsIsolatedCallbacks: Bool
    private let recoverWorkspaceWindow: ([NSWindow]) -> Bool
    private var windowObservers: [NSObjectProtocol] = []

    override init() {
        lifecycle = .shared
        allowsIsolatedCallbacks = false
        recoverWorkspaceWindow = { windows in
            guard let window = RivuneWindowRecovery.recoverableWorkspaceWindow(in: windows) else { return false }
    ...
    init(lifecycle: RivuneLifecycleRecorder, allowsIsolatedCallbacks: Bool,
         recoverWorkspaceWindow: @escaping ([NSWindow]) -> Bool = { _ in false }) {
        self.lifecycle = lifecycle
        self.allowsIsolatedCallbacks = allowsIsolatedCallbacks
        self.recoverWorkspaceWindow = recoverWorkspaceWindow
        super.init()
    }

    private var callbacksEnabled: Bool {
        allowsIsolatedCallbacks || (!RivuneLaunchContext.isIsolated && Bundle.main.bundleURL.path.hasPrefix("/Applications/"))
    }
    ...
    func applicationDidBecomeActive(_ notification: Notification) {
        guard callbacksEnabled else { return }
        lifecycle.record(.becameActive)
    }

    func applicationDidResignActive(_ notification: Notification) {
        guard callbacksEnabled else { return }
        lifecycle.record(.resignedActive)
    }

    func applicationShouldTerminateAfterLastWindowClosed(_ sender: NSApplication) -> Bool { !callbacksEnabled }

    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool {
        guard callbacksEnabled else { return true }
        let restored = !flag && recoverWorkspaceWindow(sender.windows)
        lifecycle.record(.reopenRequested, detail: flag ? "visible" : (restored ? "restore" : "create"))
        return true
    }

    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        guard callbacksEnabled else { return .terminateNow }
        lifecycle.record(.terminationRequested)
        ...
        guard admission == .ready else {
            ...
            alert.runModal()
            lifecycle.record(.terminationCancelled, detail: String(describing: admission))
            return .terminateCancel
        }
        lifecycle.record(.terminationApproved)
        return .terminateNow
    }
    ...
    private func installWindowObservers() {
        ...
        let visibility = center.addObserver(forName: NSWindow.didChangeOcclusionStateNotification, object: nil, queue: .main) { [weak self] note in
            guard let self, let window = note.object as? NSWindow,
                  window.level == .normal, window.canBecomeMain,
                  window.title == RivuneBrand.displayName else { return }
            let kind: RivuneLifecycleEvent.Kind = window.occlusionState.contains(.visible) ? .windowVisible : .windowHidden
            Task { @MainActor in self.lifecycle.record(kind) }
        }
        let closing = center.addObserver(forName: NSWindow.willCloseNotification, object: nil, queue: .main) { [weak self] note in
            guard let self, let window = note.object as? NSWindow,
                  window.level == .normal, window.canBecomeMain,
                  window.title == RivuneBrand.displayName else { return }
            Task { @MainActor in self.lifecycle.record(.windowClosed) }
        }
        windowObservers = [visibility, closing]
    }
}
