...
export function useHostResults(snapshot:HostSnapshot|null, disconnected:boolean, bridge:unknown){
 ...
 function close(restore=true){
  ...
  if(restore)requestAnimationFrame(()=>{
   ...
   if(target?.isConnected&&target.getClientRects().length&&getComputedStyle(target).visibility==='visible'&&!target.closest('[inert]'))target.focus();else heading.current?.focus();
  });
 }
