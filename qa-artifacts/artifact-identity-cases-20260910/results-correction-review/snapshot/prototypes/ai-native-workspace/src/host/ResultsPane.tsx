...
export function ResultsPane({records,state,runLabels,onSelect,onRetry,onBack,onClose,notice}:ResultsPaneProps){
 ...
 const pane=useRef<HTMLElement>(null),listScroll=useRef(0);
 ...
 useLayoutEffect(()=>{
  ...
  if(key==='list'&&backTarget.current){(rows.current.get(backTarget.current)??heading.current)?.focus({preventScroll:true});if(pane.current)pane.current.scrollTop=listScroll.current;backTarget.current=null;}
 ...
 return <aside ref={pane} id="host-results" className="host-results" aria-labelledby={title} onKeyDown={e=>{
  ...
  {selected?<>
   ...
   <ul>{[...records].sort((a,b)=>b.createdAt.localeCompare(a.createdAt)||a.artifactID.localeCompare(b.artifactID)).map(a=><li key={a.artifactID}>
    <button type="button" ref={node=>{if(node)rows.current.set(a.artifactID,node);else rows.current.delete(a.artifactID);}} aria-label={`${a.displayName}, ${description(a)}, ${a.artifactID}`} onClick={()=>{listScroll.current=pane.current?.scrollTop??0;onSelect(a);}}><strong>{a.displayName}</strong><span>{description(a)}</span></button>
   </li>)}</ul>
  </>}
 </aside>;
}
