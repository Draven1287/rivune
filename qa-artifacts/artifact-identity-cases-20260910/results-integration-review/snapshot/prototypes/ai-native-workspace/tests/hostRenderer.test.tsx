...
teamFormTests.push(['results-flow','Results actual adapter/list/text/focus and preserved chat',async f=>{
 ...
 await openResults();assert(!f.calls.some(c=>c.name==='inspectArtifact'),'List mounting inspected content');assert(document.activeElement===container.querySelector('.host-results h2'),'Results heading not focused');if(innerWidth<=1000){assert(chat.inert&&getComputedStyle(chat).visibility==='hidden','Narrow chat not mounted inert');}
...
teamFormTests.push(['results-navigation','Results failed navigation retains selection, successful navigation closes',async f=>{
 ...
 // Desktop sidebar remains operable. At narrow widths exercise the host's navigation
 // result through a synthetic snapshot update; selecting Conversations intentionally closes Results.
 if(innerWidth>1000){await click('Other saved chat');assert(resultText()?.value===texts[0],'Rejected navigation cleared Results');reject=false;await click('Other saved chat');}
 else {f.snapshot.activeConversationID='other-chat';await act(async()=>f.emit({}));}
