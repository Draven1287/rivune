...
enum SettingsSection: String, CaseIterable, Identifiable, Hashable {
    ...
    var searchTerms: String {
        ...
        case .appearance: "theme dark black glass motion animation startup arrival replay accessibility graphite orbit cosmos background"
    ...
    var nativeDestination: NativeSettingsDestination {
        switch self {
...
struct SettingsView: View {
    ...
    @AppStorage(RivuneAppearancePolicy.presetKey) private var backgroundPreset = ""
    @AppStorage("rivune.appearance.presetDim") private var presetBackgroundDim: Double?
    @AppStorage("rivune.appearance.presetStars") private var presetBackgroundStars: Bool?
    ...
    var body: some View {
        ...
        .onChange(of: selectedSection) { _, section in
            if embeddedSection == nil, let section { store.requestedSettingsSection = section.nativeDestination }
        }
    ...
    private var settingsSidebar: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            Button(action: closeSettings) {
                Label("Back to workspace", systemImage: "arrow.left")
    ...
    private func closeSettings() {
        if let onClose { onClose() } else { store.closeWorkspaceSettings(); dismiss() }
    }
    ...
    private func openSection(_ section: SettingsSection) {
        if embeddedSection == nil {
            selectedSection = section
            store.requestedSettingsSection = section.nativeDestination
        } else { store.presentSettings(section.nativeDestination) }
    }
        ...
        SettingsCard(title: "Conversation", symbol: "text.bubble", accent: RivunePalette.rivune) {
            VStack(spacing: 15) {
                ...
                HStack {
                    ...
                    RivuneSelect(title: "Default mode", selection: $store.defaultMode,
                        options: IntelligenceModeSelectionOption.available().map { ($0.title, $0.mode) })
                }
            }
        }
    }
    ...
    private var appearanceSection: some View {
        SettingsCard(title: "Appearance", symbol: "circle.lefthalf.filled", accent: RivunePalette.claude) {
            VStack(alignment: .leading, spacing: 15) {
                ...
                Text(selectedBackgroundPreset == nil
                     ? "Your previous appearance is unchanged. Choose a background when you are ready."
                     : "Choose the atmosphere around your work. Reading surfaces stay consistent.")
                    .font(.callout).foregroundStyle(RivunePalette.secondaryText)
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 120, maximum: 220), spacing: 10)], alignment: .leading, spacing: 10) {
                    ForEach(RivuneBackgroundPreset.allCases) { preset in
                        RivuneBackgroundPresetCard(preset: preset, isSelected: selectedBackgroundPreset == preset,
                                                  stars: presetBackgroundStars ?? stars, dim: presetBackgroundDim ?? backgroundDim) {
                            backgroundPreset = preset.rawValue
                        }
                    }
                }
                if selectedBackgroundPreset != nil {
                    Button("Use previous appearance") { backgroundPreset = "" }
                        .buttonStyle(RivuneButtonStyle())
                        .accessibilityHint("Restores your saved galaxy, stars and brightness choices.")
                } else {
                    Toggle("Milky Way background", isOn: $galaxy)
                        .toggleStyle(RivuneCheckmarkStyle())
                }
                if selectedBackgroundPreset == nil || selectedBackgroundPreset == .cosmos {
                    Toggle("Stars", isOn: appearanceStarsBinding).toggleStyle(RivuneCheckmarkStyle())
                }
                RivuneBackgroundBrightnessControl(
                    value: appearanceBrightnessBinding,
                    range: selectedBackgroundPreset == nil ? 0.15...0.85 : 0.15...0.70
                )
                Toggle(isOn: $store.subtleMotionEnabled) {
                    SettingLabel(title: "Subtle motion", detail: "Use subtle movement while models collaborate", symbol: "move.3d")
                }
                .toggleStyle(RivuneCheckmarkStyle())
                Text("Backgrounds are still images and drawings; this motion setting applies to collaboration indicators.")
                ...
                Button {
                    store.showSettings = false
                    store.replayStartup()
                } label: { Label("Replay arrival", systemImage: "play.circle") }
                .buttonStyle(RivuneButtonStyle())
                .frame(maxWidth: .infinity, alignment: .leading)
                Button {
                    store.showSettings = false
                    store.showAccountSetup = true
                } label: { Label("Restart setup", systemImage: "person.crop.circle.badge.plus") }
                .buttonStyle(RivuneButtonStyle())
                .frame(maxWidth: .infinity, alignment: .leading)
            }
        }
    ...
    private var selectedBackgroundPreset: RivuneBackgroundPreset? {
        RivuneAppearancePolicy.preset(from: backgroundPreset)
    }

    private var appearanceStarsBinding: Binding<Bool> {
        Binding(get: { selectedBackgroundPreset == nil ? stars : (presetBackgroundStars ?? stars) },
                set: { if selectedBackgroundPreset == nil { stars = $0 } else { presetBackgroundStars = $0 } })
    }

    private var appearanceBrightnessBinding: Binding<Double> {
        Binding(get: { 1 - (selectedBackgroundPreset == nil ? backgroundDim : (presetBackgroundDim ?? backgroundDim)) },
                set: { if selectedBackgroundPreset == nil { backgroundDim = 1 - $0 } else { presetBackgroundDim = 1 - $0 } })
    }
...
struct ConnectionSetupSheet: View {
    ...
    private func providerRow(mode: IntelligenceMode, name: String, detail: String, readiness: ProviderReadiness) -> some View {
        VStack(alignment: .leading, spacing: 13) {
            HStack(spacing: 11) {
                ...
                VStack(alignment: .trailing, spacing: 6) {
                    HStack(spacing: 5) {
                        ...
                        Text(isPreview ? "Preview" : readiness == .unavailable ? "Not verified" : readiness.label)
                            .font(.system(size: 11, weight: .medium))
                            .multilineTextAlignment(.trailing)
                    }
            ...
            if readiness == .unavailable && !isPreview {
                VStack(alignment: .leading, spacing: 8) {
                    Text(ProviderStatusRecovery.unknown.message)
                        .font(.system(size: 11))
                        .foregroundStyle(RivunePalette.secondaryText)
                        .fixedSize(horizontal: false, vertical: true)
                    Button("Check connection", systemImage: "arrow.clockwise") {
                        guard !isPreview, store.startupPhase != .checking else { return }
                        store.refreshConnections()
                    }
                    .buttonStyle(.plain)
                    .disabled(isPreview || store.startupPhase == .checking)
                    .accessibilityLabel("Check configured connections")
                    .help("Refreshes configured connection checks. Does not send a chat message.")
                }
            }
