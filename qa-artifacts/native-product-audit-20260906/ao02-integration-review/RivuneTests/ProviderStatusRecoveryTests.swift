...
final class ProviderStatusRecoveryTests: XCTestCase {
    func testSanitizedStatusClassificationAndActions() {
        ...
        XCTAssertTrue(ProviderStatusRecovery.signInRequired.action == .openProviderSetup)
        XCTAssertTrue(ProviderStatusRecovery.executableMissing.action == .openProviderSetup)
        XCTAssertTrue(ProviderStatusRecovery.unknown.action == .checkConnection)
        XCTAssertTrue(ProviderStatusRecovery.signedIn.action == .none)
        count += 4; print("PASS: four typed next-action mappings")
        let marker = "SECRET_FIXTURE_ONLY"
        let outcome = classify(.claude, 1, "authentication \(marker)")
        XCTAssertTrue(!outcome.message.contains(marker) && outcome == .unknown)
        count += 1; print("PASS: diagnostics excluded from user-facing copy")
        print("RESULT: \(count)/\(count) fixture checks passed")
    }
}
