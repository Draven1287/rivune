...
let fixtures = [
("invalid-selection", "[{\"turns\":[{\"selectedArtifact\":false}]}]"),
("invalid-selection-plus-null-turn", "[{\"turns\":[{\"selectedArtifact\":false},null]}]"),
("invalid-selection-plus-number-turn", "[{\"turns\":[{\"selectedArtifact\":false},7]}]")
]
for (name, json) in fixtures { print("\(name): recoveryRequired=\(Check.hasInvalidArtifactSelections(Data(json.utf8)))") }
