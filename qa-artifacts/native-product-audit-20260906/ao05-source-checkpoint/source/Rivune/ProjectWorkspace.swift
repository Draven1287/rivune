...
struct ResponseArtifactCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text(artifact.summary).lineLimit(fullSummary ? nil : 4).textSelection(.enabled)
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\(artifact.files.count) files · \(artifact.byteCount.formatted()) bytes").font(.caption).foregroundStyle(RivunePalette.secondaryText)
                }
                Spacer()
                #if os(macOS)
            ...
            Text(artifact.files.map(\.path).joined(separator: "  ·  ")).font(.caption.monospaced()).textSelection(.enabled)
            Text("Preview the files, then choose where to save").font(.caption).foregroundStyle(RivunePalette.secondaryText)
