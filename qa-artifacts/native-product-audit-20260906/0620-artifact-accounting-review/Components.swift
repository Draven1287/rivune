    ...
    var body: some View {
        VStack(spacing: 7) {
            ...
            HStack(spacing: 10) {
                ...
                if let team = store.savedTeam?.configuration,
                   let manager = team.members.first(where: { $0.memberID == team.orchestratorMemberID }) {
                    Text("Orchestrator: \(manager.displayName)").font(.caption).foregroundStyle(RivunePalette.secondaryText)
                }
                Spacer(minLength: 0)
            }
                ...
                if let selected = store.draftArtifact {
                    HStack(alignment: .top) {
                        VStack(alignment: .leading, spacing: 4) {
                            Label("Selected response · \(selected.files.count) complete files", systemImage: "doc.on.doc")
                            Text(selected.files.map(\.path).joined(separator: " · ")).lineLimit(2)
                            Text("Included with your next request, even when conversation memory is off.")
                        }.font(.caption)
                        Spacer()
                        Button { store.draftArtifact = nil } label: { Image(systemName: "xmark") }
                            .buttonStyle(.plain).accessibilityLabel("Remove selected generated files")
                    }.padding(12).background(RivunePalette.control, in: RoundedRectangle(cornerRadius: 10)).padding(10)
                }
...
private struct ComposerPermissionMenu: View {
    ...
    var body: some View {
        RivuneActionMenu(title: "Tools and permissions", actions: [
            ...
            .init(title: "Remove all selected files", systemImage: "xmark.circle", isEnabled: !store.draftAttachments.isEmpty || store.draftArtifact != nil) {
