...
async function capture(name){
 if(name.includes('anchor')) await page.waitForFunction(()=>{const el=document.querySelector(location.hash);return el&&Math.abs(el.getBoundingClientRect().top-parseFloat(getComputedStyle(document.documentElement).scrollPaddingTop))<3;});
