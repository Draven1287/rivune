...
extension RivuneStore {
    ...
    func routeDescription(for mode: IntelligenceMode) -> String {
        switch mode {
        case .chatGPT:
            currentCodexRoute.transportKind == .api ? "OpenAI API · connection check: \(apiProbes[.openAI]?.modelID ?? "model unavailable")" : "Codex CLI · requested \(codexModel.title) · \(codexEffort.title) reasoning"
        case .claude:
            currentClaudeRoute.transportKind == .api ? "Anthropic API · connection check: \(apiProbes[.anthropic]?.modelID ?? "model unavailable")" : "Claude Code CLI · requested \(claudeModel.title) · \(claudeEffort.title) reasoning"
        case .together, .council, .swarm: "\(routeDescription(for: .chatGPT)) + \(routeDescription(for: .claude))"
        }
    }
