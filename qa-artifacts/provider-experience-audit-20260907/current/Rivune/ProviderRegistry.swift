...
struct AIExecutionRoute: Codable, Hashable, Sendable {
...
struct CLICapabilitySnapshot: Sendable {
    ...
    var codexObservedAt: Date?
    var claudeObservedAt: Date?
    ...
    static func localCodexModels(home: URL = FileManager.default.homeDirectoryForCurrentUser) -> [CodexModel] {
        localCodexMetadata(home: home).models
    }
    static func localCodexMetadata(home: URL = FileManager.default.homeDirectoryForCurrentUser) -> (models: [CodexModel], observedAt: Date?) {
        ...
        guard let info = try? url.resourceValues(forKeys: [.isRegularFileKey, .fileSizeKey, .contentModificationDateKey]),
              ...
              let modified = info.contentModificationDate, (0...604_800).contains(Date().timeIntervalSince(modified)),
              let data = try? Data(contentsOf: url) else { return ([], nil) }
        return (codexCache(data), modified)
    }
