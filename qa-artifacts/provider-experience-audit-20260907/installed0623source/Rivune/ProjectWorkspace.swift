...
/// Each viewer owns a unique private copy; it never writes to a selected project.
final class ResponseArtifactStage {
    let rootURL: URL
    let answerID: UUID
    init(artifact: ResponseArtifact) throws {
        let validated = try ResponseArtifact.parse(answer: artifact.rawResponse, answerID: artifact.answerID)
        answerID = validated.answerID
        rootURL = FileManager.default.temporaryDirectory.appendingPathComponent("Rivune-artifact-\(answerID)-\(UUID())", isDirectory: true)
        ...
        do {
            for file in validated.files {
                let target = rootURL.appendingPathComponent(file.path)
                try FileManager.default.createDirectory(at: target.deletingLastPathComponent(), withIntermediateDirectories: true)
                try Data(file.content.utf8).write(to: target, options: .withoutOverwriting)
            }
        } catch { try? FileManager.default.removeItem(at: rootURL); throw error }
    }
...
final class ProjectWorkspace: Sendable {
    ...
    func prepare(answer: String) throws -> ProjectChangeSet {
        let manifest = try ResponseArtifact.parse(answer: answer, answerID: UUID())
...
final class ProjectWorkspaceSession: ObservableObject {
    ...
    func cancelFolderLoad() {
        folderGeneration = UUID()
        isLoadingFolder = false
        message = "Folder loading cancelled. Choose a local folder to try again."
    }

    func select(_ url: URL, onReady: (() -> Void)? = nil) {
        let generation = UUID()
        ...
        // Cloud file hydration can block in the filesystem. Keep it off the UI
        // actor; cancellation revokes admission even if the OS read finishes late.
        Task {
            let result = await Task.detached(priority: .userInitiated) {
                Result { () -> (ProjectWorkspace, ProjectSnapshot) in
                    let candidate = try ProjectWorkspace(rootURL: url)
                    return (candidate, try candidate.snapshot())
                }
            }.value
            guard folderGeneration == generation else {
                if didAccess { url.stopAccessingSecurityScopedResource() }
                return
            }
            isLoadingFolder = false
            switch result {
            case .success(let (candidate, files)):
        ...
        Task {
            ...
            guard folderGeneration == generation, isLoadingFolder else { return }
            cancelFolderLoad()
            message = "The folder is taking too long to read. Download its files in Finder or choose a local folder, then try again."
        }
    }
...
struct ProjectWorkspaceView: View {
    ...
    private var finalAnswer: String? {
        ...
        case .together, .council, .swarm: return turn.combinedAnswer?.content
        }
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if session.isLoadingFolder {
                VStack(spacing: 12) {
                    ProgressView("Reading folder…")
                    Button("Cancel", action: session.cancelFolderLoad)
                }.frame(maxWidth: .infinity, maxHeight: .infinity)
            } else if let workspace = session.workspace {
                HStack(spacing: 12) {
...
@MainActor
final class StaticPreviewStatus: ObservableObject {
    @Published var loading = true
    @Published var failure: String?
    var onFinish: (() -> Void)?
    func finish(error: String? = nil) { loading = false; failure = error; onFinish?() }
}

struct StaticPreviewPolicy {
    // WebKit content blocker regexes do not support alternation. Compile each
    // protocol family separately instead of (https?|wss?|ftp).
    static let networkRules = #"[{"trigger":{"url-filter":"^https?://"},"action":{"type":"block"}},{"trigger":{"url-filter":"^wss?://"},"action":{"type":"block"}},{"trigger":{"url-filter":"^ftp://"},"action":{"type":"block"}}]"#
    static func allows(_ url: URL, root: URL, download: Bool) -> Bool {
        let root = root.resolvingSymlinksInPath().standardizedFileURL.path
        return url.isFileURL && !download && url.resolvingSymlinksInPath().standardizedFileURL.path.hasPrefix(root + "/")
    }
}

private struct StaticProjectPreview: View {
    let rootURL: URL
    let revision: UUID
    @StateObject private var status: StaticPreviewStatus
    @State private var retry = UUID()
    init(rootURL: URL, revision: UUID, status: StaticPreviewStatus? = nil) {
        self.rootURL = rootURL
        self.revision = revision
        _status = StateObject(wrappedValue: status ?? StaticPreviewStatus())
    }
    var body: some View {
        ZStack {
            StaticProjectWebView(rootURL: rootURL, revision: revision, retry: retry, status: status)
            if let failure = status.failure {
                VStack(spacing: 12) {
                    Text("Preview could not load").font(.headline)
                    Text(failure).font(.callout).multilineTextAlignment(.center)
                    Button("Retry preview") { retry = UUID() }
                }.padding(24).frame(maxWidth: .infinity, maxHeight: .infinity)
                    .background(RivunePalette.surface)
            } else if status.loading {
                ProgressView("Loading local preview…").padding(20).background(RivunePalette.surface, in: RoundedRectangle(cornerRadius: 12))
            }
        }.frame(maxWidth: .infinity, maxHeight: .infinity)
    }
}

/// SwiftUI can update an NSView before its sheet has attached or laid out.
/// Admit the first navigation only once WebKit has a real window and viewport.
@MainActor
final class MountedStaticWebView: WKWebView {
    private var readyTask: Task<Void, Never>?
    var pendingLoad: (() -> Void)? { didSet { admitWhenMounted() } }
    ...
    func cancelMountAdmission() {
        readyTask?.cancel()
        readyTask = nil
        pendingLoad = nil
    }
    private func admitWhenMounted() {
        guard window != nil, bounds.width > 0, bounds.height > 0, pendingLoad != nil, readyTask == nil else { return }
        // Defer state publication outside SwiftUI's layout/update callback. This
        // is an attachment condition, not a timer or an arbitrary delay.
        readyTask = Task { @MainActor [weak self] in
            guard let self else { return }
            self.readyTask = nil
            guard !Task.isCancelled, self.window != nil, self.bounds.width > 0, self.bounds.height > 0 else { return }
            let action = self.pendingLoad
...
struct StaticProjectWebView: NSViewRepresentable {
    let rootURL: URL
    let revision: UUID
    let retry: UUID
    let status: StaticPreviewStatus
    func makeCoordinator() -> Coordinator { Coordinator(rootURL: rootURL, status: status) }
    static func makeView(coordinator: Coordinator) -> MountedStaticWebView {
        let configuration = WKWebViewConfiguration()
        configuration.websiteDataStore = .nonPersistent()
        configuration.defaultWebpagePreferences.allowsContentJavaScript = false
        let view = MountedStaticWebView(frame: .zero, configuration: configuration)
        view.navigationDelegate = coordinator
        return view
    }
    func makeNSView(context: Context) -> MountedStaticWebView { Self.makeView(coordinator: context.coordinator) }
    func updateNSView(_ view: MountedStaticWebView, context: Context) {
        let coordinator = context.coordinator
        view.pendingLoad = { [weak view, weak coordinator] in
            guard let view, let coordinator else { return }
            coordinator.load(view, rootURL: rootURL, revision: revision, retry: retry)
        }
    }
    static func dismantleNSView(_ view: MountedStaticWebView, coordinator: Coordinator) {
        view.cancelMountAdmission()
        coordinator.cancel(); view.stopLoading(); view.navigationDelegate = nil
    }
    @MainActor final class Coordinator: NSObject, WKNavigationDelegate {
        var rootURL: URL
        private var revision: UUID?
        private var retry: UUID?
        private var generation = UUID()
        private var navigation: WKNavigation?
        private var loadTask: Task<Void, Never>?
        private var timeout: Task<Void, Never>?
        private let status: StaticPreviewStatus
        init(rootURL: URL, status: StaticPreviewStatus) { self.rootURL = rootURL; self.status = status }
        func cancel() { generation = UUID(); navigation = nil; loadTask?.cancel(); timeout?.cancel() }
        func load(_ view: WKWebView, rootURL: URL, revision: UUID, retry: UUID) {
            let rootURL = rootURL.resolvingSymlinksInPath().standardizedFileURL
            guard self.revision != revision || self.retry != retry || self.rootURL != rootURL else { return }
            cancel(); view.stopLoading()
            self.revision = revision; self.retry = retry
            self.rootURL = rootURL.resolvingSymlinksInPath().standardizedFileURL
            let ticket = generation
            loadTask = Task { @MainActor [weak self, weak view] in
                guard let self, let view else { return }
                self.status.loading = true; self.status.failure = nil
                do {
                    let rules = try await WKContentRuleListStore.default().compileContentRuleList(forIdentifier: "RivuneStaticPreviewNoNetworkV2", encodedContentRuleList: StaticPreviewPolicy.networkRules)
                    guard !Task.isCancelled, self.generation == ticket else { return }
                    guard let rules else { self.status.finish(error: "The network blocker is unavailable. Preview remains blocked."); return }
                    view.configuration.userContentController.removeAllContentRuleLists()
                    view.configuration.userContentController.add(rules)
                    self.navigation = view.loadFileURL(self.rootURL.appendingPathComponent("index.html"), allowingReadAccessTo: self.rootURL)
                    guard self.navigation != nil else {
                        self.status.finish(error: "The local index.html file could not be opened."); return
                    }
                    self.timeout = Task { @MainActor [weak self, weak view] in
                        do { try await Task.sleep(for: .seconds(15)) } catch { return }
                        guard let self, self.generation == ticket, self.status.loading else { return }
                        self.navigation = nil; view?.stopLoading(); self.status.finish(error: "Loading timed out. Check that index.html and its local files are available, then retry.")
                    }
                } catch {
                    guard !Task.isCancelled, self.generation == ticket else { return }
                    self.status.finish(error: "The local preview could not prepare its network blocker. No page was loaded.")
                }
            }
        }
        func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction) async -> WKNavigationActionPolicy {
            guard let url = navigationAction.request.url,
                  StaticPreviewPolicy.allows(url, root: rootURL, download: navigationAction.shouldPerformDownload) else {
                if navigationAction.targetFrame?.isMainFrame == true {
                    timeout?.cancel(); status.finish(error: "This preview only opens files inside the selected project folder.")
                }
                return .cancel
            }
            return .allow
        }
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) { guard navigation === self.navigation else { return }; self.navigation = nil; timeout?.cancel(); status.finish() }
        func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) { guard navigation === self.navigation else { return }; failed(error) }
        func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) { guard navigation === self.navigation else { return }; failed(error) }
        private func failed(_ error: Error) {
            guard (error as NSError).code != NSURLErrorCancelled else { return }
            timeout?.cancel(); status.finish(error: "WebKit could not open the local page. Check the project files and retry.")
        }
        func webViewWebContentProcessDidTerminate(_ webView: WKWebView) { timeout?.cancel(); status.finish(error: "The preview process stopped. Retry to reload the local page.") }
    }
}
#endif
...
struct RivuneApprovedProjectContext: Equatable, Sendable {
    let approvedInstructions: String?
    let documents: [PromptAttachment]
}
...
enum RivuneProjectFiles {
    ...
    static func approvedContext(for project: RivuneProject, approved: Bool) throws -> RivuneApprovedProjectContext {
        guard approved else { return .init(approvedInstructions: nil, documents: []) }
        let instructions = project.instructions.trimmingCharacters(in: .whitespacesAndNewlines)
        let documents = try project.files.filter(\.included).map(attachment)
        guard documents.count <= 6,
              RivuneStore.preparedAttachmentContext(documents) != nil,
              instructions.utf8.count <= 8_000,
              instructions.utf8.count + documents.reduce(0, { $0 + $1.byteCount }) <= 20_000 else {
            throw ProjectLibraryError.message("Selected project context exceeds the 20 KB request budget. Select fewer files or shorten the instructions.")
        }
        return .init(approvedInstructions: instructions.isEmpty ? nil : instructions, documents: documents)
    }
}
...
extension RivuneStore {
    ...
    var projectRecipients: String {
        switch mode { case .together, .council, .swarm: "ChatGPT and Claude"; case .chatGPT: "ChatGPT"; case .claude: "Claude" }
    }
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    ForEach(project.files) { file in
                        HStack(alignment: .top) {
                            ...
                            Spacer()
                            RivuneActionMenu(title: "Actions for \(file.name)", actions: [
                                RivuneMenuAction(title: "Reveal in Finder") { do { NSWorkspace.shared.activateFileViewerSelecting([try RivuneProjectFiles.resolve(file)]) } catch { notice = error.localizedDescription } },
                                RivuneMenuAction(title: "Refresh reference") { refresh(file, project: project) },
                                RivuneMenuAction(title: "Remove reference", destructive: true) { var changed = project; changed.files.removeAll { $0.id == file.id }; store.updateProject(changed) }
                            ])
                        }.padding(.vertical, 6)
                    }
                }.padding(10)
...
struct ResponseArtifactCard: View {
    let artifact: ResponseArtifact
    var onContinue: (() -> Void)? = nil
    @State private var presentedArtifact: ResponseArtifact?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text(artifact.summary)
                ...
                .lineLimit(fullSummary ? nil : 4)
                .textSelection(.enabled)
            if artifact.summary.count > 320 {
                Button(fullSummary ? "Show less" : "Read full summary") { fullSummary.toggle() }.buttonStyle(.plain)
            }
            HStack(spacing: 12) {
                Image(systemName: artifact.hasPreview ? "macwindow" : "doc.on.doc").font(.title2)
                VStack(alignment: .leading, spacing: 4) {
                    Text(artifact.hasPreview ? "Website files" : "Generated files").font(.headline)
                    Text("\(artifact.files.count) files · \(artifact.byteCount.formatted()) bytes").font(RivuneTypography.supporting).foregroundStyle(RivunePalette.secondaryText)
                }
                Spacer()
                #if os(macOS)
                Button(artifact.hasPreview ? "Preview" : "Open files") { presentedArtifact = artifact }
                    .buttonStyle(.borderedProminent).tint(RivunePalette.rivune)
                #endif
            }.padding(16).background(RivunePalette.control, in: RoundedRectangle(cornerRadius: 12))
            Text(artifact.files.map(\.path).joined(separator: "  ·  ")).font(RivuneTypography.supporting.monospaced()).textSelection(.enabled)
            Text("Preview the files, then choose where to save").font(RivuneTypography.supporting).foregroundStyle(RivunePalette.secondaryText)
            if let onContinue { Button("Continue editing these files", action: onContinue).buttonStyle(.bordered) }
            DisclosureGroup("Original response") { MarkdownResponseText(content: artifact.rawResponse) }
                .font(RivuneTypography.supporting)
        }
        #if os(macOS)
        .sheet(item: $presentedArtifact) { selected in ResponseArtifactViewer(artifact: selected) }
        #endif
    }
}

#if os(macOS)
struct ResponseArtifactViewer: View {
    let artifact: ResponseArtifact
    var previewStatus: StaticPreviewStatus? = nil
    @Environment(\.dismiss) private var dismiss
    @StateObject private var saveSession = ProjectWorkspaceSession()
    @State private var stage: ResponseArtifactStage?
    @State private var stagingError: String?
    @State private var selectedPath: String?
    @State private var tab = "Preview"
    @State private var showOriginal = false
    @State private var checks: [ProjectStructuralCheck] = []

    var body: some View {
        VStack(spacing: 0) {
            HStack {
                VStack(alignment: .leading, spacing: 3) {
                    Text(artifact.hasPreview ? "Website preview" : "Generated files").font(.title3.weight(.semibold))
                    Text("\(artifact.files.count) files · from this response").font(.caption).foregroundStyle(RivunePalette.secondaryText)
                }
                Spacer()
                Button("Save to folder…", action: chooseSaveFolder).disabled(saveSession.isLoadingFolder)
                Button { dismiss() } label: { Image(systemName: "xmark").frame(width: 28, height: 28) }
                    .buttonStyle(.plain).accessibilityLabel("Close artifact")
            }.padding(20)
            HStack {
                Picker("Artifact view", selection: $tab) {
                    if artifact.hasPreview { Text("Preview").tag("Preview") }
                    Text("Files").tag("Files")
                    if tab == "Save" || saveSession.proposal != nil || saveSession.receipt != nil { Text("Review save").tag("Save") }
            ...
            if tab == "Preview" {
                if let stage {
                    StaticProjectPreview(rootURL: stage.rootURL, revision: artifact.answerID, status: previewStatus)
                } else if let stagingError {
                    VStack(spacing: 12) {
                        Text(stagingError)
                        Button("Retry preview", action: preparePreview)
                    }.frame(maxWidth: .infinity, maxHeight: .infinity)
                ...
                HSplitView {
                            ...
                            ForEach(artifact.files) { file in
                                Button { selectedPath = file.path } label: {
                                    HStack {
                                        ...
                                        Spacer()
                                    }.padding(10).background(selectedPath == file.path ? RivunePalette.control : .clear, in: RoundedRectangle(cornerRadius: 8))
                                }.buttonStyle(.plain)
                            }
                        }.padding(12)
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                            Text(selectedPath ?? "Choose a file").font(.headline)
                            Spacer()
                            if tab == "Save" { Toggle("Original", isOn: $showOriginal).toggleStyle(.switch) }
                            Button("Copy file") {
                                NSPasteboard.general.clearContents()
                                NSPasteboard.general.setString(selectedContent, forType: .string)
                            }
                            ShareLink(item: selectedContent) { Text("Share contents") }
                        }
                        ScrollView([.horizontal, .vertical]) {
                            Text(selectedContent).font(.system(size: 12, design: .monospaced)).textSelection(.enabled)
                                .frame(maxWidth: .infinity, alignment: .topLeading)
                        }
                    }.padding(16).frame(minWidth: 360, maxWidth: .infinity, maxHeight: .infinity)
                }
            }
            Divider()
            VStack(alignment: .leading, spacing: 8) {
                if tab == "Save" {
                    if let root = saveSession.workspace?.rootURL {
                        Text("Destination: \(root.path)").font(.caption).textSelection(.enabled)
                    }
                    if saveSession.isLoadingFolder {
                        HStack { ProgressView("Reading folder…"); Button("Cancel", action: saveSession.cancelFolderLoad) }
                    }
                    if let message = saveSession.message { Text(message).font(.caption) }
                    HStack {
                        Text("Review the proposed files before applying. Existing files may be replaced.").font(.caption)
                        Spacer()
                        if saveSession.receipt != nil {
                            Button("Revert last save") { saveSession.revert() }
                        }
                        Button("Apply reviewed changes") { saveSession.apply() }
                            .buttonStyle(.borderedProminent).tint(RivunePalette.rivune).disabled(saveSession.proposal == nil)
                    }
                } else {
                    Text("Static preview · JavaScript and remote requests blocked. Preview uses a separate copy of this response.").font(.caption)
                    DisclosureGroup("File checks") {
                        ForEach(checks) { check in
                            Label(check.detail, systemImage: check.passed ? "checkmark.circle" : "exclamationmark.circle").font(.caption)
                        }
                        Text("Structural checks only. Browser interactions and visual quality have not been verified by Rivune.").font(.caption)
                    }
                }
            }.padding(16).foregroundStyle(RivunePalette.secondaryText)
        }
        .frame(minWidth: 780, idealWidth: 1000, minHeight: 580, idealHeight: 740)
        .background(RivunePalette.canvas).foregroundStyle(RivunePalette.primaryText).preferredColorScheme(.dark)
        .onAppear {
            selectedPath = artifact.files.first?.path
            if !artifact.hasPreview { tab = "Files" }
            preparePreview()
        }
    }

    private var selectedContent: String {
        if tab == "Save", showOriginal {
            return saveSession.proposal?.changes.first(where: { $0.path == selectedPath })?.originalData
                .flatMap { String(data: $0, encoding: .utf8) } ?? "No original contents in this proposal."
        }
        return artifact.files.first(where: { $0.path == selectedPath })?.content ?? ""
    }

    private func preparePreview() {
        do {
            let next = try ResponseArtifactStage(artifact: artifact)
            checks = try ProjectWorkspace(rootURL: next.rootURL).structuralChecks()
            stage = next
            stagingError = nil
        } catch { stagingError = error.localizedDescription }
    }

    private func chooseSaveFolder() {
        let panel = NSOpenPanel()
        panel.title = "Save generated files"
        panel.message = "Choose or create a folder. Review the proposed changes before applying them."
        panel.canChooseDirectories = true
        panel.canChooseFiles = false
        panel.canCreateDirectories = true
        panel.allowsMultipleSelection = false
        if panel.runModal() == .OK, let url = panel.url {
            saveSession.select(url) { saveSession.prepare(answer: artifact.rawResponse) }
