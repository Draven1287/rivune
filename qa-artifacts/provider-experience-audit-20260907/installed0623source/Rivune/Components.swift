...
struct ResponseCard: View {
    ...
    var isPrimary = false
    var onContinueArtifact: ((AIAnswer) -> Void)? = nil
    ...
    private var cardHeader: some View {
        HStack(spacing: 10) {
            ...
            if answer != nil {
                ...
                if let onRetry {
                    RivuneActionMenu(title: "Response actions", actions: [
                        .init(title: retryLabel, systemImage: "arrow.clockwise", action: onRetry)
                    ])
                }
                if let answer {
                    ShareLink(item: answer.content) { Image(systemName: "square.and.arrow.up").frame(width: 32, height: 32) }
                        .buttonStyle(.plain).accessibilityLabel("Share response")
                }
            }
        }
    }
    ...
    private func answerBody(_ answer: AIAnswer) -> some View {
        if isPrimary, let artifact = try? ResponseArtifact.parse(answer: answer.content, answerID: answer.id) {
            ResponseArtifactCard(artifact: artifact, onContinue: onContinueArtifact.map { action in { action(answer) } })
        } else if isPrimary && ResponseArtifact.looksLikeManifest(answer.content) {
            Label("This file result is incomplete or unsupported.", systemImage: "exclamationmark.triangle")
            Text("Ask for complete files with safe relative paths. No files were saved.").font(.caption)
            DisclosureGroup("Original response") { MarkdownResponseText(content: answer.content) }
        } else {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 14) {
                    RivuneSelect(title: "Collaboration detail", selection: $selectedSection, options: CollaborationInspectorSection.allCases.map { ($0.title, $0) })
                    .frame(maxWidth: 430)
...
#if os(macOS)
/// Attached only behind the message input, providing its real window and bounds.
private final class ComposerFocusAnchorView: NSView {
    override func hitTest(_ point: NSPoint) -> NSView? { nil }
}
private struct ComposerFocusAnchor: NSViewRepresentable {
    let anchor: ComposerFocusAnchorView
    func makeNSView(context: Context) -> ComposerFocusAnchorView { anchor }
    func updateNSView(_ nsView: ComposerFocusAnchorView, context: Context) {}
}
#endif
...
struct ComposerView: View {
    ...
    @State private var restoreComposerAfterSettings = false
    #if os(macOS)
    @State private var composerFocusAnchor = ComposerFocusAnchorView()
    @State private var settingsFocusOwner: NSWindow?
    @State private var settingsComposerSelection: NSRange?
    #endif
    ...
    var body: some View {
        VStack(spacing: 7) {
            ...
            VStack(spacing: 0) {
                #if os(macOS)
                composerConfigurationRow
                    .padding(.horizontal, RivuneInterface.composerContentInset)
                    .padding(.top, 10)
                    .sheet(isPresented: $showTeamEditor) { RivuneTeamEditor(store: store) }
                #endif
                if let selected = store.draftArtifact {
                    SelectedArtifactContext(artifact: selected) { store.draftArtifact = nil }
                }
                ...
                TextField("Send a message", text: $store.composerText, axis: .vertical)
                    ...
                    #if os(macOS)
                    .background(ComposerFocusAnchor(anchor: composerFocusAnchor).allowsHitTesting(false).accessibilityHidden(true))
                    #endif
                    .onSubmit {
                    ...
                    .padding(.horizontal, RivuneInterface.composerContentInset)
                    .padding(.top, store.draftAttachments.isEmpty && store.draftArtifact == nil ? 16 : 9)
                    .padding(.bottom, 12)
            ...
            HStack(spacing: 7) {
                ...
                Spacer()
                ViewThatFits(in: .horizontal) {
                    Text(store.draftAttachments.isEmpty && store.draftArtifact == nil ? "Tools off · no attachments" : "Tools off · selected files")
                    Text("Tools off")
                }
            }
            .font(.caption2)
        ...
        .task(id: store.newConversationFocusRequest) {
            guard let request = store.newConversationFocusRequest else { return }
            // The new Home composer may mount after the sidebar action changes
            // selection. A consumed request cannot focus later chat navigation.
            await Task.yield()
            guard !Task.isCancelled, store.newConversationFocusRequest == request else { return }
            guard store.selectedConversationID == nil, !store.showSettings,
                  !showDocumentPicker, !showTeamEditor, !showConfigurationPanel else {
                store.consumeNewConversationFocusRequest(request)
                return
            }
            #if os(macOS)
            guard NSApp.isActive, let owner = composerFocusAnchor.window,
                  NSApp.keyWindow === owner, owner.attachedSheet == nil,
                  !composerFocusAnchor.isHiddenOrHasHiddenAncestor,
                  composerFocusAnchor.bounds.width > 0, composerFocusAnchor.bounds.height > 0 else {
                store.consumeNewConversationFocusRequest(request)
                return
            }
            #endif
            ...
            #if os(macOS)
            await Task.yield()
            guard !Task.isCancelled, store.newConversationFocusRequest == request,
                  store.selectedConversationID == nil, !store.showSettings,
                  !showDocumentPicker, !showTeamEditor, !showConfigurationPanel,
                  NSApp.isActive, composerFocusAnchor.window === owner,
                  NSApp.keyWindow === owner, owner.attachedSheet == nil else {
                store.consumeNewConversationFocusRequest(request)
                return
            }
            if composerFocused, let editor = owner.firstResponder as? NSTextView,
               editor.window === owner, editor.string == store.composerText,
               composerFocusAnchor.convert(composerFocusAnchor.bounds, to: nil)
                .intersects(editor.convert(editor.visibleRect, to: nil)) {
                // A restored unsent Home draft must receive appended typing,
                // rather than be selected and overwritten by macOS focus.
                editor.setSelectedRange(NSRange(location: (editor.string as NSString).length, length: 0))
            }
            #endif
            store.consumeNewConversationFocusRequest(request)
        }
        .onReceive(store.$showSettings.removeDuplicates()) { presented in
            // Published delivers before the workspace is disabled and SwiftUI clears focus.
            if presented {
                restoreComposerAfterSettings = composerFocused
                #if os(macOS)
                settingsFocusOwner = composerFocused ? NSApp.keyWindow : nil
                settingsComposerSelection = composerFocused ? (settingsFocusOwner?.firstResponder as? NSTextView)?.selectedRange() : nil
                #endif
            }
        }
        .onChange(of: store.workspaceReturnFocusRevision) { _, _ in
            guard restoreComposerAfterSettings else { return }
            restoreComposerAfterSettings = false
            #if os(macOS)
            let owner = settingsFocusOwner
            let selection = settingsComposerSelection
            settingsFocusOwner = nil
            settingsComposerSelection = nil
            #endif
            Task { @MainActor in
                await Task.yield()
                guard !store.showSettings else { return }
                #if os(macOS)
                guard NSApp.isActive, let owner, NSApp.keyWindow === owner, owner.attachedSheet == nil else { return }
                #endif
                composerFocused = true
                #if os(macOS)
                // macOS text fields select all when focus is programmatically restored.
                // Restore the previous caret/selection before the next keystroke.
                await Task.yield()
                guard !store.showSettings, NSApp.isActive, NSApp.keyWindow === owner,
                      let selection, let editor = owner.firstResponder as? NSTextView else { return }
    ...
    private var composerToolbar: some View {
        #if os(macOS)
        HStack(spacing: RivuneInterface.composerControlGap) {
            attachmentControl
            ComposerPermissionMenu(store: store, onChooseDocuments: { showDocumentPicker = true })
    ...
    #if os(macOS)
    @ViewBuilder
    private var composerConfigurationRow: some View {
        if store.mode == .council {
            ViewThatFits(in: .horizontal) {
                HStack(spacing: RivuneInterface.composerControlGap) {
                    teamControl
                    managerLabel.fixedSize(horizontal: true, vertical: false)
                    Spacer(minLength: 0)
                }
                VStack(alignment: .leading, spacing: 4) {
                    teamControl
                    managerLabel.fixedSize(horizontal: false, vertical: true)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
            }
        } else {
            HStack {
                ViewThatFits(in: .horizontal) {
                    ConfigurationMenu(store: store, opensBelow: isEmbedded, isExpanded: $showConfigurationPanel)
                    ConfigurationMenu(store: store, compact: true, opensBelow: isEmbedded, isExpanded: $showConfigurationPanel)
                }
                Spacer(minLength: 0)
            }
        }
    ...
    private var teamControl: some View {
        Button { showTeamEditor = true } label: {
            HStack(spacing: RivuneInterface.composerControlGap) {
                Label("Team · \(store.savedTeam?.configuration.members.count ?? 2)", systemImage: "person.2")
                Image(systemName: "chevron.down").font(.caption)
            }
        }
        .buttonStyle(RivuneButtonStyle())
        .accessibilityLabel("Council team")
        .accessibilityHint("Choose the manager and team for new requests")
    }

    private var managerLabel: some View {
        Text(managerSummary)
            .font(RivuneTypography.supporting)
            .foregroundStyle(RivunePalette.secondaryText)
            .help(managerHelp)
    }

    private var managerSummary: String {
        guard let team = store.savedTeam?.configuration,
              let manager = team.members.first(where: { $0.memberID == team.orchestratorMemberID }) else {
            return "Manager · Automatic"
        }
        return "Manager · \(manager.displayName) · \(manager.requestedModelID ?? "Account default") · \(manager.requestedEffort ?? "Automatic")"
    }

    private var managerHelp: String {
        if store.savedTeam == nil {
            return "Rivune chooses among members that finish their answers. Choose a specific manager in Team."
        }
    ...
    private var attachmentControl: some View {
        RivuneActionMenu(title: "Add files", actions: [
            .init(title: "Attach a text document", systemImage: "doc.badge.plus") { showDocumentPicker = true },
            .init(title: "Photos — unavailable", systemImage: "photo", isEnabled: false) {},
            .init(title: "Camera — unavailable", systemImage: "camera", isEnabled: false) {},
            .init(title: "About supported files", systemImage: "info.circle") {
                store.showPrototypeNotice("Rivune accepts selected UTF-8 text and code files up to 20 KB total")
            }
        ]) {
...
private struct SelectedArtifactContext: View {
    let artifact: ArtifactContinuation
    let remove: () -> Void

    var body: some View {
        HStack(alignment: .top, spacing: 8) {
            VStack(alignment: .leading, spacing: RivuneInterface.contextLineGap) {
                Label("Selected response · \(artifact.files.count) complete files", systemImage: "doc.on.doc")
                    .font(RivuneTypography.supporting.weight(.medium))
                Text(artifact.files.map(\.path).joined(separator: " · "))
                    .lineLimit(2)
                Text("Included with your next request, even when conversation memory is off.")
                    .lineLimit(2)
            }
            Spacer(minLength: 0)
            Button(action: remove) {
                Image(systemName: "xmark")
                    .frame(width: RivuneInterface.contextRemoveTarget, height: RivuneInterface.contextRemoveTarget)
                    .contentShape(Rectangle())
            }
            .buttonStyle(.plain)
            .accessibilityLabel("Remove selected generated files")
        }
        .font(RivuneTypography.supporting)
        .foregroundStyle(RivunePalette.secondaryText)
        .padding(.horizontal, RivuneInterface.contextInset)
        .padding(.vertical, 6)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(RivunePalette.control, in: RoundedRectangle(cornerRadius: 10))
        .padding(.horizontal, RivuneInterface.composerContentInset - RivuneInterface.contextInset)
        .padding(.top, 6)
    }
}
...
private struct ComposerPermissionMenu: View {
    ...
    var body: some View {
        RivuneActionMenu(title: "Tools and permissions", actions: [
            .init(title: "Text-only CLI session", systemImage: "checkmark", section: "Current access") {
            ...
            .init(title: "Choose files to share…", systemImage: "doc.badge.plus", action: onChooseDocuments),
            .init(title: "Remove attachments and generated files", systemImage: "xmark.circle", isEnabled: !store.draftAttachments.isEmpty || store.draftArtifact != nil) {
                store.draftAttachments.map(\.id).forEach(store.removeAttachment)
                store.draftArtifact = nil
            },
            .init(title: "Workspace editing", section: "Unavailable in this build", isEnabled: false) {},
            .init(title: "Shell commands", isEnabled: false) {},
            .init(title: "Unrestricted access", isEnabled: false) {},
            .init(title: "About permissions", systemImage: "info.circle", section: "About") {
            ...
            if showsTitle {
                ...
                ComposerIconLabel(symbol: "checkmark.shield", isActive: !store.draftAttachments.isEmpty || store.draftArtifact != nil)
            }
        }
    ...
    private var permissionAccessibilityValue: String {
        let count = store.draftAttachments.count + (store.draftArtifact?.files.count ?? 0)
        guard count > 0 else {
            return "Text-only CLI; no files attached"
        }
        return "Text-only CLI; \(count) selected files attached"
    }
}
...
private struct ContextWindowMenu: View {
    ...
    var body: some View {
        if let artifact = store.draftArtifact {
            RivuneActionMenu(title: "Request context", actions: [
                .init(title: "Use previous messages", systemImage: "text.bubble", isSelected: store.memoryEnabled) { contextBinding.wrappedValue.toggle() },
                .init(title: "\(artifact.files.count) complete files selected", systemImage: "doc.on.doc") {
                    store.showPrototypeNotice("The complete selected files are included even when previous messages are off")
                },
                .init(title: "Request size checked before sending", systemImage: "info.circle") {
                    store.showPrototypeNotice("Rivune checks the complete encoded request before sending. If it is too large, your draft and selected files stay here")
                }
            ]) {
            ...
            .accessibilityLabel("Request context")
            .accessibilityValue("\(artifact.files.count) complete files selected; request size checked before sending")
        } else {
            estimatedContextMenu
        }
    }

    private var estimatedContextMenu: some View {
        RivuneActionMenu(title: "Input estimate", actions: [
            .init(title: "Use previous messages", systemImage: "text.bubble", isSelected: store.memoryEnabled) { contextBinding.wrappedValue.toggle() },
            .init(title: "≈ \(approximateTokenText) tokens", section: "Next request estimate") {
                store.showPrototypeNotice("About \(approximateTokenText) tokens are currently prepared for the next request")
            },
            .init(title: "\(formattedPreparedBytes) prepared") {
            ...
            .init(title: "How the meter works", systemImage: "info.circle") {
...
private struct ConfigurationMenu: View {
    ...
    var body: some View {
        Button {
            if store.mode == .council { showTeam = true; return }
            ...
            HStack(spacing: 5) {
                ...
                if store.mode == .council {
                    Text("Council team").font(.caption.weight(.medium))
                } else {
            ...
            .frame(height: RivuneInterface.composerControlHeight)
            .background(
        ...
        .sheet(isPresented: $showTeam) { RivuneTeamEditor(store: store) }
...
private struct ModelConfigurationPanel: View {
    ...
    private var visibleProvider: IntelligenceMode {
        (store.mode == .together || store.mode == .council) ? editingProvider : store.mode
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
                ...
                if store.mode == .together || store.mode == .council {
                    HStack(spacing: 6) {
                        providerTab(.chatGPT)
                        providerTab(.claude)
                        Spacer(minLength: 0)
...
private struct ComposerConfigurationLabel: View {
    ...
    private var configurationText: Text {
        ...
        case .together, .council, .swarm:
            return Text(compact ? store.mode.displayName : "\(store.mode.displayName) · 2")
        }
        #endif
    }
    ...
    var body: some View {
        ...
        case .paragraph(let text):
            Text(inlineMarkdown(text))
                .font(RivuneTypography.responseBody)
                .lineSpacing(RivuneInterface.responseLineSpacing)
...
enum ResponseTextParser {
    ...
    private static func normalizeSectionBreaks(in source: String) -> String {
        ...
        // Legacy section guessing is only for unformatted, single-line prose.
        // Applying it to Markdown splits headings such as "## Recommendation"
        // and can mutate code or table cells before the parser sees them.
        guard normalized.count > 160, !normalized.contains("\n"),
              !normalized.trimmingCharacters(in: .whitespaces).hasPrefix("#"),
              !normalized.contains("```") else { return normalized }
...
/// Edits a separate draft; every submitted request retains its own frozen team.
struct RivuneTeamEditor: View {
    @ObservedObject var store: RivuneStore
    @Environment(\.dismiss) private var dismiss
    @State private var members: [TeamMemberConfiguration]
    @State private var orchestrator: String
    ...
    init(store: RivuneStore) {
        self.store = store
        let team = store.savedTeam?.configuration ?? store.suggestedTeam
        _members = State(initialValue: team.members)
        _orchestrator = State(initialValue: team.orchestratorMemberID)
        _fallbacks = State(initialValue: team.fallback.memberIDs)
    }

    private var orderedFallbackCandidates: [TeamMemberConfiguration] {
        let eligible = members.filter { $0.memberID != orchestrator }
        let selected = fallbacks.compactMap { id in eligible.first { $0.memberID == id } }
        return selected + eligible.filter { !fallbacks.contains($0.memberID) }
    }
        ...
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Council · independent answers, one reviewed result")
                        .font(.caption).foregroundStyle(RivunePalette.secondaryText)
                }
                Spacer()
                ModelPickerCloseButton { dismiss() }
            }
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    Text("Orchestrator").font(.headline)
                    RivuneSelect(title: "Orchestrator", selection: $orchestrator, options: members.map { ($0.displayName, $0.memberID) })
                        .onChange(of: orchestrator) { _, value in fallbacks.removeAll { $0 == value } }
                    Text("This member contributes an independent answer, then reviews the team’s answers in a separate session.")
                        .font(.caption).foregroundStyle(RivunePalette.secondaryText)
                    ForEach(members, id: \.memberID) { member in memberCard(member) }
                    Button("Add member", systemImage: "plus") {
                        members.append(.init(memberID: UUID().uuidString, displayName: "Member \(members.count + 1)", routeRef: .init(.codexCLI), requestedModelID: nil, requestedEffort: nil))
                    }.buttonStyle(RivuneButtonStyle()).disabled(members.count >= 6)
                    Button { advanced.toggle() } label: {
                        Label("If the orchestrator fails", systemImage: advanced ? "chevron.down" : "chevron.right")
                    }.buttonStyle(.plain)
                    if advanced {
                        Text("Stop unless you select a fallback. Fallbacks run in the order shown and must have a successful independent answer.").font(.caption)
                        ForEach(orderedFallbackCandidates, id: \.memberID) { member in
                            HStack {
                                Button {
                                    if fallbacks.contains(member.memberID) { fallbacks.removeAll { $0 == member.memberID } }
                                    else { fallbacks.append(member.memberID) }
                                } label: {
                                    Label(member.displayName, systemImage: fallbacks.contains(member.memberID) ? "checkmark.square.fill" : "square")
                                }.buttonStyle(.plain)
                                Spacer()
                                if let index = fallbacks.firstIndex(of: member.memberID) {
                                    Text("\(index + 1)").font(.caption)
                                    Button("Move earlier", systemImage: "arrow.up") {
                                        guard let current = fallbacks.firstIndex(of: member.memberID), current > 0 else { return }
                                        fallbacks.swapAt(current, current - 1)
                                    }
                                        .labelStyle(.iconOnly).buttonStyle(.plain).disabled(index == 0)
                                }
                            }
                        }
                    }
                    Text("Up to 2 calls at once, and 1 per provider connection. Account capacity and resolved model are unknown until reported by the provider.")
                        .font(.caption).foregroundStyle(RivunePalette.secondaryText)
                    Text("Swarm and automatic workflow selection are not available in this build.")
                        .font(.caption).foregroundStyle(RivunePalette.tertiaryText)
                }.padding(.trailing, 6)
            }
            if let error { Text(error).font(.caption).foregroundStyle(Color.red) }
            HStack {
                Text("Changes apply to new requests.").font(.caption).foregroundStyle(RivunePalette.secondaryText)
                Spacer()
                Button("Cancel") { dismiss() }.buttonStyle(RivuneButtonStyle()).keyboardShortcut(.cancelAction)
                Button("Save team") {
                    do {
                        try store.saveTeam(.init(members: members, orchestratorMemberID: orchestrator, fallback: fallbacks.isEmpty ? .stop : .orderedMemberIDs(fallbacks)))
                        store.mode = .council
                        dismiss()
                    } catch { self.error = error.localizedDescription }
                }.buttonStyle(RivuneButtonStyle()).keyboardShortcut(.defaultAction)
            }
        }
        .padding(24).frame(width: 590, height: 670)
        .background(RivunePalette.surface).foregroundStyle(RivunePalette.primaryText)
    }

    private func memberCard(_ member: TeamMemberConfiguration) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                TextField("Member name", text: Binding(get: { members.first { $0.memberID == member.memberID }?.displayName ?? member.displayName }, set: { update(member, name: $0) }))
                    .textFieldStyle(.plain).font(.headline).accessibilityLabel("\(member.displayName) name")
                if member.memberID == orchestrator { Text("Orchestrator").font(.caption).foregroundStyle(RivunePalette.rivune) }
                Button("Remove \(member.displayName)", systemImage: "minus.circle") {
                    members.removeAll { $0.memberID == member.memberID }
                    fallbacks.removeAll { $0 == member.memberID }
                    if orchestrator == member.memberID { orchestrator = members[0].memberID }
                }.labelStyle(.iconOnly).buttonStyle(.plain).disabled(members.count <= 2)
            }
            RivuneSelect(title: "\(member.displayName) provider", selection: Binding(get: { member.routeRef }, set: { update(member, route: $0, reset: true) }), options: [("Codex CLI", TeamRouteReference(.codexCLI)), ("Claude Code CLI", TeamRouteReference(.claudeCodeCLI))])
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 5) {
                    Text("Model").font(.caption)
                    RivuneSelect(title: "\(member.displayName) model", selection: Binding(get: { member.requestedModelID ?? "" }, set: { update(member, model: $0, resetEffort: true) }), options: modelOptions(member))
                }
                VStack(alignment: .leading, spacing: 5) {
                    Text("Reasoning").font(.caption)
                    RivuneSelect(title: "\(member.displayName) reasoning", selection: Binding(get: { member.requestedEffort ?? "" }, set: { update(member, effort: $0) }), options: effortOptions(member))
                }
            }
            let ready = store.teamAdmissionEvidence.accepts(member)
            Label(ready ? "Connection ready · requested options" : "Unavailable · reconnect or update this member", systemImage: ready ? "checkmark.circle" : "exclamationmark.circle")
                .font(.caption).foregroundStyle(ready ? RivunePalette.secondaryText : RivunePalette.claude)
        }.padding(14).background(RivunePalette.canvas, in: RoundedRectangle(cornerRadius: 12))
    }

    private func modelOptions(_ member: TeamMemberConfiguration) -> [(String, String)] {
        let pairs = store.teamAdmissionEvidence.routes.first { $0.reference == member.routeRef }?.supportedOptions ?? []
        var values: [(String, String)] = [("Account default", "")] + Set(pairs.compactMap(\.model)).sorted().map { ($0, $0) }
        if let saved = member.requestedModelID, !values.contains(where: { $0.1 == saved }) { values.append(("\(saved) · unavailable", saved)) }
        return values
    }
    private func effortOptions(_ member: TeamMemberConfiguration) -> [(String, String)] {
        let pairs = store.teamAdmissionEvidence.routes.first { $0.reference == member.routeRef }?.supportedOptions ?? []
        var values: [(String, String)] = [("Automatic", "")] + Set(pairs.filter { $0.model == member.requestedModelID }.compactMap(\.effort)).sorted().map { ($0.capitalized, $0) }
        if let saved = member.requestedEffort, !values.contains(where: { $0.1 == saved }) { values.append(("\(saved) · unavailable", saved)) }
        return values
    }
    private func update(_ old: TeamMemberConfiguration, name: String? = nil, route: TeamRouteReference? = nil, model: String? = nil, effort: String? = nil, reset: Bool = false, resetEffort: Bool = false) {
        guard let index = members.firstIndex(where: { $0.memberID == old.memberID }) else { return }
        members[index] = .init(memberID: old.memberID, displayName: name ?? old.displayName, routeRef: route ?? old.routeRef,
            requestedModelID: reset ? nil : (model.map { $0.isEmpty ? nil : $0 } ?? old.requestedModelID),
            requestedEffort: reset || resetEffort ? nil : (effort.map { $0.isEmpty ? nil : $0 } ?? old.requestedEffort))
    }
}
