...
export function ComposerExecutionControl({ snapshot, catalog, conversationID, confirmation, disabled, onApply, onReviewConnections, compact=false }: {
  ...
  return <>{compact&&<div className="quiet-execution-toolbar">{confirmation?<span className="quiet-confirmed-configuration">{restingSummary}</span>:<><DropdownMenu><DropdownMenuTrigger asChild><Button ref={compactTrigger} variant="ghost" size="sm" disabled={disabled} aria-label="Choose execution mode">{saved.team?'Constellation':'Direct'}<ChevronDown aria-hidden="true"/></Button></DropdownMenuTrigger><DropdownMenuContent className="quiet-menu" align="start" onCloseAutoFocus={event=>{if(open)event.preventDefault();}}><DropdownMenuItem onSelect={openEditor}>Configure mode and participants</DropdownMenuItem><DropdownMenuItem onSelect={onReviewConnections}>Review connections</DropdownMenuItem></DropdownMenuContent></DropdownMenu><Button variant="ghost" size="sm" disabled={disabled} onClick={openEditor} title={restingSummary}>{name(saved.selection?.providerID??snapshot.selectedProviderID)}<ChevronDown aria-hidden="true"/></Button></>}</div>}<details className="composer-execution" open={open} onKeyDown={event => { if (event.key === 'Escape' && open) { event.preventDefault(); event.stopPropagation(); close(); } }}>
