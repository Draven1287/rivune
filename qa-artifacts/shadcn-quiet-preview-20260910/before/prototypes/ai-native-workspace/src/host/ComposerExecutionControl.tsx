...
export function ComposerExecutionControl({ snapshot, catalog, conversationID, confirmation, disabled, onApply, onReviewConnections }: {
  ...
  function close() { if (saving) return; if(!confirmation)loadSaved(); setOpen(false); summary.current?.focus(); }
  ...
  return <><details className="composer-execution" open={open} onKeyDown={event => { if (event.key === 'Escape' && open) { event.preventDefault(); event.stopPropagation(); close(); } }}>
    <summary ref={summary} title={restingSummary} onClick={event => { event.preventDefault(); if (open) close(); else { if(!confirmation)loadSaved(); setOpen(true); } }}>{restingSummary}</summary>
