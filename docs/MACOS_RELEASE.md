...
Run `python3 scripts/test_dmg_release_guards.py` before packaging. These offline
tests exercise the actual shell signature validator with codesign-format
fixtures and verify that public mode without credentials stops before writing
artifacts. Hardened runtime is reported inside the `CodeDirectory ... flags=...`
record, not on a standalone `flags=` line. Fixture success does not establish
that a signing identity, Apple notarization, or a public installer is available.
